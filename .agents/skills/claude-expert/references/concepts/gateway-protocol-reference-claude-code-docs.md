---
title: Gateway protocol reference - Claude Code Docs
...
**Source:** [https://code.claude.com/docs/en/llm-gateway-protocol](https://code.claude.com/docs/en/llm-gateway-protocol)
...
Subagent IDs are generated fresh for each spawn. Teammate agents, the named members of an [agent team](/docs/en/agent-teams), reuse a stable name-based ID across reconnections. In both cases the ID identifies an agent, not a person or a device, so do not treat the agent ID header as a user identifier.
...
[Organization rollout](/docs/en/llm-gateway-rollout)[Monitoring](/docs/en/monitoring-usage)
