...
**Source:** [https://code.claude.com/docs/en/sandboxing](https://code.claude.com/docs/en/sandboxing)
...
* **Environment variables**: sandboxed Bash commands inherit the parent process environment by default, including any credentials set there. Use [`sandbox.credentials`](#protect-credentials) to unset specific variables for sandboxed commands, or set [`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`](/docs/en/env-vars) to strip Anthropic and cloud provider credentials from all subprocesses.
...
[Sandbox environments](/docs/en/sandbox-environments)[Model configuration](/docs/en/model-config)
