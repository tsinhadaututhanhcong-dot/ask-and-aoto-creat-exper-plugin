...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/homeassistant](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/homeassistant)
...
### 1. 创建长期访问令牌[​](#1-创建长期访问令牌 "1. 创建长期访问令牌的直接链接")
...
### 3. 启动 Gateway[​](#3-启动-gateway "3. 启动 Gateway的直接链接")
...
### `ha_list_entities`[​](#ha_list_entities "ha_list_entities的直接链接")
...
### `ha_get_state`[​](#ha_get_state "ha_get_state的直接链接")
...
### `ha_list_services`[​](#ha_list_services "ha_list_services的直接链接")
...
### `ha_call_service`[​](#ha_call_service "ha_call_service的直接链接")
...
## Gateway 平台：实时事件[​](#gateway-平台实时事件 "Gateway 平台：实时事件的直接链接")
...
### Agent 响应[​](#agent-响应 "Agent 响应的直接链接")
...
### 响应式自动化（通过 Gateway 事件）[​](#响应式自动化通过-gateway-事件 "响应式自动化（通过 Gateway 事件）的直接链接")
...
* [配置](#配置)
  + [1. 创建长期访问令牌](#1-创建长期访问令牌)
  + [2. 配置环境变量](#2-配置环境变量)
  + [3. 启动 Gateway](#3-启动-gateway)
* [可用工具](#可用工具)
...
* [Gateway 平台：实时事件](#gateway-平台实时事件)
  + [事件过滤](#事件过滤)
  + [事件格式化](#事件格式化)
  + [Agent 响应](#agent-响应)
  + [连接管理](#连接管理)
...
* [自动化示例](#自动化示例)
  + [晨间例程](#晨间例程)
  + [安全检查](#安全检查)
  + [响应式自动化（通过 Gateway 事件）](#响应式自动化通过-gateway-事件)