# LINE | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/line](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/line)
...
## 第一步：创建 LINE Messaging API 频道[​](#第一步创建-line-messaging-api-频道 "第一步：创建 LINE Messaging API 频道的直接链接")
...
## 第四步：设置 webhook URL[​](#第四步设置-webhook-url "第四步：设置 webhook URL的直接链接")
...
## 第五步：运行 gateway[​](#第五步运行-gateway "第五步：运行 gateway的直接链接")
...
## LLM 响应缓慢[​](#llm-响应缓慢 "LLM 响应缓慢的直接链接")
...
## Cron / 通知推送[​](#cron--通知推送 "Cron / 通知推送的直接链接")
...
**`send_image` 报错"LINE\_PUBLIC\_URL must be set"。** LINE Messaging API 不接受二进制上传 — 图片、音频和视频必须是可访问的 HTTPS URL。将 `LINE_PUBLIC_URL` 设置为隧道的公网主机名，适配器会自动从 `/line/media/<token>/<filename>` 提供文件服务。
...
* [机器人响应方式](#机器人响应方式)
* [第一步：创建 LINE Messaging API 频道](#第一步创建-line-messaging-api-频道)
* [第二步：暴露 webhook 端口](#第二步暴露-webhook-端口)
* [第三步：配置 Hermes](#第三步配置-hermes)
* [第四步：设置 webhook URL](#第四步设置-webhook-url)
* [第五步：运行 gateway](#第五步运行-gateway)
* [LLM 响应缓慢](#llm-响应缓慢)
* [Cron / 通知推送](#cron--通知推送)
* [环境变量参考](#环境变量参考)
* [故障排查](#故障排查)
