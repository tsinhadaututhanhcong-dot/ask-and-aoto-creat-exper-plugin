# Slack | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/slack](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/slack)
...
## 第一步：创建 Slack 应用[​](#第一步创建-slack-应用 "第一步：创建 Slack 应用的直接链接")

最快的方式是粘贴 Hermes 为你生成的 manifest（清单文件）。它会一次性声明所有内置斜杠命令（`/btw`、`/stop`、`/model`……）、所有必需的 OAuth 权限范围、所有事件订阅，并启用 Socket Mode。

### 方式 A：使用 Hermes 生成的 manifest（推荐）[​](#方式-a使用-hermes-生成的-manifest推荐 "方式 A：使用 Hermes 生成的 manifest（推荐）的直接链接")
...
### 方式 B：从头手动创建[​](#方式-b从头手动创建 "方式 B：从头手动创建的直接链接")
...
## 第二步：配置 Bot Token 权限范围[​](#第二步配置-bot-token-权限范围 "第二步：配置 Bot Token 权限范围的直接链接")
...
## 第三步：启用 Socket Mode[​](#第三步启用-socket-mode "第三步：启用 Socket Mode的直接链接")
...
## 第四步：订阅事件[​](#第四步订阅事件 "第四步：订阅事件的直接链接")
...
## 第五步：启用 Messages Tab[​](#第五步启用-messages-tab "第五步：启用 Messages Tab的直接链接")
...
## 第六步：将应用安装到工作区[​](#第六步将应用安装到工作区 "第六步：将应用安装到工作区的直接链接")
...
## 第七步：查找用于白名单的用户 ID[​](#第七步查找用于白名单的用户-id "第七步：查找用于白名单的用户 ID的直接链接")
...
## 第八步：配置 Hermes[​](#第八步配置-hermes "第八步：配置 Hermes的直接链接")
...
## 第九步：将机器人邀请到频道[​](#第九步将机器人邀请到频道 "第九步：将机器人邀请到频道的直接链接")
...
### 更新后刷新斜杠命令[​](#更新后刷新斜杠命令 "更新后刷新斜杠命令的直接链接")
...
### 旧版 `/hermes <子命令>` 仍然有效[​](#旧版-hermes-子命令-仍然有效 "旧版-hermes-子命令-仍然有效的直接链接")
...
### 在话题（thread）中使用命令（`!cmd` 前缀）[​](#在话题thread中使用命令cmd-前缀 "在话题thread中使用命令cmd-前缀的直接链接")
...
### 高级：仅输出斜杠命令数组[​](#高级仅输出斜杠命令数组 "高级：仅输出斜杠命令数组的直接链接")
...
| `platforms.slack.extra.cron_continuable_surface` | `"thread"` | [可继续 cron 任务](/docs/zh-Hans/user-guide/features/cron)的投递方式。`"thread"` 为每次投递新建专用话题（默认）；`"in_channel"` 直接平铺投递到频道时间线。使用 `in_channel` 时需搭配 `reply_in_thread: false`（及 `require_mention: false`），纯文本回复即可继续任务。 |
...
### 频道白名单（`allowed_channels`）[​](#频道白名单allowed_channels "频道白名单allowed_channels的直接链接")
...
另见：[管理员/用户斜杠命令分离](/docs/zh-Hans/reference/slash-commands#permissions-and-adminuser-split)。

### 未授权用户处理[​](#未授权用户处理 "未授权用户处理的直接链接")
...
### OAuth Token 文件[​](#oauth-token-文件 "OAuth Token 文件的直接链接")
...
## 按频道设置 Prompt[​](#按频道设置-prompt "按频道设置 Prompt的直接链接")
...
| "not\_authed" 或 "invalid\_auth" 错误 | 重新生成 Bot Token 和 App Token，更新 `.env` |
...
* [概述](#概述)
* [第一步：创建 Slack 应用](#第一步创建-slack-应用)
  + [方式 A：使用 Hermes 生成的 manifest（推荐）](#方式-a使用-hermes-生成的-manifest推荐)
  + [方式 B：从头手动创建](#方式-b从头手动创建)
* [第二步：配置 Bot Token 权限范围](#第二步配置-bot-token-权限范围)
* [第三步：启用 Socket Mode](#第三步启用-socket-mode)
* [第四步：订阅事件](#第四步订阅事件)
* [第五步：启用 Messages Tab](#第五步启用-messages-tab)
* [第六步：将应用安装到工作区](#第六步将应用安装到工作区)
* [第七步：查找用于白名单的用户 ID](#第七步查找用于白名单的用户-id)
* [第八步：配置 Hermes](#第八步配置-hermes)
* [第九步：将机器人邀请到频道](#第九步将机器人邀请到频道)
* [斜杠命令](#斜杠命令)
  + [更新后刷新斜杠命令](#更新后刷新斜杠命令)
  + [旧版 `/hermes <子命令>` 仍然有效](#旧版-hermes-子命令-仍然有效)
  + [在话题（thread）中使用命令（`!cmd` 前缀）](#在话题thread中使用命令cmd-前缀)
  + [高级：仅输出斜杠命令数组](#高级仅输出斜杠命令数组)
* [机器人的响应方式](#机器人的响应方式)
* [配置选项](#配置选项)
  + [话题与回复行为](#话题与回复行为)
  + [会话隔离](#会话隔离)
  + [提及与触发行为](#提及与触发行为)
  + [频道白名单（`allowed_channels`）](#频道白名单allowed_channels)
  + [未授权用户处理](#未授权用户处理)
  + [语音转录](#语音转录)
  + [完整示例](#完整示例)
* [主频道](#主频道)
* [多工作区支持](#多工作区支持)
  + [配置](#配置)
  + [OAuth Token 文件](#oauth-token-文件)
  + [工作原理](#工作原理)
* [语音消息](#语音消息)
* [按频道设置 Prompt](#按频道设置-prompt)
* [按频道绑定技能](#按频道绑定技能)
* [故障排除](#故障排除)
  + [快速检查清单](#快速检查清单)
* [安全](#安全)
