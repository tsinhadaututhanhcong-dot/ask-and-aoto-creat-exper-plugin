...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/devops/devops-hermes-s6-container-supervision](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/devops/devops-hermes-s6-container-supervision)
...
| Related skills | [`hermes-agent`](/docs/zh-Hans/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent), `hermes-agent-dev` |
...
## Architecture at a glance[​](#architecture-at-a-glance "Architecture at a glance的直接链接")
...
## Key files[​](#key-files "Key files的直接链接")
...
## Why Architecture B (CMD as main program, not s6-supervised)[​](#why-architecture-b-cmd-as-main-program-not-s6-supervised "Why Architecture B (CMD as main program, not s6-supervised)的直接链接")
...
## Quick recipes[​](#quick-recipes "Quick recipes的直接链接")

### Verify s6 is PID 1 in a running container[​](#verify-s6-is-pid-1-in-a-running-container "Verify s6 is PID 1 in a running container的直接链接")
...
### Inspect a profile gateway service[​](#inspect-a-profile-gateway-service "Inspect a profile gateway service的直接链接")
...
### Bring a service up/down manually[​](#bring-a-service-updown-manually "Bring a service up/down manually的直接链接")
...
### Watch the cont-init reconciler log[​](#watch-the-cont-init-reconciler-log "Watch the cont-init reconciler log的直接链接")
...
### Add a new static service[​](#add-a-new-static-service "Add a new static service的直接链接")
...
### Change the per-profile gateway run command[​](#change-the-per-profile-gateway-run-command "Change the per-profile gateway run command的直接链接")
...
### Run the docker test harness[​](#run-the-docker-test-harness "Run the docker test harness的直接链接")
...
## Common pitfalls[​](#common-pitfalls "Common pitfalls的直接链接")

### "command not found" via `docker exec`[​](#command-not-found-via-docker-exec "command-not-found-via-docker-exec的直接链接")
...
### Profile directory ownership[​](#profile-directory-ownership "Profile directory ownership的直接链接")
...
### Files written by `docker exec` are root-owned[​](#files-written-by-docker-exec-are-root-owned "files-written-by-docker-exec-are-root-owned的直接链接")
...
### Service slot exists but s6-svstat says "s6-supervise not running"[​](#service-slot-exists-but-s6-svstat-says-s6-supervise-not-running "Service slot exists but s6-svstat says \"s6-supervise not running\"的直接链接")
...
### Gateway starts then immediately exits (`down (exitcode 1)` in svstat)[​](#gateway-starts-then-immediately-exits-down-exitcode-1-in-svstat "gateway-starts-then-immediately-exits-down-exitcode-1-in-svstat的直接链接")
...
### Reconciler skipped a profile[​](#reconciler-skipped-a-profile "Reconciler skipped a profile的直接链接")
...
### "Help, the container exits 143!"[​](#help-the-container-exits-143 "\"Help, the container exits 143!\"的直接链接")
...
## Related skills[​](#related-skills "Related skills的直接链接")
...
* [Key files](#key-files)
* [Why Architecture B (CMD as main program, not s6-supervised)](#why-architecture-b-cmd-as-main-program-not-s6-supervised)
* [Quick recipes](#quick-recipes)
  + [Verify s6 is PID 1 in a running container](#verify-s6-is-pid-1-in-a-running-container)
  + [Inspect a profile gateway service](#inspect-a-profile-gateway-service)
  + [Bring a service up/down manually](#bring-a-service-updown-manually)
  + [Watch the cont-init reconciler log](#watch-the-cont-init-reconciler-log)
  + [Add a new static service](#add-a-new-static-service)
  + [Change the per-profile gateway run command](#change-the-per-profile-gateway-run-command)
  + [Run the docker test harness](#run-the-docker-test-harness)
* [Common pitfalls](#common-pitfalls)
  + ["command not found" via `docker exec`](#command-not-found-via-docker-exec)
  + [Profile directory ownership](#profile-directory-ownership)
  + [Files written by `docker exec` are root-owned](#files-written-by-docker-exec-are-root-owned)
  + [Service slot exists but s6-svstat says "s6-supervise not running"](#service-slot-exists-but-s6-svstat-says-s6-supervise-not-running)
  + [Gateway starts then immediately exits (`down (exitcode 1)` in svstat)](#gateway-starts-then-immediately-exits-down-exitcode-1-in-svstat)
  + [Reconciler skipped a profile](#reconciler-skipped-a-profile)
  + ["Help, the container exits 143!"](#help-the-container-exits-143)
