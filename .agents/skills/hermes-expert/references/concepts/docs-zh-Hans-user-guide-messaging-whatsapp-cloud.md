...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/whatsapp-cloud](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/whatsapp-cloud)
...
If those constraints don't work for your use case, the [Baileys bridge integration](/docs/zh-Hans/user-guide/messaging/whatsapp.md) is the alternative — personal account, no public URL needed, but unofficial and ban-prone.

Which one should I use?

* **Cloud API (this guide)** — running a real business bot, want stability, fine with the Meta verification + template paperwork
* **[Baileys bridge](/docs/zh-Hans/user-guide/messaging/whatsapp.md)** — personal projects, quick demos, single-user setups, willing to risk the bot phone number's account
...
## Creating the Meta app[​](#creating-the-meta-app "Creating the Meta app的直接链接")
...
## Permanent token (production)[​](#permanent-token-production "Permanent token (production)的直接链接")
...
## Exposing Hermes to the internet[​](#exposing-hermes-to-the-internet "Exposing Hermes to the internet的直接链接")
...
### Cloudflare Tunnel (recommended)[​](#cloudflare-tunnel-recommended "Cloudflare Tunnel (recommended)的直接链接")
...
### Your own domain + reverse proxy[​](#your-own-domain--reverse-proxy "Your own domain + reverse proxy的直接链接")
...
## Configuring the webhook on Meta's side[​](#configuring-the-webhook-on-metas-side "Configuring the webhook on Meta's side的直接链接")
...
## Recipient whitelist (Meta-side)[​](#recipient-whitelist-meta-side "Recipient whitelist (Meta-side)的直接链接")
...
## Allowlist (Hermes-side)[​](#allowlist-hermes-side "Allowlist (Hermes-side)的直接链接")
...
## Polishing your bot's WhatsApp profile[​](#polishing-your-bots-whatsapp-profile "Polishing your bot's WhatsApp profile的直接链接")
...
## Configuration reference[​](#configuration-reference "Configuration reference的直接链接")
...
| **`WHATSAPP_CLOUD_ALLOWED_USERS`** | — | Comma-separated wa\_ids allowed to message the bot. |
...
| `WHATSAPP_CLOUD_HOME_CHANNEL` | — | wa\_id to use as the bot's home channel (for cron jobs etc). |
...
## Features[​](#features "Features的直接链接")

### Inbound[​](#inbound "Inbound的直接链接")
...
### Outbound[​](#outbound "Outbound的直接链接")
...
### Interactive UX[​](#interactive-ux "Interactive UX的直接链接")
...
### Read receipts and typing indicator[​](#read-receipts-and-typing-indicator "Read receipts and typing indicator的直接链接")
...
### Voice messages[​](#voice-messages "Voice messages的直接链接")
...
## Known limitations[​](#known-limitations "Known limitations的直接链接")

### 24-hour conversation window[​](#24-hour-conversation-window "24-hour conversation window的直接链接")
...
### Group chats[​](#group-chats "Group chats的直接链接")
...
### Outbound rate limit[​](#outbound-rate-limit "Outbound rate limit的直接链接")
...
### Setup verification fails ("URL couldn't be validated") in Meta dashboard[​](#setup-verification-fails-url-couldnt-be-validated-in-meta-dashboard "Setup verification fails (\"URL couldn't be validated\") in Meta dashboard的直接链接")
...
### `graph error 100`: Object with ID '...' does not exist[​](#graph-error-100-object-with-id--does-not-exist "graph-error-100-object-with-id--does-not-exist的直接链接")
...
### `graph error 190`: Authentication Error[​](#graph-error-190-authentication-error "graph-error-190-authentication-error的直接链接")
...
### `graph error 131047`: Re-engagement message[​](#graph-error-131047-re-engagement-message "graph-error-131047-re-engagement-message的直接链接")
...
### Inbound message: `media metadata fetch failed (status=401)`[​](#inbound-message-media-metadata-fetch-failed-status401 "inbound-message-media-metadata-fetch-failed-status401的直接链接")

Same 401 root causes as outbound (`graph error 190`) — the access token is invalid or expired. Fix the token.

### Bot replies appear as raw JSON / tool-call leakage[​](#bot-replies-appear-as-raw-json--tool-call-leakage "Bot replies appear as raw JSON / tool-call leakage的直接链接")
...
### STT (voice note transcription) returns empty / "could not transcribe"[​](#stt-voice-note-transcription-returns-empty--could-not-transcribe "STT (voice note transcription) returns empty / \"could not transcribe\"的直接链接")
...
## Security notes[​](#security-notes "Security notes的直接链接")
...
## Comparison to the Baileys bridge[​](#comparison-to-the-baileys-bridge "Comparison to the Baileys bridge的直接链接")
...
* [WhatsApp (Baileys bridge) Setup](/docs/zh-Hans/user-guide/messaging/whatsapp.md) — the alternative integration for personal projects.
* [Messaging Platforms overview](/docs/zh-Hans/user-guide/messaging/index.md) — all messaging integrations at a glance.

* [Quick start](#quick-start)
* [Prerequisites](#prerequisites)
* [Creating the Meta app](#creating-the-meta-app)
* [Permanent token (production)](#permanent-token-production)
* [Exposing Hermes to the internet](#exposing-hermes-to-the-internet)
  + [Cloudflare Tunnel (recommended)](#cloudflare-tunnel-recommended)
  + [ngrok](#ngrok)
  + [Your own domain + reverse proxy](#your-own-domain--reverse-proxy)
* [Configuring the webhook on Meta's side](#configuring-the-webhook-on-metas-side)
* [Recipient whitelist (Meta-side)](#recipient-whitelist-meta-side)
* [Allowlist (Hermes-side)](#allowlist-hermes-side)
* [Polishing your bot's WhatsApp profile](#polishing-your-bots-whatsapp-profile)
* [Configuration reference](#configuration-reference)
* [Features](#features)
  + [Inbound](#inbound)
  + [Outbound](#outbound)
  + [Interactive UX](#interactive-ux)
  + [Read receipts and typing indicator](#read-receipts-and-typing-indicator)
  + [Voice messages](#voice-messages)
* [Known limitations](#known-limitations)
  + [24-hour conversation window](#24-hour-conversation-window)
  + [Group chats](#group-chats)
  + [Outbound rate limit](#outbound-rate-limit)
* [Troubleshooting](#troubleshooting)
  + [Setup verification fails ("URL couldn't be validated") in Meta dashboard](#setup-verification-fails-url-couldnt-be-validated-in-meta-dashboard)
  ...
  + [Bot replies appear as raw JSON / tool-call leakage](#bot-replies-appear-as-raw-json--tool-call-leakage)
  + [STT (voice note transcription) returns empty / "could not transcribe"](#stt-voice-note-transcription-returns-empty--could-not-transcribe)
* [Security notes](#security-notes)
* [Comparison to the Baileys bridge](#comparison-to-the-baileys-bridge)
* [See also](#see-also)