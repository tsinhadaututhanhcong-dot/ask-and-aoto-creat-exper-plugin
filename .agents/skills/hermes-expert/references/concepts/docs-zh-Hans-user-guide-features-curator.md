...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/curator](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/curator)

本页总览

Curator 是针对 **agent 创建的技能**的后台维护流程。它跟踪每个技能被查看、使用和修补的频率，将长期未使用的技能经历 `active → stale → archived` 状态流转，并定期启动一个短暂的辅助模型审查，提出合并或修补漂移的建议。

它的存在是为了防止通过[自我改进循环](/docs/zh-Hans/user-guide/features/skills#agent-managed-skills-skill_manage-tool)创建的技能无限堆积。每次 agent 解决新问题并保存技能时，该技能都会落入 `~/.hermes/skills/`。若没有维护，最终会出现数十个范围狭窄的近似重复项，污染技能目录并浪费 token（令牌）。
...
### 在更便宜的辅助模型上运行审查[​](#在更便宜的辅助模型上运行审查 "在更便宜的辅助模型上运行审查的直接链接")
...
## "agent 创建"的含义[​](#agent-创建的含义 "\"agent 创建\"的含义的直接链接")
...
## 固定技能[​](#pinning-a-skill "固定技能的直接链接")
...
### 摘要中的重命名映射[​](#摘要中的重命名映射 "摘要中的重命名映射的直接链接")
...
## 恢复已归档的技能[​](#恢复已归档的技能 "恢复已归档的技能的直接链接")
...
* [技能系统](/docs/zh-Hans/user-guide/features/skills)——技能的总体工作原理及创建技能的自我改进循环
* [内存](/docs/zh-Hans/user-guide/features/memory)——维护长期记忆的并行后台审查
* [捆绑技能目录](/docs/zh-Hans/reference/skills-catalog)
* [Issue #7816](https://github.com/NousResearch/hermes-agent/issues/7816)——原始提案与设计讨论

* [运行方式](#运行方式)
* [配置](#配置)
  + [在更便宜的辅助模型上运行审查](#在更便宜的辅助模型上运行审查)
* [CLI](#cli)
* [备份与回滚](#备份与回滚)
* ["agent 创建"的含义](#agent-创建的含义)
