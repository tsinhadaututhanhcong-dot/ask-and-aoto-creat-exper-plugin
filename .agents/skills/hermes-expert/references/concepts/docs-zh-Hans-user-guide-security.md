# 安全 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/security](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/security)
...
对于破坏性会话斜杠命令（`/clear`、`/new` / `/reset`、`/undo`、`/exit --delete`），CLI 在执行前也会提示确认。参见[斜杠命令——破坏性命令的确认提示](/docs/zh-Hans/reference/slash-commands#confirmation-prompts-for-destructive-commands)。

### 硬性黑名单（始终生效的底线）[​](#硬性黑名单始终生效的底线 "硬性黑名单（始终生效的底线）的直接链接")
...
### 审批流程（CLI）[​](#审批流程cli "审批流程（CLI）的直接链接")
...
### 审批流程（Gateway/消息平台）[​](#审批流程gateway消息平台 "审批流程（Gateway/消息平台）的直接链接")
...
## 用户授权（Gateway）[​](#用户授权gateway "用户授权（Gateway）的直接链接")

运行消息 gateway 时，Hermes 通过分层授权系统控制谁可以与机器人交互。

### 授权检查顺序[​](#授权检查顺序 "授权检查顺序的直接链接")
...
### Docker 安全标志[​](#docker-安全标志 "Docker 安全标志的直接链接")
...
## 终端后端安全对比[​](#终端后端安全对比 "终端后端安全对比的直接链接")
...
## 环境变量透传[​](#environment-variable-passthrough "环境变量透传的直接链接")
...
### 凭据文件透传（OAuth token 等）[​](#credential-file-passthrough "凭据文件透传（OAuth token 等）的直接链接")
...
### 各沙箱的过滤规则[​](#各沙箱的过滤规则 "各沙箱的过滤规则的直接链接")

| 沙箱 | 默认过滤 | 透传覆盖 |
| --- | --- | --- |
| **execute\_code** | 阻止名称中包含 `KEY`、`TOKEN`、`SECRET`、`PASSWORD`、`CREDENTIAL`、`PASSWD`、`AUTH` 的变量；仅允许安全前缀变量通过 | ✅ 透传变量绕过两项检查 |
...
## MCP 凭据处理[​](#mcp-凭据处理 "MCP 凭据处理的直接链接")

MCP（Model Context Protocol）服务器子进程接收**经过过滤的环境**，以防止意外泄露凭据。

### 安全环境变量[​](#安全环境变量 "安全环境变量的直接链接")
...
完整详情请参见配置指南中的[网站黑名单](/docs/zh-Hans/user-guide/configuration#website-blocklist)。

### SSRF 防护[​](#ssrf-防护 "SSRF 防护的直接链接")
...
#### 有意允许私有 URL[​](#有意允许私有-url "有意允许私有 URL的直接链接")
...
### Tirith 预执行安全扫描[​](#tirith-预执行安全扫描 "Tirith 预执行安全扫描的直接链接")
...
Tirith 为 Linux（x86\_64 / aarch64）和 macOS（x86\_64 / arm64）提供预构建二进制文件。在没有预构建二进制文件的平台（Windows 等）上，tirith 会被静默跳过——模式匹配防护仍然运行，CLI 不会显示"不可用"横幅。若要在 Windows 上使用 tirith，请在 WSL 下运行 Hermes。

Tirith 的判定与审批流程集成：安全命令直接通过，可疑和被阻止的命令会触发用户审批，并附上完整的 tirith 发现（严重性、标题、描述、更安全的替代方案）。用户可以批准或拒绝——默认选择为拒绝，以确保无人值守场景的安全。

### 上下文文件注入防护[​](#上下文文件注入防护 "上下文文件注入防护的直接链接")
...
## 生产部署最佳实践[​](#生产部署最佳实践 "生产部署最佳实践的直接链接")

### Gateway 部署检查清单[​](#gateway-部署检查清单 "Gateway 部署检查清单的直接链接")
...
### 保护 API 密钥[​](#保护-api-密钥 "保护 API 密钥的直接链接")
...
## 供应链安全公告检查[​](#供应链安全公告检查 "供应链安全公告检查的直接链接")
...
### 可选依赖的懒加载安装[​](#可选依赖的懒加载安装 "可选依赖的懒加载安装的直接链接")
...
* [危险命令审批](#危险命令审批)
  + [审批模式](#审批模式)
  + [YOLO 模式](#yolo-模式)
  + [硬性黑名单（始终生效的底线）](#硬性黑名单始终生效的底线)
  + [审批超时](#审批超时)
  + [触发审批的条件](#触发审批的条件)
  + [审批流程（CLI）](#审批流程cli)
  + [审批流程（Gateway/消息平台）](#审批流程gateway消息平台)
  + [永久允许列表](#永久允许列表)
* [用户授权（Gateway）](#用户授权gateway)
  + [授权检查顺序](#授权检查顺序)
  + [平台允许列表](#平台允许列表)
  + [DM 配对系统](#dm-配对系统)
* [容器隔离](#容器隔离)
  + [Docker 安全标志](#docker-安全标志)
  + [资源限制](#资源限制)
  + [文件系统持久化](#文件系统持久化)
* [终端后端安全对比](#终端后端安全对比)
...
* [MCP 凭据处理](#mcp-凭据处理)
  ...
  + [网站访问策略](#网站访问策略)
  + [SSRF 防护](#ssrf-防护)
  + [Tirith 预执行安全扫描](#tirith-预执行安全扫描)
  + [上下文文件注入防护](#上下文文件注入防护)
* [生产部署最佳实践](#生产部署最佳实践)
  + [Gateway 部署检查清单](#gateway-部署检查清单)
  + [保护 API 密钥](#保护-api-密钥)
  + [网络隔离](#网络隔离)
* [供应链安全公告检查](#供应链安全公告检查)
  + [可选依赖的懒加载安装](#可选依赖的懒加载安装)
