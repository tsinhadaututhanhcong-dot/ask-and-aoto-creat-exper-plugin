# 凭证池 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/credential-pools](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/credential-pools)

本页总览

凭证池允许你为同一提供商注册多个 API 密钥或 OAuth 令牌。当某个密钥触达速率限制或计费配额时，Hermes 会自动轮换到下一个健康密钥——在不切换提供商的情况下保持会话持续运行。

这与[备用提供商](/docs/zh-Hans/user-guide/features/fallback-providers)不同，后者会切换到*另一个*提供商。凭证池是同一提供商内的轮换；备用提供商是跨提供商的故障转移。池会优先尝试——如果池中所有密钥都耗尽，*才会*激活备用提供商。
...
## 委托与子代理共享[​](#委托与子代理共享 "委托与子代理共享的直接链接")
