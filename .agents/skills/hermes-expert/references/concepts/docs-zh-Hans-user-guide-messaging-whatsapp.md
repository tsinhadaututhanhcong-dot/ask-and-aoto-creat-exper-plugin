...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/whatsapp](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/whatsapp)
...
## 第一步：运行配置向导[​](#第一步运行配置向导 "第一步：运行配置向导的直接链接")
...
## 第二步：获取第二个手机号（机器人模式）[​](#第二步获取第二个手机号机器人模式 "第二步：获取第二个手机号（机器人模式）的直接链接")
...
这与 [Signal 群组白名单](/docs/zh-Hans/reference/environment-variables) 保持一致。
如需使用配对流程，请移除这两个变量，改用
[私信配对系统](/docs/zh-Hans/user-guide/security#dm-pairing-system)。
...
## 消息格式与投递[​](#消息格式与投递 "消息格式与投递的直接链接")

WhatsApp 支持**流式（渐进式）响应**——机器人在 AI 生成文本时实时编辑消息，与 Discord 和 Telegram 一样。在内部，WhatsApp 被归类为 TIER\_MEDIUM 平台（投递能力中等）。

### 分块[​](#分块 "分块的直接链接")

长响应会自动按每块 **4,096 个字符**拆分为多条消息（WhatsApp 的实际显示上限）。无需任何配置——gateway 会自动处理拆分并按顺序发送各块。

### WhatsApp 兼容 Markdown[​](#whatsapp-兼容-markdown "WhatsApp 兼容 Markdown的直接链接")
...
| **macOS："Node.js not installed"但终端中 node 可用** | launchd 服务不继承你的 shell PATH。运行 `hermes gateway install` 将当前 PATH 重新快照到 plist 中，然后运行 `hermes gateway start`。详见 [Gateway 服务文档](/docs/zh-Hans/user-guide/messaging/#macos-launchd)。 |
...
* [两种模式](#两种模式)
* [前置条件](#前置条件)
* [第一步：运行配置向导](#第一步运行配置向导)
* [第二步：获取第二个手机号（机器人模式）](#第二步获取第二个手机号机器人模式)
...
* [消息格式与投递](#消息格式与投递)
  + [分块](#分块)
  + [WhatsApp 兼容 Markdown](#whatsapp-兼容-markdown)
  + [工具进度](#工具进度)
* [故障排查](#故障排查)
* [安全](#安全)
