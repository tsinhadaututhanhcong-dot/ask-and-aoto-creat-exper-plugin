# 环境变量 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables)

本页总览

所有变量均写入 `~/.hermes/.env`。也可以使用 `hermes config set VAR value` 进行设置。

## LLM 提供商[​](#llm-提供商 "LLM 提供商的直接链接")
...
| `AZURE_AUTHORITY_HOST` | 主权云 authority 覆盖（例如 Azure Government 使用 `https://login.microsoftonline.us`）。参见 [Azure Foundry 指南](/docs/zh-Hans/guides/azure-foundry#sovereign-clouds-government-china) |
...
| `HERMES_GIT_BASH_PATH` | **仅 Windows。** 覆盖终端工具的 `bash.exe` 发现路径。可指向任意 bash——完整 Git-for-Windows 安装、通过符号链接的 WSL bash、MSYS2、Cygwin。安装程序会自动将其设置为所配置的 PortableGit。参见 [Windows（原生）指南](/docs/zh-Hans/user-guide/windows-native#how-hermes-runs-shell-commands-on-windows) |
...
## 提供商认证（OAuth）[​](#提供商认证oauth "提供商认证（OAuth）的直接链接")
...
### Langfuse 可观测性[​](#langfuse-可观测性 "Langfuse 可观测性的直接链接")

内置 [`observability/langfuse`](/docs/zh-Hans/user-guide/features/built-in-plugins#observabilitylangfuse) 插件的环境变量。在 `~/.hermes/.env` 中设置。在这些变量生效之前，还必须启用该插件（`hermes plugins enable observability/langfuse`，或在 `hermes plugins` 中勾选）。
...
### Nous Tool Gateway[​](#nous-tool-gateway "Nous Tool Gateway的直接链接")

这些变量为付费 Nous 订阅者或自托管 gateway 部署配置 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)。大多数用户无需设置——gateway 通过 `hermes model` 或 `hermes tools` 自动配置。
...
## 容器资源（Docker、Singularity、Modal、Daytona）[​](#容器资源dockersingularitymodaldaytona "容器资源（Docker、Singularity、Modal、Daytona）的直接链接")
...
| `DISCORD_ALLOW_MENTION_EVERYONE` | 允许 bot ping `@everyone`/`@here`（默认：`false`）。参见 [Mention 控制](/docs/zh-Hans/user-guide/messaging/discord#mention-control)。 |
...
| `FEISHU_ALLOW_BOTS` | `none`（默认）/`mentions`/`all`——接受来自其他 bot 的入站消息。参见 [bot 间消息传递](/docs/zh-Hans/user-guide/messaging/feishu#bot-to-bot-messaging) |
...
| `GATEWAY_PROXY_URL` | 将消息转发到的远程 Hermes API 服务器 URL（[代理模式](/docs/zh-Hans/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)）。设置后，gateway 仅处理平台 I/O——所有 agent 工作委托给远程服务器。也可通过 `config.yaml` 中的 `gateway.proxy_url` 配置。 |
...
### Microsoft Graph（Teams 会议）[​](#microsoft-graphteams-会议 "Microsoft Graph（Teams 会议）的直接链接")

用于即将推出的 Teams 会议摘要流水线的 Microsoft Graph REST 客户端的仅应用凭证。Azure 门户操作步骤和所需 API 权限详见[注册 Microsoft Graph 应用程序](/docs/zh-Hans/guides/microsoft-graph-app-registration)。
...
### Microsoft Graph Webhook 监听器[​](#microsoft-graph-webhook-监听器 "Microsoft Graph Webhook 监听器的直接链接")

Graph 事件（Teams 会议、日历、聊天等）的入站变更通知监听器。设置和安全加固详见 [Microsoft Graph Webhook 监听器](/docs/zh-Hans/user-guide/messaging/msgraph-webhook)。
...
### Teams 会议摘要投递[​](#teams-会议摘要投递 "Teams 会议摘要投递的直接链接")

仅在启用 [`teams_pipeline` 插件](/docs/zh-Hans/user-guide/messaging/msgraph-webhook)时使用。设置也可在 `config.yaml` 的 `platforms.teams.extra` 下配置——两者都设置时环境变量优先。参见 [Microsoft Teams → 会议摘要投递](/docs/zh-Hans/user-guide/messaging/teams#meeting-summary-delivery-teams-meeting-pipeline)。
...
### LINE Messaging API[​](#line-messaging-api "LINE Messaging API的直接链接")

由内置 LINE 平台插件（`plugins/platforms/line/`）使用。完整设置详见 [消息 Gateway → LINE](/docs/zh-Hans/user-guide/messaging/line)。
...
### ntfy（推送通知）[​](#ntfy推送通知 "ntfy（推送通知）的直接链接")
...
在使用不受信任的话题部署前，请参阅 [ntfy 消息指南](/docs/zh-Hans/user-guide/messaging/ntfy)——特别是**身份模型**部分。

### 高级消息调优[​](#高级消息调优 "高级消息调优的直接链接")
...
## Agent 行为[​](#agent-行为 "Agent 行为的直接链接")
...
| `CODEX_HOME` | 启用 [Codex 应用服务器运行时](/docs/zh-Hans/user-guide/features/codex-app-server-runtime)时，覆盖 Codex CLI 读取其配置 + 认证的目录（默认：`~/.codex`）。Hermes 的迁移将托管块写入 `<CODEX_HOME>/config.toml`。 |
...
| 变量 | 描述 |
| --- | --- |
| `HERMES_TUI` | 设为 `1` 时启动 [TUI](/docs/zh-Hans/user-guide/tui) 而非经典 CLI。等同于传入 `--tui`。 |
...
## 上下文压缩（仅 config.yaml）[​](#上下文压缩仅-configyaml "上下文压缩（仅 config.yaml）的直接链接")
...
## 回退提供商（仅 config.yaml）[​](#回退提供商仅-configyaml "回退提供商（仅 config.yaml）的直接链接")
...
详见 [回退提供商](/docs/zh-Hans/user-guide/features/fallback-providers)。

## 提供商路由（仅 config.yaml）[​](#提供商路由仅-configyaml "提供商路由（仅 config.yaml）的直接链接")
...
* [LLM 提供商](#llm-提供商)
* [提供商认证（OAuth）](#提供商认证oauth)
* [工具 API](#工具-api)
  + [Langfuse 可观测性](#langfuse-可观测性)
  + [Nous Tool Gateway](#nous-tool-gateway)
* [终端后端](#终端后端)
* [SSH 后端](#ssh-后端)
* [容器资源（Docker、Singularity、Modal、Daytona）](#容器资源dockersingularitymodaldaytona)
* [持久 Shell](#持久-shell)
* [消息平台](#消息平台)
  + [Microsoft Graph（Teams 会议）](#microsoft-graphteams-会议)
  + [Microsoft Graph Webhook 监听器](#microsoft-graph-webhook-监听器)
  + [Teams 会议摘要投递](#teams-会议摘要投递)
...
* [界面](#界面)
* [会话设置](#会话设置)
* [上下文压缩（仅 config.yaml）](#上下文压缩仅-configyaml)
* [辅助任务覆盖](#辅助任务覆盖)
* [回退提供商（仅 config.yaml）](#回退提供商仅-configyaml)
* [提供商路由（仅 config.yaml）](#提供商路由仅-configyaml)

## Related Files
> **LLM Navigation:** Các tệp dưới đây được liên kết trực tiếp từ tài liệu này. Hãy đọc chúng nếu cần thêm ngữ cảnh.

- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/azure-foundry](./docs-zh-Hans-guides-azure-foundry.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/codex-app-server-runtime](./docs-zh-Hans-user-guide-features-codex-app-server-runtime.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/line](./docs-zh-Hans-user-guide-messaging-line.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/ntfy](./docs-zh-Hans-user-guide-messaging-ntfy.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/windows-native](./docs-zh-Hans-user-guide-windows-native.md)
