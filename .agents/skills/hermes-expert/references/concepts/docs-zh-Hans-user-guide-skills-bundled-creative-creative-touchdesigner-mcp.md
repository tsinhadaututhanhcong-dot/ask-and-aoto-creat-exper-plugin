...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-touchdesigner-mcp](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-touchdesigner-mcp)
...
| 相关 skill | [`native-mcp`](/docs/zh-Hans/user-guide/skills/bundled/mcp/mcp-native-mcp), [`ascii-video`](/docs/zh-Hans/user-guide/skills/bundled/creative/creative-ascii-video), [`manim-video`](/docs/zh-Hans/user-guide/skills/bundled/creative/creative-manim-video), `hermes-video` |
...
4. **优先使用原生 MCP 工具，而非 td\_execute\_python。** 使用 `td_create_operator`、`td_set_operator_pars`、`td_get_errors` 等。仅在复杂多步骤逻辑时回退到 `td_execute_python`。
...
### 手动步骤（一次性，无法自动化）[​](#手动步骤一次性无法自动化 "手动步骤（一次性，无法自动化）的直接链接")
...
### 第 0 步：探索（构建任何内容之前）[​](#第-0-步探索构建任何内容之前 "第 0 步：探索（构建任何内容之前）的直接链接")
...
### 第 1 步：清理 + 构建[​](#第-1-步清理--构建 "第 1 步：清理 + 构建的直接链接")
...
### 第 2 步：设置参数[​](#第-2-步设置参数 "第 2 步：设置参数的直接链接")
...
### 第 3 步：连线[​](#第-3-步连线 "第 3 步：连线的直接链接")
...
### 第 4 步：验证[​](#第-4-步验证 "第 4 步：验证的直接链接")
...
### 第 5 步：显示 / 捕获[​](#第-5-步显示--捕获 "第 5 步：显示 / 捕获的直接链接")
...
## MCP 工具快速参考[​](#mcp-工具快速参考 "MCP 工具快速参考的直接链接")
...
### 录制前：检查清单[​](#录制前检查清单 "录制前：检查清单的直接链接")
...
## 音频响应式 GLSL（经过验证的方案）[​](#音频响应式-glsl经过验证的方案 "音频响应式 GLSL（经过验证的方案）的直接链接")

### 正确的信号链（2026 年 4 月测试）[​](#正确的信号链2026-年-4-月测试 "正确的信号链（2026 年 4 月测试）的直接链接")
...
### 关键音频响应式规则（经验证）[​](#关键音频响应式规则经验证 "关键音频响应式规则（经验证）的直接链接")
...
### GLSL 频谱采样[​](#glsl-频谱采样 "GLSL 频谱采样的直接链接")
...
* [架构](#架构)
* [设置（自动化）](#设置自动化)
  + [手动步骤（一次性，无法自动化）](#手动步骤一次性无法自动化)
* [环境说明](#环境说明)
* [工作流程](#工作流程)
  + [第 0 步：探索（构建任何内容之前）](#第-0-步探索构建任何内容之前)
  + [第 1 步：清理 + 构建](#第-1-步清理--构建)
  + [第 2 步：设置参数](#第-2-步设置参数)
  + [第 3 步：连线](#第-3-步连线)
  + [第 4 步：验证](#第-4-步验证)
  + [第 5 步：显示 / 捕获](#第-5-步显示--捕获)
* [MCP 工具快速参考](#mcp-工具快速参考)
* [关键实现规则](#关键实现规则)
* [录制 / 导出视频](#录制--导出视频)
  + [录制前：检查清单](#录制前检查清单)
* [音频响应式 GLSL（经过验证的方案）](#音频响应式-glsl经过验证的方案)
  + [正确的信号链（2026 年 4 月测试）](#正确的信号链2026-年-4-月测试)
  + [关键音频响应式规则（经验证）](#关键音频响应式规则经验证)
  + [GLSL 频谱采样](#glsl-频谱采样)
* [算子快速参考](#算子快速参考)
* [安全说明](#安全说明)
* [参考资料](#参考资料)
