# Kanban 教程 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban-tutorial](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban-tutorial)

本页总览

Hermes Kanban 系统所设计的四个使用场景的完整演示，需在浏览器中打开 dashboard。如果你还没有阅读 [Kanban 概述](/docs/zh-Hans/user-guide/features/kanban)，请先从那里开始——本文假设你已了解 task（任务）、run（运行）、assignee（负责人）和 dispatcher（调度器）的概念。
...
dashboard 是**你**观察系统最便捷的地方。dispatcher 生成的 agent worker 不会看到 dashboard 或 CLI——它们通过专用的 `kanban_*` [工具集](/docs/zh-Hans/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_list`、`kanban_complete`、`kanban_block`、`kanban_heartbeat`、`kanban_comment`、`kanban_create`、`kanban_link`、`kanban_unblock`）来操作看板。三个界面——dashboard、CLI、worker 工具——都通过同一个每看板独立的 SQLite 数据库（默认看板为 `~/.hermes/kanban.db`，后续创建的任意看板为 `~/.hermes/kanban/boards/<slug>/kanban.db`）进行路由，因此无论变更来自哪一侧，每个看板的数据始终一致。

本教程全程使用 `default` 看板。如果你需要多个隔离队列（每个项目/仓库/领域一个），请参阅概述中的[看板（多项目）](/docs/zh-Hans/user-guide/features/kanban#boards-multi-project)——相同的 CLI/dashboard/worker 流程适用于每个看板，且 worker 在物理上无法看到其他看板上的任务。

在本教程中，**标注为 `bash` 的代码块是*你*运行的命令。** 标注为 `# worker tool calls` 的代码块是生成的 worker 模型发出的工具调用——展示在这里是为了让你能端到端地了解整个循环，而不是让你自己去运行它们。

## 看板概览[​](#看板概览 "看板概览的直接链接")

![Kanban board overview](/docs/zh-Hans/assets/images/01-board-overview-dd98ac39283d95534d6df59f90df98b0.png)

从左到右共六列：

* **Triage（分类）** — 原始想法。默认情况下，dispatcher 会对此处的任务自动运行**分解器**（orchestrator 驱动的扇出）：它读取你的 profile 名册和描述，生成一张子任务图，将任务路由给最合适的专家，同时保持原始任务作为父任务存活，以便在所有子任务完成后 orchestrator 重新唤醒来判断完成情况。点击 kanban 页面顶部的 **Orchestration: Auto/Manual** 切换按钮来切换模式。在 Manual 模式下（或没有 orchestrator profile 的配置中），点击卡片上的 **⚗ Decompose**，或运行 `hermes kanban decompose <id>` / `/kanban decompose <id>`。对于不需要扇出的单个任务，**✨ Specify** 会进行一次性规格重写（目标、方法、验收标准）并将任务提升到 `todo`。在 `config.yaml` 的 `auxiliary.kanban_decomposer` 和 `auxiliary.triage_specifier` 下配置相关模型。参见主 Kanban 指南中的[自动与手动编排](/docs/zh-Hans/user-guide/features/kanban#auto-vs-manual-orchestration)。
...
![Board with lanes by profile off](/docs/zh-Hans/assets/images/02-board-flat-019a7cf4f943c0ff4d95ffa7d579bf25.png)

## 场景一 — 独立开发者交付功能[​](#场景一--独立开发者交付功能 "场景一 — 独立开发者交付功能的直接链接")
...
![Solo dev — completed schema task drawer](/docs/zh-Hans/assets/images/03-drawer-schema-task-217f2993df9ebe7c0f2a678df97d934e.png)
...
## 场景二 — 集群并行处理[​](#场景二--集群并行处理 "场景二 — 集群并行处理的直接链接")
...
![Fleet view filtered to transcribe tasks](/docs/zh-Hans/assets/images/07-fleet-transcribes-cf7345a3270df63f9336252cada8da61.png)
...
## 场景三 — 角色流水线与重试[​](#场景三--角色流水线与重试 "场景三 — 角色流水线与重试的直接链接")

这正是 Kanban 相比普通 TODO 列表的价值所在。PM 编写规格说明，工程师实现，审查者拒绝第一次尝试，工程师修改后再次尝试，审查者批准。

dashboard 视图，按 `auth-project` 筛选：

![Pipeline view for a multi-role feature](/docs/zh-Hans/assets/images/08-pipeline-auth-2dc4dcc631cf371f649ad70ab1e71611.png)
...
![Implementation task with two runs — blocked then completed](/docs/zh-Hans/assets/images/04b-drawer-retry-history-scrolled-356e4195a55572cfc522c5236f02c594.png)
...
![Reviewer&#39;s drawer view of the pipeline](/docs/zh-Hans/assets/images/09-drawer-pipeline-review-12ea45e97703570d5e889812347a0778.png)
...
## 场景四 — 熔断器与崩溃恢复[​](#场景四--熔断器与崩溃恢复 "场景四 — 熔断器与崩溃恢复的直接链接")
...
### 熔断器 — 持续性失败[​](#熔断器--持续性失败 "熔断器 — 持续性失败的直接链接")
...
![Circuit breaker — 2 spawn_failed + 1 gave_up](/docs/zh-Hans/assets/images/11-drawer-gave-up-04cdec442b239748a9904b8651efde1d.png)
...
### 崩溃恢复 — worker 在运行中途死亡[​](#崩溃恢复--worker-在运行中途死亡 "崩溃恢复 — worker 在运行中途死亡的直接链接")
...
![Crash and recovery — 1 crashed + 1 completed](/docs/zh-Hans/assets/images/06-drawer-crash-recovery-17edcbea131d965bb8dc7283e5622e35.png)
...
## 结构化交接 — `summary` 和 `metadata` 的重要性[​](#结构化交接--summary-和-metadata-的重要性 "结构化交接--summary-和-metadata-的重要性的直接链接")
...
## 检查当前正在运行的任务[​](#检查当前正在运行的任务 "检查当前正在运行的任务的直接链接")

作为补充——以下是一个仍在执行中的任务的抽屉视图（场景一中的 API 实现，已被 `backend-dev` 认领但尚未完成）：

![Claimed, in-flight task](/docs/zh-Hans/assets/images/10-drawer-in-flight-0b6c924e986c7862439f00d776e432f4.png)
...
* [Kanban 概述](/docs/zh-Hans/user-guide/features/kanban) — 完整的数据模型、事件词汇表和 CLI 参考。
...
* [看板概览](#看板概览)
  + [平铺视图](#平铺视图)
* [场景一 — 独立开发者交付功能](#场景一--独立开发者交付功能)
* [场景二 — 集群并行处理](#场景二--集群并行处理)
* [场景三 — 角色流水线与重试](#场景三--角色流水线与重试)
* [场景四 — 熔断器与崩溃恢复](#场景四--熔断器与崩溃恢复)
  + [熔断器 — 持续性失败](#熔断器--持续性失败)
  + [崩溃恢复 — worker 在运行中途死亡](#崩溃恢复--worker-在运行中途死亡)
* [结构化交接 — `summary` 和 `metadata` 的重要性](#结构化交接--summary-和-metadata-的重要性)
* [检查当前正在运行的任务](#检查当前正在运行的任务)
* [后续步骤](#后续步骤)
