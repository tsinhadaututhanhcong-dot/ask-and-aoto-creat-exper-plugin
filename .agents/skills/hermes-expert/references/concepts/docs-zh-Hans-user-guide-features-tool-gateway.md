# Nous Tool Gateway（工具网关） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tool-gateway](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tool-gateway)
...
## 如何启用 Tool Gateway[​](#如何启用-tool-gateway "如何启用 Tool Gateway的直接链接")

### 在模型配置流程中[​](#在模型配置流程中 "在模型配置流程中的直接链接")
...
### 通过 `hermes tools`[​](#通过-hermes-tools "通过-hermes-tools的直接链接")
...
## 切回直连 Key[​](#切回直连-key "切回直连 Key的直接链接")
...
* [进阶：自建网关](#进阶自建网关)
* [常见问题](#常见问题)
  + [需要删掉已有的 API Key 吗？](#需要删掉已有的-api-key-吗)
  + [能否部分工具走网关、部分走直连？](#能否部分工具走网关部分走直连)
  + [订阅到期会怎样？](#订阅到期会怎样)
  + [与「消息网关」（各聊天平台）是否冲突？](#与消息网关各聊天平台是否冲突)
  + [Modal 算在 Tool Gateway 里吗？](#modal-算在-tool-gateway-里吗)