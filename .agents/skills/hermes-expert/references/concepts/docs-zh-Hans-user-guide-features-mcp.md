# MCP（模型上下文协议） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/mcp](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/mcp)
...
## MCP 能给你带来什么[​](#mcp-能给你带来什么 "MCP 能给你带来什么的直接链接")
...
## 两种 MCP 服务器[​](#两种-mcp-服务器 "两种 MCP 服务器的直接链接")

### Stdio 服务器[​](#stdio-服务器 "Stdio 服务器的直接链接")
...
### HTTP 服务器[​](#http-服务器 "HTTP 服务器的直接链接")
...
### 最简 stdio 示例[​](#最简-stdio-示例 "最简 stdio 示例的直接链接")
...
### 最简 HTTP 示例[​](#最简-http-示例 "最简 HTTP 示例的直接链接")
...
## Hermes 注册 MCP 工具的方式[​](#hermes-注册-mcp-工具的方式 "Hermes 注册 MCP 工具的方式的直接链接")
...
## MCP 实用工具[​](#mcp-实用工具 "MCP 实用工具的直接链接")
...
## 按服务器过滤[​](#按服务器过滤 "按服务器过滤的直接链接")

你可以控制每个 MCP 服务器向 Hermes 贡献哪些工具，从而精细管理工具命名空间。

### 完全禁用某个服务器[​](#完全禁用某个服务器 "完全禁用某个服务器的直接链接")
...
### 白名单过滤服务器工具[​](#白名单过滤服务器工具 "白名单过滤服务器工具的直接链接")
...
### 黑名单过滤服务器工具[​](#黑名单过滤服务器工具 "黑名单过滤服务器工具的直接链接")
...
### 同样可过滤实用工具[​](#同样可过滤实用工具 "同样可过滤实用工具的直接链接")
...
### Stdio 环境变量过滤[​](#stdio-环境变量过滤 "Stdio 环境变量过滤的直接链接")

对于 stdio 服务器，Hermes 不会盲目传递你的完整 shell 环境。

只有显式配置的 `env` 加上安全基线才会被传递。这减少了意外泄露密钥的风险。

### 配置层面的暴露控制[​](#配置层面的暴露控制 "配置层面的暴露控制的直接链接")
...
### GitHub 服务器，仅暴露最小 issue 管理接口[​](#github-服务器仅暴露最小-issue-管理接口 "GitHub 服务器，仅暴露最小 issue 管理接口的直接链接")
...
### Stripe 服务器，移除危险操作[​](#stripe-服务器移除危险操作 "Stripe 服务器，移除危险操作的直接链接")
...
### 文件系统服务器，限定单个项目根目录[​](#文件系统服务器限定单个项目根目录 "文件系统服务器，限定单个项目根目录的直接链接")
...
### 为什么资源或 prompt 实用工具没有出现？[​](#为什么资源或-prompt-实用工具没有出现 "为什么资源或 prompt 实用工具没有出现？的直接链接")
...
## MCP Sampling 支持[​](#mcp-sampling-支持 "MCP Sampling 支持的直接链接")
...
## 将 Hermes 作为 MCP 服务器运行[​](#将-hermes-作为-mcp-服务器运行 "将 Hermes 作为 MCP 服务器运行的直接链接")
...
### MCP 客户端配置[​](#mcp-客户端配置 "MCP 客户端配置的直接链接")
...
* [CLI 命令](/docs/zh-Hans/reference/cli-commands)
* [斜杠命令](/docs/zh-Hans/reference/slash-commands)
* [常见问题](/docs/zh-Hans/reference/faq)

* [MCP 能给你带来什么](#mcp-能给你带来什么)
* [快速开始](#快速开始)
* [两种 MCP 服务器](#两种-mcp-服务器)
  + [Stdio 服务器](#stdio-服务器)
  + [HTTP 服务器](#http-服务器)
* [基本配置参考](#基本配置参考)
  + [常用字段](#常用字段)
  + [最简 stdio 示例](#最简-stdio-示例)
  + [最简 HTTP 示例](#最简-http-示例)
* [内置预设](#内置预设)
* [Hermes 注册 MCP 工具的方式](#hermes-注册-mcp-工具的方式)
* [MCP 实用工具](#mcp-实用工具)
  + [重要说明](#重要说明)
* [按服务器过滤](#按服务器过滤)
  + [完全禁用某个服务器](#完全禁用某个服务器)
  + [白名单过滤服务器工具](#白名单过滤服务器工具)
  + [黑名单过滤服务器工具](#黑名单过滤服务器工具)
  + [优先级规则](#优先级规则)
  + [同样可过滤实用工具](#同样可过滤实用工具)
  + [完整示例](#完整示例)
* [如果所有工具都被过滤掉会怎样？](#如果所有工具都被过滤掉会怎样)
* [运行时行为](#运行时行为)
  + [发现时机](#发现时机)
  + [动态工具发现](#动态工具发现)
  + [重新加载](#重新加载)
  + [工具集](#工具集)
* [安全模型](#安全模型)
  + [Stdio 环境变量过滤](#stdio-环境变量过滤)
  + [配置层面的暴露控制](#配置层面的暴露控制)
* [示例用例](#示例用例)
  + [GitHub 服务器，仅暴露最小 issue 管理接口](#github-服务器仅暴露最小-issue-管理接口)
  + [Stripe 服务器，移除危险操作](#stripe-服务器移除危险操作)
  + [文件系统服务器，限定单个项目根目录](#文件系统服务器限定单个项目根目录)
* [故障排查](#故障排查)
  + [MCP 服务器无法连接](#mcp-服务器无法连接)
  + [工具未出现](#工具未出现)
  + [为什么资源或 prompt 实用工具没有出现？](#为什么资源或-prompt-实用工具没有出现)
* [并行工具调用](#并行工具调用)
* [MCP Sampling 支持](#mcp-sampling-支持)
* [将 Hermes 作为 MCP 服务器运行](#将-hermes-作为-mcp-服务器运行)
  + [适用场景](#适用场景)
  + [快速开始](#快速开始-1)
  + [MCP 客户端配置](#mcp-客户端配置)
  + [可用工具](#可用工具)
