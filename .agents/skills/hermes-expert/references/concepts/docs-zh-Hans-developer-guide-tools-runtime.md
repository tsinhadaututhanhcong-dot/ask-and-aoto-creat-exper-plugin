# 工具运行时 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/tools-runtime](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/tools-runtime)
...
## 工具注册模型[​](#工具注册模型 "工具注册模型的直接链接")

每个工具模块在导入时调用 `registry.register(...)`。

`model_tools.py` 负责导入/发现工具模块，并构建供模型使用的 schema 列表。

### `registry.register()` 的工作原理[​](#registryregister-的工作原理 "registryregister-的工作原理的直接链接")
...
### 发现机制：`discover_builtin_tools()`[​](#发现机制discover_builtin_tools "发现机制discover_builtin_tools的直接链接")
...
## 工具可用性检查（`check_fn`）[​](#工具可用性检查check_fn "工具可用性检查check_fn的直接链接")
...
## Toolset 解析[​](#toolset-解析 "Toolset 解析的直接链接")
...
### `get_tool_definitions()` 如何过滤工具[​](#get_tool_definitions-如何过滤工具 "get_tool_definitions-如何过滤工具的直接链接")
...
### 旧版 toolset 名称[​](#旧版-toolset-名称 "旧版 toolset 名称的直接链接")
...
### 调度流程：模型 tool\_call → handler 执行[​](#调度流程模型-tool_call--handler-执行 "调度流程：模型 tool_call → handler 执行的直接链接")
...
### Agent 循环工具[​](#agent-循环工具 "Agent 循环工具的直接链接")
...
## DANGEROUS\_PATTERNS 审批流程[​](#dangerous_patterns-审批流程 "DANGEROUS_PATTERNS 审批流程的直接链接")
...
## 终端/运行时环境[​](#终端运行时环境 "终端/运行时环境的直接链接")
...
* [Toolsets 参考](/docs/zh-Hans/reference/toolsets-reference)
* [内置工具参考](/docs/zh-Hans/reference/tools-reference)
* [Agent 循环内部机制](/docs/zh-Hans/developer-guide/agent-loop)
* [ACP 内部机制](/docs/zh-Hans/developer-guide/acp-internals)

* [工具注册模型](#工具注册模型)
  + [`registry.register()` 的工作原理](#registryregister-的工作原理)
  + [发现机制：`discover_builtin_tools()`](#发现机制discover_builtin_tools)
* [工具可用性检查（`check_fn`）](#工具可用性检查check_fn)
* [Toolset 解析](#toolset-解析)
  + [`get_tool_definitions()` 如何过滤工具](#get_tool_definitions-如何过滤工具)
  + [旧版 toolset 名称](#旧版-toolset-名称)
* [调度](#调度)
  + [调度流程：模型 tool\_call → handler 执行](#调度流程模型-tool_call--handler-执行)
  + [错误包装](#错误包装)
  + [Agent 循环工具](#agent-循环工具)
  + [异步桥接](#异步桥接)
* [DANGEROUS\_PATTERNS 审批流程](#dangerous_patterns-审批流程)
* [终端/运行时环境](#终端运行时环境)
* [并发](#并发)
* [相关文档](#相关文档)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/tools-reference](./docs-zh-Hans-reference-tools-reference.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/toolsets-reference](./docs-zh-Hans-reference-toolsets-reference.md)
