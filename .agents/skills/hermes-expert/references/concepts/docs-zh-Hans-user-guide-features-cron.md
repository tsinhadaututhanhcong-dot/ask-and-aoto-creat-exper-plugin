...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/cron](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/cron)

本页总览

使用自然语言或 cron 表达式调度自动运行的任务。Hermes 通过单一 `cronjob` 工具暴露 cron 管理能力，采用动作式操作，而非分散的 schedule/list/remove 工具。

## Cron 当前能做什么[​](#cron-当前能做什么 "Cron 当前能做什么的直接链接")
...
## 创建定时任务[​](#创建定时任务 "创建定时任务的直接链接")

### 在聊天中使用 `/cron`[​](#在聊天中使用-cron "在聊天中使用-cron的直接链接")
...
## 附带 skill 的 cron 任务[​](#附带-skill-的-cron-任务 "附带 skill 的 cron 任务的直接链接")

Cron 任务可以在运行 prompt（提示词）之前加载一个或多个 skill。

### 单个 skill[​](#单个-skill "单个 skill的直接链接")
...
### 多个 skill[​](#多个-skill "多个 skill的直接链接")
...
## 在指定项目目录中运行任务[​](#在指定项目目录中运行任务 "在指定项目目录中运行任务的直接链接")
...
## 在指定 profile 中运行 cron 任务[​](#在指定-profile-中运行-cron-任务 "在指定 profile 中运行 cron 任务的直接链接")
...
### Gateway 调度器行为[​](#gateway-调度器行为 "Gateway 调度器行为的直接链接")
...
### Telegram cron 话题（`TELEGRAM_CRON_THREAD_ID`）[​](#telegram-cron-话题telegram_cron_thread_id "telegram-cron-话题telegram_cron_thread_id的直接链接")
...
### 可继续任务（回复 cron 投递）[​](#可继续任务回复-cron-投递 "可继续任务（回复 cron 投递）的直接链接")
...
#### 平铺频道内继续（Slack）[​](#平铺频道内继续slack "平铺频道内继续（Slack）的直接链接")
...
in\_channel 任务——都会加入同一段滚动对话。这是「平铺在频道中」的固有取舍，与
...
## 无 agent 模式（纯脚本任务）[​](#无-agent-模式纯脚本任务 "无 agent 模式（纯脚本任务）的直接链接")
...
### Agent 为你设置这些[​](#agent-为你设置这些 "Agent 为你设置这些的直接链接")
...
参见[纯脚本 Cron 任务指南](/docs/zh-Hans/guides/cron-script-only)获取实际示例。

## 通过 `context_from` 串联任务[​](#通过-context_from-串联任务 "通过-context_from-串联任务的直接链接")
...
## Provider 恢复[​](#provider-恢复 "Provider 恢复的直接链接")
...
* **轮换到下一个凭证**，即同一 provider 的[凭证池](/docs/zh-Hans/user-guide/configuration#credential-pool-strategies)中的下一个
...
### 相对延迟（一次性）[​](#相对延迟一次性 "相对延迟（一次性）的直接链接")
...
### Cron 表达式[​](#cron-表达式 "Cron 表达式的直接链接")
...
## 以编程方式管理任务[​](#以编程方式管理任务 "以编程方式管理任务的直接链接")
...
## Cron 任务可用的工具集[​](#cron-任务可用的工具集 "Cron 任务可用的工具集的直接链接")
...
### 完全跳过 agent：`wakeAgent`[​](#完全跳过-agentwakeagent "完全跳过-agentwakeagent的直接链接")
...
#### 实用方案：低成本预运行门控[​](#实用方案低成本预运行门控 "实用方案：低成本预运行门控的直接链接")
...
### 串联任务：`context_from`[​](#串联任务context_from "串联任务context_from的直接链接")
...
## 自包含的 prompt 仍然重要[​](#自包含的-prompt-仍然重要 "自包含的 prompt 仍然重要的直接链接")
...
* [Cron 当前能做什么](#cron-当前能做什么)
* [创建定时任务](#创建定时任务)
  + [在聊天中使用 `/cron`](#在聊天中使用-cron)
  + [从独立 CLI](#从独立-cli)
  + [通过自然对话](#通过自然对话)
* [附带 skill 的 cron 任务](#附带-skill-的-cron-任务)
  + [单个 skill](#单个-skill)
  + [多个 skill](#多个-skill)
* [在指定项目目录中运行任务](#在指定项目目录中运行任务)
* [在指定 profile 中运行 cron 任务](#在指定-profile-中运行-cron-任务)
* [编辑任务](#编辑任务)
  + [聊天](#聊天)
  + [独立 CLI](#独立-cli)
* [生命周期操作](#生命周期操作)
  ...
  + [独立 CLI](#独立-cli-1)
* [工作原理](#工作原理)
  + [Gateway 调度器行为](#gateway-调度器行为)
* [投递选项](#投递选项)
  + [路由意图（`all`）](#路由意图all)
  + [Telegram cron 话题（`TELEGRAM_CRON_THREAD_ID`）](#telegram-cron-话题telegram_cron_thread_id)
  + [响应包装](#响应包装)
  + [可继续任务（回复 cron 投递）](#可继续任务回复-cron-投递)
  + [静默抑制](#静默抑制)
* [脚本超时](#脚本超时)
* [无 agent 模式（纯脚本任务）](#无-agent-模式纯脚本任务)
  + [Agent 为你设置这些](#agent-为你设置这些)
* [通过 `context_from` 串联任务](#通过-context_from-串联任务)
* [Provider 恢复](#provider-恢复)
* [调度格式](#调度格式)
  + [相对延迟（一次性）](#相对延迟一次性)
  + [间隔（周期性）](#间隔周期性)
  + [Cron 表达式](#cron-表达式)
...
* [重复行为](#重复行为)
* [以编程方式管理任务](#以编程方式管理任务)
* [Cron 任务可用的工具集](#cron-任务可用的工具集)
  + [完全跳过 agent：`wakeAgent`](#完全跳过-agentwakeagent)
  + [串联任务：`context_from`](#串联任务context_from)
* [任务存储](#任务存储)
* [自包含的 prompt 仍然重要](#自包含的-prompt-仍然重要)
* [安全性](#安全性)

## Related Files
> **LLM Navigation:** Các tệp dưới đây được liên kết trực tiếp từ tài liệu này. Hãy đọc chúng nếu cần thêm ngữ cảnh.

- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/cron-script-only](./docs-zh-Hans-guides-cron-script-only.md)
