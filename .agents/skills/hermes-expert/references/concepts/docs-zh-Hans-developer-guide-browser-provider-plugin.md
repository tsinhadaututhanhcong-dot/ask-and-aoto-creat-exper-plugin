# Browser Provider Plugins | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/browser-provider-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/browser-provider-plugin)
...
Browser backends are one of several **backend plugins** Hermes supports. The others (with their own ABCs) are [Web Search Provider Plugins](/docs/zh-Hans/developer-guide/web-search-provider-plugin) (which this ABC deliberately mirrors), [Image Generation](/docs/zh-Hans/developer-guide/image-gen-provider-plugin), [Video Generation](/docs/zh-Hans/developer-guide/video-gen-provider-plugin), [Memory Providers](/docs/zh-Hans/developer-guide/memory-provider-plugin), [Context Engines](/docs/zh-Hans/developer-guide/context-engine-plugin), [Secret Sources](/docs/zh-Hans/developer-guide/secret-source-plugin), and [Model Providers](/docs/zh-Hans/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Hermes Plugin](/docs/zh-Hans/developer-guide/plugins).

## How it fits together[​](#how-it-fits-together "How it fits together的直接链接")
...
## Discovery[​](#discovery "Discovery的直接链接")
...
## Directory structure[​](#directory-structure "Directory structure的直接链接")
...
## The BrowserProvider ABC[​](#the-browserprovider-abc "The BrowserProvider ABC的直接链接")
...
### The session-metadata contract[​](#the-session-metadata-contract "The session-metadata contract的直接链接")
...
### `get_setup_schema()` — the `hermes tools` picker row[​](#get_setup_schema--the-hermes-tools-picker-row "get_setup_schema--the-hermes-tools-picker-row的直接链接")
...
## Users configure it[​](#users-configure-it "Users configure it的直接链接")
...
## Reference implementations[​](#reference-implementations "Reference implementations的直接链接")
...
## Checklist[​](#checklist "Checklist的直接链接")
...
* [The BrowserProvider ABC](#the-browserprovider-abc)
  + [The session-metadata contract](#the-session-metadata-contract)
  + [`get_setup_schema()` — the `hermes tools` picker row](#get_setup_schema--the-hermes-tools-picker-row)
* [Users configure it](#users-configure-it)
