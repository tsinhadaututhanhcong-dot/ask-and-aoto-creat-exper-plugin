# 可交付成果模式（聊天中的 Artifacts） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/deliverable-mode](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/deliverable-mode)
...
## 支持的文件扩展名[​](#支持的文件扩展名 "支持的文件扩展名的直接链接")
...
## 引导 Agent 生成 Artifacts[​](#引导-agent-生成-artifacts "引导 Agent 生成 Artifacts的直接链接")
...
## Kanban：Artifacts 随完成通知一并发送[​](#kanbanartifacts-随完成通知一并发送 "Kanban：Artifacts 随完成通知一并发送的直接链接")
...
## 通过 MCP 连接更多服务[​](#通过-mcp-连接更多服务 "通过 MCP 连接更多服务的直接链接")
...
通过 `~/.hermes/config.yaml` 中的 `mcp_servers` 部分安装 MCP 服务器。完整配置指南请参阅 [MCP 集成](/docs/zh-Hans/user-guide/features/mcp)。

## 与 Perplexity Computer in Slack 的对比[​](#与-perplexity-computer-in-slack-的对比 "与 Perplexity Computer in Slack 的对比的直接链接")
...
* [工作原理](#工作原理)
* [支持的文件扩展名](#支持的文件扩展名)
* [引导 Agent 生成 Artifacts](#引导-agent-生成-artifacts)
* [Kanban：Artifacts 随完成通知一并发送](#kanbanartifacts-随完成通知一并发送)
* [通过 MCP 连接更多服务](#通过-mcp-连接更多服务)
* [与 Perplexity Computer in Slack 的对比](#与-perplexity-computer-in-slack-的对比)
