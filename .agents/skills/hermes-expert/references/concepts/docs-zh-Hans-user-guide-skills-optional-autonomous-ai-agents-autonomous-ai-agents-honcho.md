# Honcho | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho)
...
| 相关 skills | [`hermes-agent`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) |
...
### 云端（app.honcho.dev）[​](#云端apphonchodev "云端（app.honcho.dev）的直接链接")
...
### 冷启动 / 热启动 Prompt 选择[​](#冷启动--热启动-prompt-选择 "冷启动 / 热启动 Prompt 选择的直接链接")
...
## 三个正交调节维度[​](#三个正交调节维度 "三个正交调节维度的直接链接")

Honcho 的辩证行为由三个独立维度控制。每个维度可单独调整，互不影响：

### 节奏（何时）[​](#节奏何时 "节奏（何时）的直接链接")
...
### 创建带 Honcho peer 的配置文件[​](#创建带-honcho-peer-的配置文件 "创建带 Honcho peer 的配置文件的直接链接")
...
### 为现有配置文件补充创建[​](#为现有配置文件补充创建 "为现有配置文件补充创建的直接链接")

```
hermes honcho sync    # creates host blocks for all profiles that don't have one yet
```

### 按配置文件配置[​](#按配置文件配置 "按配置文件配置的直接链接")
...
## Agent 使用模式[​](#agent-使用模式 "Agent 使用模式的直接链接")

Honcho 记忆激活时 Hermes 的使用指南。

### 对话开始时[​](#对话开始时 "对话开始时的直接链接")
...
### 当用户分享需要记住的内容时[​](#当用户分享需要记住的内容时 "当用户分享需要记住的内容时的直接链接")
...
### 当用户询问历史上下文 / 需要召回具体内容时[​](#当用户询问历史上下文--需要召回具体内容时 "当用户询问历史上下文 / 需要召回具体内容时的直接链接")
...
### 何时使用 `peer: "ai"`[​](#何时使用-peer-ai "何时使用-peer-ai的直接链接")
...
### 上下文预算与注入[​](#上下文预算与注入 "上下文预算与注入的直接链接")
...
* [使用场景](#使用场景)
* [设置](#设置)
  + [云端（app.honcho.dev）](#云端apphonchodev)
  + [自托管](#自托管)
  + [验证](#验证)
* [架构](#架构)
  + [基础上下文注入](#基础上下文注入)
  + [冷启动 / 热启动 Prompt 选择](#冷启动--热启动-prompt-选择)
  + [Peers](#peers)
  + [观察](#观察)
  + [会话](#会话)
  + [召回模式](#召回模式)
* [三个正交调节维度](#三个正交调节维度)
  ...
  + [级别（强度）](#级别强度)
* [多配置文件设置](#多配置文件设置)
  + [创建带 Honcho peer 的配置文件](#创建带-honcho-peer-的配置文件)
  + [为现有配置文件补充创建](#为现有配置文件补充创建)
  + [按配置文件配置](#按配置文件配置)
* [工具](#工具)
  ...
  + [双向 peer 定向](#双向-peer-定向)
* [Agent 使用模式](#agent-使用模式)
  + [对话开始时](#对话开始时)
  + [当用户分享需要记住的内容时](#当用户分享需要记住的内容时)
  + [当用户询问历史上下文 / 需要召回具体内容时](#当用户询问历史上下文--需要召回具体内容时)
  + [何时使用 `peer: "ai"`](#何时使用-peer-ai)
  + [何时不调用工具](#何时不调用工具)
  + [节奏感知](#节奏感知)
* [配置参考](#配置参考)
  + [关键设置](#关键设置)
...
* [故障排查](#故障排查)
  ...
  + [记忆未跨会话持久化](#记忆未跨会话持久化)
  + [配置文件未获得自己的 peer](#配置文件未获得自己的-peer)
  + [控制台中的观察更改未生效](#控制台中的观察更改未生效)
  + [消息被截断](#消息被截断)
  + [上下文注入过大](#上下文注入过大)
  + [会话摘要缺失](#会话摘要缺失)
* [CLI 命令](#cli-命令)
