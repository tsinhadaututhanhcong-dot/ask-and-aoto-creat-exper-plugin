# AI 提供商 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers)

本页总览

本页介绍如何为 Hermes Agent 配置推理提供商——从 OpenRouter、Anthropic 等云端 API，到 Ollama、vLLM 等自托管端点，再到高级路由与故障转移配置。使用 Hermes 至少需要配置一个提供商。

## 推理提供商[​](#推理提供商 "推理提供商的直接链接")
...
| **xAI Grok OAuth（SuperGrok）** | `hermes model` → "xAI Grok OAuth (SuperGrok / Premium+)"——浏览器登录，无需 API key。参见[指南](/docs/zh-Hans/guides/xai-grok-oauth) |
...
官方 API key 路径请参见专属的 [Google Gemini 指南](/docs/zh-Hans/guides/google-gemini)。

模型 key 别名

在 `model:` 配置节中，可以使用 `default:` 或 `model:` 作为模型 ID 的键名。`model: { default: my-model }` 和 `model: { model: my-model }` 效果完全相同。

### Nous Portal[​](#nous-portal "Nous Portal的直接链接")

[Nous Portal](https://portal.nousresearch.com) 是 Nous Research 的统一订阅网关，也是**运行 Hermes Agent 的推荐方式**。一次 OAuth 登录即可访问 300+ 前沿智能体模型（Claude、GPT、Gemini、DeepSeek、Qwen、Kimi、GLM、MiniMax、Grok 等），以及 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、浏览器自动化）和 [Nous Chat](https://chat.nousresearch.com)——费用从你的 Nous 订阅中扣除，无需单独管理各提供商账户。
...
**完整详情：** 参见专属的 [Nous Portal 集成页面](/docs/zh-Hans/integrations/nous-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 Nous Portal 运行 Hermes Agent](/docs/zh-Hans/guides/run-hermes-with-nous-portal)。
...
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），Hermes 将这些任务路由到你的**主聊天模型**——即你在 `hermes model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/docs/zh-Hans/user-guide/configuration#auxiliary-models)。

Nous Tool Gateway

付费 Nous Portal 订阅者还可访问 **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`hermes setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `hermes model` 或 `hermes tools` 按工具启用。随时使用 `hermes portal info` 查看路由状态。

### 模型管理的两个命令[​](#模型管理的两个命令 "模型管理的两个命令的直接链接")
...
### Anthropic（原生）[​](#anthropic原生 "Anthropic（原生）的直接链接")
...
### GitHub Copilot[​](#github-copilot "GitHub Copilot的直接链接")
...
### 一等 API Key 提供商[​](#一等-api-key-提供商 "一等 API Key 提供商的直接链接")
...
### xAI（Grok）— Responses API + Prompt 缓存[​](#xaigrok-responses-api--prompt-缓存 "xAI（Grok）— Responses API + Prompt 缓存的直接链接")
...
SuperGrok 和 X Premium+ 订阅者可以用浏览器 OAuth 登录，无需 API key——在 `hermes model` 中选择 **xAI Grok OAuth (SuperGrok / Premium+)**，或运行 `hermes auth add xai-oauth`。同一 OAuth bearer token 会被 xAI 直连工具（TTS、图像生成、视频生成、转录）自动复用。完整流程参见 [xAI Grok OAuth 指南](/docs/zh-Hans/guides/xai-grok-oauth)——如果 Hermes 运行在远程主机上，还需参见 [SSH / 远程主机上的 OAuth](/docs/zh-Hans/guides/oauth-over-ssh) 了解所需的 `ssh -L` 隧道配置。
...
xAI 还提供专属 TTS 端点（`/v1/tts`）。在 `hermes tools` → 语音与 TTS 中选择 **xAI TTS**，或参见[语音与 TTS](/docs/zh-Hans/user-guide/features/tts#text-to-speech) 页面了解配置。

### NovitaAI[​](#novitaai "NovitaAI的直接链接")
...
### Ollama Cloud — 托管 Ollama 模型，OAuth + API Key[​](#ollama-cloud--托管-ollama-模型oauth--api-key "Ollama Cloud — 托管 Ollama 模型，OAuth + API Key的直接链接")
...
### AWS Bedrock[​](#aws-bedrock "AWS Bedrock的直接链接")
...
参见 [AWS Bedrock 指南](/docs/zh-Hans/guides/aws-bedrock)，了解 IAM 配置、区域选择和跨区域推理的详细步骤。

### Qwen Portal（OAuth）[​](#qwen-portaloauth "Qwen Portal（OAuth）的直接链接")
...
Qwen OAuth 与 Qwen Cloud（阿里 DashScope）
...
### 阿里云（Coding Plan）[​](#阿里云coding-plan "阿里云（Coding Plan）的直接链接")
...
### MiniMax（OAuth）[​](#minimaxoauth "MiniMax（OAuth）的直接链接")
...
MiniMax OAuth 与 API key

`minimax-oauth` 使用 MiniMax 面向消费者的 portal，通过 OAuth 登录——无需设置计费。`minimax` 和 `minimax-cn` 提供商使用 `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY`——用于程序化访问。完整流程参见 [MiniMax OAuth 指南](/docs/zh-Hans/guides/minimax-oauth)。

### NVIDIA NIM[​](#nvidia-nim "NVIDIA NIM的直接链接")
...
### GMI Cloud[​](#gmi-cloud "GMI Cloud的直接链接")
...
### Hugging Face 推理提供商[​](#hugging-face-推理提供商 "Hugging Face 推理提供商的直接链接")
...
## 自定义与自托管 LLM 提供商[​](#自定义与自托管-llm-提供商 "自定义与自托管 LLM 提供商的直接链接")
...
### 使用 `/model` 切换模型[​](#使用-model-切换模型 "使用-model-切换模型的直接链接")
...
### Ollama — 本地模型，零配置[​](#ollama--本地模型零配置 "Ollama — 本地模型，零配置的直接链接")
...
### vLLM — 高性能 GPU 推理[​](#vllm--高性能-gpu-推理 "vLLM — 高性能 GPU 推理的直接链接")
...
### SGLang — 带 RadixAttention 的快速服务[​](#sglang--带-radixattention-的快速服务 "SGLang — 带 RadixAttention 的快速服务的直接链接")
...
SGLang 默认最大输出 128 tokens

如果响应看起来被截断，在请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。SGLang 的默认值是每次响应仅 128 tokens（如果请求中未指定）。

---

### llama.cpp / llama-server — CPU 与 Metal 推理[​](#llamacpp--llama-server--cpu-与-metal-推理 "llama.cpp / llama-server — CPU 与 Metal 推理的直接链接")
...
从 [Hugging Face](https://huggingface.co/models?library=gguf) 下载 GGUF 模型。Q4\_K\_M 量化在质量与内存使用之间提供最佳平衡。

---

### LM Studio — 带本地模型的桌面应用[​](#lm-studio--带本地模型的桌面应用 "LM Studio — 带本地模型的桌面应用的直接链接")
...
### WSL2 网络（Windows 用户）[​](#wsl2-网络windows-用户 "WSL2 网络（Windows 用户）的直接链接")
...
#### 方式 1：镜像网络模式（推荐）[​](#方式-1镜像网络模式推荐 "方式 1：镜像网络模式（推荐）的直接链接")
...
#### 方式 2：使用 Windows 主机 IP（Windows 10 / 旧版本）[​](#方式-2使用-windows-主机-ipwindows-10--旧版本 "方式 2：使用 Windows 主机 IP（Windows 10 / 旧版本）的直接链接")
...
#### 服务器绑定地址（NAT 模式必需）[​](#服务器绑定地址nat-模式必需 "服务器绑定地址（NAT 模式必需）的直接链接")
...
#### Windows 防火墙[​](#windows-防火墙 "Windows 防火墙的直接链接")
...
### 本地模型故障排查[​](#本地模型故障排查 "本地模型故障排查的直接链接")

以下问题影响与 Hermes 配合使用的**所有**本地推理服务器。

#### 从 WSL2 连接 Windows 托管模型服务器时"连接被拒绝"[​](#从-wsl2-连接-windows-托管模型服务器时连接被拒绝 "从 WSL2 连接 Windows 托管模型服务器时\"连接被拒绝\"的直接链接")
...
#### 工具调用以文本形式出现而非执行[​](#工具调用以文本形式出现而非执行 "工具调用以文本形式出现而非执行的直接链接")
...
#### 模型似乎忘记上下文或给出不连贯的响应[​](#模型似乎忘记上下文或给出不连贯的响应 "模型似乎忘记上下文或给出不连贯的响应的直接链接")
...
#### 启动时显示"Context limit: 2048 tokens"[​](#启动时显示context-limit-2048-tokens "启动时显示\"Context limit: 2048 tokens\"的直接链接")
...
#### 响应在句子中间被截断[​](#响应在句子中间被截断 "响应在句子中间被截断的直接链接")
...
2. **上下文耗尽** — 模型填满了上下文窗口。增加 `model.context_length` 或在 Hermes 中启用[上下文压缩](/docs/zh-Hans/user-guide/configuration#context-compression)。

---

### LiteLLM Proxy — 多提供商网关[​](#litellm-proxy--多提供商网关 "LiteLLM Proxy — 多提供商网关的直接链接")
...
### ClawRouter — 成本优化路由[​](#clawrouter--成本优化路由 "ClawRouter — 成本优化路由的直接链接")
...
### 命名自定义提供商[​](#命名自定义提供商 "命名自定义提供商的直接链接")
...
### 实战配置：Together AI、Groq、Perplexity[​](#实战配置together-aigroqperplexity "实战配置：Together AI、Groq、Perplexity的直接链接")
...
#### Together AI[​](#together-ai "Together AI的直接链接")
...
#### Perplexity[​](#perplexity "Perplexity的直接链接")
...
#### 在单个配置中使用多个提供商[​](#在单个配置中使用多个提供商 "在单个配置中使用多个提供商的直接链接")
...
## 可选 API Key[​](#可选-api-key "可选 API Key的直接链接")
...
### 自托管 Firecrawl[​](#自托管-firecrawl "自托管 Firecrawl的直接链接")

默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你希望在本地运行 Firecrawl，可以将 Hermes 指向自托管实例。完整配置说明参见 Firecrawl 的 [SELF\_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
## OpenRouter 提供商路由[​](#openrouter-提供商路由 "OpenRouter 提供商路由的直接链接")
...
## OpenRouter Pareto Code 路由器[​](#openrouter-pareto-code-路由器 "OpenRouter Pareto Code 路由器的直接链接")
...
* 要将 Pareto Code 路由器用于特定**辅助任务**（压缩、视觉等）而非主智能体，在该任务下设置 `extra_body.plugins`——参见[辅助模型 → OpenRouter 路由与辅助任务的 Pareto Code](/docs/zh-Hans/user-guide/configuration#openrouter-routing--pareto-code-for-auxiliary-tasks)。

## 故障转移提供商[​](#故障转移提供商 "故障转移提供商的直接链接")
...
故障转移仅通过 `config.yaml` 配置——或通过 `hermes fallback` 交互式配置。有关触发时机、链推进方式以及与辅助任务和委托的交互，参见[故障转移提供商](/docs/zh-Hans/user-guide/features/fallback-providers)。

---

## 另请参阅[​](#另请参阅 "另请参阅的直接链接")

* [配置](/docs/zh-Hans/user-guide/configuration) — 通用配置（目录结构、配置优先级、终端后端、记忆、压缩等）
* [环境变量](/docs/zh-Hans/reference/environment-variables) — 所有环境变量的完整参考

* [推理提供商](#推理提供商)
  + [Nous Portal](#nous-portal)
  + [模型管理的两个命令](#模型管理的两个命令)
  + [Anthropic（原生）](#anthropic原生)
  + [GitHub Copilot](#github-copilot)
  + [一等 API Key 提供商](#一等-api-key-提供商)
  + [xAI（Grok）— Responses API + Prompt 缓存](#xaigrok-responses-api--prompt-缓存)
  + [NovitaAI](#novitaai)
  + [Ollama Cloud — 托管 Ollama 模型，OAuth + API Key](#ollama-cloud--托管-ollama-模型oauth--api-key)
  + [AWS Bedrock](#aws-bedrock)
  + [Qwen Portal（OAuth）](#qwen-portaloauth)
  + [阿里云（Coding Plan）](#阿里云coding-plan)
  + [MiniMax（OAuth）](#minimaxoauth)
  + [NVIDIA NIM](#nvidia-nim)
  + [GMI Cloud](#gmi-cloud)
  + [StepFun](#stepfun)
  + [Hugging Face 推理提供商](#hugging-face-推理提供商)
* [自定义与自托管 LLM 提供商](#自定义与自托管-llm-提供商)
  + [通用配置](#通用配置)
  + [使用 `/model` 切换模型](#使用-model-切换模型)
  + [Ollama — 本地模型，零配置](#ollama--本地模型零配置)
  + [vLLM — 高性能 GPU 推理](#vllm--高性能-gpu-推理)
  + [SGLang — 带 RadixAttention 的快速服务](#sglang--带-radixattention-的快速服务)
  + [llama.cpp / llama-server — CPU 与 Metal 推理](#llamacpp--llama-server--cpu-与-metal-推理)
  + [LM Studio — 带本地模型的桌面应用](#lm-studio--带本地模型的桌面应用)
  + [WSL2 网络（Windows 用户）](#wsl2-网络windows-用户)
  + [本地模型故障排查](#本地模型故障排查)
  + [LiteLLM Proxy — 多提供商网关](#litellm-proxy--多提供商网关)
  + [ClawRouter — 成本优化路由](#clawrouter--成本优化路由)
  + [其他兼容提供商](#其他兼容提供商)
  + [上下文长度检测](#上下文长度检测)
  + [命名自定义提供商](#命名自定义提供商)
  + [实战配置：Together AI、Groq、Perplexity](#实战配置together-aigroqperplexity)
  + [选择合适的配置](#选择合适的配置)
* [可选 API Key](#可选-api-key)
  + [自托管 Firecrawl](#自托管-firecrawl)
* [OpenRouter 提供商路由](#openrouter-提供商路由)
* [OpenRouter Pareto Code 路由器](#openrouter-pareto-code-路由器)
* [故障转移提供商](#故障转移提供商)
* [另请参阅](#另请参阅)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/aws-bedrock](./docs-zh-Hans-guides-aws-bedrock.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/google-gemini](./docs-zh-Hans-guides-google-gemini.md)
