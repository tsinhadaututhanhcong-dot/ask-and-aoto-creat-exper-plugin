# Node Inspect 调试器 — 调试 Node | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger)
...
| 相关 skill | [`systematic-debugging`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`python-debugpy`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-python-debugpy), [`debugging-hermes-tui-commands`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-debugging-hermes-tui-commands) |
...
* tui\_gateway 子进程（`_SlashWorker`、PTY bridge workers）行为异常
* 需要检查闭包中某个值，而不打补丁就无法用 `console.log` 获取
* 性能分析：附加到运行中的进程以采集 CPU profile 或堆快照

**不适用于：** 一分钟内用 `console.log` 就能解决的问题。断点调试开销较大，只在收益明显时使用。

## 快速参考：`node inspect` REPL[​](#快速参考node-inspect-repl "快速参考node-inspect-repl的直接链接")
...
## 附加到运行中的进程[​](#附加到运行中的进程 "附加到运行中的进程的直接链接")
...
## 程序化 CDP（从终端脚本化）[​](#程序化-cdp从终端脚本化 "程序化 CDP（从终端脚本化）的直接链接")
...
## 调试 Hermes ui-tui[​](#调试-hermes-ui-tui "调试 Hermes ui-tui的直接链接")

TUI 基于 Ink + tsx 构建。两种常见场景：

### 在开发模式下调试单个 Ink 组件[​](#在开发模式下调试单个-ink-组件 "在开发模式下调试单个 Ink 组件的直接链接")
...
### 调试运行中的 `hermes --tui`[​](#调试运行中的-hermes---tui "调试运行中的-hermes---tui的直接链接")
...
### 调试 `_SlashWorker` / PTY 子进程[​](#调试-_slashworker--pty-子进程 "调试-_slashworker--pty-子进程的直接链接")

这些是 Python 进程，不是 Node——请使用 `python-debugpy` skill。只有 Node 部分（Ink UI、tui\_gateway client、`ui-tui/` 下的 tsx-run 测试）使用本 skill。

## 在调试器下运行 Vitest 测试[​](#在调试器下运行-vitest-测试 "在调试器下运行 Vitest 测试的直接链接")
...
## 堆快照与 CPU Profile（非交互式）[​](#堆快照与-cpu-profile非交互式 "堆快照与 CPU Profile（非交互式）的直接链接")
...
* [概述](#概述)
* [使用时机](#使用时机)
* [快速参考：`node inspect` REPL](#快速参考node-inspect-repl)
* [附加到运行中的进程](#附加到运行中的进程)
* [程序化 CDP（从终端脚本化）](#程序化-cdp从终端脚本化)
* [调试 Hermes ui-tui](#调试-hermes-ui-tui)
  + [在开发模式下调试单个 Ink 组件](#在开发模式下调试单个-ink-组件)
  + [调试运行中的 `hermes --tui`](#调试运行中的-hermes---tui)
  + [调试 `_SlashWorker` / PTY 子进程](#调试-_slashworker--pty-子进程)
* [在调试器下运行 Vitest 测试](#在调试器下运行-vitest-测试)
* [堆快照与 CPU Profile（非交互式）](#堆快照与-cpu-profile非交互式)
* [常见陷阱](#常见陷阱)
* [验证清单](#验证清单)
* [一键配方](#一键配方)
