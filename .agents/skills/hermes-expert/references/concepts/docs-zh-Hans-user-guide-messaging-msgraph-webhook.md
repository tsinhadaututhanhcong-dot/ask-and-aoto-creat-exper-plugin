# Microsoft Graph Webhook 监听器 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/msgraph-webhook](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/msgraph-webhook)
...
* Microsoft Graph 应用凭据——[注册 Microsoft Graph 应用程序](/docs/zh-Hans/guides/microsoft-graph-app-registration)
...
大多数设置也有对应的环境变量（`MSGRAPH_WEBHOOK_*`），在 gateway 启动时合并到配置中（例外是 `host`，它仅可通过配置文件设置——参见上方说明）——参见[环境变量参考](/docs/zh-Hans/reference/environment-variables#microsoft-graph-teams-meetings)。

## 安全加固[​](#安全加固 "安全加固的直接链接")

### clientState 是主要的认证检查[​](#clientstate-是主要的认证检查 "clientState 是主要的认证检查的直接链接")
...
### 源 IP 白名单（生产部署）[​](#源-ip-白名单生产部署 "源 IP 白名单（生产部署）的直接链接")
...
### HTTPS 终止[​](#https-终止 "HTTPS 终止的直接链接")
...
* [注册 Microsoft Graph 应用程序](/docs/zh-Hans/guides/microsoft-graph-app-registration) — Azure 应用注册前提条件
* [环境变量 → Microsoft Graph](/docs/zh-Hans/reference/environment-variables#microsoft-graph-teams-meetings) — 完整环境变量列表
* [Microsoft Teams 机器人设置](/docs/zh-Hans/user-guide/messaging/teams) — 允许用户在 Teams 中与 Hermes 聊天的另一平台

* [前提条件](#前提条件)
* [快速开始](#快速开始)
* [配置](#配置)
* [安全加固](#安全加固)
  + [clientState 是主要的认证检查](#clientstate-是主要的认证检查)
  + [源 IP 白名单（生产部署）](#源-ip-白名单生产部署)
  + [HTTPS 终止](#https-终止)
  + [响应规范](#响应规范)
* [故障排查](#故障排查)
* [相关文档](#相关文档)
