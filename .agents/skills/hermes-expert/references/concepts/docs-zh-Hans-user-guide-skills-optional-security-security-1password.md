# 1Password — 设置并使用 1Password CLI (op) | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/security/security-1password](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/security/security-1password)
...
### 服务账户（推荐用于 Hermes）[​](#服务账户推荐用于-hermes "服务账户（推荐用于 Hermes）的直接链接")
...
### 桌面应用集成（交互式）[​](#桌面应用集成交互式 "桌面应用集成（交互式）的直接链接")

1. 在 1Password 桌面应用中启用：设置 → 开发者 → 与 1Password CLI 集成
2. 确保应用已解锁
3. 运行 `op signin` 并通过生物识别提示授权

### Connect 服务器（自托管）[​](#connect-服务器自托管 "Connect 服务器（自托管）的直接链接")
...
## Hermes 执行模式（桌面应用流程）[​](#hermes-执行模式桌面应用流程 "Hermes 执行模式（桌面应用流程）的直接链接")
...
### 以密钥环境变量运行命令[​](#以密钥环境变量运行命令 "以密钥环境变量运行命令的直接链接")
...
## CI / 无头环境说明[​](#ci--无头环境说明 "CI / 无头环境说明的直接链接")
...
* [使用场景](#使用场景)
* [认证方式](#认证方式)
  + [服务账户（推荐用于 Hermes）](#服务账户推荐用于-hermes)
  + [桌面应用集成（交互式）](#桌面应用集成交互式)
  + [Connect 服务器（自托管）](#connect-服务器自托管)
* [设置步骤](#设置步骤)
* [Hermes 执行模式（桌面应用流程）](#hermes-执行模式桌面应用流程)
* [常用操作](#常用操作)
  + [读取密钥](#读取密钥)
  + [获取 OTP](#获取-otp)
  + [注入模板](#注入模板)
  + [以密钥环境变量运行命令](#以密钥环境变量运行命令)
* [使用限制](#使用限制)
* [CI / 无头环境说明](#ci--无头环境说明)
* [参考资料](#参考资料)