...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/open-webui](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/open-webui)
...
### 2. 启动 Hermes Agent gateway[​](#2-启动-hermes-agent-gateway "2. 启动 Hermes Agent gateway的直接链接")
...
### 3. 验证 API 服务器可访问[​](#3-验证-api-服务器可访问 "3. 验证 API 服务器可访问的直接链接")
...
### 4. 启动 Open WebUI[​](#4-启动-open-webui "4. 启动 Open WebUI的直接链接")
...
## 通过管理员 UI 配置[​](#通过管理员-ui-配置 "通过管理员 UI 配置的直接链接")
...
## API 类型：Chat Completions 与 Responses[​](#api-类型chat-completions-与-responses "API 类型：Chat Completions 与 Responses的直接链接")
...
### 使用 Chat Completions（推荐）[​](#使用-chat-completions推荐 "使用 Chat Completions（推荐）的直接链接")

这是默认模式，无需额外配置。Open WebUI 发送标准 OpenAI 格式请求，Hermes Agent 相应响应。每个请求包含完整的对话历史。

### 使用 Responses API[​](#使用-responses-api "使用 Responses API的直接链接")
...
### Hermes Agent（API 服务器）[​](#hermes-agentapi-服务器 "Hermes Agent（API 服务器）的直接链接")
...
| `API_SERVER_KEY` | *（必填）* | 用于认证的 Bearer token（令牌）。需与 `OPENAI_API_KEY` 匹配。 |

### Open WebUI[​](#open-webui "Open WebUI的直接链接")
...
### 下拉列表中没有模型[​](#下拉列表中没有模型 "下拉列表中没有模型的直接链接")
...
## 多用户设置与 Profiles[​](#多用户设置与-profiles "多用户设置与 Profiles的直接链接")

要为每个用户运行独立的 Hermes 实例——各自拥有独立的配置、记忆和技能——请使用 [profiles](/docs/zh-Hans/user-guide/profiles)。每个 profile 在不同端口上运行自己的 API 服务器，并自动将 profile 名称作为模型名称公告给 Open WebUI。

### 1. 创建 profiles 并配置 API 服务器[​](#1-创建-profiles-并配置-api-服务器 "1. 创建 profiles 并配置 API 服务器的直接链接")
...
### 2. 启动各 gateway[​](#2-启动各-gateway "2. 启动各 gateway的直接链接")
...
### 3. 在 Open WebUI 中添加连接[​](#3-在-open-webui-中添加连接 "3. 在 Open WebUI 中添加连接的直接链接")
...
## Linux Docker（无 Docker Desktop）[​](#linux-docker无-docker-desktop "Linux Docker（无 Docker Desktop）的直接链接")
...
* [快速设置](#快速设置)
  ...
  + [2. 启动 Hermes Agent gateway](#2-启动-hermes-agent-gateway)
  + [3. 验证 API 服务器可访问](#3-验证-api-服务器可访问)
  + [4. 启动 Open WebUI](#4-启动-open-webui)
  + [5. 打开 UI](#5-打开-ui)
* [Docker Compose 设置](#docker-compose-设置)
* [通过管理员 UI 配置](#通过管理员-ui-配置)
* [API 类型：Chat Completions 与 Responses](#api-类型chat-completions-与-responses)
  + [使用 Chat Completions（推荐）](#使用-chat-completions推荐)
  + [使用 Responses API](#使用-responses-api)
* [工作原理](#工作原理)
* [配置参考](#配置参考)
  + [Hermes Agent（API 服务器）](#hermes-agentapi-服务器)
  + [Open WebUI](#open-webui)
* [故障排查](#故障排查)
  + [下拉列表中没有模型](#下拉列表中没有模型)
  + [连接测试通过但模型无法加载](#连接测试通过但模型无法加载)
  + [响应耗时很长](#响应耗时很长)
  + ["Invalid API key" 错误](#invalid-api-key-错误)
* [多用户设置与 Profiles](#多用户设置与-profiles)
  + [1. 创建 profiles 并配置 API 服务器](#1-创建-profiles-并配置-api-服务器)
  + [2. 启动各 gateway](#2-启动各-gateway)
  + [3. 在 Open WebUI 中添加连接](#3-在-open-webui-中添加连接)
* [Linux Docker（无 Docker Desktop）](#linux-docker无-docker-desktop)
