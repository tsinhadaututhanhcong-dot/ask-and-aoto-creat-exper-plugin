# Python Debugpy — 调试 Python：pdb REPL + debugpy 远程（DAP） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-python-debugpy](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-python-debugpy)
...
| 相关 skill | [`systematic-debugging`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-hermes-tui-commands`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-debugging-hermes-tui-commands) |
...
* 需要逐步执行某个函数并观察集合的变化
* 长期运行的进程（hermes gateway、tui\_gateway）出现异常且无法重启
...
## pdb 快速参考[​](#pdb-快速参考 "pdb 快速参考的直接链接")
...
## 方案 1：本地断点[​](#方案-1本地断点 "方案 1：本地断点的直接链接")
...
## 方案 2：在 pdb 下启动脚本（无需修改源码）[​](#方案-2在-pdb-下启动脚本无需修改源码 "方案 2：在 pdb 下启动脚本（无需修改源码）的直接链接")
...
## 方案 3：调试 pytest 测试[​](#方案-3调试-pytest-测试 "方案 3：调试 pytest 测试的直接链接")
...
## 方案 4：对任意异常进行事后分析[​](#方案-4对任意异常进行事后分析 "方案 4：对任意异常进行事后分析的直接链接")
...
## 方案 5：使用 debugpy 进行远程调试（附加到运行中的进程）[​](#方案-5使用-debugpy-进行远程调试附加到运行中的进程 "方案 5：使用 debugpy 进行远程调试（附加到运行中的进程）的直接链接")

适用于长期运行的进程：Hermes gateway、tui\_gateway、daemon，或已出现异常且无法干净重启的进程。
...
### 模式 A：修改源码——进程在启动时等待调试器[​](#模式-a修改源码进程在启动时等待调试器 "模式 A：修改源码——进程在启动时等待调试器的直接链接")
...
### 模式 B：无需修改源码——使用 `-m debugpy` 启动[​](#模式-b无需修改源码使用--m-debugpy-启动 "模式-b无需修改源码使用--m-debugpy-启动的直接链接")
...
### 模式 C：附加到已运行的进程[​](#模式-c附加到已运行的进程 "模式 C：附加到已运行的进程的直接链接")
...
### 从终端连接客户端[​](#从终端连接客户端 "从终端连接客户端的直接链接")
...
## 调试 Hermes 特定进程[​](#调试-hermes-特定进程 "调试 Hermes 特定进程的直接链接")

### 测试[​](#测试 "测试的直接链接")

参见方案 3。始终添加 `-p no:xdist` 或在不使用 xdist 的情况下运行单个测试。

### `run_agent.py` / CLI — 一次性运行[​](#run_agentpy--cli--一次性运行 "run_agentpy--cli--一次性运行的直接链接")

最简单：在可疑行附近添加 `breakpoint()`，然后正常运行 `hermes`。控制权将在暂停点返回到你的终端。

### `tui_gateway` 子进程（由 `hermes --tui` 启动）[​](#tui_gateway-子进程由-hermes---tui-启动 "tui_gateway-子进程由-hermes---tui-启动的直接链接")
...
### `_SlashWorker` 子进程[​](#_slashworker-子进程 "_slashworker-子进程的直接链接")

相同模式——在 worker 的 `exec` 路径中使用 `remote-pdb` 的 `set_trace()`。该 worker 在多次 slash 命令间持续存在，因此第一次触发会阻塞直到你连接；后续 slash 命令正常通过，除非你重新设置断点。

### Gateway（`gateway/run.py`）[​](#gatewaygatewayrunpy "gatewaygatewayrunpy的直接链接")
...
* [pdb 快速参考](#pdb-快速参考)
* [方案 1：本地断点](#方案-1本地断点)
* [方案 2：在 pdb 下启动脚本（无需修改源码）](#方案-2在-pdb-下启动脚本无需修改源码)
* [方案 3：调试 pytest 测试](#方案-3调试-pytest-测试)
* [方案 4：对任意异常进行事后分析](#方案-4对任意异常进行事后分析)
* [方案 5：使用 debugpy 进行远程调试（附加到运行中的进程）](#方案-5使用-debugpy-进行远程调试附加到运行中的进程)
  + [安装](#安装)
  + [模式 A：修改源码——进程在启动时等待调试器](#模式-a修改源码进程在启动时等待调试器)
  + [模式 B：无需修改源码——使用 `-m debugpy` 启动](#模式-b无需修改源码使用--m-debugpy-启动)
  + [模式 C：附加到已运行的进程](#模式-c附加到已运行的进程)
  + [从终端连接客户端](#从终端连接客户端)
* [调试 Hermes 特定进程](#调试-hermes-特定进程)
  + [测试](#测试)
  + [`run_agent.py` / CLI — 一次性运行](#run_agentpy--cli--一次性运行)
  + [`tui_gateway` 子进程（由 `hermes --tui` 启动）](#tui_gateway-子进程由-hermes---tui-启动)
  + [`_SlashWorker` 子进程](#_slashworker-子进程)
  + [Gateway（`gateway/run.py`）](#gatewaygatewayrunpy)
* [常见陷阱](#常见陷阱)
* [验证清单](#验证清单)
* [一次性速查方案](#一次性速查方案)
