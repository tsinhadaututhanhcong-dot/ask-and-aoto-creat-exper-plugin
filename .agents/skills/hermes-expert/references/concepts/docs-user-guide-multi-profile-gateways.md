...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/multi-profile-gateways](https://hermes-agent.nousresearch.com/docs/user-guide/multi-profile-gateways)

On this page

Operate multiple [profiles](/docs/user-guide/profiles) — each with its own bot tokens,
...
[Profiles](/docs/user-guide/profiles) for the basics.
...
## Alternative: one gateway for all profiles (multiplexing)[​](#alternative-one-gateway-for-all-profiles-multiplexing "Direct link to Alternative: one gateway for all profiles (multiplexing)")
...
### When to prefer multiplexing[​](#when-to-prefer-multiplexing "Direct link to When to prefer multiplexing")
...
### How to opt in[​](#how-to-opt-in "Direct link to How to opt in")
...
### What changes when multiplexing is on[​](#what-changes-when-multiplexing-is-on "Direct link to What changes when multiplexing is on")

Enabling the flag changes how a few things behave. All of these revert the
moment the flag is off.

#### 1. Secondary profiles must not start their own gateway[​](#1-secondary-profiles-must-not-start-their-own-gateway "Direct link to 1. Secondary profiles must not start their own gateway")
...
#### 2. HTTP-inbound platforms are reached via a `/p/<profile>/` URL prefix[​](#2-http-inbound-platforms-are-reached-via-a-pprofile-url-prefix "Direct link to 2-http-inbound-platforms-are-reached-via-a-pprofile-url-prefix")
...
#### 3. Per-credential platforms still need their own token per profile[​](#3-per-credential-platforms-still-need-their-own-token-per-profile "Direct link to 3. Per-credential platforms still need their own token per profile")
...
#### 4. Session keys are namespaced by profile[​](#4-session-keys-are-namespaced-by-profile "Direct link to 4. Session keys are namespaced by profile")
...
#### 5. One PID/lock and one status surface[​](#5-one-pidlock-and-one-status-surface "Direct link to 5. One PID/lock and one status surface")
...
#### What does **not** change[​](#what-does-not-change "Direct link to what-does-not-change")
...
## Start, stop, or restart all gateways at once[​](#start-stop-or-restart-all-gateways-at-once "Direct link to Start, stop, or restart all gateways at once")
...
## Manage one profile[​](#manage-one-profile "Direct link to Manage one profile")
...
## Service files[​](#service-files "Direct link to Service files")
...
## Identify what's actually running[​](#identify-whats-actually-running "Direct link to Identify what's actually running")
...
## Editing configuration[​](#editing-configuration "Direct link to Editing configuration")
...
## Keeping the host awake[​](#keeping-the-host-awake "Direct link to Keeping the host awake")

The gateway process can run all day, but the operating system will still try
to sleep when idle. Two patterns:

### macOS — `caffeinate`[​](#macos--caffeinate "Direct link to macos--caffeinate")
...
### Linux — `systemd-inhibit` or `loginctl`[​](#linux--systemd-inhibit-or-loginctl "Direct link to linux--systemd-inhibit-or-loginctl")
...
## Token-conflict safety[​](#token-conflict-safety "Direct link to Token-conflict safety")
...
### "Could not find service in domain for user gui: 501"[​](#could-not-find-service-in-domain-for-user-gui-501 "Direct link to \"Could not find service in domain for user gui: 501\"")
...
### Stale PID after a crash[​](#stale-pid-after-a-crash "Direct link to Stale PID after a crash")
...
### Forcing a hard reset of one service[​](#forcing-a-hard-reset-of-one-service "Direct link to Forcing a hard reset of one service")
