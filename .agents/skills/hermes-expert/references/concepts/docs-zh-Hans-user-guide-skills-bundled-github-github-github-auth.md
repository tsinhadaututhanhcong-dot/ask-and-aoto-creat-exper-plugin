...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/github/github-github-auth](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/github/github-github-auth)
...
| 相关 skill | [`github-pr-workflow`](/docs/zh-Hans/user-guide/skills/bundled/github/github-github-pr-workflow), [`github-code-review`](/docs/zh-Hans/user-guide/skills/bundled/github/github-github-code-review), [`github-issues`](/docs/zh-Hans/user-guide/skills/bundled/github/github-github-issues), [`github-repo-management`](/docs/zh-Hans/user-guide/skills/bundled/github/github-github-repo-management) |
...
## 方法一：仅 Git 认证（无 gh，无 sudo）[​](#方法一仅-git-认证无-gh无-sudo "方法一：仅 Git 认证（无 gh，无 sudo）的直接链接")

适用于任何已安装 `git` 的机器，无需 root 权限。

### 选项 A：HTTPS 配合个人访问令牌（推荐）[​](#选项-ahttps-配合个人访问令牌推荐 "选项 A：HTTPS 配合个人访问令牌（推荐）的直接链接")
...
### 选项 B：SSH 密钥认证[​](#选项-bssh-密钥认证 "选项 B：SSH 密钥认证的直接链接")
...
## 方法二：gh CLI 认证[​](#方法二gh-cli-认证 "方法二：gh CLI 认证的直接链接")

若已安装 `gh`，一步即可完成 API 访问和 git 凭据配置。

### 浏览器交互登录（桌面环境）[​](#浏览器交互登录桌面环境 "浏览器交互登录（桌面环境）的直接链接")
...
### 基于令牌登录（无头环境 / SSH 服务器）[​](#基于令牌登录无头环境--ssh-服务器 "基于令牌登录（无头环境 / SSH 服务器）的直接链接")
...
## 不使用 gh 调用 GitHub API[​](#不使用-gh-调用-github-api "不使用 gh 调用 GitHub API的直接链接")

当 `gh` 不可用时，仍可使用 `curl` 配合个人访问令牌访问完整的 GitHub API。其他 GitHub skill 的降级方案均采用此方式。

### 为 API 调用设置令牌[​](#为-api-调用设置令牌 "为 API 调用设置令牌的直接链接")
...
### 从 Git 凭据中提取令牌[​](#从-git-凭据中提取令牌 "从 Git 凭据中提取令牌的直接链接")
...
### 辅助函数：检测认证方式[​](#辅助函数检测认证方式 "辅助函数：检测认证方式的直接链接")
...
* [检测流程](#检测流程)
* [方法一：仅 Git 认证（无 gh，无 sudo）](#方法一仅-git-认证无-gh无-sudo)
  + [选项 A：HTTPS 配合个人访问令牌（推荐）](#选项-ahttps-配合个人访问令牌推荐)
  + [选项 B：SSH 密钥认证](#选项-bssh-密钥认证)
* [方法二：gh CLI 认证](#方法二gh-cli-认证)
  + [浏览器交互登录（桌面环境）](#浏览器交互登录桌面环境)
  + [基于令牌登录（无头环境 / SSH 服务器）](#基于令牌登录无头环境--ssh-服务器)
  + [验证](#验证)
* [不使用 gh 调用 GitHub API](#不使用-gh-调用-github-api)
  + [为 API 调用设置令牌](#为-api-调用设置令牌)
  + [从 Git 凭据中提取令牌](#从-git-凭据中提取令牌)
  + [辅助函数：检测认证方式](#辅助函数检测认证方式)
* [故障排查](#故障排查)
