# 内置工具参考 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/tools-reference](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/tools-reference)
...
除内置工具外，Hermes 还可从 MCP 服务器动态加载工具。MCP 工具以 `mcp_<server>_` 为前缀（例如，`github` MCP 服务器的 `mcp_github_create_issue`）。配置方法见 [MCP 集成](/docs/zh-Hans/user-guide/features/mcp)。

## `browser` 工具集[​](#browser-工具集 "browser-工具集的直接链接")
...
## `browser` 工具集（CDP 门控工具）[​](#browser-工具集cdp-门控工具 "browser-工具集cdp-门控工具的直接链接")
...
## `delegation` 工具集[​](#delegation-工具集 "delegation-工具集的直接链接")

| 工具 | 描述 | 所需环境 |
| --- | --- | --- |
| `delegate_task` | 生成一个或多个子 agent，在隔离上下文中处理任务。每个子 agent 拥有独立的对话、终端会话和工具集。仅返回最终摘要——中间工具结果不会进入你的上下文窗口。两种… | — |

## `feishu_doc` 工具集[​](#feishu_doc-工具集 "feishu_doc-工具集的直接链接")
...
## `feishu_drive` 工具集[​](#feishu_drive-工具集 "feishu_drive-工具集的直接链接")
...
## `file` 工具集[​](#file-工具集 "file-工具集的直接链接")
...
## `homeassistant` 工具集[​](#homeassistant-工具集 "homeassistant-工具集的直接链接")
...
| `ha_list_entities` | 列出 Home Assistant 实体。可按域（light、switch、climate、sensor、binary\_sensor、cover、fan 等）或区域名称（客厅、厨房、卧室等）过滤。 | — |
| `ha_list_services` | 列出用于设备控制的可用 Home Assistant 服务（动作）。显示每种设备类型可执行的操作及其接受的参数。用于发现如何控制通过 `ha_list_entities` 找到的设备。 | — |

## `computer_use` 工具集[​](#computer_use-工具集 "computer_use-工具集的直接链接")
...
**Honcho 工具**（`honcho_profile`、`honcho_search`、`honcho_context`、`honcho_reasoning`、`honcho_conclude`）不再是内置工具。它们通过 `plugins/memory/honcho/` 的 Honcho 记忆提供者插件提供。安装和使用方法见 [Memory Providers](/docs/zh-Hans/user-guide/features/memory-providers)。

## `image_gen` 工具集[​](#image_gen-工具集 "image_gen-工具集的直接链接")

| 工具 | 描述 | 所需环境 |
| --- | --- | --- |
| `image_generate` | 使用 FAL.ai 从文本 prompt（提示词）生成高质量图片。底层模型由用户配置（默认：FLUX 2 Klein 9B，生成时间低于 1 秒），agent 不可选择。返回单个图片 URL。使用… 显示。 | FAL\_KEY |

## `kanban` 工具集[​](#kanban-工具集 "kanban-工具集的直接链接")

在以下情况下注册：(a) agent 由 kanban 调度器生成（设置了 `HERMES_KANBAN_TASK` 环境变量），或 (b) 在显式启用 `kanban` 工具集的 profile 中运行。任务范围的 worker 使用生命周期工具处理其分配的任务；编排器 profile 还额外获得 `kanban_list` 和 `kanban_unblock` 等看板路由工具。完整工作流见 [Kanban 多 Agent](/docs/zh-Hans/user-guide/features/kanban)。
...
## `skills` 工具集[​](#skills-工具集 "skills-工具集的直接链接")
...
## `terminal` 工具集[​](#terminal-工具集 "terminal-工具集的直接链接")
...
## `video_gen` 工具集[​](#video_gen-工具集 "video_gen-工具集的直接链接")
...
单个 `video_generate` 工具涵盖两种模态——传入 `image_url` 可为静态图片制作动画，省略则从文本生成。活跃后端自动路由到正确的端点。工具描述在会话启动时重建，以反映活跃后端的实际能力（模态、宽高比、分辨率、时长范围、最大参考图片数、音频支持）。后端开发见 [视频生成提供者插件](/docs/zh-Hans/developer-guide/video-gen-provider-plugin)。
...
## `web` 工具集[​](#web-工具集 "web-工具集的直接链接")

| 工具 | 描述 | 所需环境 |
| --- | --- | --- |
| `web_search` | 在网络上搜索信息。默认返回最多 5 条结果，包含标题、URL 和描述。接受可选的 `limit`（1-100，默认 5）。查询直接传递给配置的后端，因此当后端支持时，`site:domain`、`filetype:pdf`、`intitle:word`、`-term`、`"exact phrase"` 等运算符可能有效。 | EXA\_API\_KEY 或 PARALLEL\_API\_KEY 或 FIRECRAWL\_API\_KEY 或 TAVILY\_API\_KEY |
| `web_extract` | 从网页 URL 提取内容。以 Markdown 格式返回页面内容。也支持 PDF URL——直接传入 PDF 链接即可转换为 Markdown 文本。5000 字符以下的页面返回完整 Markdown；更大的页面由 LLM 摘要处理。 | EXA\_API\_KEY 或 PARALLEL\_API\_KEY 或 FIRECRAWL\_API\_KEY 或 TAVILY\_API\_KEY |

## `x_search` 工具集[​](#x_search-工具集 "x_search-工具集的直接链接")

| 工具 | 描述 | 所需环境 |
| --- | --- | --- |
| `x_search` | 使用 xAI 内置的 `x_search` Responses 工具搜索 X（Twitter）帖子、主页和话题串。用于获取 X 上的当前讨论、反应或观点，而非通用网页。默认关闭——通过 `hermes tools` → 🐦 X (Twitter) Search 选择启用。仅在配置了 xAI 凭据时注册 schema（check\_fn 门控）。 | XAI\_API\_KEY **或** xAI Grok OAuth（SuperGrok / Premium+）登录 |

## `tts` 工具集[​](#tts-工具集 "tts-工具集的直接链接")
...
## `discord` 工具集[​](#discord-工具集 "discord-工具集的直接链接")
...
## `discord_admin` 工具集[​](#discord_admin-工具集 "discord_admin-工具集的直接链接")
...
## `spotify` 工具集[​](#spotify-工具集 "spotify-工具集的直接链接")
...
## `hermes-yuanbao` 工具集[​](#hermes-yuanbao-工具集 "hermes-yuanbao-工具集的直接链接")
...
* [`browser` 工具集](#browser-工具集)
* [`browser` 工具集（CDP 门控工具）](#browser-工具集cdp-门控工具)
* [`clarify` 工具集](#clarify-工具集)
* [`code_execution` 工具集](#code_execution-工具集)
* [`cronjob` 工具集](#cronjob-工具集)
* [`delegation` 工具集](#delegation-工具集)
* [`feishu_doc` 工具集](#feishu_doc-工具集)
* [`feishu_drive` 工具集](#feishu_drive-工具集)
* [`file` 工具集](#file-工具集)
* [`homeassistant` 工具集](#homeassistant-工具集)
* [`computer_use` 工具集](#computer_use-工具集)
* [`image_gen` 工具集](#image_gen-工具集)
* [`kanban` 工具集](#kanban-工具集)
* [`memory` 工具集](#memory-工具集)
* [`messaging` 工具集](#messaging-工具集)
* [`session_search` 工具集](#session_search-工具集)
* [`skills` 工具集](#skills-工具集)
* [`terminal` 工具集](#terminal-工具集)
* [`todo` 工具集](#todo-工具集)
* [`vision` 工具集](#vision-工具集)
* [`video` 工具集](#video-工具集)
* [`video_gen` 工具集](#video_gen-工具集)
* [`web` 工具集](#web-工具集)
* [`x_search` 工具集](#x_search-工具集)
* [`tts` 工具集](#tts-工具集)
* [`discord` 工具集](#discord-工具集)
* [`discord_admin` 工具集](#discord_admin-工具集)
* [`spotify` 工具集](#spotify-工具集)
* [`hermes-yuanbao` 工具集](#hermes-yuanbao-工具集)
