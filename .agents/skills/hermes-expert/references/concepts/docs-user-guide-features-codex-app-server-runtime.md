...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/features/codex-app-server-runtime](https://hermes-agent.nousresearch.com/docs/user-guide/features/codex-app-server-runtime)
...
## What tools the model actually has[​](#what-tools-the-model-actually-has "Direct link to What tools the model actually has")
...
### 1. Codex's built-in toolset (always on)[​](#1-codexs-built-in-toolset-always-on "Direct link to 1. Codex's built-in toolset (always on)")
...
### 2. Native Codex plugins (auto-migrated from your `codex plugin` install)[​](#2-native-codex-plugins-auto-migrated-from-your-codex-plugin-install "Direct link to 2-native-codex-plugins-auto-migrated-from-your-codex-plugin-install")
...
### 3. Hermes tool callback (MCP server, registered in `~/.codex/config.toml`)[​](#3-hermes-tool-callback-mcp-server-registered-in-codexconfigtoml "Direct link to 3-hermes-tool-callback-mcp-server-registered-in-codexconfigtoml")
...
### What's NOT available on this runtime[​](#whats-not-available-on-this-runtime "Direct link to What's NOT available on this runtime")
...
## Workflow features (`/goal`, kanban, cron)[​](#workflow-features-goal-kanban-cron "Direct link to workflow-features-goal-kanban-cron")

### `/goal` (the Ralph loop)[​](#goal-the-ralph-loop "Direct link to goal-the-ralph-loop")
...
### Kanban (multi-agent worktree dispatch)[​](#kanban-multi-agent-worktree-dispatch "Direct link to Kanban (multi-agent worktree dispatch)")
...
## Self-improvement loop (memory + skill nudges)[​](#self-improvement-loop-memory--skill-nudges "Direct link to Self-improvement loop (memory + skill nudges)")
...
| --- | --- | --- |
| `_turns_since_memory` increments | per user prompt, in run\_conversation pre-loop | same code path, before the early-return |
...
## How approvals work[​](#how-approvals-work "Direct link to How approvals work")
...
## Permission profiles[​](#permission-profiles "Direct link to Permission profiles")
...
## Auxiliary tasks and ChatGPT subscription token cost[​](#auxiliary-tasks-and-chatgpt-subscription-token-cost "Direct link to Auxiliary tasks and ChatGPT subscription token cost")
...
## Editing `~/.codex/config.toml` safely[​](#editing-codexconfigtoml-safely "Direct link to editing-codexconfigtoml-safely")
...
## Multi-profile / multi-tenant setups[​](#multi-profile--multi-tenant-setups "Direct link to Multi-profile / multi-tenant setups")
...
## HOME environment variable passthrough[​](#home-environment-variable-passthrough "Direct link to HOME environment variable passthrough")
...
## MCP server migration[​](#mcp-server-migration "Direct link to MCP server migration")
...
| `url` + `headers` | streamable\_http transport |
...
## Native Codex plugin migration[​](#native-codex-plugin-migration "Direct link to Native Codex plugin migration")
...
## Hermes tool callback (the new MCP server)[​](#hermes-tool-callback-the-new-mcp-server "Direct link to Hermes tool callback (the new MCP server)")
...
* [Why](#why)
* [What tools the model actually has](#what-tools-the-model-actually-has)
  + [1. Codex's built-in toolset (always on)](#1-codexs-built-in-toolset-always-on)
  + [2. Native Codex plugins (auto-migrated from your `codex plugin` install)](#2-native-codex-plugins-auto-migrated-from-your-codex-plugin-install)
  + [3. Hermes tool callback (MCP server, registered in `~/.codex/config.toml`)](#3-hermes-tool-callback-mcp-server-registered-in-codexconfigtoml)
  + [What's NOT available on this runtime](#whats-not-available-on-this-runtime)
* [Workflow features (`/goal`, kanban, cron)](#workflow-features-goal-kanban-cron)
  + [`/goal` (the Ralph loop)](#goal-the-ralph-loop)
  + [Kanban (multi-agent worktree dispatch)](#kanban-multi-agent-worktree-dispatch)
  + [Cron jobs](#cron-jobs)
* [Trade-offs](#trade-offs)
* [Prerequisites](#prerequisites)
* [Enabling](#enabling)
* [Self-improvement loop (memory + skill nudges)](#self-improvement-loop-memory--skill-nudges)
* [How approvals work](#how-approvals-work)
* [Permission profiles](#permission-profiles)
* [Auxiliary tasks and ChatGPT subscription token cost](#auxiliary-tasks-and-chatgpt-subscription-token-cost)
* [Editing `~/.codex/config.toml` safely](#editing-codexconfigtoml-safely)
* [Multi-profile / multi-tenant setups](#multi-profile--multi-tenant-setups)
* [HOME environment variable passthrough](#home-environment-variable-passthrough)
* [MCP server migration](#mcp-server-migration)
* [Native Codex plugin migration](#native-codex-plugin-migration)
* [Hermes tool callback (the new MCP server)](#hermes-tool-callback-the-new-mcp-server)
* [Disabling](#disabling)
