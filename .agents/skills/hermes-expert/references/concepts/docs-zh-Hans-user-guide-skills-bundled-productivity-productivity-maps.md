# Maps — 通过 OpenStreetMap/OSRM 进行地理编码、POI、路线、时区查询 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-maps](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-maps)
...
### search — 地理编码地名[​](#search--地理编码地名 "search — 地理编码地名的直接链接")
...
### reverse — 坐标转地址[​](#reverse--坐标转地址 "reverse — 坐标转地址的直接链接")
...
### nearby — 按分类查找地点[​](#nearby--按分类查找地点 "nearby — 按分类查找地点的直接链接")
...
46 个分类：restaurant、cafe、bar、hospital、pharmacy、hotel、guest\_house、
...
### distance — 行程距离与时间[​](#distance--行程距离与时间 "distance — 行程距离与时间的直接链接")
...
### directions — 逐步导航[​](#directions--逐步导航 "directions — 逐步导航的直接链接")
...
### timezone — 坐标对应时区[​](#timezone--坐标对应时区 "timezone — 坐标对应时区的直接链接")
...
### area — 地点的边界框与面积[​](#area--地点的边界框与面积 "area — 地点的边界框与面积的直接链接")
...
### bbox — 在边界框内搜索[​](#bbox--在边界框内搜索 "bbox — 在边界框内搜索的直接链接")
...
## 处理 Telegram 位置图钉[​](#处理-telegram-位置图钉 "处理 Telegram 位置图钉的直接链接")
...
* [前置条件](#前置条件)
* [命令](#命令)
  + [search — 地理编码地名](#search--地理编码地名)
  + [reverse — 坐标转地址](#reverse--坐标转地址)
  + [nearby — 按分类查找地点](#nearby--按分类查找地点)
  + [distance — 行程距离与时间](#distance--行程距离与时间)
  + [directions — 逐步导航](#directions--逐步导航)
  + [timezone — 坐标对应时区](#timezone--坐标对应时区)
  + [area — 地点的边界框与面积](#area--地点的边界框与面积)
  + [bbox — 在边界框内搜索](#bbox--在边界框内搜索)
* [处理 Telegram 位置图钉](#处理-telegram-位置图钉)
* [工作流示例](#工作流示例)
* [注意事项](#注意事项)
* [验证](#验证)