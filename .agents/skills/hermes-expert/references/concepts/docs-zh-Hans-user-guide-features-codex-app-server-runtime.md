# Codex App-Server 运行时（可选） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/codex-app-server-runtime](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/codex-app-server-runtime)
...
* **Hermes 的丰富工具一并可用**——web\_search、web\_extract、浏览器自动化、视觉、图像生成、技能和 TTS 通过 MCP 回调提供。Codex 会回调 Hermes 获取其自身没有内置的工具。
* **记忆与技能提示持续生效**——Codex 的事件被投影为 Hermes 的消息格式，使自我改进循环看到正常的对话记录。

## 模型实际拥有哪些工具[​](#模型实际拥有哪些工具 "模型实际拥有哪些工具的直接链接")

这是大多数用户最想提前了解的部分。当此运行时开启时，执行你的轮次的模型拥有三个独立的工具来源：

### 1. Codex 内置工具集（始终开启）[​](#1-codex-内置工具集始终开启 "1. Codex 内置工具集（始终开启）的直接链接")
...
### 2. 原生 Codex 插件（从你的 `codex plugin` 安装中自动迁移）[​](#2-原生-codex-插件从你的-codex-plugin-安装中自动迁移 "2-原生-codex-插件从你的-codex-plugin-安装中自动迁移的直接链接")
...
### 3. Hermes 工具回调（MCP server，注册在 `~/.codex/config.toml` 中）[​](#3-hermes-工具回调mcp-server注册在-codexconfigtoml-中 "3-hermes-工具回调mcp-server注册在-codexconfigtoml-中的直接链接")
...
* **`image_generate`** — 通过 Hermes 的 image\_gen 插件链生成图像。
...
### 此运行时上不可用的工具[​](#此运行时上不可用的工具 "此运行时上不可用的工具的直接链接")
...
## 工作流功能（`/goal`、kanban、cron）[​](#工作流功能goalkanbancron "工作流功能goalkanbancron的直接链接")

### `/goal`（Ralph 循环）[​](#goalralph-循环 "goalralph-循环的直接链接")
...
### Kanban（多 agent 工作树分发）[​](#kanban多-agent-工作树分发 "Kanban（多 agent 工作树分发）的直接链接")
...
* Codex 完整工具集（shell、apply\_patch、update\_plan、view\_image、web\_search）——worker 原生完成实际任务
* 已迁移的 Codex 插件——Linear、GitHub 等
* 用于 browser\_\*、vision、image\_gen、技能、TTS 的 Hermes 工具回调
...
**尚未经过专项测试。** Cron 任务通过 `cronjob` → `AIAgent.run_conversation` 运行，与 CLI 的代码路径相同。如果 cron 任务的配置中有 `openai_runtime: codex_app_server`，它将在 Codex 上运行。相同的工具可用性规则适用——Codex 内置工具 + 插件 + MCP 回调可用，agent 循环工具（delegate\_task、memory、session\_search、todo）不可用。如果你的 cron 任务依赖这些工具，请将 cron 限定在使用默认运行时的配置文件中。
...
## 自我改进循环（记忆 + 技能提示）[​](#自我改进循环记忆--技能提示 "自我改进循环（记忆 + 技能提示）的直接链接")
...
|  | 默认运行时 | Codex 运行时 |
| --- | --- | --- |
| `_turns_since_memory` 递增 | 每个用户 prompt，在 run\_conversation 预循环中 | 相同代码路径，在提前返回之前 |
...
## 辅助任务与 ChatGPT 订阅 token 消耗[​](#辅助任务与-chatgpt-订阅-token-消耗 "辅助任务与 ChatGPT 订阅 token 消耗的直接链接")
...
## 安全编辑 `~/.codex/config.toml`[​](#安全编辑-codexconfigtoml "安全编辑-codexconfigtoml的直接链接")
...
## 多配置文件 / 多租户设置[​](#多配置文件--多租户设置 "多配置文件 / 多租户设置的直接链接")
...
## HOME 环境变量透传[​](#home-环境变量透传 "HOME 环境变量透传的直接链接")
...
## MCP server 迁移[​](#mcp-server-迁移 "MCP server 迁移的直接链接")
...
## 原生 Codex 插件迁移[​](#原生-codex-插件迁移 "原生 Codex 插件迁移的直接链接")
...
## Hermes 工具回调（新 MCP server）[​](#hermes-工具回调新-mcp-server "Hermes 工具回调（新 MCP server）的直接链接")
...
* 多轮对话
* 通过 Hermes UI 进行 `commandExecution` 和 `fileChange`（apply\_patch）审批
...
* 通过 Codex 使用 Hermes web\_search（已实时验证："OpenAI Codex CLI – Getting Started" 端到端返回结果）
...
* [为什么使用](#为什么使用)
* [模型实际拥有哪些工具](#模型实际拥有哪些工具)
  + [1. Codex 内置工具集（始终开启）](#1-codex-内置工具集始终开启)
  + [2. 原生 Codex 插件（从你的 `codex plugin` 安装中自动迁移）](#2-原生-codex-插件从你的-codex-plugin-安装中自动迁移)
  + [3. Hermes 工具回调（MCP server，注册在 `~/.codex/config.toml` 中）](#3-hermes-工具回调mcp-server注册在-codexconfigtoml-中)
  + [此运行时上不可用的工具](#此运行时上不可用的工具)
* [工作流功能（`/goal`、kanban、cron）](#工作流功能goalkanbancron)
  + [`/goal`（Ralph 循环）](#goalralph-循环)
  + [Kanban（多 agent 工作树分发）](#kanban多-agent-工作树分发)
  + [Cron 任务](#cron-任务)
* [权衡对比](#权衡对比)
* [前提条件](#前提条件)
* [启用](#启用)
* [自我改进循环（记忆 + 技能提示）](#自我改进循环记忆--技能提示)
* [审批流程](#审批流程)
* [权限配置文件](#权限配置文件)
* [辅助任务与 ChatGPT 订阅 token 消耗](#辅助任务与-chatgpt-订阅-token-消耗)
* [安全编辑 `~/.codex/config.toml`](#安全编辑-codexconfigtoml)
* [多配置文件 / 多租户设置](#多配置文件--多租户设置)
* [HOME 环境变量透传](#home-环境变量透传)
* [MCP server 迁移](#mcp-server-迁移)
* [原生 Codex 插件迁移](#原生-codex-插件迁移)
* [Hermes 工具回调（新 MCP server）](#hermes-工具回调新-mcp-server)
* [禁用](#禁用)
* [限制](#限制)
* [架构](#架构)