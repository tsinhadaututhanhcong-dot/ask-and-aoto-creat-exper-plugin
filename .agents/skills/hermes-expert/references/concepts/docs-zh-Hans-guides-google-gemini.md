...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/google-gemini](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/google-gemini)
...
### 原生 Gemini API[​](#原生-gemini-api "原生 Gemini API的直接链接")
...
### 优先使用原生端点[​](#优先使用原生端点 "优先使用原生端点的直接链接")
...
### 通过 Gemini API 使用 Gemma[​](#通过-gemini-api-使用-gemma "通过 Gemini API 使用 Gemma的直接链接")
...
## 会话中途切换模型[​](#会话中途切换模型 "会话中途切换模型的直接链接")
...
### "Gemini native client requires an API key"[​](#gemini-native-client-requires-an-api-key "\"Gemini native client requires an API key\"的直接链接")
...
### "This Google API key is on the free tier"[​](#this-google-api-key-is-on-the-free-tier "\"This Google API key is on the free tier\"的直接链接")
...
### "404 model not found"[​](#404-model-not-found "\"404 model not found\"的直接链接")

所选模型对你的账号、地区或密钥不可用。重新运行 `hermes model` 并从当前列表中选择其他 Gemini 模型。

### Gemma 模型未显示在 `hermes model` 中[​](#gemma-模型未显示在-hermes-model-中 "gemma-模型未显示在-hermes-model-中的直接链接")

Hermes 默认可能会在选择器中隐藏低吞吐量的 Gemma 模型。如果你有意评估某个模型，请直接在 `~/.hermes/config.yaml` 中设置模型 ID。

### Gemma 出现 "429 quota exceeded"[​](#gemma-出现-429-quota-exceeded "Gemma 出现 \"429 quota exceeded\"的直接链接")

通过 Gemini API 提供的 Gemma 模型适合评估使用，但其 Gemini API 免费层级上限较低。请将其用于兼容性测试，然后切换到付费 Gemini 模型或其他 provider 以进行持续的 Agent 会话。

### 已配置 OpenAI 兼容端点[​](#已配置-openai-兼容端点 "已配置 OpenAI 兼容端点的直接链接")
...
### 工具调用因 schema 错误而失败[​](#工具调用因-schema-错误而失败 "工具调用因 schema 错误而失败的直接链接")

升级 Hermes 并重新运行 `hermes model`。原生 Gemini 适配器会针对 Gemini 更严格的函数声明格式对工具 schema 进行清理；旧版本或自定义端点可能不支持此功能。

## 相关链接[​](#相关链接 "相关链接的直接链接")

* [AI Providers](/docs/zh-Hans/integrations/providers)
* [Configuration](/docs/zh-Hans/user-guide/configuration)
* [Fallback Providers](/docs/zh-Hans/user-guide/features/fallback-providers)
* [AWS Bedrock](/docs/zh-Hans/guides/aws-bedrock) — 使用 AWS 凭据的原生云 provider 集成
...
* [可用模型](#可用模型)
  + [最新别名](#最新别名)
  + [通过 Gemini API 使用 Gemma](#通过-gemini-api-使用-gemma)
* [会话中途切换模型](#会话中途切换模型)
* [诊断](#诊断)
...
* [故障排查](#故障排查)
  ...
  + [Gemma 模型未显示在 `hermes model` 中](#gemma-模型未显示在-hermes-model-中)
  + [Gemma 出现 "429 quota exceeded"](#gemma-出现-429-quota-exceeded)
  + [已配置 OpenAI 兼容端点](#已配置-openai-兼容端点)
  + [工具调用因 schema 错误而失败](#工具调用因-schema-错误而失败)
* [相关链接](#相关链接)
