...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/mattermost](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/mattermost)
...
### Mattermost 中的会话模型[​](#mattermost-中的会话模型 "Mattermost 中的会话模型的直接链接")
...
## 第一步：启用机器人账户[​](#第一步启用机器人账户 "第一步：启用机器人账户的直接链接")
...
## 第二步：创建机器人账户[​](#第二步创建机器人账户 "第二步：创建机器人账户的直接链接")
...
## 第三步：将机器人添加到频道[​](#第三步将机器人添加到频道 "第三步：将机器人添加到频道的直接链接")
...
## 第四步：查找你的 Mattermost 用户 ID[​](#第四步查找你的-mattermost-用户-id "第四步：查找你的 Mattermost 用户 ID的直接链接")
...
## 第五步：配置 Hermes Agent[​](#第五步配置-hermes-agent "第五步：配置 Hermes Agent的直接链接")

### 方式 A：交互式配置（推荐）[​](#方式-a交互式配置推荐 "方式 A：交互式配置（推荐）的直接链接")
...
| `MATTERMOST_FREE_RESPONSE_CHANNELS` | *（无）* | 逗号分隔的频道 ID，机器人在这些频道中无需 `@提及` 即可响应，即使 require\_mention 为 true。 |
...
### WebSocket 断开连接/重连循环[​](#websocket-断开连接重连循环 "WebSocket 断开连接/重连循环的直接链接")
...
### 启动时出现"Failed to authenticate"[​](#启动时出现failed-to-authenticate "启动时出现\"Failed to authenticate\"的直接链接")
...
### "User not allowed"/机器人忽略你[​](#user-not-allowed机器人忽略你 "\"User not allowed\"/机器人忽略你的直接链接")
...
* [Hermes 的行为方式](#hermes-的行为方式)
  + [Mattermost 中的会话模型](#mattermost-中的会话模型)
* [第一步：启用机器人账户](#第一步启用机器人账户)
* [第二步：创建机器人账户](#第二步创建机器人账户)
* [第三步：将机器人添加到频道](#第三步将机器人添加到频道)
* [第四步：查找你的 Mattermost 用户 ID](#第四步查找你的-mattermost-用户-id)
* [第五步：配置 Hermes Agent](#第五步配置-hermes-agent)
  + [方式 A：交互式配置（推荐）](#方式-a交互式配置推荐)
  + [方式 B：手动配置](#方式-b手动配置)
...
* [故障排查](#故障排查)
  ...
  + [WebSocket 断开连接/重连循环](#websocket-断开连接重连循环)
  + [启动时出现"Failed to authenticate"](#启动时出现failed-to-authenticate)
  + [机器人离线](#机器人离线)
  + ["User not allowed"/机器人忽略你](#user-not-allowed机器人忽略你)
