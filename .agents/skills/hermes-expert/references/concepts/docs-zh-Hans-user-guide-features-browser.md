# 浏览器自动化 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/browser](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/browser)
...
如果您拥有付费 [Nous Portal](https://portal.nousresearch.com) 订阅，可通过 **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** 使用浏览器自动化功能，无需单独的 API 密钥。新安装可运行 `hermes setup --portal` 登录并一次性开启所有 gateway 工具；已有安装可通过 `hermes model` 或 `hermes tools` 选择 **Nous Subscription** 作为浏览器提供商。

### Browserbase 云端模式[​](#browserbase-云端模式 "Browserbase 云端模式的直接链接")
...
### Browser Use 云端模式[​](#browser-use-云端模式 "Browser Use 云端模式的直接链接")
...
### Firecrawl 云端模式[​](#firecrawl-云端模式 "Firecrawl 云端模式的直接链接")
...
### 混合路由：公网 URL 使用云端，LAN/localhost 使用本地[​](#混合路由公网-url-使用云端lanlocalhost-使用本地 "混合路由：公网 URL 使用云端，LAN/localhost 使用本地的直接链接")
...
### Camofox 本地模式[​](#camofox-本地模式 "Camofox 本地模式的直接链接")
...
#### 持久化浏览器会话[​](#持久化浏览器会话 "持久化浏览器会话的直接链接")
...
##### Hermes 的行为[​](#hermes-的行为 "Hermes 的行为的直接链接")
...
##### Hermes 不做的事[​](#hermes-不做的事 "Hermes 不做的事的直接链接")
...
##### 验证是否正常工作[​](#验证是否正常工作 "验证是否正常工作的直接链接")
...
#### 外部管理的 Camofox 会话[​](#外部管理的-camofox-会话 "外部管理的 Camofox 会话的直接链接")
...
#### VNC 实时查看[​](#vnc-实时查看 "VNC 实时查看的直接链接")

当 Camofox 以有头模式运行（带可见浏览器窗口）时，其健康检查响应中会暴露 VNC 端口。Hermes 自动发现此信息，并在导航响应中包含 VNC URL，Agent 可分享链接供您实时查看浏览器。

### 通过 CDP 连接本地 Chromium 系浏览器（`/browser connect`）[​](#通过-cdp-连接本地-chromium-系浏览器browser-connect "通过-cdp-连接本地-chromium-系浏览器browser-connect的直接链接")
...
### WSL2 + Windows Chrome：优先使用 MCP 而非 `/browser connect`[​](#wsl2--windows-chrome优先使用-mcp-而非-browser-connect "wsl2--windows-chrome优先使用-mcp-而非-browser-connect的直接链接")
...
### 安装 agent-browser CLI[​](#安装-agent-browser-cli "安装 agent-browser CLI的直接链接")
...
### `browser_navigate`[​](#browser_navigate "browser_navigate的直接链接")
...
### `browser_snapshot`[​](#browser_snapshot "browser_snapshot的直接链接")
...
### `browser_click`[​](#browser_click "browser_click的直接链接")
...
### `browser_type`[​](#browser_type "browser_type的直接链接")
...
### `browser_scroll`[​](#browser_scroll "browser_scroll的直接链接")
...
### `browser_press`[​](#browser_press "browser_press的直接链接")
...
### `browser_back`[​](#browser_back "browser_back的直接链接")

在浏览器历史记录中返回上一页。

### `browser_get_images`[​](#browser_get_images "browser_get_images的直接链接")

列出当前页面上所有图片及其 URL 和 alt 文本。适用于查找需要分析的图片。

### `browser_vision`[​](#browser_vision "browser_vision的直接链接")
...
### `browser_console`[​](#browser_console "browser_console的直接链接")
...
### `browser_cdp`[​](#browser_cdp "browser_cdp的直接链接")
...
### `browser_dialog`[​](#browser_dialog "browser_dialog的直接链接")
...
* [概述](#概述)
* [配置](#配置)
  + [Browserbase 云端模式](#browserbase-云端模式)
  + [Browser Use 云端模式](#browser-use-云端模式)
  + [Firecrawl 云端模式](#firecrawl-云端模式)
  + [混合路由：公网 URL 使用云端，LAN/localhost 使用本地](#混合路由公网-url-使用云端lanlocalhost-使用本地)
  + [Camofox 本地模式](#camofox-本地模式)
  + [通过 CDP 连接本地 Chromium 系浏览器（`/browser connect`）](#通过-cdp-连接本地-chromium-系浏览器browser-connect)
  + [WSL2 + Windows Chrome：优先使用 MCP 而非 `/browser connect`](#wsl2--windows-chrome优先使用-mcp-而非-browser-connect)
  + [本地浏览器模式](#本地浏览器模式)
  + [可选环境变量](#可选环境变量)
  + [安装 agent-browser CLI](#安装-agent-browser-cli)
* [可用工具](#可用工具)
