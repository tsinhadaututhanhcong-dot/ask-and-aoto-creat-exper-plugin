# 操作 Teams 会议流水线 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/operate-teams-meeting-pipeline](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/operate-teams-meeting-pipeline)

本页总览

本指南适用于已通过 [Teams Meetings](/docs/zh-Hans/user-guide/messaging/teams-meetings) 启用该功能之后的操作阶段。
...
## 核心操作员命令[​](#核心操作员命令 "核心操作员命令的直接链接")

### 验证配置快照[​](#验证配置快照 "验证配置快照的直接链接")
...
### 检查 token 健康状态[​](#检查-token-健康状态 "检查 token 健康状态的直接链接")
...
### 续期即将到期的订阅[​](#续期即将到期的订阅 "续期即将到期的订阅的直接链接")
...
### 自动化订阅续期（生产环境必须配置）[​](#自动化订阅续期生产环境必须配置 "自动化订阅续期（生产环境必须配置）的直接链接")
...
#### 方式一：Hermes cron（若已运行 Hermes gateway，推荐此方式）[​](#方式一hermes-cron若已运行-hermes-gateway推荐此方式 "方式一：Hermes cron（若已运行 Hermes gateway，推荐此方式）的直接链接")
...
#### 方式二：systemd timer（推荐用于 Linux 生产部署）[​](#方式二systemd-timer推荐用于-linux-生产部署 "方式二：systemd timer（推荐用于 Linux 生产部署）的直接链接")
...
#### 方式三：普通 crontab[​](#方式三普通-crontab "方式三：普通 crontab的直接链接")
...
#### 验证续期是否正常工作[​](#验证续期是否正常工作 "验证续期是否正常工作的直接链接")
...
### 重放已存储的任务[​](#重放已存储的任务 "重放已存储的任务的直接链接")

```
hermes teams-pipeline run <job-id>
```

### 干运行会议产物拉取[​](#干运行会议产物拉取 "干运行会议产物拉取的直接链接")
...
## 日常运行手册[​](#日常运行手册 "日常运行手册的直接链接")

### 首次设置后[​](#首次设置后 "首次设置后的直接链接")
...
### 变更 webhook URL 或投递目标前[​](#变更-webhook-url-或投递目标前 "变更 webhook URL 或投递目标前的直接链接")
...
### 任务停留在重试状态或在摘要生成前失败[​](#任务停留在重试状态或在摘要生成前失败 "任务停留在重试状态或在摘要生成前失败的直接链接")
...
### 摘要已生成但未投递到 Teams[​](#摘要已生成但未投递到-teams "摘要已生成但未投递到 Teams的直接链接")
...
### 重复或意外的重放[​](#重复或意外的重放 "重复或意外的重放的直接链接")
...
## 投递模式决策指南[​](#投递模式决策指南 "投递模式决策指南的直接链接")
...
* [Teams Meetings 设置](/docs/zh-Hans/user-guide/messaging/teams-meetings)
* [Microsoft Teams bot 设置](/docs/zh-Hans/user-guide/messaging/teams)

* [核心操作员命令](#核心操作员命令)
  + [验证配置快照](#验证配置快照)
  + [检查 token 健康状态](#检查-token-健康状态)
  + [检查订阅](#检查订阅)
  + [续期即将到期的订阅](#续期即将到期的订阅)
  + [自动化订阅续期（生产环境必须配置）](#自动化订阅续期生产环境必须配置)
  + [查看最近的任务](#查看最近的任务)
  + [重放已存储的任务](#重放已存储的任务)
  + [干运行会议产物拉取](#干运行会议产物拉取)
* [日常运行手册](#日常运行手册)
  + [首次设置后](#首次设置后)
  + [每日或定期检查](#每日或定期检查)
  + [变更 webhook URL 或投递目标前](#变更-webhook-url-或投递目标前)
* [故障排查](#故障排查)
  + [未创建任何任务](#未创建任何任务)
  + [任务停留在重试状态或在摘要生成前失败](#任务停留在重试状态或在摘要生成前失败)
  + [摘要已生成但未投递到 Teams](#摘要已生成但未投递到-teams)
  + [重复或意外的重放](#重复或意外的重放)
* [上线检查清单](#上线检查清单)
* [投递模式决策指南](#投递模式决策指南)
* [操作员工作表](#操作员工作表)
