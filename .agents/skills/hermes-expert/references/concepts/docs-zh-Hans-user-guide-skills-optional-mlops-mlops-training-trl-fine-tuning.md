# 使用 TRL 进行微调 — TRL：面向 LLM RLHF 的 SFT、DPO、PPO、GRPO 及奖励建模 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-training-trl-fine-tuning](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-training-trl-fine-tuning)
...
### 工作流 1：完整 RLHF 流水线（SFT → 奖励模型 → PPO）[​](#工作流-1完整-rlhf-流水线sft--奖励模型--ppo "工作流 1：完整 RLHF 流水线（SFT → 奖励模型 → PPO）的直接链接")
...
### 工作流 2：使用 DPO 进行简单偏好对齐[​](#工作流-2使用-dpo-进行简单偏好对齐 "工作流 2：使用 DPO 进行简单偏好对齐的直接链接")
...
### 工作流 3：使用 GRPO 进行内存高效的在线 RL[​](#工作流-3使用-grpo-进行内存高效的在线-rl "工作流 3：使用 GRPO 进行内存高效的在线 RL的直接链接")

以最小内存占用进行强化学习训练。

关于深入的 GRPO 指导——奖励函数设计、关键训练洞察（损失行为、模式崩溃、调参）以及高级多阶段模式——请参阅 **[references/grpo-training.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/training/trl-fine-tuning/references/grpo-training.md)**。生产就绪的训练脚本位于 **[templates/basic\_grpo\_training.py](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/training/trl-fine-tuning/templates/basic_grpo_training.py)**。
...
## 何时使用 TRL 及替代方案[​](#何时使用-trl-及替代方案 "何时使用 TRL 及替代方案的直接链接")
...
* [快速开始](#快速开始)
* [常见工作流](#常见工作流)
  + [工作流 1：完整 RLHF 流水线（SFT → 奖励模型 → PPO）](#工作流-1完整-rlhf-流水线sft--奖励模型--ppo)
  + [工作流 2：使用 DPO 进行简单偏好对齐](#工作流-2使用-dpo-进行简单偏好对齐)
  + [工作流 3：使用 GRPO 进行内存高效的在线 RL](#工作流-3使用-grpo-进行内存高效的在线-rl)
* [何时使用 TRL 及替代方案](#何时使用-trl-及替代方案)
* [常见问题](#常见问题)
* [高级主题](#高级主题)
* [硬件要求](#硬件要求)
* [资源](#资源)