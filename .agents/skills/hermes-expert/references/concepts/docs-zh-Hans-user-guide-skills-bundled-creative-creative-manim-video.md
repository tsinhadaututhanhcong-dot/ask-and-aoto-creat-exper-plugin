# Manim Video — Manim CE 动画：3Blue1Brown 数学/算法视频 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-manim-video](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-manim-video)
...
### 第一步：规划（plan.md）[​](#第一步规划planmd "第一步：规划（plan.md）的直接链接")

在写任何代码之前，先编写 `plan.md`。完整模板参见 `references/scene-planning.md`。

### 第二步：编码（script.py）[​](#第二步编码scriptpy "第二步：编码（script.py）的直接链接")
...
### LaTeX 使用原始字符串[​](#latex-使用原始字符串 "LaTeX 使用原始字符串的直接链接")
...
### 边缘文字 buff >= 0.5[​](#边缘文字-buff--05 "边缘文字 buff >= 0.5的直接链接")

```
label.to_edge(DOWN, buff=0.5)  # never < 0.5
```

### 替换文字前先 FadeOut[​](#替换文字前先-fadeout "替换文字前先 FadeOut的直接链接")

```
self.play(ReplacementTransform(note1, note2))  # not Write(note2) on top
```

### 永远不要对未添加的 Mobject 执行动画[​](#永远不要对未添加的-mobject-执行动画 "永远不要对未添加的 Mobject 执行动画的直接链接")
...
| `references/updaters-and-trackers.md` | ValueTracker、add\_updater、always\_redraw、基于时间的 updater、模式 |
...
## 创意发散（仅在用户要求实验性/创意性/独特输出时使用）[​](#创意发散仅在用户要求实验性创意性独特输出时使用 "创意发散（仅在用户要求实验性/创意性/独特输出时使用）的直接链接")
...
* [创作方向](#创作方向)
  ...
  + [场景间差异化](#场景间差异化)
* [工作流程](#工作流程)
  + [第一步：规划（plan.md）](#第一步规划planmd)
  + [第二步：编码（script.py）](#第二步编码scriptpy)
  + [第三步：渲染](#第三步渲染)
  + [第四步：拼接](#第四步拼接)
  + [第五步：审查](#第五步审查)
* [关键实现注意事项](#关键实现注意事项)
  + [LaTeX 使用原始字符串](#latex-使用原始字符串)
  + [边缘文字 buff >= 0.5](#边缘文字-buff--05)
  + [替换文字前先 FadeOut](#替换文字前先-fadeout)
  + [永远不要对未添加的 Mobject 执行动画](#永远不要对未添加的-mobject-执行动画)
* [性能目标](#性能目标)
* [参考文档](#参考文档)
* [创意发散（仅在用户要求实验性/创意性/独特输出时使用）](#创意发散仅在用户要求实验性创意性独特输出时使用)
