# 更新与卸载 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/updating](https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/updating)
...
1. **配对数据快照** — 保存一份轻量级的更新前状态快照（涵盖 `~/.hermes/pairing/`、飞书评论规则及其他运行时修改的状态文件）。可通过 [快照与回滚](/docs/zh-Hans/user-guide/checkpoints-and-rollback) 中描述的快照恢复流程进行恢复，或从 Hermes 写入 `~/.hermes/` 目录旁的最新快速快照 zip 文件中提取。
...
### 仅预览：`hermes update --check`[​](#仅预览hermes-update---check "仅预览hermes-update---check的直接链接")

想在拉取前确认是否有更新？运行 `hermes update --check` — 它会获取并与 `origin/main` 比较提交。不修改任何文件，不重启 gateway。适合在以"是否有更新"为条件的脚本和 cron 任务中使用。

### 完整更新前备份：`--backup`[​](#完整更新前备份--backup "完整更新前备份--backup的直接链接")
...
### Windows：另一个 `hermes.exe` 正在运行[​](#windows另一个-hermesexe-正在运行 "windows另一个-hermesexe-正在运行的直接链接")
...
### 更新后建议的验证步骤[​](#更新后建议的验证步骤 "更新后建议的验证步骤的直接链接")
...
### 终端在更新中途断开连接[​](#终端在更新中途断开连接 "终端在更新中途断开连接的直接链接")
...
### Nix 用户注意事项[​](#nix-用户注意事项 "Nix 用户注意事项的直接链接")
...
详情参见 [Nix 安装](/docs/zh-Hans/getting-started/nix-setup)。
...
* [更新](#更新)
  + [更新过程](#更新过程)
  + [仅预览：`hermes update --check`](#仅预览hermes-update---check)
  + [完整更新前备份：`--backup`](#完整更新前备份--backup)
  + [Windows：另一个 `hermes.exe` 正在运行](#windows另一个-hermesexe-正在运行)
  + [更新后建议的验证步骤](#更新后建议的验证步骤)
  + [终端在更新中途断开连接](#终端在更新中途断开连接)
  + [查看当前版本](#查看当前版本)
  + [从消息平台更新](#从消息平台更新)
  + [手动更新](#手动更新)
  + [回滚说明](#回滚说明)
  + [Nix 用户注意事项](#nix-用户注意事项)
* [卸载](#卸载)
  + [手动卸载](#手动卸载)
