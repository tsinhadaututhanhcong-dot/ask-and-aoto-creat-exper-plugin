# WeCom（企业微信） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/wecom](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/wecom)
...
### 第一步：创建 AI Bot[​](#第一步创建-ai-bot "第一步：创建 AI Bot的直接链接")

#### 推荐方式：扫码创建（一条命令）[​](#推荐方式扫码创建一条命令 "推荐方式：扫码创建（一条命令）的直接链接")
...
#### 备选方式：手动设置[​](#备选方式手动设置 "备选方式：手动设置的直接链接")
...
### 第三步：启动网关[​](#第三步启动网关 "第三步：启动网关的直接链接")
...
| `allow_from` | `[]` | 允许私聊的用户 ID（当 dm\_policy=allowlist 时） |
| `group_allow_from` | `[]` | 允许的群组 ID（当 group\_policy=allowlist 时） |
| `groups` | `{}` | 按群组配置（见下文） |
...
### 按群组的发送者白名单[​](#按群组的发送者白名单 "按群组的发送者白名单的直接链接")
...
### AES 加密媒体解密[​](#aes-加密媒体解密 "AES 加密媒体解密的直接链接")
...
## 回复模式流式响应[​](#回复模式流式响应 "回复模式流式响应的直接链接")
...
## 连接与重连[​](#连接与重连 "连接与重连的直接链接")

适配器在 `wss://openws.work.weixin.qq.com` 维护与 WeCom 网关的持久 WebSocket 连接。

### 连接生命周期[​](#连接生命周期 "连接生命周期的直接链接")

1. **连接：** 建立 WebSocket 连接，并发送包含 bot\_id 和 secret 的 `aibot_subscribe` 认证帧。
...
| `WeCom websocket closed during authentication` | 网络问题或凭据不正确。验证 bot\_id 和 secret。 |

* [前提条件](#前提条件)
* [设置](#设置)
  + [第一步：创建 AI Bot](#第一步创建-ai-bot)
...
* [访问策略](#访问策略)
  + [私聊策略](#私聊策略)
  + [群组策略](#群组策略)
  + [按群组的发送者白名单](#按群组的发送者白名单)
* [媒体支持](#媒体支持)
  + [入站（接收）](#入站接收)
  + [AES 加密媒体解密](#aes-加密媒体解密)
  + [出站（发送）](#出站发送)
* [回复模式流式响应](#回复模式流式响应)
* [连接与重连](#连接与重连)
  + [连接生命周期](#连接生命周期)
