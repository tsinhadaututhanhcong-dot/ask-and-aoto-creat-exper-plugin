...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/xai-grok-oauth](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/xai-grok-oauth)
...
## 直连 xAI 工具（TTS / 图像 / 视频 / 转录 / X 搜索）[​](#直连-xai-工具tts--图像--视频--转录--x-搜索 "直连 xAI 工具（TTS / 图像 / 视频 / 转录 / X 搜索）的直接链接")
...
### Token 过期——未自动重新登录[​](#token-过期未自动重新登录 "Token 过期——未自动重新登录的直接链接")
...
回环重定向类 provider（Spotify、MCP 服务器）请参阅 [OAuth over SSH / Remote Hosts](/docs/zh-Hans/guides/oauth-over-ssh)。

### 登录成功后 HTTP 403（套餐 / 权限问题）[​](#登录成功后-http-403套餐--权限问题 "登录成功后 HTTP 403（套餐 / 权限问题）的直接链接")
...
### 运行时出现"No xAI credentials found"错误[​](#运行时出现no-xai-credentials-found错误 "运行时出现\"No xAI credentials found\"错误的直接链接")
...
* [OAuth over SSH / Remote Hosts](/docs/zh-Hans/guides/oauth-over-ssh) — 如果 Hermes 与浏览器不在同一台机器上，必读
...
* [语音与 TTS](/docs/zh-Hans/user-guide/features/tts)
...
* [直连 xAI 工具（TTS / 图像 / 视频 / 转录 / X 搜索）](#直连-xai-工具tts--图像--视频--转录--x-搜索)
  + [模型](#模型)
* [环境变量](#环境变量)
* [故障排查](#故障排查)
  + [Token 过期——未自动重新登录](#token-过期未自动重新登录)
  + [授权超时](#授权超时)
  + [从远程服务器登录](#从远程服务器登录)
  + [登录成功后 HTTP 403（套餐 / 权限问题）](#登录成功后-http-403套餐--权限问题)
  + [运行时出现"No xAI credentials found"错误](#运行时出现no-xai-credentials-found错误)
* [退出登录](#退出登录)
* [另请参阅](#另请参阅)
