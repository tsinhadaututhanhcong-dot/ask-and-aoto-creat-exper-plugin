# Cron 故障排查 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/cron-troubleshooting](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/cron-troubleshooting)
...
## 任务未触发[​](#任务未触发 "任务未触发的直接链接")

### 检查 1：确认任务存在且处于活跃状态[​](#检查-1确认任务存在且处于活跃状态 "检查 1：确认任务存在且处于活跃状态的直接链接")
...
### 检查 2：确认调度表达式正确[​](#检查-2确认调度表达式正确 "检查 2：确认调度表达式正确的直接链接")
...
### 检查 3：gateway 是否正在运行？[​](#检查-3gateway-是否正在运行 "检查 3：gateway 是否正在运行？的直接链接")
...
### 检查 4：检查系统时钟和时区[​](#检查-4检查系统时钟和时区 "检查 4：检查系统时钟和时区的直接链接")
...
### 检查 1：确认投递目标正确[​](#检查-1确认投递目标正确 "检查 1：确认投递目标正确的直接链接")
...
### 检查 2：检查 `[SILENT]` 的使用[​](#检查-2检查-silent-的使用 "检查-2检查-silent-的使用的直接链接")
...
### 检查 3：平台 token 权限[​](#检查-3平台-token-权限 "检查 3：平台 token 权限的直接链接")
...
### 检查 4：响应包装[​](#检查-4响应包装 "检查 4：响应包装的直接链接")
...
## Skill 加载失败[​](#skill-加载失败 "Skill 加载失败的直接链接")

### 检查 1：确认 skill 已安装[​](#检查-1确认-skill-已安装 "检查 1：确认 skill 已安装的直接链接")
...
### 检查 2：检查 skill 名称与 skill 文件夹名称[​](#检查-2检查-skill-名称与-skill-文件夹名称 "检查 2：检查 skill 名称与 skill 文件夹名称的直接链接")
...
### 检查 3：依赖交互式工具的 skill[​](#检查-3依赖交互式工具的-skill "检查 3：依赖交互式工具的 skill的直接链接")
...
### 检查 4：多 skill 加载顺序[​](#检查-4多-skill-加载顺序 "检查 4：多 skill 加载顺序的直接链接")
...
## 任务错误与失败[​](#任务错误与失败 "任务错误与失败的直接链接")

### 检查 1：查看近期任务输出[​](#检查-1查看近期任务输出 "检查 1：查看近期任务输出的直接链接")
...
### 检查 2：常见错误模式[​](#检查-2常见错误模式 "检查 2：常见错误模式的直接链接")
...
### 检查 3：锁竞争[​](#检查-3锁竞争 "检查 3：锁竞争的直接链接")
...
### 检查 4：jobs.json 的权限[​](#检查-4jobsjson-的权限 "检查 4：jobs.json 的权限的直接链接")
...
*完整的 cron 参考文档，请参阅 [用 Cron 自动化一切](/docs/zh-Hans/guides/automate-with-cron) 和 [定时任务（Cron）](/docs/zh-Hans/user-guide/features/cron)。*

* [任务未触发](#任务未触发)
  + [检查 1：确认任务存在且处于活跃状态](#检查-1确认任务存在且处于活跃状态)
  + [检查 2：确认调度表达式正确](#检查-2确认调度表达式正确)
  + [检查 3：gateway 是否正在运行？](#检查-3gateway-是否正在运行)
  + [检查 4：检查系统时钟和时区](#检查-4检查系统时钟和时区)
* [投递失败](#投递失败)
  + [检查 1：确认投递目标正确](#检查-1确认投递目标正确)
  + [检查 2：检查 `[SILENT]` 的使用](#检查-2检查-silent-的使用)
  + [检查 3：平台 token 权限](#检查-3平台-token-权限)
  + [检查 4：响应包装](#检查-4响应包装)
* [Skill 加载失败](#skill-加载失败)
  + [检查 1：确认 skill 已安装](#检查-1确认-skill-已安装)
  + [检查 2：检查 skill 名称与 skill 文件夹名称](#检查-2检查-skill-名称与-skill-文件夹名称)
  + [检查 3：依赖交互式工具的 skill](#检查-3依赖交互式工具的-skill)
  + [检查 4：多 skill 加载顺序](#检查-4多-skill-加载顺序)
* [任务错误与失败](#任务错误与失败)
  + [检查 1：查看近期任务输出](#检查-1查看近期任务输出)
  + [检查 2：常见错误模式](#检查-2常见错误模式)
  + [检查 3：锁竞争](#检查-3锁竞争)
  + [检查 4：jobs.json 的权限](#检查-4jobsjson-的权限)
* [性能问题](#性能问题)
  + [任务启动缓慢](#任务启动缓慢)
  + [过多任务重叠](#过多任务重叠)
  + [脚本输出过大](#脚本输出过大)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/automate-with-cron](./docs-zh-Hans-guides-automate-with-cron.md)
