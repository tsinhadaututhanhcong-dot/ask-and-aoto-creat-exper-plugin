# 扩展 Dashboard | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/extending-the-dashboard](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/extending-the-dashboard)
...
如果只是想使用 dashboard，请参阅 [Web Dashboard](/docs/zh-Hans/user-guide/features/web-dashboard)。如果想为终端 CLI（而非 Web Dashboard）换肤，请参阅 [Skins & Themes](/docs/zh-Hans/user-guide/features/skins) —— CLI 皮肤系统与 dashboard 主题无关。
...
### 快速上手——你的第一个主题[​](#快速上手你的第一个主题 "快速上手——你的第一个主题的直接链接")
...
### 调色板、字体排版、布局[​](#调色板字体排版布局 "调色板、字体排版、布局的直接链接")

这三个块是主题的核心。每个块相互独立——覆盖其中一个，其余保持不变。

#### 调色板（3 层）[​](#调色板3-层 "调色板（3 层）的直接链接")
...
### 主题资源（图片作为 CSS 变量）[​](#主题资源图片作为-css-变量 "主题资源（图片作为 CSS 变量）的直接链接")
...
### 原始 `customCSS`[​](#原始-customcss "原始-customcss的直接链接")
...
### 完整主题 YAML 参考[​](#完整主题-yaml-参考 "完整主题 YAML 参考的直接链接")
...
### 快速上手——你的第一个插件[​](#快速上手你的第一个插件 "快速上手——你的第一个插件的直接链接")
...
* `plugin.yaml` + `__init__.py` — CLI/gateway 插件（[参见插件页面](/docs/zh-Hans/user-guide/features/plugins)）。
* `dashboard/manifest.json` + `dashboard/dist/index.js` — dashboard UI 插件。
* `dashboard/plugin_api.py` — dashboard 后端路由。

三者均非必须；按需包含所需层次即可。

### Manifest 参考[​](#manifest-参考 "Manifest 参考的直接链接")
...
### Plugin SDK[​](#plugin-sdk "Plugin SDK的直接链接")
...
#### 调用内置 Hermes 端点[​](#调用内置-hermes-端点 "调用内置 Hermes 端点的直接链接")
...
完整列表参见 [Web Dashboard → REST API](/docs/zh-Hans/user-guide/features/web-dashboard#rest-api)。

### Shell 插槽[​](#shell-插槽 "Shell 插槽的直接链接")
...
#### 重复注册与 HMR[​](#重复注册与-hmr "重复注册与 HMR的直接链接")

如果同一个 `(plugin, slot)` 对被注册两次，后一次调用会替换前一次——这与 React HMR 期望插件重新挂载时的行为一致。

### 替换内置页面（`tab.override`）[​](#替换内置页面taboverride "替换内置页面taboverride的直接链接")
...
### 增强内置页面（页面级插槽）[​](#增强内置页面页面级插槽 "增强内置页面（页面级插槽）的直接链接")
...
### 仅插槽插件（`tab.hidden`）[​](#仅插槽插件tabhidden "仅插槽插件tabhidden的直接链接")
...
### 后端 API 路由[​](#后端-api-路由 "后端 API 路由的直接链接")
...
#### 访问 Hermes 内部模块[​](#访问-hermes-内部模块 "访问 Hermes 内部模块的直接链接")
...
### 插件自定义 CSS[​](#插件自定义-css "插件自定义 CSS的直接链接")
...
#### 插件加载生命周期[​](#插件加载生命周期 "插件加载生命周期的直接链接")
...
## 主题 + 插件组合演示[​](#主题--插件组合演示 "主题 + 插件组合演示的直接链接")
...
### `window` 上的 SDK[​](#window-上的-sdk "window-上的-sdk的直接链接")
...
* [目录](#目录)
* [主题](#主题)
  + [快速上手——你的第一个主题](#快速上手你的第一个主题)
  + [调色板、字体排版、布局](#调色板字体排版布局)
  + [布局变体](#布局变体)
