# 视频生成 Provider 插件 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin)
...
视频生成与[图像生成 Provider 插件](/docs/zh-Hans/developer-guide/image-gen-provider-plugin)几乎一一对应——如果你已构建过图像生成后端，对其结构应已了然于胸。主要区别在于：`capabilities()` 方法用于声明模态（modality）/宽高比/时长，以及路由约定（传入 `image_url` 则使用图生视频，省略则使用文生视频——provider 在内部选择正确的端点）。

## 统一接口（一个工具，两种模态）[​](#统一接口一个工具两种模态 "统一接口（一个工具，两种模态）的直接链接")
...
## VideoGenProvider ABC[​](#videogenprovider-abc "VideoGenProvider ABC的直接链接")
...
## `video_generate` 参数模式[​](#video_generate-参数模式 "video_generate-参数模式的直接链接")
...
## 模型系列与端点路由（FAL 模式）[​](#模型系列与端点路由fal-模式 "模型系列与端点路由（FAL 模式）的直接链接")
...
* [统一接口（一个工具，两种模态）](#统一接口一个工具两种模态)
* [发现机制](#发现机制)
* [目录结构](#目录结构)
* [VideoGenProvider ABC](#videogenprovider-abc)
* [插件清单](#插件清单)
* [`video_generate` 参数模式](#video_generate-参数模式)
* [模型系列与端点路由（FAL 模式）](#模型系列与端点路由fal-模式)
* [选择优先级](#选择优先级)
* [响应结构](#响应结构)
* [产物保存位置](#产物保存位置)
* [测试](#测试)
