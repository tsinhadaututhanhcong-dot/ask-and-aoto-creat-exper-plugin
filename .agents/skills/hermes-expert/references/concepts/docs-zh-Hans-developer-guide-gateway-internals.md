# Gateway 内部机制 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/gateway-internals](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/gateway-internals)
...
3. **GatewayRunner.\_handle\_message()** 接收事件：
...
支持线程的平台（Telegram 论坛话题、Discord 线程、Slack 线程）可能在 chat\_id 部分包含线程 ID。**切勿手动构造会话键** — 请始终使用 `gateway/session.py` 中的 `build_session_key()`。

### 两级消息守卫[​](#两级消息守卫 "两级消息守卫的直接链接")
...
### 运行中 Agent 守卫[​](#运行中-agent-守卫 "运行中 Agent 守卫的直接链接")
...
* **显式目标投递** — `send_message` 工具指定 `telegram:-1001234567890`，或通过 [`hermes send` CLI](/docs/zh-Hans/guides/pipe-script-output) 封装同一工具供 shell 脚本使用
* **跨平台投递** — 投递至与原始消息不同的平台
...
### Gateway Hook 事件[​](#gateway-hook-事件 "Gateway Hook 事件的直接链接")
...
### 内存刷写生命周期[​](#内存刷写生命周期 "内存刷写生命周期的直接链接")
...
* [消息 Gateway（用户指南）](/docs/zh-Hans/user-guide/messaging)
...
* [授权](#授权)
  + [DM 配对流程](#dm-配对流程)
* [斜杠命令分发](#斜杠命令分发)
  + [运行中 Agent 守卫](#运行中-agent-守卫)
* [配置来源](#配置来源)
* [平台适配器](#平台适配器)
  + [Token 锁](#token-锁)
* [投递路径](#投递路径)
* [Hooks](#hooks)
  + [Gateway Hook 事件](#gateway-hook-事件)
* [内存提供者集成](#内存提供者集成)
  + [内存刷写生命周期](#内存刷写生命周期)
* [后台维护](#后台维护)
* [进程管理](#进程管理)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/pipe-script-output](./docs-zh-Hans-guides-pipe-script-output.md)
