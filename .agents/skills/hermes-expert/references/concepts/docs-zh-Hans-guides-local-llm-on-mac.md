# 在 Mac 上运行本地 LLM | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/local-llm-on-mac](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/local-llm-on-mac)
...
| --- | --- | --- | --- |
| Qwen3.5-9B-Q4\_K\_M (GGUF) | 5.3 GB | ~10 GB（含量化 KV 缓存） | llama.cpp |
...
## 方案 A：llama.cpp[​](#方案-allamacpp "方案 A：llama.cpp的直接链接")
...
### 内存受限系统的优化[​](#内存受限系统的优化 "内存受限系统的优化的直接链接")
...
如果仍然内存不足，优先减小上下文大小（`-c`），然后尝试更小的量化级别（Q3\_K\_M 代替 Q4\_K\_M）。
...
## 方案 B：通过 omlx 使用 MLX[​](#方案-b通过-omlx-使用-mlx "方案 B：通过 omlx 使用 MLX的直接链接")
...
## 基准测试：llama.cpp vs MLX[​](#基准测试llamacpp-vs-mlx "基准测试：llama.cpp vs MLX的直接链接")

两个后端在同一台机器（Apple M5 Max，128 GB 统一内存）上测试，使用相同模型（Qwen3.5-9B），量化级别相当（GGUF 使用 Q4\_K\_M，MLX 使用 mxfp4）。五个不同 prompt，每个运行三次，后端顺序测试以避免资源竞争。

### 结果[​](#结果 "结果的直接链接")

| 指标 | llama.cpp (Q4\_K\_M) | MLX (mxfp4) | 胜者 |
| --- | --- | --- | --- |
...
## 连接 Hermes[​](#连接-hermes "连接 Hermes的直接链接")
...
* [方案 A：llama.cpp](#方案-allamacpp)
  ...
  + [测试](#测试)
  + [获取模型名称](#获取模型名称)
* [方案 B：通过 omlx 使用 MLX](#方案-b通过-omlx-使用-mlx)
  + [安装](#安装-1)
  + [下载模型](#下载模型-1)
  + [启动服务器](#启动服务器-1)
  + [测试](#测试-1)
  + [列出可用模型](#列出可用模型)
* [基准测试：llama.cpp vs MLX](#基准测试llamacpp-vs-mlx)
  + [结果](#结果)
  + [含义解读](#含义解读)
  + [如何选择？](#如何选择)
* [连接 Hermes](#连接-hermes)
