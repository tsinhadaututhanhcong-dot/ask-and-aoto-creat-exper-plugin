# Fastmcp — 使用 FastMCP 在 Python 中构建、测试、检查、安装和部署 MCP 服务器 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mcp/mcp-fastmcp](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mcp/mcp-fastmcp)
...
| 相关 skill | [`native-mcp`](/docs/zh-Hans/user-guide/skills/bundled/mcp/mcp-native-mcp), [`mcporter`](/docs/zh-Hans/user-guide/skills/optional/mcp/mcp-mcporter) |
...
### 1. 选择最小可行的服务器形态[​](#1-选择最小可行的服务器形态 "1. 选择最小可行的服务器形态的直接链接")
...
### 2. 从模板脚手架生成[​](#2-从模板脚手架生成 "2. 从模板脚手架生成的直接链接")
...
### 3. 优先实现工具[​](#3-优先实现工具 "3. 优先实现工具的直接链接")
...
### 4. 仅在有帮助时添加资源和 Prompt[​](#4-仅在有帮助时添加资源和-prompt "4. 仅在有帮助时添加资源和 Prompt的直接链接")
...
### 5. 集成前先测试服务器[​](#5-集成前先测试服务器 "5. 集成前先测试服务器的直接链接")
...
### 6. 本地验证通过后安装到客户端[​](#6-本地验证通过后安装到客户端 "6. 本地验证通过后安装到客户端的直接链接")
...
### 7. 本地契约稳定后再部署[​](#7-本地契约稳定后再部署 "7. 本地契约稳定后再部署的直接链接")
...
### API 封装模式[​](#api-封装模式 "API 封装模式的直接链接")
...
### FastMCP 命令缺失[​](#fastmcp-命令缺失 "FastMCP 命令缺失的直接链接")
...
### `fastmcp inspect` 失败[​](#fastmcp-inspect-失败 "fastmcp-inspect-失败的直接链接")
...
### 工具在 Python 中正常但通过 CLI 不工作[​](#工具在-python-中正常但通过-cli-不工作 "工具在 Python 中正常但通过 CLI 不工作的直接链接")
...
* [包含文件](#包含文件)
  ...
  + [参考资料](#参考资料)
* [工作流](#工作流)
  + [1. 选择最小可行的服务器形态](#1-选择最小可行的服务器形态)
  + [2. 从模板脚手架生成](#2-从模板脚手架生成)
  + [3. 优先实现工具](#3-优先实现工具)
  + [4. 仅在有帮助时添加资源和 Prompt](#4-仅在有帮助时添加资源和-prompt)
  + [5. 集成前先测试服务器](#5-集成前先测试服务器)
  + [6. 本地验证通过后安装到客户端](#6-本地验证通过后安装到客户端)
  + [7. 本地契约稳定后再部署](#7-本地契约稳定后再部署)
* [常见模式](#常见模式)
  + [API 封装模式](#api-封装模式)
  + [数据库模式](#数据库模式)
  + [文件处理器模式](#文件处理器模式)
* [质量标准](#质量标准)
* [故障排查](#故障排查)
  + [FastMCP 命令缺失](#fastmcp-命令缺失)
  + [`fastmcp inspect` 失败](#fastmcp-inspect-失败)
  + [工具在 Python 中正常但通过 CLI 不工作](#工具在-python-中正常但通过-cli-不工作)
  + [Hermes 无法看到已部署的服务器](#hermes-无法看到已部署的服务器)
* [参考资料](#参考资料-1)
