...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/web-dashboard](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/web-dashboard)
...
### Status（状态）[​](#status状态 "Status（状态）的直接链接")
...
### Config（配置）[​](#config配置 "Config（配置）的直接链接")
...
### API Keys（API 密钥）[​](#api-keysapi-密钥 "API Keys（API 密钥）的直接链接")
...
### Sessions（会话）[​](#sessions会话 "Sessions（会话）的直接链接")
...
### Analytics（分析）[​](#analytics分析 "Analytics（分析）的直接链接")
...
### Skills（技能）[​](#skills技能 "Skills（技能）的直接链接")
...
## `/reload` 斜杠命令[​](#reload-斜杠命令 "reload-斜杠命令的直接链接")
...
## REST API[​](#rest-api "REST API的直接链接")

Web Dashboard 暴露了一个供前端使用的 REST API。你也可以直接调用这些端点进行自动化操作：

### GET /api/status[​](#get-apistatus "GET /api/status的直接链接")

返回 agent 版本、gateway 状态、平台状态和活跃会话数。

### GET /api/sessions[​](#get-apisessions "GET /api/sessions的直接链接")

返回最近 20 个会话的元数据（模型、token 数、时间戳、预览）。

### GET /api/config[​](#get-apiconfig "GET /api/config的直接链接")

以 JSON 格式返回当前 `config.yaml` 内容。

### GET /api/config/defaults[​](#get-apiconfigdefaults "GET /api/config/defaults的直接链接")

返回默认配置值。

### GET /api/config/schema[​](#get-apiconfigschema "GET /api/config/schema的直接链接")

返回描述每个配置字段的 schema——类型、描述、类别，以及适用时的选项。前端使用此 schema 为每个字段渲染正确的输入控件。

### PUT /api/config[​](#put-apiconfig "PUT /api/config的直接链接")

保存新配置。请求体：`{"config": {...}}`。

### GET /api/env[​](#get-apienv "GET /api/env的直接链接")

返回所有已知环境变量，包含其设置/未设置状态、脱敏值、描述和类别。

### PUT /api/env[​](#put-apienv "PUT /api/env的直接链接")

设置环境变量。请求体：`{"key": "VAR_NAME", "value": "secret"}`。

### DELETE /api/env[​](#delete-apienv "DELETE /api/env的直接链接")

删除环境变量。请求体：`{"key": "VAR_NAME"}`。

### GET /api/sessions/{session\_id}[​](#get-apisessionssession_id "GET /api/sessions/{session_id}的直接链接")

返回单个会话的元数据。

### GET /api/sessions/{session\_id}/messages[​](#get-apisessionssession_idmessages "GET /api/sessions/{session_id}/messages的直接链接")

返回会话的完整消息历史，包含工具调用和时间戳。

### GET /api/sessions/search[​](#get-apisessionssearch "GET /api/sessions/search的直接链接")

对消息内容进行全文搜索。查询参数：`q`。返回匹配的会话 ID 和高亮片段。

### DELETE /api/sessions/{session\_id}[​](#delete-apisessionssession_id "DELETE /api/sessions/{session_id}的直接链接")

删除会话及其消息历史。

### GET /api/logs[​](#get-apilogs "GET /api/logs的直接链接")

返回日志行。查询参数：`file`（agent/errors/gateway）、`lines`（数量）、`level`、`component`。

### GET /api/analytics/usage[​](#get-apianalyticsusage "GET /api/analytics/usage的直接链接")

返回 token 用量、成本和会话分析。查询参数：`days`（默认 30）。响应包含每日明细和按模型聚合数据。

### GET /api/cron/jobs[​](#get-apicronjobs "GET /api/cron/jobs的直接链接")

返回所有已配置的定时任务，包含其状态、计划和运行历史。

### POST /api/cron/jobs[​](#post-apicronjobs "POST /api/cron/jobs的直接链接")

创建新定时任务。请求体：`{"prompt": "...", "schedule": "0 9 * * *", "name": "...", "deliver": "local"}`。

### POST /api/cron/jobs/{job\_id}/pause[​](#post-apicronjobsjob_idpause "POST /api/cron/jobs/{job_id}/pause的直接链接")

暂停定时任务。

### POST /api/cron/jobs/{job\_id}/resume[​](#post-apicronjobsjob_idresume "POST /api/cron/jobs/{job_id}/resume的直接链接")

恢复已暂停的定时任务。

### POST /api/cron/jobs/{job\_id}/trigger[​](#post-apicronjobsjob_idtrigger "POST /api/cron/jobs/{job_id}/trigger的直接链接")

在计划之外立即触发定时任务。

### DELETE /api/cron/jobs/{job\_id}[​](#delete-apicronjobsjob_id "DELETE /api/cron/jobs/{job_id}的直接链接")

删除定时任务。

### GET /api/skills[​](#get-apiskills "GET /api/skills的直接链接")

返回所有技能，包含其名称、描述、类别和启用状态。

### PUT /api/skills/toggle[​](#put-apiskillstoggle "PUT /api/skills/toggle的直接链接")

启用或禁用技能。请求体：`{"name": "skill-name", "enabled": true}`。

### GET /api/tools/toolsets[​](#get-apitoolstoolsets "GET /api/tools/toolsets的直接链接")
...
如需构建自定义主题、添加插件标签页、注入 shell 插槽或暴露插件专属 REST 端点，请参阅 **[扩展 Dashboard](/docs/zh-Hans/user-guide/features/extending-the-dashboard)**——完整指南涵盖：
...
* [页面](#页面)
  ...
  + [Chat（聊天）](#chat聊天)
  + [Config（配置）](#config配置)
  + [API Keys（API 密钥）](#api-keysapi-密钥)
  + [Sessions（会话）](#sessions会话)
  + [Logs（日志）](#logs日志)
  + [Analytics（分析）](#analytics分析)
  + [Cron（定时任务）](#cron定时任务)
  + [Skills（技能）](#skills技能)
* [`/reload` 斜杠命令](#reload-斜杠命令)
* [REST API](#rest-api)
