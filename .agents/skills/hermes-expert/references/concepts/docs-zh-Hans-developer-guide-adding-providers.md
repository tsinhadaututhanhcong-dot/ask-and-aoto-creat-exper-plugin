# 添加 Provider | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/adding-providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/adding-providers)
...
## 首先选择实现路径[​](#首先选择实现路径 "首先选择实现路径的直接链接")

### 路径 A——OpenAI 兼容 provider[​](#路径-aopenai-兼容-provider "路径 A——OpenAI 兼容 provider的直接链接")
...
### 路径 B——原生 provider[​](#路径-b原生-provider "路径 B——原生 provider的直接链接")
...
### 每个内置 provider 都必须修改[​](#每个内置-provider-都必须修改 "每个内置 provider 都必须修改的直接链接")
...
### 原生 / 非 OpenAI provider 额外需要[​](#原生--非-openai-provider-额外需要 "原生 / 非 OpenAI provider 额外需要的直接链接")

10. `agent/<provider>_adapter.py`
11. `run_agent.py`
12. 如果需要 provider SDK，则修改 `pyproject.toml`

## 快速路径：简单 API key provider[​](#快速路径简单-api-key-provider "快速路径：简单 API key provider的直接链接")
...
参见 `plugins/model-providers/nvidia/` 或 `plugins/model-providers/gmi/` 作为模板，以及完整的 [Model Provider Plugin 指南](/docs/zh-Hans/developer-guide/model-provider-plugin)，了解字段参考、hook 用法和端到端示例。

## 完整路径：OAuth 和复杂 provider[​](#完整路径oauth-和复杂-provider "完整路径：OAuth 和复杂 provider的直接链接")
...
## 第 1 步：选择一个规范的 provider id[​](#第-1-步选择一个规范的-provider-id "第 1 步：选择一个规范的 provider id的直接链接")
...
## 第 2 步：在 `hermes_cli/auth.py` 中添加认证元数据[​](#第-2-步在-hermes_cliauthpy-中添加认证元数据 "第-2-步在-hermes_cliauthpy-中添加认证元数据的直接链接")
...
## 第 3 步：在 `hermes_cli/models.py` 中添加模型目录和别名[​](#第-3-步在-hermes_climodelspy-中添加模型目录和别名 "第-3-步在-hermes_climodelspy-中添加模型目录和别名的直接链接")
...
## 第 4 步：在 `hermes_cli/runtime_provider.py` 中解析运行时数据[​](#第-4-步在-hermes_cliruntime_providerpy-中解析运行时数据 "第-4-步在-hermes_cliruntime_providerpy-中解析运行时数据的直接链接")
...
## 第 5 步：在 `hermes_cli/main.py` 中接线 CLI[​](#第-5-步在-hermes_climainpy-中接线-cli "第-5-步在-hermes_climainpy-中接线-cli的直接链接")
...
## 第 6 步：保持辅助调用正常工作[​](#第-6-步保持辅助调用正常工作 "第 6 步：保持辅助调用正常工作的直接链接")

这里有两个文件需要关注：

### `agent/auxiliary_client.py`[​](#agentauxiliary_clientpy "agentauxiliary_clientpy的直接链接")
...
### `agent/model_metadata.py`[​](#agentmodel_metadatapy "agentmodel_metadatapy的直接链接")

为 provider 的模型添加上下文长度，以保持 token 预算、压缩阈值和限制的合理性。

## 第 7 步：如果 provider 是原生的，添加适配器和 `run_agent.py` 支持[​](#第-7-步如果-provider-是原生的添加适配器和-run_agentpy-支持 "第-7-步如果-provider-是原生的添加适配器和-run_agentpy-支持的直接链接")
...
### `run_agent.py`[​](#run_agentpy "run_agentpy的直接链接")
...
### Prompt 缓存和 provider 专属请求字段[​](#prompt-缓存和-provider-专属请求字段 "Prompt 缓存和 provider 专属请求字段的直接链接")
...
## 第 8 步：测试[​](#第-8-步测试 "第 8 步：测试的直接链接")
...
## 第 9 步：实时验证[​](#第-9-步实时验证 "第 9 步：实时验证的直接链接")
...
## 第 10 步：更新用户文档[​](#第-10-步更新用户文档 "第 10 步：更新用户文档的直接链接")
...
## OpenAI 兼容 provider 清单[​](#openai-兼容-provider-清单 "OpenAI 兼容 provider 清单的直接链接")
...
## 原生 provider 清单[​](#原生-provider-清单 "原生 provider 清单的直接链接")
...
### 1. 将 provider 添加到 auth 但未添加到模型解析[​](#1-将-provider-添加到-auth-但未添加到模型解析 "1. 将 provider 添加到 auth 但未添加到模型解析的直接链接")

这会导致凭据解析正确，而 `/model` 和 `provider:model` 输入失败。

### 2. 忘记 `config["model"]` 可以是字符串或字典[​](#2-忘记-configmodel-可以是字符串或字典 "2-忘记-configmodel-可以是字符串或字典的直接链接")

大量 provider 选择代码必须对两种形式进行规范化。

### 3. 假设必须使用内置 provider[​](#3-假设必须使用内置-provider "3. 假设必须使用内置 provider的直接链接")

如果该服务只是 OpenAI 兼容的，自定义 provider 可能已经以更少的维护成本解决了用户问题。

### 4. 忘记辅助路径[​](#4-忘记辅助路径 "4. 忘记辅助路径的直接链接")

主聊天路径可能正常工作，而摘要、记忆刷新或视觉辅助失败，因为辅助路由从未更新。

### 5. 原生 provider 分支隐藏在 `run_agent.py` 中[​](#5-原生-provider-分支隐藏在-run_agentpy-中 "5-原生-provider-分支隐藏在-run_agentpy-中的直接链接")

搜索 `api_mode` 和 `self.client.`。不要假设显而易见的请求路径是唯一的。

### 6. 将 OpenRouter 专属字段发送给其他 provider[​](#6-将-openrouter-专属字段发送给其他-provider "6. 将 OpenRouter 专属字段发送给其他 provider的直接链接")

provider 路由等字段只属于支持它们的 provider。

### 7. 更新了 `hermes model` 但未更新 `hermes setup`[​](#7-更新了-hermes-model-但未更新-hermes-setup "7-更新了-hermes-model-但未更新-hermes-setup的直接链接")

两个流程都需要了解该 provider。

## 实现时的好搜索目标[​](#实现时的好搜索目标 "实现时的好搜索目标的直接链接")
...
* [Provider 运行时解析](/docs/zh-Hans/developer-guide/provider-runtime)
* [架构](/docs/zh-Hans/developer-guide/architecture)
* [贡献指南](/docs/zh-Hans/developer-guide/contributing)

* [心智模型](#心智模型)
* [首先选择实现路径](#首先选择实现路径)
  + [路径 A——OpenAI 兼容 provider](#路径-aopenai-兼容-provider)
  + [路径 B——原生 provider](#路径-b原生-provider)
* [文件清单](#文件清单)
  + [每个内置 provider 都必须修改](#每个内置-provider-都必须修改)
  + [原生 / 非 OpenAI provider 额外需要](#原生--非-openai-provider-额外需要)
* [快速路径：简单 API key provider](#快速路径简单-api-key-provider)
* [完整路径：OAuth 和复杂 provider](#完整路径oauth-和复杂-provider)
* [第 1 步：选择一个规范的 provider id](#第-1-步选择一个规范的-provider-id)
* [第 2 步：在 `hermes_cli/auth.py` 中添加认证元数据](#第-2-步在-hermes_cliauthpy-中添加认证元数据)
* [第 3 步：在 `hermes_cli/models.py` 中添加模型目录和别名](#第-3-步在-hermes_climodelspy-中添加模型目录和别名)
* [第 4 步：在 `hermes_cli/runtime_provider.py` 中解析运行时数据](#第-4-步在-hermes_cliruntime_providerpy-中解析运行时数据)
* [第 5 步：在 `hermes_cli/main.py` 中接线 CLI](#第-5-步在-hermes_climainpy-中接线-cli)
* [第 6 步：保持辅助调用正常工作](#第-6-步保持辅助调用正常工作)
  + [`agent/auxiliary_client.py`](#agentauxiliary_clientpy)
  + [`agent/model_metadata.py`](#agentmodel_metadatapy)
* [第 7 步：如果 provider 是原生的，添加适配器和 `run_agent.py` 支持](#第-7-步如果-provider-是原生的添加适配器和-run_agentpy-支持)
  + [新适配器文件](#新适配器文件)
  + [`run_agent.py`](#run_agentpy)
  + [Prompt 缓存和 provider 专属请求字段](#prompt-缓存和-provider-专属请求字段)
* [第 8 步：测试](#第-8-步测试)
* [第 9 步：实时验证](#第-9-步实时验证)
* [第 10 步：更新用户文档](#第-10-步更新用户文档)
* [OpenAI 兼容 provider 清单](#openai-兼容-provider-清单)
* [原生 provider 清单](#原生-provider-清单)
* [常见陷阱](#常见陷阱)
  + [1. 将 provider 添加到 auth 但未添加到模型解析](#1-将-provider-添加到-auth-但未添加到模型解析)
  + [2. 忘记 `config["model"]` 可以是字符串或字典](#2-忘记-configmodel-可以是字符串或字典)
  + [3. 假设必须使用内置 provider](#3-假设必须使用内置-provider)
  + [4. 忘记辅助路径](#4-忘记辅助路径)
  + [5. 原生 provider 分支隐藏在 `run_agent.py` 中](#5-原生-provider-分支隐藏在-run_agentpy-中)
  + [6. 将 OpenRouter 专属字段发送给其他 provider](#6-将-openrouter-专属字段发送给其他-provider)
  + [7. 更新了 `hermes model` 但未更新 `hermes setup`](#7-更新了-hermes-model-但未更新-hermes-setup)
* [实现时的好搜索目标](#实现时的好搜索目标)
* [相关文档](#相关文档)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/provider-runtime](./docs-zh-Hans-developer-guide-provider-runtime.md)
