# 语音模式 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/voice-mode](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/voice-mode)

本页总览

Hermes Agent 支持在 CLI 和消息平台上进行完整的语音交互。通过麦克风与 Agent 对话，听取语音回复，并在 Discord 语音频道中进行实时语音对话。

如需包含推荐配置和实际使用模式的实践指南，请参阅 [使用 Hermes 的语音模式](/docs/zh-Hans/guides/use-voice-mode-with-hermes)。

## 前提条件[​](#前提条件 "前提条件的直接链接")

使用语音功能前，请确保已完成以下准备：

1. **已安装 Hermes Agent** — 通过安装脚本（参见 [安装](/docs/zh-Hans/getting-started/installation)）
...
付费的 [Nous Portal](/docs/zh-Hans/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`hermes setup --portal` 可一次性完成两项配置。
...
### Python 包[​](#python-包 "Python 包的直接链接")
...
## CLI 语音模式[​](#cli-语音模式 "CLI 语音模式的直接链接")
...
### 幻觉过滤器[​](#幻觉过滤器 "幻觉过滤器的直接链接")

Whisper 有时会从静音或背景噪音中生成幻觉文字（如"Thank you for watching"、"Subscribe"等）。Agent 使用包含 26 个已知幻觉短语（覆盖多种语言）的列表以及能捕获重复变体的正则表达式模式对其进行过滤。

---

## Gateway 语音回复（Telegram 和 Discord）[​](#gateway-语音回复telegram-和-discord "Gateway 语音回复（Telegram 和 Discord）的直接链接")

如果尚未设置消息机器人，请参阅对应平台的指南：

* [Telegram 设置指南](/docs/zh-Hans/user-guide/messaging/telegram)
* [Discord 设置指南](/docs/zh-Hans/user-guide/messaging/discord)
...
### Discord：频道与私信[​](#discord频道与私信 "Discord：频道与私信的直接链接")
...
## Discord 语音频道[​](#discord-语音频道 "Discord 语音频道的直接链接")

最具沉浸感的语音功能：Bot 加入 Discord 语音频道，监听用户发言，转录语音，通过 Agent 处理后，在语音频道中语音回复。

### 设置[​](#设置 "设置的直接链接")

#### 1. Discord Bot 权限[​](#1-discord-bot-权限 "1. Discord Bot 权限的直接链接")

如果您已为文字功能设置了 Discord Bot（参见 [Discord 设置指南](/docs/zh-Hans/user-guide/messaging/discord)），需要额外添加语音权限。
...
#### 2. 特权 Gateway Intents[​](#2-特权-gateway-intents "2. 特权 Gateway Intents的直接链接")
...
**Message Content Intent** 为必需项。**Server Members Intent** 仅在 `DISCORD_ALLOWED_USERS` 列表使用用户名时才需要 — 如果使用数字用户 ID，可以关闭。语音频道中 SSRC → user\_id 的映射来自 Discord 语音 WebSocket 上的 SPEAKING opcode，**不**需要 Server Members Intent。

#### 3. Opus 编解码器[​](#3-opus-编解码器 "3. Opus 编解码器的直接链接")
...
### STT 提供商对比[​](#stt-提供商对比 "STT 提供商对比的直接链接")
...
### TTS 提供商对比[​](#tts-提供商对比 "TTS 提供商对比的直接链接")
...
### "No audio device found"（CLI）[​](#no-audio-device-foundcli "\"No audio device found\"（CLI）的直接链接")
...
### Bot 在 Discord 服务器频道中不响应[​](#bot-在-discord-服务器频道中不响应 "Bot 在 Discord 服务器频道中不响应的直接链接")
...
* [CLI 语音模式](#cli-语音模式)
  ...
  + [流式 TTS](#流式-tts)
  + [幻觉过滤器](#幻觉过滤器)
* [Gateway 语音回复（Telegram 和 Discord）](#gateway-语音回复telegram-和-discord)
  + [Discord：频道与私信](#discord频道与私信)
  + [命令](#命令)
  + [模式](#模式)
  + [平台投递](#平台投递)
* [Discord 语音频道](#discord-语音频道)
  + [设置](#设置)
...
* [配置参考](#配置参考)
  ...
  + [TTS 提供商对比](#tts-提供商对比)
* [故障排查](#故障排查)
  + ["No audio device found"（CLI）](#no-audio-device-foundcli)
  + [Bot 在 Discord 服务器频道中不响应](#bot-在-discord-服务器频道中不响应)
  + [Bot 加入语音频道但听不到我说话](#bot-加入语音频道但听不到我说话)
  + [Bot 能听到我说话但不响应](#bot-能听到我说话但不响应)
  + [Bot 有文字回复但语音频道中没有声音](#bot-有文字回复但语音频道中没有声音)
  + [Whisper 返回乱码文字](#whisper-返回乱码文字)
