# 模型提供商插件 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/model-provider-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/model-provider-plugin)
...
模型提供商插件是**提供商插件**的第三种类型。其他两种分别是 [Memory Provider 插件](/docs/zh-Hans/developer-guide/memory-provider-plugin)（跨会话知识）和 [Context Engine 插件](/docs/zh-Hans/developer-guide/context-engine-plugin)（上下文压缩策略）。三者均遵循相同的"放入目录、声明 profile、无需编辑仓库"模式。
...
## 最简示例——一个简单的 API key 提供商[​](#最简示例一个简单的-api-key-提供商 "最简示例——一个简单的 API key 提供商的直接链接")
...
## ProviderProfile 字段[​](#providerprofile-字段 "ProviderProfile 字段的直接链接")
...
| `default_max_tokens` | `int | None` | 提供商级别的 max\_tokens 上限（Nvidia：16384） |
| `default_aux_model` | str | 用于辅助任务（压缩、视觉、摘要）的廉价模型 |

## 可覆盖的 hook[​](#可覆盖的-hook "可覆盖的 hook的直接链接")
...
## Hook 参考示例[​](#hook-参考示例 "Hook 参考示例的直接链接")
...
## 用户覆盖——不修改仓库替换内置提供商[​](#用户覆盖不修改仓库替换内置提供商 "用户覆盖——不修改仓库替换内置提供商的直接链接")
...
## api\_mode 选择[​](#api_mode-选择 "api_mode 选择的直接链接")
...
## 通用 PluginManager 集成[​](#通用-pluginmanager-集成 "通用 PluginManager 集成的直接链接")
...
完整的入口点设置请参阅 [构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins#distribute-via-pip)。

## 相关页面[​](#相关页面 "相关页面的直接链接")

* [Provider Runtime](/docs/zh-Hans/developer-guide/provider-runtime) — 解析优先级及各层读取 profile 的位置
* [添加提供商](/docs/zh-Hans/developer-guide/adding-providers) — 新推理后端的端到端检查清单（涵盖快速插件路径和完整 CLI/auth 集成）
* [Memory Provider 插件](/docs/zh-Hans/developer-guide/memory-provider-plugin)
* [Context Engine 插件](/docs/zh-Hans/developer-guide/context-engine-plugin)
* [构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins) — 通用插件编写指南

* [发现机制](#发现机制)
* [目录结构](#目录结构)
* [最简示例——一个简单的 API key 提供商](#最简示例一个简单的-api-key-提供商)
* [ProviderProfile 字段](#providerprofile-字段)
* [可覆盖的 hook](#可覆盖的-hook)
* [Hook 参考示例](#hook-参考示例)
* [用户覆盖——不修改仓库替换内置提供商](#用户覆盖不修改仓库替换内置提供商)
* [api\_mode 选择](#api_mode-选择)
* [认证类型](#认证类型)
* [发现时机](#发现时机)
* [测试你的插件](#测试你的插件)
* [通用 PluginManager 集成](#通用-pluginmanager-集成)
* [通过 pip 分发](#通过-pip-分发)
* [相关页面](#相关页面)
