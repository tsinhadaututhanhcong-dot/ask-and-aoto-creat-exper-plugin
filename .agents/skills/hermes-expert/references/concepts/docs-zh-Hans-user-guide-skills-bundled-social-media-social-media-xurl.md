# Xurl — 通过 xurl CLI 使用 X/Twitter：发帖、搜索、私信、媒体、v2 API | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/social-media/social-media-xurl](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/social-media/social-media-xurl)
...
## 密钥安全（强制要求）[​](#密钥安全强制要求 "密钥安全（强制要求）的直接链接")
...
## 一次性用户配置（用户在 agent 外执行）[​](#一次性用户配置用户在-agent-外执行 "一次性用户配置（用户在 agent 外执行）的直接链接")
...
### 用户、时间线、提及[​](#用户时间线提及 "用户、时间线、提及的直接链接")
...
## 原始 API 访问[​](#原始-api-访问 "原始 API 访问的直接链接")
...
### 发布带图片的帖子[​](#发布带图片的帖子 "发布带图片的帖子的直接链接")
...
### 多应用（凭据已手动预配置）[​](#多应用凭据已手动预配置 "多应用（凭据已手动预配置）的直接链接")
...
## Agent 工作流[​](#agent-工作流 "Agent 工作流的直接链接")
...
* 上游 CLI：[https://github.com/xdevplatform/xurl（X](https://github.com/xdevplatform/xurl%EF%BC%88X) 开发者平台团队，Chris Park 等）
...
* [密钥安全（强制要求）](#密钥安全强制要求)
* [安装](#安装)
* [一次性用户配置（用户在 agent 外执行）](#一次性用户配置用户在-agent-外执行)
* [快速参考](#快速参考)
* [命令详情](#命令详情)
  + [发帖](#发帖)
  + [读取与搜索](#读取与搜索)
