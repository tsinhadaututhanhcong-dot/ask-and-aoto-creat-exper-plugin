...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/google_chat](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/google_chat)
...
## 第一步：创建或选择 GCP 项目[​](#第一步创建或选择-gcp-项目 "第一步：创建或选择 GCP 项目的直接链接")
...
## 第二步：启用两个 API[​](#第二步启用两个-api "第二步：启用两个 API的直接链接")
...
## 第三步：创建 Service Account[​](#第三步创建-service-account "第三步：创建 Service Account的直接链接")
...
## 第四步：创建 Pub/Sub topic 和订阅[​](#第四步创建-pubsub-topic-和订阅 "第四步：创建 Pub/Sub topic 和订阅的直接链接")
...
## 第五步：在 topic 上配置 IAM 绑定（关键）[​](#第五步在-topic-上配置-iam-绑定关键 "第五步：在 topic 上配置 IAM 绑定（关键）的直接链接")
...
## 第六步：在订阅上配置 IAM 绑定[​](#第六步在订阅上配置-iam-绑定 "第六步：在订阅上配置 IAM 绑定的直接链接")
...
## 第七步：配置 Chat 应用[​](#第七步配置-chat-应用 "第七步：配置 Chat 应用的直接链接")
...
## 第八步：在测试 space 中安装机器人[​](#第八步在测试-space-中安装机器人 "第八步：在测试 space 中安装机器人的直接链接")
...
## 第九步：配置 Hermes[​](#第九步配置-hermes "第九步：配置 Hermes的直接链接")
...
### 一次性宿主机设置[​](#一次性宿主机设置 "一次性宿主机设置的直接链接")
...
### 每用户授权（在 Chat 中操作）[​](#每用户授权在-chat-中操作 "每用户授权（在 Chat 中操作）的直接链接")
...
* [概览](#概览)
* [第一步：创建或选择 GCP 项目](#第一步创建或选择-gcp-项目)
* [第二步：启用两个 API](#第二步启用两个-api)
* [第三步：创建 Service Account](#第三步创建-service-account)
* [第四步：创建 Pub/Sub topic 和订阅](#第四步创建-pubsub-topic-和订阅)
* [第五步：在 topic 上配置 IAM 绑定（关键）](#第五步在-topic-上配置-iam-绑定关键)
* [第六步：在订阅上配置 IAM 绑定](#第六步在订阅上配置-iam-绑定)
* [第七步：配置 Chat 应用](#第七步配置-chat-应用)
* [第八步：在测试 space 中安装机器人](#第八步在测试-space-中安装机器人)
* [第九步：配置 Hermes](#第九步配置-hermes)
* [格式化与功能](#格式化与功能)
* [第十步：原生附件投递（可选）](#第十步原生附件投递可选)
  + [为何需要单独的流程](#为何需要单独的流程)
  + [一次性宿主机设置](#一次性宿主机设置)
  + [每用户授权（在 Chat 中操作）](#每用户授权在-chat-中操作)
  + [Scope](#scope)
  + [多用户行为](#多用户行为)
* [故障排查](#故障排查)
* [安全说明](#安全说明)