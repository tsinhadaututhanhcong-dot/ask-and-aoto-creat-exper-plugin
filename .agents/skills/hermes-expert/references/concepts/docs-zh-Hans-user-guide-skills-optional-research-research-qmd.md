# Qmd | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-qmd](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-qmd)
...
| 相关 skill | [`obsidian`](/docs/zh-Hans/user-guide/skills/bundled/note-taking/note-taking-obsidian), [`native-mcp`](/docs/zh-Hans/user-guide/skills/bundled/mcp/mcp-native-mcp), [`arxiv`](/docs/zh-Hans/user-guide/skills/bundled/research/research-arxiv) |
...
### Node.js >= 22（必需）[​](#nodejs--22必需 "Node.js >= 22（必需）的直接链接")
...
### SQLite 扩展支持（仅 macOS）[​](#sqlite-扩展支持仅-macos "SQLite 扩展支持（仅 macOS）的直接链接")
...
| 模型 | 用途 | 大小 |
| --- | --- | --- |
| embeddinggemma-300M-Q8\_0 | 向量 embedding（嵌入） | ~300MB |
| qwen3-reranker-0.6b-q8\_0 | 结果重排序 | ~640MB |
...
### 2. 添加上下文描述[​](#2-添加上下文描述 "2. 添加上下文描述的直接链接")
...
### 3. 生成 Embedding[​](#3-生成-embedding "3. 生成 Embedding的直接链接")
...
### 快速关键词搜索（BM25）[​](#快速关键词搜索bm25 "快速关键词搜索（BM25）的直接链接")
...
### 混合搜索 + 重排序（最佳质量）[​](#混合搜索--重排序最佳质量 "混合搜索 + 重排序（最佳质量）的直接链接")
...
### 结构化多模式查询[​](#结构化多模式查询 "结构化多模式查询的直接链接")
...
### 查询语法（lex/BM25 模式）[​](#查询语法lexbm25-模式 "查询语法（lex/BM25 模式）的直接链接")
...
### HyDE（假设文档 Embedding）[​](#hyde假设文档-embedding "HyDE（假设文档 Embedding）的直接链接")
...
### 方案 B：HTTP Daemon 模式（快速，重度使用推荐）[​](#方案-bhttp-daemon-模式快速重度使用推荐 "方案 B：HTTP Daemon 模式（快速，重度使用推荐）的直接链接")
...
### 保持 Daemon 持续运行[​](#保持-daemon-持续运行 "保持 Daemon 持续运行的直接链接")
...
#### Linux（systemd 用户服务）[​](#linuxsystemd-用户服务 "Linux（systemd 用户服务）的直接链接")
...
### MCP 工具参考[​](#mcp-工具参考 "MCP 工具参考的直接链接")
...
## CLI 用法（不使用 MCP）[​](#cli-用法不使用-mcp "CLI 用法（不使用 MCP）的直接链接")
...
## 搜索流水线工作原理[​](#搜索流水线工作原理 "搜索流水线工作原理的直接链接")
...
### "首次运行时模型正在下载"[​](#首次运行时模型正在下载 "\"首次运行时模型正在下载\"的直接链接")

正常现象 — qmd 首次使用时会自动下载约 2GB 的 GGUF 模型。
这是一次性操作。

### 冷启动延迟（约 19s）[​](#冷启动延迟约-19s "冷启动延迟（约 19s）的直接链接")
...
### macOS："unable to load extension"[​](#macosunable-to-load-extension "macOS：\"unable to load extension\"的直接链接")

安装 Homebrew SQLite：`brew install sqlite`
然后确保其在系统 SQLite 之前出现在 PATH 中。

### "未找到集合"[​](#未找到集合 "\"未找到集合\"的直接链接")

运行 `qmd collection add <path> --name <name>` 添加目录，
然后运行 `qmd embed` 进行索引。

### Embedding 模型覆盖（CJK/多语言）[​](#embedding-模型覆盖cjk多语言 "Embedding 模型覆盖（CJK/多语言）的直接链接")
...
* [使用场景](#使用场景)
* [前置条件](#前置条件)
  + [Node.js >= 22（必需）](#nodejs--22必需)
  + [SQLite 扩展支持（仅 macOS）](#sqlite-扩展支持仅-macos)
  + [安装 qmd](#安装-qmd)
  + [验证安装](#验证安装)
* [快速参考](#快速参考)
* [设置流程](#设置流程)
  + [1. 添加集合](#1-添加集合)
  + [2. 添加上下文描述](#2-添加上下文描述)
  + [3. 生成 Embedding](#3-生成-embedding)
  + [4. 验证](#4-验证)
* [搜索模式](#搜索模式)
  + [快速关键词搜索（BM25）](#快速关键词搜索bm25)
  + [语义向量搜索](#语义向量搜索)
  + [混合搜索 + 重排序（最佳质量）](#混合搜索--重排序最佳质量)
  + [结构化多模式查询](#结构化多模式查询)
  + [查询语法（lex/BM25 模式）](#查询语法lexbm25-模式)
  + [HyDE（假设文档 Embedding）](#hyde假设文档-embedding)
  + [限定集合范围](#限定集合范围)
  + [输出格式](#输出格式)
* [MCP 集成（推荐）](#mcp-集成推荐)
  + [方案 A：Stdio 模式（简单）](#方案-astdio-模式简单)
  + [方案 B：HTTP Daemon 模式（快速，重度使用推荐）](#方案-bhttp-daemon-模式快速重度使用推荐)
  + [保持 Daemon 持续运行](#保持-daemon-持续运行)
  + [MCP 工具参考](#mcp-工具参考)
* [CLI 用法（不使用 MCP）](#cli-用法不使用-mcp)
* [搜索流水线工作原理](#搜索流水线工作原理)
* [最佳实践](#最佳实践)
* [故障排查](#故障排查)
  + ["首次运行时模型正在下载"](#首次运行时模型正在下载)
  + [冷启动延迟（约 19s）](#冷启动延迟约-19s)
  + [macOS："unable to load extension"](#macosunable-to-load-extension)
  + ["未找到集合"](#未找到集合)
  + [Embedding 模型覆盖（CJK/多语言）](#embedding-模型覆盖cjk多语言)
* [数据存储](#数据存储)
* [参考资料](#参考资料)
