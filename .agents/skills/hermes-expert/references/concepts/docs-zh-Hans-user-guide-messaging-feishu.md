# 飞书 / Lark | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/feishu](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/feishu)
...
## 第一步：创建飞书 / Lark 应用[​](#第一步创建飞书--lark-应用 "第一步：创建飞书 / Lark 应用的直接链接")

### 推荐：扫码创建（一条命令）[​](#推荐扫码创建一条命令 "推荐：扫码创建（一条命令）的直接链接")
...
## 第二步：选择连接模式[​](#第二步选择连接模式 "第二步：选择连接模式的直接链接")

### 推荐：WebSocket 模式[​](#推荐websocket-模式 "推荐：WebSocket 模式的直接链接")
...
### 可选：Webhook 模式[​](#可选webhook-模式 "可选：Webhook 模式的直接链接")
...
## 第三步：配置 Hermes[​](#第三步配置-hermes "第三步：配置 Hermes的直接链接")

### 方式 A：交互式配置[​](#方式-a交互式配置 "方式 A：交互式配置的直接链接")
...
## Home Chat[​](#home-chat "Home Chat的直接链接")
...
如果白名单为空，任何能访问机器人的人都可能使用它。在群聊中，消息处理前会根据发送者的 open\_id 检查白名单。

### Webhook 加密密钥[​](#webhook-加密密钥 "Webhook 加密密钥的直接链接")
...
### 验证 Token[​](#验证-token "验证 Token的直接链接")
...
## 机器人间消息传递[​](#机器人间消息传递 "机器人间消息传递的直接链接")
...
### 飞书应用所需配置[​](#飞书应用所需配置 "飞书应用所需配置的直接链接")
...
## 文档评论智能回复[​](#文档评论智能回复 "文档评论智能回复的直接链接")
...
### 飞书应用所需配置[​](#飞书应用所需配置-1 "飞书应用所需配置的直接链接")
...
## Markdown 渲染与 Post 回退[​](#markdown-渲染与-post-回退 "Markdown 渲染与 Post 回退的直接链接")
...
### 按聊天串行化[​](#按聊天串行化 "按聊天串行化的直接链接")

同一聊天中的消息串行处理（每次一条），以保持对话连贯性。每个聊天有独立的锁，不同聊天的消息并发处理。

## 速率限制（Webhook 模式）[​](#速率限制webhook-模式 "速率限制（Webhook 模式）的直接链接")

在 webhook 模式下，适配器对每个 IP 强制执行速率限制，防止滥用：

* **窗口：** 60 秒滑动窗口
* **限制：** 每个（app\_id, path, IP）三元组每窗口 120 次请求
* **追踪上限：** 最多追踪 4096 个唯一键（防止内存无限增长）

超出限制的请求将收到 HTTP 429（请求过多）。

### Webhook 异常追踪[​](#webhook-异常追踪 "Webhook 异常追踪的直接链接")
...
## WebSocket 调优[​](#websocket-调优 "WebSocket 调优的直接链接")
...
| `FEISHU_ALLOWED_USERS` | — | *（空）* | 用户白名单的逗号分隔 open\_id 列表 |
...
| `FEISHU_VERIFICATION_TOKEN` | — | *（空）* | webhook payload 认证的验证 token |
| `FEISHU_GROUP_POLICY` | — | `allowlist` | 群消息策略：`open`、`allowlist`、`disabled` |
| `FEISHU_BOT_OPEN_ID` | — | *（空）* | 机器人的 open\_id（用于 @提及 检测） |
| `FEISHU_BOT_USER_ID` | — | *（空）* | 机器人的 user\_id（用于 @提及 检测） |
...
| `Another local Hermes gateway is already using this Feishu app_id` | 同一时间只能有一个 Hermes 实例使用相同的 app\_id。请先停止另一个 gateway。 |
...
* [第一步：创建飞书 / Lark 应用](#第一步创建飞书--lark-应用)
  + [推荐：扫码创建（一条命令）](#推荐扫码创建一条命令)
  + [备选：手动配置](#备选手动配置)
* [第二步：选择连接模式](#第二步选择连接模式)
  + [推荐：WebSocket 模式](#推荐websocket-模式)
  + [可选：Webhook 模式](#可选webhook-模式)
* [第三步：配置 Hermes](#第三步配置-hermes)
  + [方式 A：交互式配置](#方式-a交互式配置)
  + [方式 B：手动配置](#方式-b手动配置)
* [第四步：启动 Gateway](#第四步启动-gateway)
* [Home Chat](#home-chat)
* [安全](#安全)
  + [用户白名单](#用户白名单)
  + [Webhook 加密密钥](#webhook-加密密钥)
  + [验证 Token](#验证-token)
* [群消息策略](#群消息策略)
  + [机器人身份](#机器人身份)
* [机器人间消息传递](#机器人间消息传递)
...
* [媒体支持](#媒体支持)
  + [入站（接收）](#入站接收)
  + [出站（发送）](#出站发送)
* [Markdown 渲染与 Post 回退](#markdown-渲染与-post-回退)
* [处理状态表情回应](#处理状态表情回应)
* [突发保护与批处理](#突发保护与批处理)
  + [文本批处理](#文本批处理)
  + [媒体批处理](#媒体批处理)
  + [按聊天串行化](#按聊天串行化)
* [速率限制（Webhook 模式）](#速率限制webhook-模式)
  + [Webhook 异常追踪](#webhook-异常追踪)
* [WebSocket 调优](#websocket-调优)
* [按群访问控制](#按群访问控制)
* [去重](#去重)
* [所有环境变量](#所有环境变量)
* [故障排查](#故障排查)
