# TUI | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/tui](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/tui)

本页总览

TUI 是 Hermes 的现代前端——一个终端 UI（用户界面），与 [Classic CLI](/docs/zh-Hans/user-guide/cli) 共享同一 Python 运行时。相同的 agent、相同的会话、相同的斜杠命令；交互界面更简洁、响应更流畅。
...
Classic CLI 仍作为默认方式保留。[CLI 界面](/docs/zh-Hans/user-guide/cli)中记录的所有内容——斜杠命令、快捷命令、skill 预加载、personality、多行输入、中断——在 TUI 中均完全一致。

## 为什么选择 TUI[​](#为什么选择-tui "为什么选择 TUI的直接链接")
...
同样的 [skins](/docs/zh-Hans/user-guide/features/skins) 和 [personalities](/docs/zh-Hans/user-guide/features/personality) 均适用。会话中途使用 `/skin ares`、`/personality pirate` 切换，UI 实时重绘。完整的可定制键列表及其对 classic 与 TUI 的适用范围，请参阅 [Skins & Themes](/docs/zh-Hans/user-guide/features/skins)——TUI 支持 banner 调色板、UI 颜色、prompt 字形/颜色、会话显示、补全菜单、选区背景色、`tool_prefix` 和 `help_header`。

### 可折叠的 banner 区块[​](#可折叠的-banner-区块 "可折叠的 banner 区块的直接链接")
...
快捷键与 [Classic CLI](/docs/zh-Hans/user-guide/cli#keybindings) 完全一致。仅有以下行为差异：
...
其他所有斜杠命令（包括已安装的 skill、快捷命令和 personality 切换）与 classic CLI 完全一致。请参阅[斜杠命令参考](/docs/zh-Hans/reference/slash-commands)。

## LaTeX 数学渲染[​](#latex-数学渲染 "LaTeX 数学渲染的直接链接")
...
各 skin 的状态栏颜色和阈值与 classic CLI 共享——请参阅 [Skins](/docs/zh-Hans/user-guide/features/skins) 了解自定义方式。
...
会话生命周期、搜索、压缩和导出，请参阅[会话](/docs/zh-Hans/user-guide/sessions)。

## TUI 如何与其 gateway 通信[​](#tui-如何与其-gateway-通信 "TUI 如何与其 gateway 通信的直接链接")
...
如果你希望多个界面共享同一组会话，请使用共享的 `~/.hermes/state.db`（参见[会话](/docs/zh-Hans/user-guide/sessions)）或 Web 仪表板的内嵌聊天（参见 [Web Dashboard](/docs/zh-Hans/user-guide/features/web-dashboard#chat)）——而不是手动设置 gateway URL。

## 回退到 Classic CLI[​](#回退到-classic-cli "回退到 Classic CLI的直接链接")
...
* [CLI 界面](/docs/zh-Hans/user-guide/cli) — 完整的斜杠命令和快捷键参考（共享）
* [会话](/docs/zh-Hans/user-guide/sessions) — 恢复、分支和历史记录
* [Skins & Themes](/docs/zh-Hans/user-guide/features/skins) — 自定义 banner、状态栏和浮层主题
* [语音模式](/docs/zh-Hans/user-guide/features/voice-mode) — 在两种界面中均可使用
* [配置](/docs/zh-Hans/user-guide/configuration) — 所有配置键

* [启动](#启动)
* [为什么选择 TUI](#为什么选择-tui)
  + [可折叠的 banner 区块](#可折叠的-banner-区块)
* [环境要求](#环境要求)
  + [外部预构建](#外部预构建)
* [快捷键](#快捷键)
* [斜杠命令](#斜杠命令)
* [LaTeX 数学渲染](#latex-数学渲染)
* [浅色终端检测](#浅色终端检测)
* [忙碌指示器样式](#忙碌指示器样式)
* [自动恢复](#自动恢复)
* [状态栏](#状态栏)
* [配置](#配置)
* [会话](#会话)
* [TUI 如何与其 gateway 通信](#tui-如何与其-gateway-通信)
* [回退到 Classic CLI](#回退到-classic-cli)
* [另请参阅](#另请参阅)
