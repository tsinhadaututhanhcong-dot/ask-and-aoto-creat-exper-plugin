...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/telegram](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/telegram)
...
## 第一步：通过 BotFather 创建机器人[​](#第一步通过-botfather-创建机器人 "第一步：通过 BotFather 创建机器人的直接链接")
...
## 第二步：自定义机器人（可选）[​](#第二步自定义机器人可选 "第二步：自定义机器人（可选）的直接链接")
...
## 第三步：隐私模式（群组关键设置）[​](#第三步隐私模式群组关键设置 "第三步：隐私模式（群组关键设置）的直接链接")
...
### 如何关闭隐私模式[​](#如何关闭隐私模式 "如何关闭隐私模式的直接链接")
...
### 观察群组消息但不自动回复[​](#观察群组消息但不自动回复 "观察群组消息但不自动回复的直接链接")
...
## 第四步：获取你的用户 ID[​](#第四步获取你的用户-id "第四步：获取你的用户 ID的直接链接")
...
**方法二：** 向 [@get\_id\_bot](https://t.me/get_id_bot) 发送消息——另一个可靠的选项。

保存这个数字，下一步会用到。

## 第五步：配置 Hermes[​](#第五步配置-hermes "第五步：配置 Hermes的直接链接")

### 方式 A：交互式设置（推荐）[​](#方式-a交互式设置推荐 "方式 A：交互式设置（推荐）的直接链接")
...
### 方式 B：手动配置[​](#方式-b手动配置 "方式 B：手动配置的直接链接")
...
## 从 Docker 后端终端发送生成的文件[​](#从-docker-后端终端发送生成的文件 "从 Docker 后端终端发送生成的文件的直接链接")
...
### 支持的 `MEDIA:` 文件扩展名[​](#支持的-media-文件扩展名 "支持的-media-文件扩展名的直接链接")
...
## Webhook 模式[​](#webhook-模式 "Webhook 模式的直接链接")
...
### 云部署示例（Fly.io）[​](#云部署示例flyio "云部署示例（Fly.io）的直接链接")
...
### 话题模式下的 Cron 投递[​](#话题模式下的-cron-投递 "话题模式下的 Cron 投递的直接链接")
...
### 接收语音（语音转文字）[​](#接收语音语音转文字 "接收语音（语音转文字）的直接链接")
...
#### 跳过 STT：将原始音频文件传递给 Agent[​](#跳过-stt将原始音频文件传递给-agent "跳过 STT：将原始音频文件传递给 Agent的直接链接")
...
### 发送语音（文字转语音）[​](#发送语音文字转语音 "发送语音（文字转语音）的直接链接")
...
## 通过本地 Bot API 服务器处理大文件（>20MB）[​](#通过本地-bot-api-服务器处理大文件20mb "通过本地 Bot API 服务器处理大文件（>20MB）的直接链接")
...
### 第一步：获取 Telegram API 凭据[​](#第一步获取-telegram-api-凭据 "第一步：获取 Telegram API 凭据的直接链接")
...
### 第二步：运行 telegram-bot-api 服务器[​](#第二步运行-telegram-bot-api-服务器 "第二步：运行 telegram-bot-api 服务器的直接链接")
...
### 第三步：将机器人从公共 API 登出（一次性操作）[​](#第三步将机器人从公共-api-登出一次性操作 "第三步：将机器人从公共 API 登出（一次性操作）的直接链接")
...
### 第四步：将 Hermes 指向本地服务器[​](#第四步将-hermes-指向本地服务器 "第四步：将 Hermes 指向本地服务器的直接链接")
...
使用 `platforms.telegram.extra`，而非 `telegram.extra`
...
### 第五步：`local_mode`——磁盘上的文件访问[​](#第五步local_mode磁盘上的文件访问 "第五步local_mode磁盘上的文件访问的直接链接")
...
### 同一群组中的多个 Hermes 机器人[​](#同一群组中的多个-hermes-机器人 "同一群组中的多个 Hermes 机器人的直接链接")
...
### 故障排除：私聊正常但群组无响应[​](#故障排除私聊正常但群组无响应 "故障排除：私聊正常但群组无响应的直接链接")
...
### 群组触发配置示例[​](#群组触发配置示例 "群组触发配置示例的直接链接")
...
### `mention_patterns` 说明[​](#mention_patterns-说明 "mention_patterns-说明的直接链接")
...
## 私聊话题（Bot API 9.4）[​](#私聊话题bot-api-94 "私聊话题（Bot API 9.4）的直接链接")
...
## 多会话私聊模式（`/topic`）[​](#多会话私聊模式topic "多会话私聊模式topic的直接链接")
...
### `/topic` 子命令[​](#topic-子命令 "topic-子命令的直接链接")
...
### 私聊话题 vs 多会话私聊模式[​](#私聊话题-vs-多会话私聊模式 "私聊话题 vs 多会话私聊模式的直接链接")
...
### 创建新话题（终端用户流程）[​](#创建新话题终端用户流程 "创建新话题（终端用户流程）的直接链接")
...
### 话题内的 `/topic`（无参数）[​](#话题内的-topic无参数 "话题内的-topic无参数的直接链接")
...
* 论坛启用私聊中的 General（置顶顶部）话题被视为根大厅，无论 Telegram 是以 `message_thread_id=1` 还是无 thread\_id 投递其消息
...
### 降级 Hermes[​](#降级-hermes "降级 Hermes的直接链接")
...
## 群组论坛话题技能绑定[​](#群组论坛话题技能绑定 "群组论坛话题技能绑定的直接链接")
...
### 与私聊话题的区别[​](#与私聊话题的区别 "与私聊话题的区别的直接链接")
...
## 近期 Bot API 功能[​](#近期-bot-api-功能 "近期 Bot API 功能的直接链接")
...
### 流式传输（`gateway.streaming.transport`）[​](#流式传输gatewaystreamingtransport "流式传输gatewaystreamingtransport的直接链接")
...
## 渲染：富消息、表格和链接预览[​](#渲染富消息表格和链接预览 "渲染：富消息、表格和链接预览的直接链接")
...
### 从 PR #17686 之前迁移[​](#从-pr-17686-之前迁移 "从 PR #17686 之前迁移的直接链接")
...
### 访客 @mention 绕过（`guest_mode`）[​](#访客-mention-绕过guest_mode "访客-mention-绕过guest_mode的直接链接")
...
## 斜杠命令访问控制[​](#斜杠命令访问控制 "斜杠命令访问控制的直接链接")
...
## 交互式模型选择器[​](#交互式模型选择器 "交互式模型选择器的直接链接")
...
## 按频道 Prompt[​](#按频道-prompt "按频道 Prompt的直接链接")
...
## 交互式 Prompt（clarify）[​](#交互式-promptclarify "交互式 Prompt（clarify）的直接链接")
...
更多详情，请参阅[安全文档](/docs/zh-Hans/user-guide/security)。你也可以使用 [DM 配对](/docs/zh-Hans/user-guide/messaging#dm-pairing-alternative-to-allowlists) 进行更动态的用户授权方式。

* [第一步：通过 BotFather 创建机器人](#第一步通过-botfather-创建机器人)
* [第二步：自定义机器人（可选）](#第二步自定义机器人可选)
* [第三步：隐私模式（群组关键设置）](#第三步隐私模式群组关键设置)
  + [如何关闭隐私模式](#如何关闭隐私模式)
  + [观察群组消息但不自动回复](#观察群组消息但不自动回复)
* [第四步：获取你的用户 ID](#第四步获取你的用户-id)
* [第五步：配置 Hermes](#第五步配置-hermes)
  + [方式 A：交互式设置（推荐）](#方式-a交互式设置推荐)
  + [方式 B：手动配置](#方式-b手动配置)
  + [启动 Gateway](#启动-gateway)
* [从 Docker 后端终端发送生成的文件](#从-docker-后端终端发送生成的文件)
  + [支持的 `MEDIA:` 文件扩展名](#支持的-media-文件扩展名)
* [Webhook 模式](#webhook-模式)
  + [配置](#配置)
  + [云部署示例（Fly.io）](#云部署示例flyio)
* [代理支持](#代理支持)
* [主频道](#主频道)
  + [话题模式下的 Cron 投递](#话题模式下的-cron-投递)
* [语音消息](#语音消息)
  + [接收语音（语音转文字）](#接收语音语音转文字)
  + [发送语音（文字转语音）](#发送语音文字转语音)
* [通过本地 Bot API 服务器处理大文件（>20MB）](#通过本地-bot-api-服务器处理大文件20mb)
  + [第一步：获取 Telegram API 凭据](#第一步获取-telegram-api-凭据)
  + [第二步：运行 telegram-bot-api 服务器](#第二步运行-telegram-bot-api-服务器)
  + [第三步：将机器人从公共 API 登出（一次性操作）](#第三步将机器人从公共-api-登出一次性操作)
  + [第四步：将 Hermes 指向本地服务器](#第四步将-hermes-指向本地服务器)
  + [第五步：`local_mode`——磁盘上的文件访问](#第五步local_mode磁盘上的文件访问)
  + [第六步：测试](#第六步测试)
* [群聊使用](#群聊使用)
  + [同一群组中的多个 Hermes 机器人](#同一群组中的多个-hermes-机器人)
  + [故障排除：私聊正常但群组无响应](#故障排除私聊正常但群组无响应)
  + [群组触发配置示例](#群组触发配置示例)
  + [`mention_patterns` 说明](#mention_patterns-说明)
* [私聊话题（Bot API 9.4）](#私聊话题bot-api-94)
  + [使用场景](#使用场景)
  + [配置](#配置-1)
  + [工作原理](#工作原理)
  + [根私聊处理](#根私聊处理)
  + [技能绑定](#技能绑定)
* [多会话私聊模式（`/topic`）](#多会话私聊模式topic)
  + [`/topic` 子命令](#topic-子命令)
  + [私聊话题 vs 多会话私聊模式](#私聊话题-vs-多会话私聊模式)
  + [前提条件](#前提条件)
  + [激活流程](#激活流程)
  + [创建新话题（终端用户流程）](#创建新话题终端用户流程)
  + [自动重命名话题](#自动重命名话题)
  + [话题内的 `/new`](#话题内的-new)
  + [恢复之前的会话](#恢复之前的会话)
  + [话题内的 `/topic`（无参数）](#话题内的-topic无参数)
  + [底层实现](#底层实现)
  + [禁用多会话模式](#禁用多会话模式)
  + [降级 Hermes](#降级-hermes)
* [群组论坛话题技能绑定](#群组论坛话题技能绑定)
  + [使用场景](#使用场景-1)
  + [配置](#配置-2)
  + [工作原理](#工作原理-1)
  + [与私聊话题的区别](#与私聊话题的区别)
* [近期 Bot API 功能](#近期-bot-api-功能)
  + [流式传输（`gateway.streaming.transport`）](#流式传输gatewaystreamingtransport)
* [渲染：富消息、表格和链接预览](#渲染富消息表格和链接预览)
* [群组白名单](#群组白名单)
  + [从 PR #17686 之前迁移](#从-pr-17686-之前迁移)
  + [访客 @mention 绕过（`guest_mode`）](#访客-mention-绕过guest_mode)
* [斜杠命令访问控制](#斜杠命令访问控制)
* [交互式模型选择器](#交互式模型选择器)
* [DNS-over-HTTPS 备用 IP](#dns-over-https-备用-ip)
  + [工作原理](#工作原理-2)
  + [配置](#配置-3)
* [代理支持](#代理支持-1)
  + [支持的变量](#支持的变量)
  + [配置](#配置-4)
* [消息反应](#消息反应)
* [按频道 Prompt](#按频道-prompt)
* [故障排除](#故障排除)
* [执行审批](#执行审批)
* [交互式 Prompt（clarify）](#交互式-promptclarify)
* [推送通知音量](#推送通知音量)
* [安全](#安全)
