...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks)
...
| **[Plugin hooks](#plugin-hooks)** | [插件](/docs/zh-Hans/user-guide/features/plugins)中的 `ctx.register_hook()` | CLI + Gateway | 工具拦截、指标采集、护栏 |
...
## Gateway Event Hooks[​](#gateway-event-hooks "Gateway Event Hooks的直接链接")

Gateway hooks 在 gateway 运行期间（Telegram、Discord、Slack、WhatsApp、Teams）自动触发，不会阻塞主 agent 管道。

### 创建 Hook[​](#创建-hook "创建 Hook的直接链接")
...
#### HOOK.yaml[​](#hookyaml "HOOK.yaml的直接链接")
...
#### handler.py[​](#handlerpy "handler.py的直接链接")
...
#### 命令使用日志记录器[​](#命令使用日志记录器 "命令使用日志记录器的直接链接")
...
#### 会话开始 Webhook[​](#会话开始-webhook "会话开始 Webhook的直接链接")
...
### 教程：BOOT.md——每次 Gateway 启动时运行启动检查清单[​](#教程bootmd每次-gateway-启动时运行启动检查清单 "教程：BOOT.md——每次 Gateway 启动时运行启动检查清单的直接链接")
...
#### 第一步：编写检查清单[​](#第一步编写检查清单 "第一步：编写检查清单的直接链接")
...
#### 第二步：创建 hook[​](#第二步创建-hook "第二步：创建 hook的直接链接")
...
#### 为什么这不是内置功能[​](#为什么这不是内置功能 "为什么这不是内置功能的直接链接")
...
## Plugin Hooks[​](#plugin-hooks "Plugin Hooks的直接链接")

[插件](/docs/zh-Hans/user-guide/features/plugins)可以注册在 **CLI 和 gateway** 会话中均会触发的 hook。这些 hook 通过插件 `register()` 函数中的 `ctx.register_hook()` 以编程方式注册。
...
### `pre_tool_call`[​](#pre_tool_call "pre_tool_call的直接链接")
...
### `post_tool_call`[​](#post_tool_call "post_tool_call的直接链接")
...
### `pre_llm_call`[​](#pre_llm_call "pre_llm_call的直接链接")
...
### `post_llm_call`[​](#post_llm_call "post_llm_call的直接链接")
...
### `on_session_start`[​](#on_session_start "on_session_start的直接链接")
...
### `on_session_end`[​](#on_session_end "on_session_end的直接链接")
...
### `on_session_finalize`[​](#on_session_finalize "on_session_finalize的直接链接")
...
### `on_session_reset`[​](#on_session_reset "on_session_reset的直接链接")
...
参见 **[构建插件指南](/docs/zh-Hans/developer-guide/plugins)**，获取包含工具 schema、处理器和高级 hook 模式的完整演练。

---

### `subagent_stop`[​](#subagent_stop "subagent_stop的直接链接")
...
### `pre_gateway_dispatch`[​](#pre_gateway_dispatch "pre_gateway_dispatch的直接链接")
...
### `pre_approval_request`[​](#pre_approval_request "pre_approval_request的直接链接")
...
### `post_approval_response`[​](#post_approval_response "post_approval_response的直接链接")
...
### `transform_tool_result`[​](#transform_tool_result "transform_tool_result的直接链接")
...
### `transform_terminal_output`[​](#transform_terminal_output "transform_terminal_output的直接链接")
...
### `transform_llm_output`[​](#transform_llm_output "transform_llm_output的直接链接")
...
## Shell Hooks[​](#shell-hooks "Shell Hooks的直接链接")
...
### 配置 schema[​](#配置-schema "配置 schema的直接链接")
...
### JSON 通信协议[​](#json-通信协议 "JSON 通信协议的直接链接")
...
#### 1. 每次写入后自动格式化 Python 文件[​](#1-每次写入后自动格式化-python-文件 "1. 每次写入后自动格式化 Python 文件的直接链接")
...
#### 2. 阻断破坏性 `terminal` 命令[​](#2-阻断破坏性-terminal-命令 "2-阻断破坏性-terminal-命令的直接链接")
...
#### 3. 向每轮注入 `git status`（Claude-Code `UserPromptSubmit` 等效）[​](#3-向每轮注入-git-statusclaude-code-userpromptsubmit-等效 "3-向每轮注入-git-statusclaude-code-userpromptsubmit-等效的直接链接")
...
#### 4. 记录每次子 agent 完成[​](#4-记录每次子-agent-完成 "4. 记录每次子 agent 完成的直接链接")
...
### `hermes hooks` CLI[​](#hermes-hooks-cli "hermes-hooks-cli的直接链接")
...
* [Gateway Event Hooks](#gateway-event-hooks)
  ...
  + [可用事件](#可用事件)
  + [示例](#示例)
  + [教程：BOOT.md——每次 Gateway 启动时运行启动检查清单](#教程bootmd每次-gateway-启动时运行启动检查清单)
  + [工作原理](#工作原理)
...
* [Shell Hooks](#shell-hooks)
  ...
  + [实际示例](#实际示例)
  + [授权模型](#授权模型)
  + [`hermes hooks` CLI](#hermes-hooks-cli)
  + [安全性](#安全性)
  + [顺序与优先级](#顺序与优先级)
