# 图像生成 Provider 插件 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/image-gen-provider-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/image-gen-provider-plugin)
...
图像生成是 Hermes 支持的多种**后端插件**之一。其他插件（各有更专用的 ABC）包括：[Memory Provider 插件](/docs/zh-Hans/developer-guide/memory-provider-plugin)、[Context Engine 插件](/docs/zh-Hans/developer-guide/context-engine-plugin) 和 [Model Provider 插件](/docs/zh-Hans/developer-guide/model-provider-plugin)。通用工具/hook/CLI 插件请参阅 [构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins)。
...
## ImageGenProvider ABC[​](#imagegenprovider-abc "ImageGenProvider ABC的直接链接")
...
## 处理 base64 与 URL 输出[​](#处理-base64-与-url-输出 "处理 base64 与 URL 输出的直接链接")
...
`my_backend_imggen_package` 必须暴露一个顶层 `register` 函数。完整配置请参阅通用插件指南中的 [通过 pip 分发](/docs/zh-Hans/developer-guide/plugins#distribute-via-pip)。

## 相关页面[​](#相关页面 "相关页面的直接链接")

* [图像生成](/docs/zh-Hans/user-guide/features/image-generation) — 面向用户的功能文档
* [插件概览](/docs/zh-Hans/user-guide/features/plugins) — 所有插件类型一览
* [构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins) — 通用工具/hook/斜杠命令指南

* [发现机制](#发现机制)
* [目录结构](#目录结构)
* [ImageGenProvider ABC](#imagegenprovider-abc)
* [plugin.yaml](#pluginyaml)
* [ABC 参考](#abc-参考)
* [响应格式](#响应格式)
* [处理 base64 与 URL 输出](#处理-base64-与-url-输出)
* [用户覆盖](#用户覆盖)
* [测试](#测试)
* [参考实现](#参考实现)
* [通过 pip 分发](#通过-pip-分发)
