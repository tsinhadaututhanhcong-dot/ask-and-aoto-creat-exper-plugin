...
**Source:** [https://hermes-agent.nousresearch.com/docs/guides/operate-teams-meeting-pipeline](https://hermes-agent.nousresearch.com/docs/guides/operate-teams-meeting-pipeline)
...
## Core Operator Commands[​](#core-operator-commands "Direct link to Core Operator Commands")

### Validate the config snapshot[​](#validate-the-config-snapshot "Direct link to Validate the config snapshot")
...
### Inspect token health[​](#inspect-token-health "Direct link to Inspect token health")
...
### Inspect subscriptions[​](#inspect-subscriptions "Direct link to Inspect subscriptions")

```
hermes teams-pipeline subscriptions
```

### Renew near-expiry subscriptions[​](#renew-near-expiry-subscriptions "Direct link to Renew near-expiry subscriptions")
...
### Automating subscription renewal (REQUIRED for production)[​](#automating-subscription-renewal-required-for-production "Direct link to Automating subscription renewal (REQUIRED for production)")
...
#### Option 1: Hermes cron (recommended if you already run the Hermes gateway)[​](#option-1-hermes-cron-recommended-if-you-already-run-the-hermes-gateway "Direct link to Option 1: Hermes cron (recommended if you already run the Hermes gateway)")
...
#### Option 2: systemd timer (recommended for Linux production deployments)[​](#option-2-systemd-timer-recommended-for-linux-production-deployments "Direct link to Option 2: systemd timer (recommended for Linux production deployments)")
...
#### Option 3: Plain crontab[​](#option-3-plain-crontab "Direct link to Option 3: Plain crontab")
...
#### Verifying renewal is working[​](#verifying-renewal-is-working "Direct link to Verifying renewal is working")
...
### Inspect recent jobs[​](#inspect-recent-jobs "Direct link to Inspect recent jobs")
...
### Replay a stored job[​](#replay-a-stored-job "Direct link to Replay a stored job")

```
hermes teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches[​](#dry-run-meeting-artifact-fetches "Direct link to Dry-run meeting artifact fetches")
...
## Routine Runbook[​](#routine-runbook "Direct link to Routine Runbook")

### After first setup[​](#after-first-setup "Direct link to After first setup")
...
### Daily or periodic checks[​](#daily-or-periodic-checks "Direct link to Daily or periodic checks")
...
### Before changing webhook URLs or delivery targets[​](#before-changing-webhook-urls-or-delivery-targets "Direct link to Before changing webhook URLs or delivery targets")
...
## Failure Triage[​](#failure-triage "Direct link to Failure Triage")

### No jobs are being created[​](#no-jobs-are-being-created "Direct link to No jobs are being created")
...
### Jobs stay in retry or fail before summarization[​](#jobs-stay-in-retry-or-fail-before-summarization "Direct link to Jobs stay in retry or fail before summarization")
...
### Summaries are produced but not delivered to Teams[​](#summaries-are-produced-but-not-delivered-to-teams "Direct link to Summaries are produced but not delivered to Teams")
...
### Duplicate or unexpected replays[​](#duplicate-or-unexpected-replays "Direct link to Duplicate or unexpected replays")
...
## Go-Live Checklist[​](#go-live-checklist "Direct link to Go-Live Checklist")
...
## Delivery-Mode Decision Guide[​](#delivery-mode-decision-guide "Direct link to Delivery-Mode Decision Guide")
...
## Operator Worksheet[​](#operator-worksheet "Direct link to Operator Worksheet")
...
## Change Review Worksheet[​](#change-review-worksheet "Direct link to Change Review Worksheet")
...
* [Core Operator Commands](#core-operator-commands)
  + [Validate the config snapshot](#validate-the-config-snapshot)
  + [Inspect token health](#inspect-token-health)
  + [Inspect subscriptions](#inspect-subscriptions)
  + [Renew near-expiry subscriptions](#renew-near-expiry-subscriptions)
  + [Automating subscription renewal (REQUIRED for production)](#automating-subscription-renewal-required-for-production)
  + [Inspect recent jobs](#inspect-recent-jobs)
  + [Replay a stored job](#replay-a-stored-job)
  + [Dry-run meeting artifact fetches](#dry-run-meeting-artifact-fetches)
* [Routine Runbook](#routine-runbook)
  + [After first setup](#after-first-setup)
  + [Daily or periodic checks](#daily-or-periodic-checks)
  + [Before changing webhook URLs or delivery targets](#before-changing-webhook-urls-or-delivery-targets)
* [Failure Triage](#failure-triage)
  + [No jobs are being created](#no-jobs-are-being-created)
  + [Jobs stay in retry or fail before summarization](#jobs-stay-in-retry-or-fail-before-summarization)
  + [Summaries are produced but not delivered to Teams](#summaries-are-produced-but-not-delivered-to-teams)
  + [Duplicate or unexpected replays](#duplicate-or-unexpected-replays)
* [Go-Live Checklist](#go-live-checklist)
* [Delivery-Mode Decision Guide](#delivery-mode-decision-guide)
* [Operator Worksheet](#operator-worksheet)
* [Change Review Worksheet](#change-review-worksheet)
