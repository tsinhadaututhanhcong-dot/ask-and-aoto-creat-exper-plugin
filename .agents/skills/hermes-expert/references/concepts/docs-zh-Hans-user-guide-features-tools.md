# 工具与工具集 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tools](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tools)
...
**Honcho 跨会话记忆**作为记忆提供者插件（`plugins/memory/honcho/`）提供，而非内置工具集。安装方式请参阅 [Plugins](/docs/zh-Hans/user-guide/features/plugins)。
...
如需查看由代码派生的权威注册表，请参阅 [内置工具参考](/docs/zh-Hans/reference/tools-reference) 和 [工具集参考](/docs/zh-Hans/reference/toolsets-reference)。

Nous Tool Gateway

付费 [Nous Portal](https://portal.nousresearch.com) 订阅者可通过 **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** 使用网页搜索、图像生成、TTS 和浏览器自动化——无需单独配置 API 密钥。运行 `hermes model` 启用，或通过 `hermes tools` 配置各工具。

## 使用工具集[​](#使用工具集 "使用工具集的直接链接")
...
完整列表（包括 `hermes-cli`、`hermes-telegram` 等平台预设以及 `mcp-<server>` 等动态 MCP 工具集）请参阅 [工具集参考](/docs/zh-Hans/reference/toolsets-reference)。
...
这意味着 Docker 后端的行为类似持久化沙箱虚拟机，而非每次命令都使用全新容器。如果你执行过一次 `pip install foo`，该包在本次会话的剩余时间内均可用。如果你执行了 `cd /workspace/project`，后续的 `ls` 调用将看到该目录。完整的生命周期详情及控制 `/workspace` 和 `/root` 是否跨 Hermes 重启保留的 `container_persistent` 标志，请参阅 [配置 → Docker 后端](/docs/zh-Hans/user-guide/configuration#docker-backend)。
...
### Singularity/Apptainer[​](#singularityapptainer "Singularity/Apptainer的直接链接")
...
### Modal（无服务器云）[​](#modal无服务器云 "Modal（无服务器云）的直接链接")
