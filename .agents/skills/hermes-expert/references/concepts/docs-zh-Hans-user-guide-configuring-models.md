# 配置模型 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models)
...
[Nous Portal](/docs/zh-Hans/user-guide/features/tool-gateway) 在单一订阅下提供 300+ 个模型。全新安装后，运行 `hermes setup --portal` 即可登录并一键将 Nous 设为提供商。使用 `hermes portal info` 查看当前配置。

## Models 页面[​](#models-页面 "Models 页面的直接链接")
...
![Models 页面概览](/docs/zh-Hans/assets/images/overview-7ea6bbb1b2b6a12e2ff92ae07084c434.png)

顶部卡片为 **Model Settings** 面板。主行始终显示 agent 将为新会话启动的模型。点击 **Change** 打开选择器。

## 设置主模型[​](#设置主模型 "设置主模型的直接链接")

点击主模型行上的 **Change**：

![模型选择器对话框](/docs/zh-Hans/assets/images/picker-dialog-6fa8d5ea35b1ebcbad9b1e862822bf91.png)
...
## 设置辅助模型[​](#设置辅助模型 "设置辅助模型的直接链接")

点击 **Show auxiliary** 展开 11 个任务槽位：

![辅助面板展开状态](/docs/zh-Hans/assets/images/auxiliary-expanded-bc717f35df24e9d19ca45d5086688b56.png)

每个辅助任务默认为 `auto`，即 Hermes 对该任务也使用主模型。当某个边缘任务需要更便宜或更快的模型时，可单独覆盖该槽位。

### 常见覆盖模式[​](#常见覆盖模式 "常见覆盖模式的直接链接")
...
## 写入 `config.yaml` 的内容[​](#写入-configyaml-的内容 "写入-configyaml-的内容的直接链接")
...
### 我选择了模型，但 Hermes 切换了提供商[​](#我选择了模型但-hermes-切换了提供商 "我选择了模型，但 Hermes 切换了提供商的直接链接")
...
## 其他方法[​](#alternative-methods "其他方法的直接链接")

### CLI 斜杠命令[​](#cli-斜杠命令 "CLI 斜杠命令的直接链接")
...
然后在聊天中使用 `/model fav` 或 `/model grok`。用户别名会覆盖内置短名称（`sonnet`、`kimi`、`opus` 等）。完整参考请见[自定义模型别名](/docs/zh-Hans/reference/slash-commands#custom-model-aliases)。

### `hermes model` 子命令[​](#hermes-model-子命令 "hermes-model-子命令的直接链接")
...
### 直接编辑配置文件[​](#直接编辑配置文件 "直接编辑配置文件的直接链接")

编辑 `~/.hermes/config.yaml` 后重启相关服务。完整 schema 请见[配置参考](/docs/zh-Hans/user-guide/configuration)。
...
* [设置辅助模型](#设置辅助模型)
  + [常见覆盖模式](#常见覆盖模式)
  + [单任务覆盖](#单任务覆盖)
  + [全部重置为 auto](#全部重置为-auto)
* ["Use as" 快捷方式](#use-as-快捷方式)
* [写入 `config.yaml` 的内容](#写入-configyaml-的内容)
* [何时生效？](#何时生效)
* [故障排查](#故障排查)
  + [选择器中显示"No authenticated providers"](#选择器中显示no-authenticated-providers)
  + [主模型在运行中的聊天里未发生变化](#主模型在运行中的聊天里未发生变化)
  + [辅助覆盖"未生效"](#辅助覆盖未生效)
  + [我选择了模型，但 Hermes 切换了提供商](#我选择了模型但-hermes-切换了提供商)
* [其他方法](#alternative-methods)
  + [CLI 斜杠命令](#cli-斜杠命令)
  + [自定义别名](#自定义别名)
  + [`hermes model` 子命令](#hermes-model-子命令)
  + [直接编辑配置文件](#直接编辑配置文件)
  + [REST API](#rest-api)
