...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/skills/bundled/creative/creative-comfyui](https://hermes-agent.nousresearch.com/docs/user-guide/skills/bundled/creative/creative-comfyui)
...
| `auto_fix_deps.py` | Run check\_deps then `comfy node install` / `comfy model download` |
...
| `fetch_logs.py` | Pull traceback / status messages for a given prompt\_id |
...
`rosetta: true` (x86\_64 Python on Apple Silicon — must reinstall as ARM64).
...
4. **Missing custom nodes** — "class\_type not found" means a required node
