...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-grok](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-grok)
...
| Related skills | [`codex`](/docs/zh-Hans/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/zh-Hans/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`hermes-agent`](/docs/zh-Hans/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) |
...
## Two Orchestration Modes[​](#two-orchestration-modes "Two Orchestration Modes的直接链接")

### Mode 1: Headless (`-p`) — Non-Interactive (PREFERRED)[​](#mode-1-headless--p--non-interactive-preferred "mode-1-headless--p--non-interactive-preferred的直接链接")
...
### Mode 2: Interactive PTY — Multi-Turn TUI Sessions[​](#mode-2-interactive-pty--multi-turn-tui-sessions "Mode 2: Interactive PTY — Multi-Turn TUI Sessions的直接链接")
...
## Headless Deep Dive[​](#headless-deep-dive "Headless Deep Dive的直接链接")

### Common Flags[​](#common-flags "Common Flags的直接链接")
...
### Output Formats[​](#output-formats "Output Formats的直接链接")
...
### Background Mode (Long Tasks)[​](#background-mode-long-tasks "Background Mode (Long Tasks)的直接链接")
...
### Session Continuation[​](#session-continuation "Session Continuation的直接链接")
...
## Read-Only Audit → Markdown Note Pattern[​](#read-only-audit--markdown-note-pattern "Read-Only Audit → Markdown Note Pattern的直接链接")
...
## PR Review Patterns[​](#pr-review-patterns "PR Review Patterns的直接链接")

### Quick Review (Headless)[​](#quick-review-headless "Quick Review (Headless)的直接链接")
...
### Clone-to-temp Review (safe, no repo mutation)[​](#clone-to-temp-review-safe-no-repo-mutation "Clone-to-temp Review (safe, no repo mutation)的直接链接")
...
### Post the review[​](#post-the-review "Post the review的直接链接")

```
terminal(command="gh pr comment 42 --body '<review text>'", workdir="/path/to/repo")
```

## Parallel Issue Fixing with Worktrees[​](#parallel-issue-fixing-with-worktrees "Parallel Issue Fixing with Worktrees的直接链接")
...
## Useful Subcommands & TUI Commands[​](#useful-subcommands--tui-commands "Useful Subcommands & TUI Commands的直接链接")
...
## Config (`~/.grok/config.toml`)[​](#config-grokconfigtoml "config-grokconfigtoml的直接链接")
...
## Pitfalls & Gotchas[​](#pitfalls--gotchas "Pitfalls & Gotchas的直接链接")
...
## Rules for Hermes Agents[​](#rules-for-hermes-agents "Rules for Hermes Agents的直接链接")
...
* [Two Orchestration Modes](#two-orchestration-modes)
  + [Mode 1: Headless (`-p`) — Non-Interactive (PREFERRED)](#mode-1-headless--p--non-interactive-preferred)
  + [Mode 2: Interactive PTY — Multi-Turn TUI Sessions](#mode-2-interactive-pty--multi-turn-tui-sessions)
* [Headless Deep Dive](#headless-deep-dive)
  + [Common Flags](#common-flags)
  + [Output Formats](#output-formats)
  + [Background Mode (Long Tasks)](#background-mode-long-tasks)
  + [Session Continuation](#session-continuation)
* [Read-Only Audit → Markdown Note Pattern](#read-only-audit--markdown-note-pattern)
* [PR Review Patterns](#pr-review-patterns)
  + [Quick Review (Headless)](#quick-review-headless)
  + [Clone-to-temp Review (safe, no repo mutation)](#clone-to-temp-review-safe-no-repo-mutation)
  + [Post the review](#post-the-review)
* [Parallel Issue Fixing with Worktrees](#parallel-issue-fixing-with-worktrees)
* [Useful Subcommands & TUI Commands](#useful-subcommands--tui-commands)
* [Config (`~/.grok/config.toml`)](#config-grokconfigtoml)
