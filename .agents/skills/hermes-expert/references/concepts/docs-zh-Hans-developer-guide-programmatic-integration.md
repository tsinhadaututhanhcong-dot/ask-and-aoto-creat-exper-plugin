# 程序化集成 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/programmatic-integration](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/programmatic-integration)
...
## ACP（Agent Client Protocol）[​](#acpagent-client-protocol "ACP（Agent Client Protocol）的直接链接")
...
完整生命周期、事件桥接及审批流程：[ACP 内部机制](/docs/zh-Hans/developer-guide/acp-internals)。
...
## TUI Gateway JSON-RPC[​](#tui-gateway-json-rpc "TUI Gateway JSON-RPC的直接链接")
...
### Pi 风格 RPC 映射[​](#pi-风格-rpc-映射 "Pi 风格 RPC 映射的直接链接")
...
## 兼容 OpenAI 的 API Server[​](#兼容-openai-的-api-server "兼容 OpenAI 的 API Server的直接链接")
...
配置、请求头（`X-Hermes-Session-Id`、`X-Hermes-Session-Key`）及前端接入：[API Server](/docs/zh-Hans/user-guide/features/api-server)。

---

## 该选哪个？[​](#该选哪个 "该选哪个？的直接链接")
...
* **需要在 Python 进程内嵌入，不想启动子进程** → 直接导入 `run_agent.AIAgent`。参见 [Agent Loop](/docs/zh-Hans/developer-guide/agent-loop)。

---

## 模型热切换[​](#模型热切换 "模型热切换的直接链接")
...
## 关于 `--mode rpc` 的说明[​](#关于---mode-rpc-的说明 "关于---mode-rpc-的说明的直接链接")
...
* [TUI Gateway JSON-RPC](#tui-gateway-json-rpc)
  + [方法目录（精选）](#方法目录精选)
  + [流式返回的事件](#流式返回的事件)
  + [Pi 风格 RPC 映射](#pi-风格-rpc-映射)
* [兼容 OpenAI 的 API Server](#兼容-openai-的-api-server)
* [该选哪个？](#该选哪个)
* [模型热切换](#模型热切换)
* [关于 `--mode rpc` 的说明](#关于---mode-rpc-的说明)
