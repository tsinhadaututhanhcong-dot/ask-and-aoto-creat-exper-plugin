# Outlines — Outlines：结构化 JSON/regex/Pydantic LLM 生成 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-inference-outlines](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-inference-outlines)
...
### 使用 Pydantic 模型[​](#使用-pydantic-模型 "使用 Pydantic 模型的直接链接")
...
### 1. 受约束的 Token 采样[​](#1-受约束的-token-采样 "1. 受约束的 Token 采样的直接链接")
...
### 2. 结构化生成器[​](#2-结构化生成器 "2. 结构化生成器的直接链接")

Outlines 为不同输出类型提供专用生成器。

#### Choice 生成器[​](#choice-生成器 "Choice 生成器的直接链接")
...
#### JSON 生成器[​](#json-生成器 "JSON 生成器的直接链接")
...
#### Regex 生成器[​](#regex-生成器 "Regex 生成器的直接链接")
...
#### 整数/浮点数生成器[​](#整数浮点数生成器 "整数/浮点数生成器的直接链接")
...
### 3. 模型后端[​](#3-模型后端 "3. 模型后端的直接链接")

Outlines 支持多种本地及基于 API 的后端。

#### Transformers（Hugging Face）[​](#transformershugging-face "Transformers（Hugging Face）的直接链接")
...
#### llama.cpp[​](#llamacpp "llama.cpp的直接链接")
...
#### vLLM（高吞吐量）[​](#vllm高吞吐量 "vLLM（高吞吐量）的直接链接")
...
#### OpenAI（有限支持）[​](#openai有限支持 "OpenAI（有限支持）的直接链接")
...
### 4. Pydantic 集成[​](#4-pydantic-集成 "4. Pydantic 集成的直接链接")
...
#### Enum 与 Literal[​](#enum-与-literal "Enum 与 Literal的直接链接")
...
### 模式 1：数据提取[​](#模式-1数据提取 "模式 1：数据提取的直接链接")
...
### 模式 3：结构化表单[​](#模式-3结构化表单 "模式 3：结构化表单的直接链接")
...
### 模式 4：多实体提取[​](#模式-4多实体提取 "模式 4：多实体提取的直接链接")
...
### 模式 5：代码生成[​](#模式-5代码生成 "模式 5：代码生成的直接链接")
...
### 模式 6：批量处理[​](#模式-6批量处理 "模式 6：批量处理的直接链接")
...
### Transformers[​](#transformers "Transformers的直接链接")
...
### llama.cpp[​](#llamacpp-1 "llama.cpp的直接链接")
...
### vLLM（生产环境）[​](#vllm生产环境 "vLLM（生产环境）的直接链接")
...
### 1. 使用具体类型[​](#1-使用具体类型 "1. 使用具体类型的直接链接")
...
### 3. 对分类使用 Enum[​](#3-对分类使用-enum "3. 对分类使用 Enum的直接链接")
...
### 4. 在 Prompt 中提供上下文[​](#4-在-prompt-中提供上下文 "4. 在 Prompt 中提供上下文的直接链接")
...
### 5. 处理可选字段[​](#5-处理可选字段 "5. 处理可选字段的直接链接")
...
## 与替代方案的对比[​](#与替代方案的对比 "与替代方案的对比的直接链接")
...
* **GitHub**：[https://github.com/outlines-dev/outlines（8k+](https://github.com/outlines-dev/outlines%EF%BC%888k+) stars）
...
* [核心概念](#核心概念)
  + [1. 受约束的 Token 采样](#1-受约束的-token-采样)
  + [2. 结构化生成器](#2-结构化生成器)
  + [3. 模型后端](#3-模型后端)
  + [4. Pydantic 集成](#4-pydantic-集成)
* [常见模式](#常见模式)
  + [模式 1：数据提取](#模式-1数据提取)
  + [模式 2：分类](#模式-2分类)
  + [模式 3：结构化表单](#模式-3结构化表单)
  + [模式 4：多实体提取](#模式-4多实体提取)
  + [模式 5：代码生成](#模式-5代码生成)
  + [模式 6：批量处理](#模式-6批量处理)
* [后端配置](#后端配置)
...
* [最佳实践](#最佳实践)
  + [1. 使用具体类型](#1-使用具体类型)
  + [2. 添加约束](#2-添加约束)
  + [3. 对分类使用 Enum](#3-对分类使用-enum)
  + [4. 在 Prompt 中提供上下文](#4-在-prompt-中提供上下文)
  + [5. 处理可选字段](#5-处理可选字段)
* [与替代方案的对比](#与替代方案的对比)
* [性能特性](#性能特性)
* [资源](#资源)
