# Hermes Agent — 配置、扩展或贡献 Hermes Agent | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent)
...
| 相关 skill | [`claude-code`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`codex`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`opencode`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-opencode) |
...
### 工具与 Skill[​](#工具与-skill "工具与 Skill的直接链接")
...
### Gateway（消息平台）[​](#gateway消息平台 "Gateway（消息平台）的直接链接")
...
### Profiles[​](#profiles "Profiles的直接链接")
...
## 斜杠命令（会话内）[​](#斜杠命令会话内 "斜杠命令（会话内）的直接链接")
...
### 工具与 Skill[​](#工具与-skill-1 "工具与 Skill的直接链接")
...
### Gateway 消息中的 PII 脱敏[​](#gateway-消息中的-pii-脱敏 "Gateway 消息中的 PII 脱敏的直接链接")
...
### Shell hook 允许列表[​](#shell-hook-允许列表 "Shell hook 允许列表的直接链接")

某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.hermes/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具[​](#禁用-webbrowserimage-gen-工具 "禁用 web/browser/image-gen 工具的直接链接")

要完全阻止模型访问网络或媒体工具，打开 `hermes tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。

---

## 语音与转录[​](#语音与转录 "语音与转录的直接链接")

### STT（语音 → 文字）[​](#stt语音--文字 "STT（语音 → 文字）的直接链接")
...
### TTS（文字 → 语音）[​](#tts文字--语音 "TTS（文字 → 语音）的直接链接")
...
## 生成额外的 Hermes 实例[​](#生成额外的-hermes-实例 "生成额外的 Hermes 实例的直接链接")

将额外的 Hermes 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate\_task[​](#何时使用此方式-vs-delegate_task "何时使用此方式 vs delegate_task的直接链接")
...
### 交互式 PTY 模式（通过 tmux）[​](#交互式-pty-模式通过-tmux "交互式 PTY 模式（通过 tmux）的直接链接")

Hermes 使用 prompt\_toolkit，需要真实终端。使用 tmux 进行交互式生成：
...
### 多 Agent 协调[​](#多-agent-协调 "多 Agent 协调的直接链接")
...
* **交互式会话使用 tmux** — 原始 PTY 模式与 prompt\_toolkit 存在 `\r` vs `\n` 问题
* **定时任务使用 `cronjob` 工具而非生成进程** — 处理投递和重试

---

## 持久化与后台系统[​](#持久化与后台系统 "持久化与后台系统的直接链接")

四个系统与主对话循环并行运行。此处为快速参考；完整开发者说明位于 `AGENTS.md`，面向用户的文档位于 `website/docs/user-guide/features/`。

### 委派（`delegate_task`）[​](#委派delegate_task "委派delegate_task的直接链接")
...
### Cron（定时任务）[​](#cron定时任务 "Cron（定时任务）的直接链接")
...
### Curator（skill 生命周期）[​](#curatorskill-生命周期 "Curator（skill 生命周期）的直接链接")
...
### Kanban（多 agent 工作队列）[​](#kanban多-agent-工作队列 "Kanban（多 agent 工作队列）的直接链接")
...
## Windows 特有问题[​](#windows-特有问题 "Windows 特有问题的直接链接")
...
### 输入/键绑定[​](#输入键绑定 "输入/键绑定的直接链接")

**Alt+Enter 不插入换行。** Windows Terminal 在终端层拦截 Alt+Enter 以切换全屏——该按键永远不会到达 prompt\_toolkit。请改用 **Ctrl+Enter**。Windows Terminal 将 Ctrl+Enter 作为 LF（`c-j`）传递，与普通 Enter（`c-m` / CR）不同，CLI 仅在 `win32` 上将 `c-j` 绑定到换行插入（参见 `_bind_prompt_submit_keys` + `cli.py` 中仅限 Windows 的 `c-j` 绑定）。副作用：在 Windows 上，原始 Ctrl+J 按键也会插入换行——这是不可避免的，因为 Windows Terminal 在 Win32 控制台 API 层将 Ctrl+Enter 和 Ctrl+J 折叠为相同的键码。Windows 上 Ctrl+J 没有冲突的绑定，因此这是无害的副作用。

mintty / git-bash 行为相同（Alt+Enter 全屏），除非你在选项 → 键中禁用 Alt+Fn 快捷键。直接使用 Ctrl+Enter 更简单。

**诊断键绑定。** 运行 `python scripts/keystroke_diagnostic.py`（仓库根目录）可查看 prompt\_toolkit 在当前终端中如何识别每个按键。可回答"Shift+Enter 是否作为独立键传入？"（几乎从不——大多数终端将其折叠为普通 Enter）或"我的终端为 Ctrl+Enter 发送什么字节序列？"等问题。Ctrl+Enter = c-j 这一事实就是通过此方式确认的。

### 配置/文件[​](#配置文件 "配置/文件的直接链接")
...
### `execute_code` / 沙箱[​](#execute_code--沙箱 "execute_code--沙箱的直接链接")
...
### 添加工具（3 个文件）[​](#添加工具3-个文件 "添加工具（3 个文件）的直接链接")
...
### Agent 循环（高层概述）[​](#agent-循环高层概述 "Agent 循环（高层概述）的直接链接")
...
### 扩展系统 prompt 的执行环境块[​](#扩展系统-prompt-的执行环境块 "扩展系统 prompt 的执行环境块的直接链接")
...
* [关键路径与配置](#关键路径与配置)
  ...
  + [Toolset](#toolset)
* [安全与隐私开关](#安全与隐私开关)
  + [工具输出中的密钥脱敏](#工具输出中的密钥脱敏)
  + [Gateway 消息中的 PII 脱敏](#gateway-消息中的-pii-脱敏)
  + [命令审批提示](#命令审批提示)
  + [Shell hook 允许列表](#shell-hook-允许列表)
  + [禁用 web/browser/image-gen 工具](#禁用-webbrowserimage-gen-工具)
* [语音与转录](#语音与转录)
  + [STT（语音 → 文字）](#stt语音--文字)
  + [TTS（文字 → 语音）](#tts文字--语音)
* [生成额外的 Hermes 实例](#生成额外的-hermes-实例)
  + [何时使用此方式 vs delegate\_task](#何时使用此方式-vs-delegate_task)
  + [单次模式](#单次模式)
  + [交互式 PTY 模式（通过 tmux）](#交互式-pty-模式通过-tmux)
  + [多 Agent 协调](#多-agent-协调)
  + [会话恢复](#会话恢复)
  + [提示](#提示)
* [持久化与后台系统](#持久化与后台系统)
  + [委派（`delegate_task`）](#委派delegate_task)
  + [Cron（定时任务）](#cron定时任务)
  + [Curator（skill 生命周期）](#curatorskill-生命周期)
  + [Kanban（多 agent 工作队列）](#kanban多-agent-工作队列)
* [Windows 特有问题](#windows-特有问题)
  + [输入/键绑定](#输入键绑定)
  + [配置/文件](#配置文件)
  + [`execute_code` / 沙箱](#execute_code--沙箱)
  + [测试/贡献](#测试贡献)
  + [路径/文件系统](#路径文件系统)
* [故障排查](#故障排查)
  + [语音不工作](#语音不工作)
...
* [贡献者快速参考](#贡献者快速参考)
  + [项目结构](#项目结构)
  + [添加工具（3 个文件）](#添加工具3-个文件)
  + [添加斜杠命令](#添加斜杠命令)
  + [Agent 循环（高层概述）](#agent-循环高层概述)
  + [测试](#测试)
  + [扩展系统 prompt 的执行环境块](#扩展系统-prompt-的执行环境块)
  + [提交约定](#提交约定)
  + [关键规则](#关键规则)
...
- [https://hermes-agent.nousresearch.com/docs/](./docs.md)
- [https://hermes-agent.nousresearch.com/docs/integrations/providers](./docs-integrations-providers.md)
- [https://hermes-agent.nousresearch.com/docs/reference/cli-commands](./docs-reference-cli-commands.md)
- [https://hermes-agent.nousresearch.com/docs/reference/environment-variables](./docs-reference-environment-variables.md)
- [https://hermes-agent.nousresearch.com/docs/reference/skills-catalog](./docs-reference-skills-catalog.md)
- [https://hermes-agent.nousresearch.com/docs/reference/slash-commands](./docs-reference-slash-commands.md)
- [https://hermes-agent.nousresearch.com/docs/reference/tools-reference](./docs-reference-tools-reference.md)
...
- [https://hermes-agent.nousresearch.com/docs/user-guide/features/curator](./docs-user-guide-features-curator.md)
- [https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban](./docs-user-guide-features-kanban.md)
