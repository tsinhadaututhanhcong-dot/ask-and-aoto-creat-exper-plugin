# 语音与 TTS | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts)
...
如果你拥有付费的 [Nous Portal](https://portal.nousresearch.com) 订阅，OpenAI TTS 可通过 **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** 使用，无需单独的 OpenAI API 密钥。新安装可运行 `hermes setup --portal` 登录并一次性开启所有 gateway 工具；已有安装可通过 `hermes model` 或 `hermes tools` 选择 **Nous Subscription** 仅启用 TTS。
...
### Telegram 语音气泡与 ffmpeg[​](#telegram-语音气泡与-ffmpeg "Telegram 语音气泡与 ffmpeg的直接链接")
...
### xAI 自定义声音（声音克隆）[​](#xai-自定义声音声音克隆 "xAI 自定义声音（声音克隆）的直接链接")
...
### Piper（本地，支持 44 种语言）[​](#piper本地支持-44-种语言 "Piper（本地，支持 44 种语言）的直接链接")
...
### 自定义命令提供商[​](#自定义命令提供商 "自定义命令提供商的直接链接")
...
#### 示例：Doubao（中文 seed-tts-2.0）[​](#示例doubao中文-seed-tts-20 "示例：Doubao（中文 seed-tts-2.0）的直接链接")
...
### Python 插件提供商[​](#python-插件提供商 "Python 插件提供商的直接链接")
...
## 语音消息转录（STT）[​](#语音消息转录stt "语音消息转录（STT）的直接链接")
...
#### 示例：Doubao / Volcengine ASR[​](#示例doubao--volcengine-asr "示例：Doubao / Volcengine ASR的直接链接")
...
* [文字转语音（TTS）](#文字转语音tts)
  ...
  + [配置](#配置)
  + [输入长度限制](#输入长度限制)
  + [Telegram 语音气泡与 ffmpeg](#telegram-语音气泡与-ffmpeg)
  + [xAI 自定义声音（声音克隆）](#xai-自定义声音声音克隆)
  + [Piper（本地，支持 44 种语言）](#piper本地支持-44-种语言)
  + [自定义命令提供商](#自定义命令提供商)
  + [Python 插件提供商](#python-插件提供商)
* [语音消息转录（STT）](#语音消息转录stt)
  + [配置](#配置-1)
  + [提供商详情](#提供商详情)
  + [回退行为](#回退行为)
