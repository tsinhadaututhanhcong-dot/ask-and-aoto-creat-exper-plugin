...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code)
...
| 相关 skill | [`codex`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`hermes-agent`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent), [`opencode`](/docs/zh-Hans/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-opencode) |
...
## 两种编排模式[​](#两种编排模式 "两种编排模式的直接链接")

Hermes 以两种根本不同的方式与 Claude Code 交互。请根据任务选择合适的模式。

### 模式一：Print 模式（`-p`）— 非交互式（大多数任务的首选）[​](#模式一print-模式-p-非交互式大多数任务的首选 "模式一print-模式-p-非交互式大多数任务的首选的直接链接")
...
### 模式二：通过 tmux 的交互式 PTY — 多轮会话[​](#模式二通过-tmux-的交互式-pty--多轮会话 "模式二：通过 tmux 的交互式 PTY — 多轮会话的直接链接")
...
## PTY 对话框处理（交互模式的关键）[​](#pty-对话框处理交互模式的关键 "PTY 对话框处理（交互模式的关键）的直接链接")

Claude Code 在首次启动时最多会显示两个确认对话框。**必须**通过 tmux send-keys 处理这些对话框。

### 对话框一：工作区信任（首次访问某目录时）[​](#对话框一工作区信任首次访问某目录时 "对话框一：工作区信任（首次访问某目录时）的直接链接")
...
### 对话框二：绕过权限警告（仅在使用 --dangerously-skip-permissions 时）[​](#对话框二绕过权限警告仅在使用---dangerously-skip-permissions-时 "对话框二：绕过权限警告（仅在使用 --dangerously-skip-permissions 时）的直接链接")
...
### 健壮的对话框处理模式[​](#健壮的对话框处理模式 "健壮的对话框处理模式的直接链接")
...
## Print 模式深度解析[​](#print-模式深度解析 "Print 模式深度解析的直接链接")

### 结构化 JSON 输出[​](#结构化-json-输出 "结构化 JSON 输出的直接链接")
...
### 流式 JSON 输出[​](#流式-json-输出 "流式 JSON 输出的直接链接")
...
### 使用 JSON Schema 进行结构化提取[​](#使用-json-schema-进行结构化提取 "使用 JSON Schema 进行结构化提取的直接链接")
...
### CI/脚本的精简模式[​](#ci脚本的精简模式 "CI/脚本的精简模式的直接链接")
...
### 过载时的备用模型[​](#过载时的备用模型 "过载时的备用模型的直接链接")
...
## 完整 CLI 标志参考[​](#完整-cli-标志参考 "完整 CLI 标志参考的直接链接")

### 会话与环境[​](#会话与环境 "会话与环境的直接链接")
...
### 系统 Prompt 与上下文[​](#系统-prompt-与上下文 "系统 Prompt 与上下文的直接链接")
...
### Agent 团队[​](#agent-团队 "Agent 团队的直接链接")
...
### --allowedTools / --disallowedTools 的工具名称语法[​](#--allowedtools----disallowedtools-的工具名称语法 "--allowedTools / --disallowedTools 的工具名称语法的直接链接")
...
## 设置与配置[​](#设置与配置 "设置与配置的直接链接")

### 设置优先级（从高到低）[​](#设置优先级从高到低 "设置优先级（从高到低）的直接链接")
...
### 记忆文件（CLAUDE.md）层级[​](#记忆文件claudemd层级 "记忆文件（CLAUDE.md）层级的直接链接")
...
## 交互会话：斜杠命令[​](#交互会话斜杠命令 "交互会话：斜杠命令的直接链接")

### 会话与上下文[​](#会话与上下文 "会话与上下文的直接链接")
...
### Skills（自然语言调用）[​](#skills自然语言调用 "Skills（自然语言调用）的直接链接")
...
## 交互会话：键盘快捷键[​](#交互会话键盘快捷键 "交互会话：键盘快捷键的直接链接")
...
### 专业技巧："ultrathink"[​](#专业技巧ultrathink "专业技巧：\"ultrathink\"的直接链接")

在 prompt 中使用关键词 "ultrathink" 可在该轮次获得最大推理 effort。无论当前 `/effort` 设置如何，这都会触发最深层的思考模式。

## PR 审查模式[​](#pr-审查模式 "PR 审查模式的直接链接")

### 快速审查（Print 模式）[​](#快速审查print-模式 "快速审查（Print 模式）的直接链接")

```
terminal(command="cd /path/to/repo && git diff main...feature-branch | claude -p 'Review this diff for bugs, security issues, and style problems. Be thorough.' --max-turns 1", timeout=60)
```

### 深度审查（交互式 + Worktree）[​](#深度审查交互式--worktree "深度审查（交互式 + Worktree）的直接链接")
...
### 通过 PR 编号审查[​](#通过-pr-编号审查 "通过 PR 编号审查的直接链接")

```
terminal(command="claude -p 'Review this PR thoroughly' --from-pr 42 --max-turns 10", workdir="/path/to/repo", timeout=120)
```

### Claude Worktree 配合 tmux[​](#claude-worktree-配合-tmux "Claude Worktree 配合 tmux的直接链接")
...
## 并行 Claude 实例[​](#并行-claude-实例 "并行 Claude 实例的直接链接")
...
## CLAUDE.md — 项目上下文文件[​](#claudemd--项目上下文文件 "CLAUDE.md — 项目上下文文件的直接链接")
...
### 规则目录（模块化 CLAUDE.md）[​](#规则目录模块化-claudemd "规则目录（模块化 CLAUDE.md）的直接链接")
...
### 通过 CLI 动态定义 Agent[​](#通过-cli-动态定义-agent "通过 CLI 动态定义 Agent的直接链接")
...
## Hook — 事件触发自动化[​](#hook--事件触发自动化 "Hook — 事件触发自动化的直接链接")
...
### 全部 8 种 Hook 类型[​](#全部-8-种-hook-类型 "全部 8 种 Hook 类型的直接链接")
...
### Hook 环境变量[​](#hook-环境变量 "Hook 环境变量的直接链接")
...
### 安全 Hook 示例[​](#安全-hook-示例 "安全 Hook 示例的直接链接")
...
### Print/CI 模式中的 MCP[​](#printci-模式中的-mcp "Print/CI 模式中的 MCP的直接链接")
...
### MCP 限制与调优[​](#mcp-限制与调优 "MCP 限制与调优的直接链接")
...
## 监控交互会话[​](#监控交互会话 "监控交互会话的直接链接")

### 读取 TUI 状态[​](#读取-tui-状态 "读取 TUI 状态的直接链接")
...
### 上下文窗口健康状态[​](#上下文窗口健康状态 "上下文窗口健康状态的直接链接")
...
## Hermes Agent 规则[​](#hermes-agent-规则 "Hermes Agent 规则的直接链接")
...
* [前置条件](#前置条件)
* [两种编排模式](#两种编排模式)
  + [模式一：Print 模式（`-p`）— 非交互式（大多数任务的首选）](#模式一print-模式-p-非交互式大多数任务的首选)
  + [模式二：通过 tmux 的交互式 PTY — 多轮会话](#模式二通过-tmux-的交互式-pty--多轮会话)
* [PTY 对话框处理（交互模式的关键）](#pty-对话框处理交互模式的关键)
  + [对话框一：工作区信任（首次访问某目录时）](#对话框一工作区信任首次访问某目录时)
  + [对话框二：绕过权限警告（仅在使用 --dangerously-skip-permissions 时）](#对话框二绕过权限警告仅在使用---dangerously-skip-permissions-时)
  + [健壮的对话框处理模式](#健壮的对话框处理模式)
* [CLI 子命令](#cli-子命令)
* [Print 模式深度解析](#print-模式深度解析)
  + [结构化 JSON 输出](#结构化-json-输出)
  + [流式 JSON 输出](#流式-json-输出)
  + [双向流式传输](#双向流式传输)
  + [管道输入](#管道输入)
  + [使用 JSON Schema 进行结构化提取](#使用-json-schema-进行结构化提取)
  + [会话续接](#会话续接)
  + [CI/脚本的精简模式](#ci脚本的精简模式)
  + [过载时的备用模型](#过载时的备用模型)
* [完整 CLI 标志参考](#完整-cli-标志参考)
  + [会话与环境](#会话与环境)
  + [模型与性能](#模型与性能)
  + [权限与安全](#权限与安全)
  + [输出与输入格式](#输出与输入格式)
  + [系统 Prompt 与上下文](#系统-prompt-与上下文)
  + [调试](#调试)
  + [Agent 团队](#agent-团队)
  + [--allowedTools / --disallowedTools 的工具名称语法](#--allowedtools----disallowedtools-的工具名称语法)
* [设置与配置](#设置与配置)
  + [设置优先级（从高到低）](#设置优先级从高到低)
  + [设置中的权限](#设置中的权限)
  + [记忆文件（CLAUDE.md）层级](#记忆文件claudemd层级)
* [交互会话：斜杠命令](#交互会话斜杠命令)
  + [会话与上下文](#会话与上下文)
  + [开发与审查](#开发与审查)
  + [配置与工具](#配置与工具)
  + [自定义斜杠命令](#自定义斜杠命令)
  + [Skills（自然语言调用）](#skills自然语言调用)
* [交互会话：键盘快捷键](#交互会话键盘快捷键)
  + [通用控制](#通用控制)
  + [模式切换](#模式切换)
  + [多行输入](#多行输入)
  + [输入前缀](#输入前缀)
  + [专业技巧："ultrathink"](#专业技巧ultrathink)
* [PR 审查模式](#pr-审查模式)
  + [快速审查（Print 模式）](#快速审查print-模式)
  + [深度审查（交互式 + Worktree）](#深度审查交互式--worktree)
  + [通过 PR 编号审查](#通过-pr-编号审查)
  + [Claude Worktree 配合 tmux](#claude-worktree-配合-tmux)
* [并行 Claude 实例](#并行-claude-实例)
* [CLAUDE.md — 项目上下文文件](#claudemd--项目上下文文件)
  + [规则目录（模块化 CLAUDE.md）](#规则目录模块化-claudemd)
  + [自动记忆](#自动记忆)
* [自定义子 Agent](#自定义子-agent)
  + [Agent 位置优先级](#agent-位置优先级)
  + [创建 Agent](#创建-agent)
  + [通过 CLI 动态定义 Agent](#通过-cli-动态定义-agent)
* [Hook — 事件触发自动化](#hook--事件触发自动化)
  + [全部 8 种 Hook 类型](#全部-8-种-hook-类型)
  + [Hook 环境变量](#hook-环境变量)
  + [安全 Hook 示例](#安全-hook-示例)
* [MCP 集成](#mcp-集成)
  + [MCP 作用域](#mcp-作用域)
  + [Print/CI 模式中的 MCP](#printci-模式中的-mcp)
  + [MCP 限制与调优](#mcp-限制与调优)
* [监控交互会话](#监控交互会话)
  + [读取 TUI 状态](#读取-tui-状态)
  + [上下文窗口健康状态](#上下文窗口健康状态)
...
* [成本与性能建议](#成本与性能建议)
* [陷阱与注意事项](#陷阱与注意事项)
* [Hermes Agent 规则](#hermes-agent-规则)
