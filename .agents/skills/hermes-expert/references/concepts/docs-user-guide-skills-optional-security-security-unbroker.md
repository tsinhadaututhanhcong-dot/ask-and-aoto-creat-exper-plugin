# Unbroker — Autonomously remove your info from data-broker sites | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/security/security-unbroker](https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/security/security-unbroker)
...
## Autonomy contract[​](#autonomy-contract "Direct link to Autonomy contract")
...
| `$PDD plan <subject> --batch` | **Reduce view**: overlays ledger state, groups brokers by next action (unscanned/found/indirect/blocked/in\_progress/done), collapses ownership clusters, **orders `found` cluster-parents-first + emits a tailored `parent_playbook`**, prints `next_actions` |
...
## Batch operation (two-phase: crawl-all, then delete)[​](#batch-operation-two-phase-crawl-all-then-delete "Direct link to Batch operation (two-phase: crawl-all, then delete)")
...
## Procedure (the autonomous loop)[​](#procedure-the-autonomous-loop "Direct link to Procedure (the autonomous loop)")
...
5. **Opt-outs (when `next` says so).** Actions come pre-ordered parents-first with `steps` from each
   ...
   * **web\_form** → drive `optout_url` with `browser_navigate`/`browser_type`/`browser_click`, submit
     ...
     listing suppression).
   * **email** → `$PDD send-email <subject> <broker> --kind <ccpa|gdpr|generic> --to <addr> --listing <url>` records + discloses in one step (recipient locked to addresses the broker
   ...
   * **phone\_callback / account / gov\_id / fax / mail / voice (T3)** *without a deletion email* →
...
8. **Schedule the next wake-up.** `next` returns `next_wake_at` (earliest due re-check). Create ONE
   ...
   unbroker loop for <subject\_id>: `$PDD next` and execute all actions"*). Processing
...
* [Autonomy contract](#autonomy-contract)
* [When to Use](#when-to-use)
* [Prerequisites](#prerequisites)
* [How to Run](#how-to-run)
* [Quick Reference](#quick-reference)
* [Batch operation (two-phase: crawl-all, then delete)](#batch-operation-two-phase-crawl-all-then-delete)
* [Procedure (the autonomous loop)](#procedure-the-autonomous-loop)
* [Pitfalls](#pitfalls)
