# 轨迹格式 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/trajectory-format](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/trajectory-format)
...
## JSONL 条目格式[​](#jsonl-条目格式 "JSONL 条目格式的直接链接")

文件中每一行是一个独立的 JSON 对象。共有两种变体：

### CLI/交互式格式（来自 `_save_trajectory`）[​](#cli交互式格式来自-_save_trajectory "cli交互式格式来自-_save_trajectory的直接链接")
...
### 批量运行器格式（来自 `batch_runner.py`）[​](#批量运行器格式来自-batch_runnerpy "批量运行器格式来自-batch_runnerpy的直接链接")
...
## conversations 数组（ShareGPT 格式）[​](#conversations-数组sharegpt-格式 "conversations 数组（ShareGPT 格式）的直接链接")
...
### 加载至 HuggingFace Datasets[​](#加载至-huggingface-datasets "加载至 HuggingFace Datasets的直接链接")
...
* [文件命名规范](#文件命名规范)
* [JSONL 条目格式](#jsonl-条目格式)
  + [CLI/交互式格式（来自 `_save_trajectory`）](#cli交互式格式来自-_save_trajectory)
  + [批量运行器格式（来自 `batch_runner.py`）](#批量运行器格式来自-batch_runnerpy)
* [conversations 数组（ShareGPT 格式）](#conversations-数组sharegpt-格式)
  + [完整示例](#完整示例)
* [规范化规则](#规范化规则)
  + [推理内容标记](#推理内容标记)
  + [工具调用规范化](#工具调用规范化)
  ...
  + [系统消息](#系统消息)
* [加载轨迹](#加载轨迹)
  + [加载至 HuggingFace Datasets](#加载至-huggingface-datasets)
* [控制轨迹保存](#控制轨迹保存)