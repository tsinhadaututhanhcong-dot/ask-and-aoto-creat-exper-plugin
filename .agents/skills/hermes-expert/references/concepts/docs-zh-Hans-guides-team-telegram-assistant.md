# 教程：团队 Telegram 助手 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/team-telegram-assistant](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/team-telegram-assistant)
...
* **已在服务器或 VPS 上安装 Hermes Agent**（不是你的笔记本——机器人需要持续运行）。如尚未安装，请参阅[安装指南](/docs/zh-Hans/getting-started/installation)。
...
## 第一步：创建 Telegram 机器人[​](#第一步创建-telegram-机器人 "第一步：创建 Telegram 机器人的直接链接")
...
## 第二步：配置 Gateway[​](#第二步配置-gateway "第二步：配置 Gateway的直接链接")
...
### 查找你的用户 ID[​](#查找你的用户-id "查找你的用户 ID的直接链接")
...
## 第三步：启动 Gateway[​](#第三步启动-gateway "第三步：启动 Gateway的直接链接")
...
### 生产环境：安装为服务[​](#生产环境安装为服务 "生产环境：安装为服务的直接链接")
...
## 第四步：设置团队访问权限[​](#第四步设置团队访问权限 "第四步：设置团队访问权限的直接链接")

现在让你的队友获得访问权限。有两种方式。

### 方式 A：静态白名单[​](#方式-a静态白名单 "方式 A：静态白名单的直接链接")
...
### 方式 B：私信配对（推荐用于团队）[​](#方式-b私信配对推荐用于团队 "方式 B：私信配对（推荐用于团队）的直接链接")
...
## 第五步：配置机器人[​](#第五步配置机器人 "第五步：配置机器人的直接链接")

### 设置主频道[​](#设置主频道 "设置主频道的直接链接")
...
### 配置工具进度显示[​](#配置工具进度显示 "配置工具进度显示的直接链接")
...
### 使用 SOUL.md 设置个性[​](#使用-soulmd-设置个性 "使用 SOUL.md 设置个性的直接链接")

通过编辑 `~/.hermes/SOUL.md` 自定义机器人的沟通方式：

完整指南请参阅[在 Hermes 中使用 SOUL.md](/docs/zh-Hans/guides/use-soul-with-hermes)。
...
## 第六步：设置定时任务[​](#第六步设置定时任务 "第六步：设置定时任务的直接链接")

gateway 运行后，你可以安排定期任务，将结果投递到团队频道。

### 每日站会摘要[​](#每日站会摘要 "每日站会摘要的直接链接")
...
## 生产环境建议[​](#生产环境建议 "生产环境建议的直接链接")

### 使用 Docker 保障安全[​](#使用-docker-保障安全 "使用 Docker 保障安全的直接链接")
...
### 监控 Gateway[​](#监控-gateway "监控 Gateway的直接链接")
...
### 保持 Hermes 更新[​](#保持-hermes-更新 "保持 Hermes 更新的直接链接")
...
* **[安全指南](/docs/zh-Hans/user-guide/security)**——深入了解授权、容器隔离和命令审批
* **[消息 Gateway](/docs/zh-Hans/user-guide/messaging)**——gateway 架构、会话管理和聊天命令的完整参考
* **[Telegram 设置](/docs/zh-Hans/user-guide/messaging/telegram)**——平台专属详情，包括语音消息和 TTS
* **[定时任务](/docs/zh-Hans/user-guide/features/cron)**——高级 cron 调度，含投递选项和 cron 表达式
* **[上下文文件](/docs/zh-Hans/user-guide/features/context-files)**——用于项目知识的 AGENTS.md、SOUL.md 和 .cursorrules
* **[个性设置](/docs/zh-Hans/user-guide/features/personality)**——内置个性预设和自定义角色定义
* **添加更多平台**——同一 gateway 可同时运行 [Discord](/docs/zh-Hans/user-guide/messaging/discord)、[Slack](/docs/zh-Hans/user-guide/messaging/slack) 和 [WhatsApp](/docs/zh-Hans/user-guide/messaging/whatsapp)
...
* [我们要构建什么](#我们要构建什么)
* [前提条件](#前提条件)
* [第一步：创建 Telegram 机器人](#第一步创建-telegram-机器人)
* [第二步：配置 Gateway](#第二步配置-gateway)
  + [方式 A：交互式设置（推荐）](#方式-a交互式设置推荐)
  + [方式 B：手动配置](#方式-b手动配置)
  + [查找你的用户 ID](#查找你的用户-id)
* [第三步：启动 Gateway](#第三步启动-gateway)
  + [快速测试](#快速测试)
  + [生产环境：安装为服务](#生产环境安装为服务)
  + [验证运行状态](#验证运行状态)
* [第四步：设置团队访问权限](#第四步设置团队访问权限)
  + [方式 A：静态白名单](#方式-a静态白名单)
  + [方式 B：私信配对（推荐用于团队）](#方式-b私信配对推荐用于团队)
  + [安全注意事项](#安全注意事项)
* [第五步：配置机器人](#第五步配置机器人)
  + [设置主频道](#设置主频道)
  + [配置工具进度显示](#配置工具进度显示)
  + [使用 SOUL.md 设置个性](#使用-soulmd-设置个性)
  + [添加项目上下文](#添加项目上下文)
* [第六步：设置定时任务](#第六步设置定时任务)
  + [每日站会摘要](#每日站会摘要)
  + [服务器健康检查](#服务器健康检查)
  + [管理定时任务](#管理定时任务)
* [生产环境建议](#生产环境建议)
  + [使用 Docker 保障安全](#使用-docker-保障安全)
  + [监控 Gateway](#监控-gateway)
  + [保持 Hermes 更新](#保持-hermes-更新)
  + [日志位置](#日志位置)
* [进一步探索](#进一步探索)
