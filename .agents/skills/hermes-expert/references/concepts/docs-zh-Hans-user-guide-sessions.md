# Sessions（会话） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/sessions](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/sessions)

本页总览

Hermes Agent 自动将每次对话保存为一个 session。Session 支持对话恢复、跨 session 搜索以及完整的对话历史管理。

## Session 的工作原理[​](#session-的工作原理 "Session 的工作原理的直接链接")
...
* Token 计数（输入/输出）
* 时间戳（started\_at、ended\_at）
* 父 session ID（用于压缩触发的 session 分割）

### 哪些内容计入上下文[​](#哪些内容计入上下文 "哪些内容计入上下文的直接链接")
...
### Session 来源[​](#session-来源 "Session 来源的直接链接")
...
## CLI Session 恢复[​](#cli-session-恢复 "CLI Session 恢复的直接链接")

使用 `--continue` 或 `--resume` 从 CLI 恢复之前的对话：

### 继续上次 Session[​](#继续上次-session "继续上次 Session的直接链接")
...
### 恢复时的对话摘要[​](#恢复时的对话摘要 "恢复时的对话摘要的直接链接")

恢复 session 时，Hermes 会在输入提示符前以样式化面板显示之前对话的紧凑摘要：

![恢复 Hermes session 时显示的「上次对话」摘要面板的样式化预览。](/img/docs/session-recap.svg)
...
## Session 命名[​](#session-naming "Session 命名的直接链接")

为 session 设置人类可读的标题，便于查找和恢复。

### 自动生成标题[​](#自动生成标题 "自动生成标题的直接链接")
...
### 压缩时的自动谱系[​](#压缩时的自动谱系 "压缩时的自动谱系的直接链接")
...
### 在消息平台中使用 /title[​](#在消息平台中使用-title "在消息平台中使用 /title的直接链接")
...
## Session 管理命令[​](#session-管理命令 "Session 管理命令的直接链接")

Hermes 通过 `hermes sessions` 提供完整的 session 管理命令集：

### 列出 Session[​](#列出-session "列出 Session的直接链接")
...
### 导出 Session[​](#导出-session "导出 Session的直接链接")
...
### 删除 Session[​](#删除-session "删除 Session的直接链接")
...
### 重命名 Session[​](#重命名-session "重命名 Session的直接链接")
...
### 清理旧 Session[​](#清理旧-session "清理旧 Session的直接链接")
...
### Session 统计[​](#session-统计 "Session 统计的直接链接")
...
如需更深入的分析——token 用量、费用估算、工具分解和活动模式——请使用 [`hermes insights`](/docs/zh-Hans/reference/cli-commands#hermes-insights)。

## Session 搜索工具[​](#session-搜索工具 "Session 搜索工具的直接链接")
...
### FTS5 查询语法[​](#fts5-查询语法 "FTS5 查询语法的直接链接")
...
> *"当用户引用过去对话中的内容，或你怀疑存在相关的先前上下文时，在要求用户重复之前先使用 session\_search 召回。"*

典型触发词：「我们之前做过这个」、「还记得吗」、「上次」、「正如我提到的」，或任何当前窗口中没有的项目/人物/概念的引用。

## 各平台 Session 跟踪[​](#各平台-session-跟踪 "各平台 Session 跟踪的直接链接")

### Gateway Session[​](#gateway-session "Gateway Session的直接链接")
...
### 共享与隔离的群组 Session[​](#共享与隔离的群组-session "共享与隔离的群组 Session的直接链接")
...
### Session 重置策略[​](#session-重置策略 "Session 重置策略的直接链接")
...
### 数据库 Schema[​](#数据库-schema "数据库 Schema的直接链接")

`state.db` 中的关键表：

* **sessions** — session 元数据（id、source、user\_id、model、title、时间戳、token 计数）。标题有唯一索引（允许 NULL 标题，只有非 NULL 标题必须唯一）。
* **messages** — 完整消息历史（role、content、tool\_calls、tool\_name、token\_count）
* **messages\_fts** — 用于跨消息内容全文搜索的 FTS5 虚拟表

## Session 过期与清理[​](#session-过期与清理 "Session 过期与清理的直接链接")
...
* [Session 的工作原理](#session-的工作原理)
  + [哪些内容计入上下文](#哪些内容计入上下文)
  + [Session 来源](#session-来源)
* [CLI Session 恢复](#cli-session-恢复)
  + [继续上次 Session](#继续上次-session)
  + [按名称恢复](#按名称恢复)
  + [恢复特定 Session](#恢复特定-session)
  + [恢复时的对话摘要](#恢复时的对话摘要)
* [跨平台切换](#跨平台切换)
* [Session 命名](#session-naming)
  ...
  + [标题规则](#标题规则)
  + [压缩时的自动谱系](#压缩时的自动谱系)
  + [在消息平台中使用 /title](#在消息平台中使用-title)
* [Session 管理命令](#session-管理命令)
  + [列出 Session](#列出-session)
  + [导出 Session](#导出-session)
  + [删除 Session](#删除-session)
  + [重命名 Session](#重命名-session)
  + [清理旧 Session](#清理旧-session)
  + [Session 统计](#session-统计)
* [Session 搜索工具](#session-搜索工具)
  + [三种调用形式](#三种调用形式)
  + [FTS5 查询语法](#fts5-查询语法)
  + [可选参数](#可选参数)
  + [使用时机](#使用时机)
* [各平台 Session 跟踪](#各平台-session-跟踪)
  + [Gateway Session](#gateway-session)
  + [共享与隔离的群组 Session](#共享与隔离的群组-session)
  + [Session 重置策略](#session-重置策略)
* [存储位置](#存储位置)
  + [数据库 Schema](#数据库-schema)
* [Session 过期与清理](#session-过期与清理)
  + [自动清理](#自动清理)
  + [手动清理](#手动清理)
