# Memory Provider 插件 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/memory-provider-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/memory-provider-plugin)
...
Memory provider 是两种 **provider 插件**类型之一。另一种是 [Context Engine 插件](/docs/zh-Hans/developer-guide/context-engine-plugin)，用于替换内置的上下文压缩器。两者遵循相同的模式：单选、配置驱动、通过 `hermes plugins` 管理。
...
## MemoryProvider 抽象基类[​](#memoryprovider-抽象基类 "MemoryProvider 抽象基类的直接链接")
...
## 必须实现的方法[​](#必须实现的方法 "必须实现的方法的直接链接")

### 核心生命周期[​](#核心生命周期 "核心生命周期的直接链接")
...
## 添加 CLI 命令[​](#添加-cli-命令 "添加 CLI 命令的直接链接")
...
### 含 CLI 的目录结构[​](#含-cli-的目录结构 "含 CLI 的目录结构的直接链接")
...
* [添加 CLI 命令](#添加-cli-命令)
  ...
  + [示例](#示例)
  + [参考实现](#参考实现)
  + [含 CLI 的目录结构](#含-cli-的目录结构)
* [单 Provider 规则](#单-provider-规则)
