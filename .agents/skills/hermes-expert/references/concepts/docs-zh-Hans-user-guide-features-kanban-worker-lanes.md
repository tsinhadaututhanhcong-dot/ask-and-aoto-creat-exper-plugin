# Kanban worker lanes（工作者通道） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban-worker-lanes](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban-worker-lanes)
...
## 通道需提供的内容[​](#通道需提供的内容 "通道需提供的内容的直接链接")

要成为 kanban worker lane，集成必须提供三项内容：

### 1. assignee 字符串[​](#1-assignee-字符串 "1. assignee 字符串的直接链接")
...
### 3. 生命周期终止器[​](#3-生命周期终止器 "3. 生命周期终止器的直接链接")

每次 claim 必须以以下之一结束：

* `kanban_complete(summary=..., metadata=...)` — 任务成功，状态切换为 `done`。
* `kanban_block(reason=...)` — 任务等待人工输入，状态切换为 `blocked`。调度器在 `kanban_unblock` 运行时重新生成。
* worker 进程退出而未调用任何工具。内核回收该进程并发出 `crashed`（PID 已消亡）、`gave_up`（连续失败断路器触发）或 `timed_out`（超过 max\_runtime）。这是失败路径；健康的 worker 不会在此结束。

kanban 内核强制要求每次运行恰好由其中一项终止。既未调用任何终止工具又正常退出的 worker 将被视为崩溃。

## 输出与 review-required 约定[​](#输出与-review-required-约定 "输出与 review-required 约定的直接链接")
...
* **先将结构化元数据写入 `kanban_comment`**，因为 `kanban_block` 只携带人类可读的 `reason`。Comment 是持久的注解通道——所有与审计相关的字段（changed\_files、tests\_run、diff\_path 或 PR url、决策记录）都应放在这里。
...
## 现有通道形态[​](#现有通道形态 "现有通道形态的直接链接")

### Hermes profile 通道（默认）[​](#hermes-profile-通道默认 "Hermes profile 通道（默认）的直接链接")
...
### Orchestrator profile 通道[​](#orchestrator-profile-通道 "Orchestrator profile 通道的直接链接")
...
## 添加外部 CLI worker 通道[​](#添加外部-cli-worker-通道 "添加外部 CLI worker 通道的直接链接")
...
## 调度器处理的失败模式[​](#调度器处理的失败模式 "调度器处理的失败模式的直接链接")
...
* [Kanban 概览](/docs/zh-Hans/user-guide/features/kanban) — 面向用户的介绍。
* [Kanban 教程](/docs/zh-Hans/user-guide/features/kanban-tutorial) — 开启仪表板的完整演练。
...
* [层级结构](#层级结构)
* [通道需提供的内容](#通道需提供的内容)
  + [1. assignee 字符串](#1-assignee-字符串)
  + [2. 生成机制](#2-生成机制)
  + [3. 生命周期终止器](#3-生命周期终止器)
* [输出与 review-required 约定](#输出与-review-required-约定)
* [日志与审计追踪](#日志与审计追踪)
* [现有通道形态](#现有通道形态)
  + [Hermes profile 通道（默认）](#hermes-profile-通道默认)
  + [Orchestrator profile 通道](#orchestrator-profile-通道)
* [添加外部 CLI worker 通道](#添加外部-cli-worker-通道)
* [调度器处理的失败模式](#调度器处理的失败模式)
* [相关资源](#相关资源)
