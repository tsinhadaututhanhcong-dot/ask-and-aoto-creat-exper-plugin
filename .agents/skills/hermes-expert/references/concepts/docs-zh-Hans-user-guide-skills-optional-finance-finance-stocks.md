# Stocks — 通过 Yahoo 获取股票报价、历史、搜索、比较及加密货币数据 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-stocks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-stocks)
...
| 相关 skill | [`dcf-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-dcf-model), [`comps-analysis`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-comps-analysis), [`lbo-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-lbo-model) |
...
### `quote SYMBOL [SYMBOL2 ...]`[​](#quote-symbol-symbol2- "quote-symbol-symbol2-的直接链接")

当前价格、涨跌额、涨跌幅、成交量、52 周高低点。

### `search QUERY`[​](#search-query "search-query的直接链接")

通过公司名称查找股票代码。返回前 5 条结果：代码、名称、交易所、类型。

### `history SYMBOL [--range RANGE]`[​](#history-symbol---range-range "history-symbol---range-range的直接链接")

每日 OHLCV 数据及统计信息（最小值、最大值、均值、总回报率 %）。时间范围：`1mo`、
`3mo`、`6mo`、`1y`、`5y`。默认：`1mo`。

### `compare SYMBOL1 SYMBOL2 [...]`[​](#compare-symbol1-symbol2- "compare-symbol1-symbol2-的直接链接")

并排对比：价格、涨跌幅、52 周表现。

### `crypto SYMBOL [SYMBOL2 ...]`[​](#crypto-symbol-symbol2- "crypto-symbol-symbol2-的直接链接")
