# CLI 命令参考 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/cli-commands](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/cli-commands)

本页总览

本页介绍从 shell 运行的**终端命令**。

关于聊天内斜杠命令，请参阅 [斜杠命令参考](/docs/zh-Hans/reference/slash-commands)。
...
| `--tui` | 启动 [TUI](/docs/zh-Hans/user-guide/tui) 而非经典 CLI。等同于 `HERMES_TUI=1`。 |
...
| `hermes proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](/docs/zh-Hans/user-guide/features/subscription-proxy)。 |
| `hermes lsp` | 管理 Language Server Protocol 集成（为 write\_file/patch 提供语义诊断）。 |
...
| `hermes bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](/docs/zh-Hans/user-guide/features/skills#skill-bundles)。 |
| `hermes curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](/docs/zh-Hans/user-guide/features/curator)。 |
...
| `hermes portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)。 |
...
## `hermes chat`[​](#hermes-chat "hermes-chat的直接链接")
...
### `hermes -z <prompt>` — 脚本化单次调用[​](#hermes--z-prompt--脚本化单次调用 "hermes--z-prompt--脚本化单次调用的直接链接")
...
## `hermes model`[​](#hermes-model "hermes-model的直接链接")
...
### `/model` 斜杠命令（会话中途）[​](#model-斜杠命令会话中途 "model-斜杠命令会话中途的直接链接")
...
## `hermes gateway`[​](#hermes-gateway "hermes-gateway的直接链接")
...
使用 `hermes gateway run` 而非 `hermes gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hermes 'hermes gateway run'`。详见 [WSL FAQ](/docs/zh-Hans/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。

## `hermes lsp`[​](#hermes-lsp "hermes-lsp的直接链接")
...
完整指南、支持的语言和配置项，请参阅 [LSP — 语义诊断](/docs/zh-Hans/user-guide/features/lsp)。

## `hermes setup`[​](#hermes-setup "hermes-setup的直接链接")
...
| `--portal` | 一键 Nous Portal 设置：通过 OAuth 登录，将 Nous 设为推理 provider，并选择加入 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)。跳过向导其余部分。 |

## `hermes portal`[​](#hermes-portal "hermes-portal的直接链接")
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)。关于一键设置路径，请参阅上方的 `hermes setup --portal`。

## `hermes whatsapp`[​](#hermes-whatsapp "hermes-whatsapp的直接链接")
...
## `hermes slack`[​](#hermes-slack "hermes-slack的直接链接")
...
生成一个 Slack app manifest，将 `COMMAND_REGISTRY` 中的每条 gateway 命令（`/btw`、`/stop`、`/model` 等）注册为一等公民 Slack 斜杠命令——与 Discord 和 Telegram 保持一致。将输出粘贴到你的 Slack app 配置中：<https://api.slack.com/apps> → 你的 app → **Features → App Manifest → Edit**，然后点击 **Save**。如果 scope 或斜杠命令有变化，Slack 会提示重新安装。
...
## `hermes login` / `hermes logout` *（已弃用）*[​](#hermes-login--hermes-logout-已弃用 "hermes-login--hermes-logout-已弃用的直接链接")

警告

`hermes login` 已被移除。请使用 `hermes auth` 管理 OAuth 凭据，使用 `hermes model` 选择 provider，或使用 `hermes setup` 进行完整的交互式设置。

## `hermes auth`[​](#hermes-auth "hermes-auth的直接链接")

管理同一 provider 的密钥轮换凭据池。完整文档请参阅 [凭据池](/docs/zh-Hans/user-guide/features/credential-pools)。
...
## `hermes status`[​](#hermes-status "hermes-status的直接链接")
...
## `hermes cron`[​](#hermes-cron "hermes-cron的直接链接")
...
## `hermes kanban`[​](#hermes-kanban "hermes-kanban的直接链接")
...
完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/hermes-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/docs/zh-Hans/user-guide/features/kanban)。

## `hermes webhook`[​](#hermes-webhook "hermes-webhook的直接链接")
...
### `hermes webhook subscribe`[​](#hermes-webhook-subscribe "hermes-webhook-subscribe的直接链接")
...
## `hermes doctor`[​](#hermes-doctor "hermes-doctor的直接链接")
...
## `hermes dump`[​](#hermes-dump "hermes-dump的直接链接")
...
| **Identity** | 活跃 profile 名称、HERMES\_HOME 路径 |
...
## `hermes debug`[​](#hermes-debug "hermes-debug的直接链接")
...
## `hermes backup`[​](#hermes-backup "hermes-backup的直接链接")
...
## `hermes checkpoints`[​](#hermes-checkpoints "hermes-checkpoints的直接链接")
...
完整架构和会话内命令，请参阅 [Checkpoints 与 `/rollback`](/docs/zh-Hans/user-guide/checkpoints-and-rollback)。

## `hermes import`[​](#hermes-import "hermes-import的直接链接")
...
## `hermes logs`[​](#hermes-logs "hermes-logs的直接链接")
...
## `hermes config`[​](#hermes-config "hermes-config的直接链接")
...
## `hermes pairing`[​](#hermes-pairing "hermes-pairing的直接链接")
...
## `hermes skills`[​](#hermes-skills "hermes-skills的直接链接")
...
## `hermes bundles`[​](#hermes-bundles "hermes-bundles的直接链接")

```
hermes bundles <subcommand>
```

Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.hermes/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](/docs/zh-Hans/user-guide/features/skills#skill-bundles)。
...
## `hermes curator`[​](#hermes-curator "hermes-curator的直接链接")
...
行为和配置请参阅 [Curator](/docs/zh-Hans/user-guide/features/curator)。

## `hermes fallback`[​](#hermes-fallback "hermes-fallback的直接链接")
...
参见 [Fallback Providers](/docs/zh-Hans/user-guide/features/fallback-providers)。

## `hermes hooks`[​](#hermes-hooks "hermes-hooks的直接链接")
...
事件签名和 payload 格式请参阅 [Hooks](/docs/zh-Hans/user-guide/features/hooks)。

## `hermes memory`[​](#hermes-memory "hermes-memory的直接链接")
...
## `hermes acp`[​](#hermes-acp "hermes-acp的直接链接")
...
参见 [ACP 编辑器集成](/docs/zh-Hans/user-guide/features/acp) 和 [ACP 内部原理](/docs/zh-Hans/developer-guide/acp-internals)。

## `hermes mcp`[​](#hermes-mcp "hermes-mcp的直接链接")
...
参见 [MCP 配置参考](/docs/zh-Hans/reference/mcp-config-reference)、[在 Hermes 中使用 MCP](/docs/zh-Hans/guides/use-mcp-with-hermes) 和 [MCP 服务器模式](/docs/zh-Hans/user-guide/features/mcp#running-hermes-as-an-mcp-server)。

## `hermes plugins`[​](#hermes-plugins "hermes-plugins的直接链接")
...
参见 [Plugins](/docs/zh-Hans/user-guide/features/plugins) 和 [构建 Hermes Plugin](/docs/zh-Hans/developer-guide/plugins/)。

## `hermes tools`[​](#hermes-tools "hermes-tools的直接链接")
...
## `hermes computer-use`[​](#hermes-computer-use "hermes-computer-use的直接链接")
...
## `hermes sessions`[​](#hermes-sessions "hermes-sessions的直接链接")
...
## `hermes insights`[​](#hermes-insights "hermes-insights的直接链接")
...
## `hermes claw`[​](#hermes-claw "hermes-claw的直接链接")
...
完整的 config 键映射、SecretRef 处理详情和迁移后检查清单，请参阅\*\*[完整迁移指南](/docs/zh-Hans/guides/migrate-from-openclaw)\*\*。
...
## `hermes dashboard`[​](#hermes-dashboard "hermes-dashboard的直接链接")

```
hermes dashboard [options]
```

启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `cd ~/.hermes/hermes-agent && uv pip install -e ".[web]"`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`cd ~/.hermes/hermes-agent && uv pip install -e ".[web,pty]"`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/docs/zh-Hans/user-guide/features/web-dashboard)。
...
## `hermes profile`[​](#hermes-profile "hermes-profile的直接链接")
...
## `hermes completion`[​](#hermes-completion "hermes-completion的直接链接")
...
## `hermes update`[​](#hermes-update "hermes-update的直接链接")
...
* [CLI 界面](/docs/zh-Hans/user-guide/cli)
* [会话](/docs/zh-Hans/user-guide/sessions)
* [Skill 系统](/docs/zh-Hans/user-guide/features/skills)
* [皮肤与主题](/docs/zh-Hans/user-guide/features/skins)

* [全局入口](#全局入口)
  + [全局选项](#全局选项)
* [顶级命令](#顶级命令)
* [`hermes chat`](#hermes-chat)
  + [`hermes -z <prompt>` — 脚本化单次调用](#hermes--z-prompt--脚本化单次调用)
* [`hermes model`](#hermes-model)
  + [`/model` 斜杠命令（会话中途）](#model-斜杠命令会话中途)
...
* [`hermes login` / `hermes logout` *（已弃用）*](#hermes-login--hermes-logout-已弃用)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/acp-internals](./docs-zh-Hans-developer-guide-acp-internals.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/plugins/](./docs-zh-Hans-developer-guide-plugins.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/migrate-from-openclaw](./docs-zh-Hans-guides-migrate-from-openclaw.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/use-mcp-with-hermes](./docs-zh-Hans-guides-use-mcp-with-hermes.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/faq](./docs-zh-Hans-reference-faq.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/curator](./docs-zh-Hans-user-guide-features-curator.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban](./docs-zh-Hans-user-guide-features-kanban.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/lsp](./docs-zh-Hans-user-guide-features-lsp.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/subscription-proxy](./docs-zh-Hans-user-guide-features-subscription-proxy.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/tui](./docs-zh-Hans-user-guide-tui.md)
