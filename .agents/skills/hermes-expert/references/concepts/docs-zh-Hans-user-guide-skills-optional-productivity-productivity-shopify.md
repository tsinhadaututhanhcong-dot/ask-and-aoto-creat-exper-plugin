# Shopify — 通过 curl 使用 Shopify Admin 与 Storefront GraphQL API | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/productivity/productivity-shopify](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/productivity/productivity-shopify)
...
| 相关 skill | [`airtable`](/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-airtable), [`xurl`](/docs/zh-Hans/user-guide/skills/bundled/social-media/social-media-xurl) |
...
### 店铺信息 + 当前 API 版本[​](#店铺信息--当前-api-版本 "店铺信息 + 当前 API 版本的直接链接")

```
shop_gql '{ shop { name myshopifyDomain primaryDomain { url } currencyCode plan { displayName } } }' | jq
```

### 列出所有支持的 API 版本[​](#列出所有支持的-api-版本 "列出所有支持的 API 版本的直接链接")
...
### 搜索商品（前 20 条匹配结果）[​](#搜索商品前-20-条匹配结果 "搜索商品（前 20 条匹配结果）的直接链接")
...
### 分页获取商品（游标）[​](#分页获取商品游标 "分页获取商品（游标）的直接链接")
...
### 获取商品（含变体 + metafield）[​](#获取商品含变体--metafield "获取商品（含变体 + metafield）的直接链接")
...
### 创建含一个变体的商品[​](#创建含一个变体的商品 "创建含一个变体的商品的直接链接")
...
### 更新价格 / SKU[​](#更新价格--sku "更新价格 / SKU的直接链接")
...
### 列出最近订单（不含 `read_all_orders` 时默认最多 30 条）[​](#列出最近订单不含-read_all_orders-时默认最多-30-条 "列出最近订单不含-read_all_orders-时默认最多-30-条的直接链接")
...
### 获取单个订单（含收货地址）[​](#获取单个订单含收货地址 "获取单个订单（含收货地址）的直接链接")
...
## Metafield 与 Metaobject[​](#metafield-与-metaobject "Metafield 与 Metaobject的直接链接")
...
## Storefront API（公开只读）[​](#storefront-api公开只读 "Storefront API（公开只读）的直接链接")
...
* [API 基础](#api-基础)
* [发现](#发现)
  + [店铺信息 + 当前 API 版本](#店铺信息--当前-api-版本)
  + [列出所有支持的 API 版本](#列出所有支持的-api-版本)
* [商品](#商品)
  + [搜索商品（前 20 条匹配结果）](#搜索商品前-20-条匹配结果)
  + [分页获取商品（游标）](#分页获取商品游标)
  + [获取商品（含变体 + metafield）](#获取商品含变体--metafield)
  + [创建含一个变体的商品](#创建含一个变体的商品)
  + [更新价格 / SKU](#更新价格--sku)
* [订单](#订单)
  + [列出最近订单（不含 `read_all_orders` 时默认最多 30 条）](#列出最近订单不含-read_all_orders-时默认最多-30-条)
  + [获取单个订单（含收货地址）](#获取单个订单含收货地址)
* [客户](#客户)
* [库存](#库存)
* [Metafield 与 Metaobject](#metafield-与-metaobject)
* [Storefront API（公开只读）](#storefront-api公开只读)
* [批量操作](#批量操作)
* [Webhook](#webhook)
* [常见陷阱](#常见陷阱)
* [安全须知](#安全须知)
