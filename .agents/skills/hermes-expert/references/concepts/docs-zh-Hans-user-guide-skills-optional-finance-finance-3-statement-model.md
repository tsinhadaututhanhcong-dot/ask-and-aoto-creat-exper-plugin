# 三表模型 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-3-statement-model](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-3-statement-model)
...
| 相关 skill | [`excel-author`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-excel-author), [`pptx-author`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-pptx-author), [`dcf-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-dcf-model), [`lbo-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-lbo-model) |
...
## ⚠️ 核心原则 — 填写任何模板前必读[​](#️-核心原则--填写任何模板前必读 "⚠️ 核心原则 — 填写任何模板前必读的直接链接")
...
## 格式 — 专业蓝灰配色（除非模板/用户另有指定）[​](#格式--专业蓝灰配色除非模板用户另有指定 "格式 — 专业蓝灰配色（除非模板/用户另有指定）的直接链接")
...
### 识别模板标签页组织[​](#识别模板标签页组织 "识别模板标签页组织的直接链接")
...
### 含利润率的利润表布局[​](#含利润率的利润表布局 "含利润率的利润表布局的直接链接")
...
### 信用指标层级检查[​](#信用指标层级检查 "信用指标层级检查的直接链接")
...
### 契约合规追踪[​](#契约合规追踪 "契约合规追踪的直接链接")

如已知债务契约条款，添加明确的合规检查，将实际指标与契约阈值进行比较。

## 情景分析（基准 / 乐观 / 悲观）[​](#情景分析基准--乐观--悲观 "情景分析（基准 / 乐观 / 悲观）的直接链接")
...
## SEC 申报文件数据提取[​](#sec-申报文件数据提取 "SEC 申报文件数据提取的直接链接")
...
## 填写模型模板[​](#填写模型模板 "填写模型模板的直接链接")

本节提供填写任意三表财务模型模板的通用指引，同时保留现有公式并确保数据完整性。

### 第一步：分析模板结构[​](#第一步分析模板结构 "第一步：分析模板结构的直接链接")
...
### 第二步：在不破坏公式的前提下录入数据[​](#第二步在不破坏公式的前提下录入数据 "第二步：在不破坏公式的前提下录入数据的直接链接")
...
### 第三步：验证公式[​](#第三步验证公式 "第三步：验证公式的直接链接")
...
### 第四步：按工作表进行质量检查[​](#第四步按工作表进行质量检查 "第四步：按工作表进行质量检查的直接链接")
...
### 第五步：跨报表完整性检查[​](#第五步跨报表完整性检查 "第五步：跨报表完整性检查的直接链接")
...
### 第六步：最终审查[​](#第六步最终审查 "第六步：最终审查的直接链接")
...
## 模型验证与审计[​](#模型验证与审计 "模型验证与审计的直接链接")

本节汇总已完成模板的所有验证检查和审计程序。

### 核心勾稽项（必须始终成立）[​](#核心勾稽项必须始终成立 "核心勾稽项（必须始终成立）的直接链接")
...
## 数据来源 — 优先 MCP，其次网络回退[​](#数据来源--优先-mcp其次网络回退 "数据来源 — 优先 MCP，其次网络回退的直接链接")
...
* [环境](#环境)
* [⚠️ 核心原则 — 填写任何模板前必读](#️-核心原则--填写任何模板前必读)
* [格式 — 专业蓝灰配色（除非模板/用户另有指定）](#格式--专业蓝灰配色除非模板用户另有指定)
* [模型结构](#模型结构)
  + [识别模板标签页组织](#识别模板标签页组织)
  + [理解模板结构](#理解模板结构)
  + [预测期间](#预测期间)
* [利润率分析](#利润率分析)
  + [核心利润率指标](#核心利润率指标)
  + [含利润率的利润表布局](#含利润率的利润表布局)
* [信用指标](#信用指标)
  + [核心信用指标](#核心信用指标)
  + [信用指标层级检查](#信用指标层级检查)
  + [契约合规追踪](#契约合规追踪)
* [情景分析（基准 / 乐观 / 悲观）](#情景分析基准--乐观--悲观)
* [SEC 申报文件数据提取](#sec-申报文件数据提取)
* [填写模型模板](#填写模型模板)
  + [第一步：分析模板结构](#第一步分析模板结构)
  + [第二步：在不破坏公式的前提下录入数据](#第二步在不破坏公式的前提下录入数据)
  + [第三步：验证公式](#第三步验证公式)
  + [第四步：按工作表进行质量检查](#第四步按工作表进行质量检查)
  + [第五步：跨报表完整性检查](#第五步跨报表完整性检查)
  + [第六步：最终审查](#第六步最终审查)
* [模型验证与审计](#模型验证与审计)
  + [核心勾稽项（必须始终成立）](#核心勾稽项必须始终成立)
  + [符号规范参考](#符号规范参考)
  + [循环引用处理](#循环引用处理)
  + [检查类别](#检查类别)
  + [主检查公式](#主检查公式)
  + [快速调试流程](#快速调试流程)
* [数据来源 — 优先 MCP，其次网络回退](#数据来源--优先-mcp其次网络回退)
* [归属声明](#归属声明)
