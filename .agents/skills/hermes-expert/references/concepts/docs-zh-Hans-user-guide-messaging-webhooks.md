...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/webhooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/webhooks)
...
## 配置路由[​](#configuring-routes "配置路由的直接链接")
...
### Prompt 模板[​](#prompt-模板 "Prompt 模板的直接链接")
...
## GitHub PR 审查（分步说明）[​](#github-pr-review "GitHub PR 审查（分步说明）的直接链接")

本演练将为每个 pull request 设置自动代码审查。

### 1. 在 GitHub 中创建 webhook[​](#1-在-github-中创建-webhook "1. 在 GitHub 中创建 webhook的直接链接")
...
### 2. 添加路由配置[​](#2-添加路由配置 "2. 添加路由配置的直接链接")

按照上方示例，将 `github-pr` 路由添加到 `~/.hermes/config.yaml`。

### 3. 确保 `gh` CLI 已认证[​](#3-确保-gh-cli-已认证 "3-确保-gh-cli-已认证的直接链接")
...
## GitLab Webhook 设置[​](#gitlab-webhook-setup "GitLab Webhook 设置的直接链接")

GitLab webhook 的工作方式类似，但使用不同的认证机制。GitLab 通过 `X-Gitlab-Token` 请求头以明文字符串匹配（非 HMAC）发送 secret。

### 1. 在 GitLab 中创建 webhook[​](#1-在-gitlab-中创建-webhook "1. 在 GitLab 中创建 webhook的直接链接")
...
### 2. 添加路由配置[​](#2-添加路由配置-1 "2. 添加路由配置的直接链接")
...
## 投递选项[​](#delivery-options "投递选项的直接链接")
...
## 直接投递模式[​](#direct-delivery-mode "直接投递模式的直接链接")
...
### 何时使用直接投递[​](#何时使用直接投递 "何时使用直接投递的直接链接")
...
### 示例：从 Supabase 推送到 Telegram[​](#示例从-supabase-推送到-telegram "示例：从 Supabase 推送到 Telegram的直接链接")
...
### 示例：通过 CLI 动态订阅[​](#示例通过-cli-动态订阅 "示例：通过 CLI 动态订阅的直接链接")
...
## 动态订阅（CLI）[​](#dynamic-subscriptions "动态订阅（CLI）的直接链接")
...
### 动态订阅的工作原理[​](#动态订阅的工作原理 "动态订阅的工作原理的直接链接")
...
### agent 驱动的订阅[​](#agent-驱动的订阅 "agent 驱动的订阅的直接链接")
...
### HMAC 签名验证[​](#hmac-签名验证 "HMAC 签名验证的直接链接")
...
### Secret 为必填项[​](#secret-为必填项 "Secret 为必填项的直接链接")
...
### Prompt 注入风险[​](#prompt-注入风险 "Prompt 注入风险的直接链接")
...
## 故障排查[​](#troubleshooting "故障排查的直接链接")

### Webhook 未到达[​](#webhook-未到达 "Webhook 未到达的直接链接")
...
### Agent 未响应[​](#agent-未响应 "Agent 未响应的直接链接")
...
### `gh` CLI 错误（GitHub 评论投递）[​](#gh-cli-错误github-评论投递 "gh-cli-错误github-评论投递的直接链接")
...
## 环境变量[​](#environment-variables "环境变量的直接链接")
...
* [GitHub PR 审查（分步说明）](#github-pr-review)
  + [1. 在 GitHub 中创建 webhook](#1-在-github-中创建-webhook)
  + [2. 添加路由配置](#2-添加路由配置)
  + [3. 确保 `gh` CLI 已认证](#3-确保-gh-cli-已认证)
  + [4. 测试](#4-测试)
* [GitLab Webhook 设置](#gitlab-webhook-setup)
  + [1. 在 GitLab 中创建 webhook](#1-在-gitlab-中创建-webhook)
  + [2. 添加路由配置](#2-添加路由配置-1)
* [投递选项](#delivery-options)
* [直接投递模式](#direct-delivery-mode)
  + [何时使用直接投递](#何时使用直接投递)
  + [示例：从 Supabase 推送到 Telegram](#示例从-supabase-推送到-telegram)
  + [示例：通过 CLI 动态订阅](#示例通过-cli-动态订阅)
  + [响应状态码](#响应状态码)
  + [配置注意事项](#配置注意事项)
* [动态订阅（CLI）](#dynamic-subscriptions)
  ...
  + [测试订阅](#测试订阅)
  + [动态订阅的工作原理](#动态订阅的工作原理)
  + [agent 驱动的订阅](#agent-驱动的订阅)
* [安全性](#security)
  + [HMAC 签名验证](#hmac-签名验证)
  + [Secret 为必填项](#secret-为必填项)
  + [速率限制](#速率限制)
  + [幂等性](#幂等性)
  + [请求体大小限制](#请求体大小限制)
  + [Prompt 注入风险](#prompt-注入风险)
* [故障排查](#troubleshooting)
  + [Webhook 未到达](#webhook-未到达)
  + [签名验证失败](#签名验证失败)
  + [事件被忽略](#事件被忽略)
  + [Agent 未响应](#agent-未响应)
  + [重复响应](#重复响应)
  + [`gh` CLI 错误（GitHub 评论投递）](#gh-cli-错误github-评论投递)
