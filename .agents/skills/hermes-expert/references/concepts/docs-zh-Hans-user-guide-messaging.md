# 消息网关 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/)
...
完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/docs/zh-Hans/user-guide/features/voice-mode) 和 [Use Voice Mode with Hermes](/docs/zh-Hans/guides/use-voice-mode-with-hermes)。
...
## 聊天命令（在消息平台内使用）[​](#聊天命令在消息平台内使用 "聊天命令（在消息平台内使用）的直接链接")
...
### 私信配对（白名单的替代方案）[​](#私信配对白名单的替代方案 "私信配对（白名单的替代方案）的直接链接")
...
### 管理员与普通用户[​](#管理员与普通用户 "管理员与普通用户的直接链接")
...
#### 查看你的权限[​](#查看你的权限 "查看你的权限的直接链接")

在任意平台使用 `/whoami` 查看当前范围、你的层级（管理员 / 普通用户 / 无限制）以及你可以运行的斜杠命令。平台特定示例请参阅 [Telegram](/docs/zh-Hans/user-guide/messaging/telegram#slash-command-access-control) 和 [Discord](/docs/zh-Hans/user-guide/messaging/discord#slash-command-access-control) 页面。

## 中断 Agent[​](#中断-agent "中断 Agent的直接链接")
...
### 队列 vs 中断 vs 引导（繁忙输入模式）[​](#队列-vs-中断-vs-引导繁忙输入模式 "队列 vs 中断 vs 引导（繁忙输入模式）的直接链接")
...
### Linux（systemd）[​](#linuxsystemd "Linux（systemd）的直接链接")
...
### macOS（launchd）[​](#macoslaunchd "macOS（launchd）的直接链接")
...
* **VIRTUAL\_ENV** — 指向 Python 虚拟环境，使工具能正确解析包。
* **HERMES\_HOME** — 将网关限定到你的 Hermes 安装。
...
| Home Assistant | `hermes-homeassistant` | 完整工具 + HA 设备控制（ha\_list\_entities、ha\_get\_state、ha\_call\_service、ha\_list\_services） |
...
## 运营多平台网关[​](#运营多平台网关 "运营多平台网关的直接链接")

网关通常同时运行多个适配器（Telegram + Discord + Slack 等）。以下章节涵盖跨所有平台的日常运维操作。

### `/platform` 命令[​](#platform-命令 "platform-命令的直接链接")
...
另请参阅更广泛的状态汇总命令 [`/platforms`](/docs/zh-Hans/reference/slash-commands#info)。

### 自动熔断器[​](#自动熔断器 "自动熔断器的直接链接")
...
### 适配器暂停时的排查步骤[​](#适配器暂停时的排查步骤 "适配器暂停时的排查步骤的直接链接")
...
### 网关重启后的会话恢复[​](#网关重启后的会话恢复 "网关重启后的会话恢复的直接链接")
...
### 进度气泡清理（可选启用）[​](#进度气泡清理可选启用 "进度气泡清理（可选启用）的直接链接")
...
* [Telegram 配置](/docs/zh-Hans/user-guide/messaging/telegram)
* [Discord 配置](/docs/zh-Hans/user-guide/messaging/discord)
* [Slack 配置](/docs/zh-Hans/user-guide/messaging/slack)
* [Google Chat 配置](/docs/zh-Hans/user-guide/messaging/google_chat)
* [WhatsApp 配置](/docs/zh-Hans/user-guide/messaging/whatsapp)
* [Signal 配置](/docs/zh-Hans/user-guide/messaging/signal)
* [SMS 配置（Twilio）](/docs/zh-Hans/user-guide/messaging/sms)
* [Email 配置](/docs/zh-Hans/user-guide/messaging/email)
* [Home Assistant 集成](/docs/zh-Hans/user-guide/messaging/homeassistant)
* [Mattermost 配置](/docs/zh-Hans/user-guide/messaging/mattermost)
* [Matrix 配置](/docs/zh-Hans/user-guide/messaging/matrix)
* [DingTalk 配置](/docs/zh-Hans/user-guide/messaging/dingtalk)
* [Feishu/Lark 配置](/docs/zh-Hans/user-guide/messaging/feishu)
* [WeCom 配置](/docs/zh-Hans/user-guide/messaging/wecom)
* [WeCom Callback 配置](/docs/zh-Hans/user-guide/messaging/wecom-callback)
* [Weixin 配置（微信）](/docs/zh-Hans/user-guide/messaging/weixin)
* [BlueBubbles 配置（iMessage）](/docs/zh-Hans/user-guide/messaging/bluebubbles)
* [QQBot 配置](/docs/zh-Hans/user-guide/messaging/qqbot)
* [Yuanbao 配置](/docs/zh-Hans/user-guide/messaging/yuanbao)
* [Microsoft Teams 配置](/docs/zh-Hans/user-guide/messaging/teams)
* [Teams 会议流水线](/docs/zh-Hans/user-guide/messaging/teams-meetings)
* [Open WebUI + API Server](/docs/zh-Hans/user-guide/messaging/open-webui)
* [Webhooks](/docs/zh-Hans/user-guide/messaging/webhooks)

* [平台对比](#平台对比)
* [架构](#架构)
* [快速配置](#快速配置)
* [网关命令](#网关命令)
* [聊天命令（在消息平台内使用）](#聊天命令在消息平台内使用)
* [会话管理](#会话管理)
  + [会话持久化](#会话持久化)
  + [重置策略](#重置策略)
* [安全](#安全)
  + [私信配对（白名单的替代方案）](#私信配对白名单的替代方案)
  + [管理员与普通用户](#管理员与普通用户)
* [中断 Agent](#中断-agent)
  + [队列 vs 中断 vs 引导（繁忙输入模式）](#队列-vs-中断-vs-引导繁忙输入模式)
* [工具进度通知](#工具进度通知)
* [后台会话](#后台会话)
  + [工作原理](#工作原理)
  + [后台进程通知](#后台进程通知)
...
* [服务管理](#服务管理)
  ...
  + [macOS（launchd）](#macoslaunchd)
* [平台专属工具集](#平台专属工具集)
* [运营多平台网关](#运营多平台网关)
  + [`/platform` 命令](#platform-命令)
  + [自动熔断器](#自动熔断器)
  + [适配器暂停时的排查步骤](#适配器暂停时的排查步骤)
  + [重启通知](#重启通知)
  + [正在输入指示器](#正在输入指示器)
  + [网关重启后的会话恢复](#网关重启后的会话恢复)
  + [进度气泡清理（可选启用）](#进度气泡清理可选启用)
* [后续步骤](#后续步骤)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/google_chat](./docs-zh-Hans-user-guide-messaging-googlechat.md)
