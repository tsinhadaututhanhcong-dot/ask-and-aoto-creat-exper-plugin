# Signal | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/signal](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/signal)
...
### 安装 signal-cli[​](#安装-signal-cli "安装 signal-cli的直接链接")
...
## 第一步：关联你的 Signal 账号[​](#第一步关联你的-signal-账号 "第一步：关联你的 Signal 账号的直接链接")
...
## 第二步：启动 signal-cli 守护进程[​](#第二步启动-signal-cli-守护进程 "第二步：启动 signal-cli 守护进程的直接链接")
...
### 原生格式、引用回复与表情回应[​](#原生格式引用回复与表情回应 "原生格式、引用回复与表情回应的直接链接")
...
### 给自己发消息（单号码配置）[​](#给自己发消息单号码配置 "给自己发消息（单号码配置）的直接链接")
...
* [前提条件](#前提条件)
  + [安装 signal-cli](#安装-signal-cli)
* [第一步：关联你的 Signal 账号](#第一步关联你的-signal-账号)
* [第二步：启动 signal-cli 守护进程](#第二步启动-signal-cli-守护进程)
...
* [功能特性](#功能特性)
  + [附件](#附件)
  + [原生格式、引用回复与表情回应](#原生格式引用回复与表情回应)
  + [正在输入指示器](#正在输入指示器)
  + [手机号脱敏](#手机号脱敏)
  + [给自己发消息（单号码配置）](#给自己发消息单号码配置)
  + [健康监控](#健康监控)
* [故障排查](#故障排查)
* [安全](#安全)
* [环境变量参考](#环境变量参考)