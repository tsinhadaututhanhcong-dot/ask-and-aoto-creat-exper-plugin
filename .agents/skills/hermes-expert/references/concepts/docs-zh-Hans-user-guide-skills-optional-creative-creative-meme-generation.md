# Meme Generation — 使用 Pillow 选取模板并叠加文字，生成真实的表情包图片 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/creative/creative-meme-generation](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/creative/creative-meme-generation)
...
| 相关 skill | [`ascii-art`](/docs/zh-Hans/user-guide/skills/bundled/creative/creative-ascii-art), `generative-widgets` |
...
### 精选模板（自定义文字位置）[​](#精选模板自定义文字位置 "精选模板（自定义文字位置）的直接链接")
...
### 动态模板（来自 imgflip API）[​](#动态模板来自-imgflip-api "动态模板（来自 imgflip API）的直接链接")
...
### 模式 1：经典模板（默认）[​](#模式-1经典模板默认 "模式 1：经典模板（默认）的直接链接")
...
### 模式 2：自定义 AI 图片（当 image\_generate 可用时）[​](#模式-2自定义-ai-图片当-image_generate-可用时 "模式 2：自定义 AI 图片（当 image_generate 可用时）的直接链接")

当没有合适的经典模板，或用户想要原创内容时使用此模式。

1. 先编写说明文字。
2. 使用 `image_generate` 创建符合表情包概念的场景。图片 prompt（提示词）中**不要包含任何文字** — 文字将由脚本添加。仅描述视觉场景。
3. 从 image\_generate 结果 URL 中找到生成图片的路径。如有需要，将其下载到本地路径。
...
* [使用时机](#使用时机)
* [可用模板](#可用模板)
  + [精选模板（自定义文字位置）](#精选模板自定义文字位置)
  + [动态模板（来自 imgflip API）](#动态模板来自-imgflip-api)
* [操作流程](#操作流程)
  + [模式 1：经典模板（默认）](#模式-1经典模板默认)
  + [模式 2：自定义 AI 图片（当 image\_generate 可用时）](#模式-2自定义-ai-图片当-image_generate-可用时)
* [示例](#示例)
* [列出模板](#列出模板)
* [注意事项](#注意事项)
* [验证](#验证)
