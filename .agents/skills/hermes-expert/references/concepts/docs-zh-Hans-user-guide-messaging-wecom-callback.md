# WeCom 回调（自建应用） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/wecom-callback](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/wecom-callback)

本页总览

通过回调/webhook 模式，将 Hermes 作为企业自建应用接入企业微信（WeCom）。

WeCom Bot 与 WeCom 回调

Hermes 支持两种企业微信集成模式：

* **[WeCom Bot](/docs/zh-Hans/user-guide/messaging/wecom)** — Bot 风格，通过 WebSocket 连接。配置简单，支持群聊。
...
### 1. 在企业微信中创建自建应用[​](#1-在企业微信中创建自建应用 "1. 在企业微信中创建自建应用的直接链接")
