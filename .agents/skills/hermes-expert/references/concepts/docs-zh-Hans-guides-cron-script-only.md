# 纯脚本 Cron 任务（无 LLM） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/cron-script-only](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/cron-script-only)
...
### agent 为你决定的内容[​](#agent-为你决定的内容 "agent 为你决定的内容的直接链接")
...
### 通过聊天管理看门狗[​](#通过聊天管理看门狗 "通过聊天管理看门狗的直接链接")
...
## 通过 CLI 创建[​](#通过-cli-创建 "通过 CLI 创建的直接链接")
...
## 脚本输出与投递的映射关系[​](#脚本输出与投递的映射关系 "脚本输出与投递的映射关系的直接链接")
...
完整语法请参阅 [cron 功能参考](/docs/zh-Hans/user-guide/features/cron)。
...
## 实战示例：磁盘空间告警[​](#实战示例磁盘空间告警 "实战示例：磁盘空间告警的直接链接")
...
## 与其他模式的对比[​](#与其他模式的对比 "与其他模式的对比的直接链接")

| 方式 | 运行内容 | 适用场景 |
| --- | --- | --- |
| `cronjob --no-agent`（本页） | 你的脚本，由 Hermes 调度 | 不需要推理的周期性看门狗 / 告警 / 指标 |
| `cronjob`（默认，LLM） | 带可选预检脚本的 agent | 消息内容需要对数据进行推理时 |
| OS cron + `curl` 到 [webhook 订阅](/docs/zh-Hans/user-guide/messaging/webhooks) | 你的脚本，由 OS 调度 | 当 Hermes 本身可能不健康时（即被监控对象） |
...
* [用 Cron 自动化一切](/docs/zh-Hans/guides/automate-with-cron) — LLM 驱动的 cron 模式。
* [定时任务（Cron）参考](/docs/zh-Hans/user-guide/features/cron) — 完整计划语法、生命周期、投递路由。
* [Webhook 订阅](/docs/zh-Hans/user-guide/messaging/webhooks) — 供外部调度器使用的即发即忘 HTTP 入口。
* [Gateway 内部机制](/docs/zh-Hans/developer-guide/gateway-internals) — 投递路由器内部实现。

* [适用场景](#适用场景)
* [通过聊天创建](#通过聊天创建)
  + [示例对话](#示例对话)
  + [agent 为你决定的内容](#agent-为你决定的内容)
  + [通过聊天管理看门狗](#通过聊天管理看门狗)
* [通过 CLI 创建](#通过-cli-创建)
* [脚本输出与投递的映射关系](#脚本输出与投递的映射关系)
* [脚本规则](#脚本规则)
* [计划语法](#计划语法)
* [投递目标](#投递目标)
* [编辑与生命周期](#编辑与生命周期)
* [实战示例：磁盘空间告警](#实战示例磁盘空间告警)
* [与其他模式的对比](#与其他模式的对比)
* [相关文档](#相关文档)
