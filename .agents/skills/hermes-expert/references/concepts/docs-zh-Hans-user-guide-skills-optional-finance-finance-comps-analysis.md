# 可比公司分析 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-comps-analysis](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-comps-analysis)
...
| 相关 skills | [`excel-author`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-excel-author), [`pptx-author`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-pptx-author), [`dcf-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-dcf-model), [`lbo-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-lbo-model) |
...
## ⚠️ 关键：数据来源优先级（请先阅读）[​](#️-关键数据来源优先级请先阅读 "⚠️ 关键：数据来源优先级（请先阅读）的直接链接")
...
## ⚠️ 关键：公式优先于硬编码 + 逐步验证[​](#️-关键公式优先于硬编码--逐步验证 "⚠️ 关键：公式优先于硬编码 + 逐步验证的直接链接")
...
## 第 1 节：文档结构与设置[​](#第-1-节文档结构与设置 "第 1 节：文档结构与设置的直接链接")

### 标题块（第 1-3 行）[​](#标题块第-1-3-行 "标题块（第 1-3 行）的直接链接")
...
### 视觉约定标准（可选——用户偏好和上传的模板始终优先）[​](#视觉约定标准可选用户偏好和上传的模板始终优先 "视觉约定标准（可选——用户偏好和上传的模板始终优先）的直接链接")
...
## 第 2 节：运营统计与财务指标[​](#第-2-节运营统计与财务指标 "第 2 节：运营统计与财务指标的直接链接")

### 核心列（从这些开始）[​](#核心列从这些开始 "核心列（从这些开始）的直接链接")
...
### 可选补充（根据行业/目的选择）[​](#可选补充根据行业目的选择 "可选补充（根据行业/目的选择）的直接链接")
...
### 公式示例（以第 7 行为例）[​](#公式示例以第-7-行为例 "公式示例（以第 7 行为例）的直接链接")
...
### 统计块（公司数据之后）[​](#统计块公司数据之后 "统计块（公司数据之后）的直接链接")
...
## 第 3 节：估值倍数与投资指标[​](#第-3-节估值倍数与投资指标 "第 3 节：估值倍数与投资指标的直接链接")

### 核心估值列（从这些开始）[​](#核心估值列从这些开始 "核心估值列（从这些开始）的直接链接")
...
### 可选估值指标（根据情况选择）[​](#可选估值指标根据情况选择 "可选估值指标（根据情况选择）的直接链接")
...
## 第 4 节：注释与方法论文档[​](#第-4-节注释与方法论文档 "第 4 节：注释与方法论文档的直接链接")

### 必要组成部分[​](#必要组成部分 "必要组成部分的直接链接")
...
## 第 5 节：选择正确的指标（决策框架）[​](#第-5-节选择正确的指标决策框架 "第 5 节：选择正确的指标（决策框架）的直接链接")

### 从"我要回答什么问题？"开始[​](#从我要回答什么问题开始 "从\"我要回答什么问题？\"开始的直接链接")
...
### 行业特定指标选择[​](#行业特定指标选择 "行业特定指标选择的直接链接")
...
## 第 6 节：最佳实践与质量检查[​](#第-6-节最佳实践与质量检查 "第 6 节：最佳实践与质量检查的直接链接")
...
## 第 6 节：高级功能[​](#第-6-节高级功能 "第 6 节：高级功能的直接链接")
...
### 四分位数分析的优势[​](#四分位数分析的优势 "四分位数分析的优势的直接链接")
...
## 第 7 节：工作流程与实用技巧[​](#第-7-节工作流程与实用技巧 "第 7 节：工作流程与实用技巧的直接链接")
...
* **版本控制**：保存为"Comps\_v1\_2024-12-15"并清晰标注日期
* **协作审查**：让他人检查你的公式

### Excel 格式检查清单（可选——根据用户偏好调整）[​](#excel-格式检查清单可选根据用户偏好调整 "Excel 格式检查清单（可选——根据用户偏好调整）的直接链接")
...
## 第 8 节：示例模板布局[​](#第-8-节示例模板布局 "第 8 节：示例模板布局的直接链接")
...
## 第 9 节：行业特定补充（可选）[​](#第-9-节行业特定补充可选 "第 9 节：行业特定补充（可选）的直接链接")
...
## 第 10 节：红旗与警示信号[​](#第-10-节红旗与警示信号 "第 10 节：红旗与警示信号的直接链接")

### 数据质量问题[​](#数据质量问题 "数据质量问题的直接链接")
...
## 第 11 节：公式参考指南[​](#第-11-节公式参考指南 "第 11 节：公式参考指南的直接链接")

### 基本 Excel 公式[​](#基本-excel-公式 "基本 Excel 公式的直接链接")
...
## 数据来源——MCP 优先，网络作为备选[​](#数据来源mcp-优先网络作为备选 "数据来源——MCP 优先，网络作为备选的直接链接")
...
* [环境](#环境)
* [⚠️ 关键：数据来源优先级（请先阅读）](#️-关键数据来源优先级请先阅读)
* [概述](#概述)
* [核心理念](#核心理念)
* [⚠️ 关键：公式优先于硬编码 + 逐步验证](#️-关键公式优先于硬编码--逐步验证)
* [第 1 节：文档结构与设置](#第-1-节文档结构与设置)
  + [标题块（第 1-3 行）](#标题块第-1-3-行)
  + [视觉约定标准（可选——用户偏好和上传的模板始终优先）](#视觉约定标准可选用户偏好和上传的模板始终优先)
* [第 2 节：运营统计与财务指标](#第-2-节运营统计与财务指标)
  + [核心列（从这些开始）](#核心列从这些开始)
  + [可选补充（根据行业/目的选择）](#可选补充根据行业目的选择)
  + [公式示例（以第 7 行为例）](#公式示例以第-7-行为例)
  + [统计块（公司数据之后）](#统计块公司数据之后)
* [第 3 节：估值倍数与投资指标](#第-3-节估值倍数与投资指标)
  + [核心估值列（从这些开始）](#核心估值列从这些开始)
  + [可选估值指标（根据情况选择）](#可选估值指标根据情况选择)
  + [公式示例](#公式示例)
  + [交叉引用规则](#交叉引用规则)
  + [统计块](#统计块)
* [第 4 节：注释与方法论文档](#第-4-节注释与方法论文档)
  + [必要组成部分](#必要组成部分)
* [第 5 节：选择正确的指标（决策框架）](#第-5-节选择正确的指标决策框架)
  + [从"我要回答什么问题？"开始](#从我要回答什么问题开始)
  + [行业特定指标选择](#行业特定指标选择)
  + ["5-10 法则"](#5-10-法则)
* [第 6 节：最佳实践与质量检查](#第-6-节最佳实践与质量检查)
  + [开始之前](#开始之前)
  + [构建过程中](#构建过程中)
  + [合理性检查](#合理性检查)
  + [常见错误](#常见错误)
* [第 6 节：高级功能](#第-6-节高级功能)
  + [动态标题](#动态标题)
  + [四分位数分析的优势](#四分位数分析的优势)
  + [行业特定修改](#行业特定修改)
* [第 7 节：工作流程与实用技巧](#第-7-节工作流程与实用技巧)
  + [分步流程](#分步流程)
  + [专业技巧](#专业技巧)
  + [Excel 格式检查清单（可选——根据用户偏好调整）](#excel-格式检查清单可选根据用户偏好调整)
* [第 8 节：示例模板布局](#第-8-节示例模板布局)
* [第 9 节：行业特定补充（可选）](#第-9-节行业特定补充可选)
* [第 10 节：红旗与警示信号](#第-10-节红旗与警示信号)
  + [数据质量问题](#数据质量问题)
  + [估值红旗](#估值红旗)
  + [可比性问题](#可比性问题)
* [第 11 节：公式参考指南](#第-11-节公式参考指南)
  + [基本 Excel 公式](#基本-excel-公式)
  + [常用比率公式](#常用比率公式)
* [关键原则总结](#关键原则总结)
* [输出检查清单](#输出检查清单)
* [持续改进](#持续改进)
* [数据来源——MCP 优先，网络作为备选](#数据来源mcp-优先网络作为备选)
* [归属](#归属)
