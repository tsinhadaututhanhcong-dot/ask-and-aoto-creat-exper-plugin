# Baoyu Infographic — 信息图：21种布局 × 21种风格（信息图, 可视化） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-baoyu-infographic](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-baoyu-infographic)
...
### 第1步：分析内容[​](#第1步分析内容 "第1步：分析内容的直接链接")
...
### 第2步：生成结构化内容 → `structured-content.md`[​](#第2步生成结构化内容--structured-contentmd "第2步生成结构化内容--structured-contentmd的直接链接")
...
### 第3步：推荐组合[​](#第3步推荐组合 "第3步：推荐组合的直接链接")
...
### 第4步：确认选项[​](#第4步确认选项 "第4步：确认选项的直接链接")
...
### 第5步：生成 Prompt → `prompts/infographic.md`[​](#第5步生成-prompt--promptsinfographicmd "第5步生成-prompt--promptsinfographicmd的直接链接")
...
### 第6步：生成图像[​](#第6步生成图像 "第6步：生成图像的直接链接")

使用 `image_generate` 工具，传入第5步组装的 prompt。

* 将宽高比映射到 image\_generate 的格式：`16:9` → `landscape`，`9:16` → `portrait`，`1:1` → `square`
* 自定义比例时，选择最接近的命名宽高比
* 失败时自动重试一次
* 将生成的图像 URL/路径保存至输出目录

### 第7步：输出摘要[​](#第7步输出摘要 "第7步：输出摘要的直接链接")
...
5. **image\_generate 宽高比** — 该工具仅支持 `landscape`、`portrait` 和 `square`。自定义比例如 `3:4` 应映射到最接近的选项（此例为 portrait）。
...
* [工作流程](#工作流程)
  + [第1步：分析内容](#第1步分析内容)
  + [第2步：生成结构化内容 → `structured-content.md`](#第2步生成结构化内容--structured-contentmd)
  + [第3步：推荐组合](#第3步推荐组合)
  + [第4步：确认选项](#第4步确认选项)
  + [第5步：生成 Prompt → `prompts/infographic.md`](#第5步生成-prompt--promptsinfographicmd)
  + [第6步：生成图像](#第6步生成图像)
  + [第7步：输出摘要](#第7步输出摘要)
* [参考文件](#参考文件)
* [注意事项](#注意事项)