# 架构 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/architecture](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/architecture)
...
### Gateway 消息[​](#gateway-消息 "Gateway 消息的直接链接")
...
## 推荐阅读顺序[​](#推荐阅读顺序 "推荐阅读顺序的直接链接")

如果你是第一次接触代码库：

1. **本页** — 整体定位
2. **[Agent 循环内部机制](/docs/zh-Hans/developer-guide/agent-loop)** — AIAgent 的工作原理
3. **[Prompt 组装](/docs/zh-Hans/developer-guide/prompt-assembly)** — 系统 prompt 的构建过程
4. **[Provider 运行时解析](/docs/zh-Hans/developer-guide/provider-runtime)** — provider 的选择方式
5. **[添加 Provider](/docs/zh-Hans/developer-guide/adding-providers)** — 新增 provider 的实践指南
6. **[工具运行时](/docs/zh-Hans/developer-guide/tools-runtime)** — 工具注册表、分发、环境
7. **[会话存储](/docs/zh-Hans/developer-guide/session-storage)** — SQLite schema、FTS5、会话血缘
8. **[Gateway 内部机制](/docs/zh-Hans/developer-guide/gateway-internals)** — 消息平台 gateway
9. **[上下文压缩与 Prompt 缓存](/docs/zh-Hans/developer-guide/context-compression-and-caching)** — 压缩与缓存
10. **[ACP 内部机制](/docs/zh-Hans/developer-guide/acp-internals)** — IDE 集成

## 主要子系统[​](#主要子系统 "主要子系统的直接链接")
...
→ [Agent 循环内部机制](/docs/zh-Hans/developer-guide/agent-loop)

### Prompt 系统[​](#prompt-系统 "Prompt 系统的直接链接")
...
→ [Prompt 组装](/docs/zh-Hans/developer-guide/prompt-assembly)，[上下文压缩与 Prompt 缓存](/docs/zh-Hans/developer-guide/context-compression-and-caching)

### Provider 解析[​](#provider-解析 "Provider 解析的直接链接")
...
→ [工具运行时](/docs/zh-Hans/developer-guide/tools-runtime)

### 会话持久化[​](#会话持久化 "会话持久化的直接链接")

基于 SQLite 的会话存储，带 FTS5 全文检索。会话具有血缘追踪（跨压缩的父/子关系）、按平台隔离，以及带竞争处理的原子写入。

→ [会话存储](/docs/zh-Hans/developer-guide/session-storage)

### 消息 Gateway[​](#消息-gateway "消息 Gateway的直接链接")

长驻进程，包含 20 个平台适配器、统一会话路由、用户授权（白名单 + DM 配对）、斜杠命令分发、hook 系统、cron 触发和后台维护。

→ [Gateway 内部机制](/docs/zh-Hans/developer-guide/gateway-internals)
...
→ [插件指南](/docs/zh-Hans/developer-guide/plugins)，[记忆提供者插件](/docs/zh-Hans/developer-guide/memory-provider-plugin)

### Cron[​](#cron "Cron的直接链接")

一等公民的 agent 任务（非 shell 任务）。任务以 JSON 存储，支持多种调度格式，可附加 skill 和脚本，并可向任意平台投递。

→ [Cron 内部机制](/docs/zh-Hans/developer-guide/cron-internals)

### ACP 集成[​](#acp-集成 "ACP 集成的直接链接")

通过 stdio/JSON-RPC 将 Hermes 作为编辑器原生 agent 暴露给 VS Code、Zed 和 JetBrains。

→ [ACP 内部机制](/docs/zh-Hans/developer-guide/acp-internals)

### 轨迹[​](#轨迹 "轨迹的直接链接")

从 agent 会话生成 ShareGPT 格式的轨迹，用于训练数据生成。

→ [轨迹与训练格式](/docs/zh-Hans/developer-guide/trajectory-format)
...
| **松耦合** | 可选子系统（MCP、插件、记忆提供者、RL 环境）使用注册表模式和 check\_fn 门控，而非硬依赖。 |
| **Profile 隔离** | 每个 profile（`hermes -p <name>`）拥有独立的 HERMES\_HOME、配置、记忆、会话和 gateway PID。多个 profile 可并发运行。 |

## 文件依赖链[​](#文件依赖链 "文件依赖链的直接链接")
...
* [数据流](#数据流)
  + [CLI 会话](#cli-会话)
  + [Gateway 消息](#gateway-消息)
  + [Cron 任务](#cron-任务)
* [推荐阅读顺序](#推荐阅读顺序)
* [主要子系统](#主要子系统)
  + [Agent 循环](#agent-循环)
  + [Prompt 系统](#prompt-系统)
  + [Provider 解析](#provider-解析)
  + [工具系统](#工具系统)
  + [会话持久化](#会话持久化)
  + [消息 Gateway](#消息-gateway)
  + [插件系统](#插件系统)
  + [Cron](#cron)
  + [ACP 集成](#acp-集成)
  + [轨迹](#轨迹)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/agent-loop](./docs-zh-Hans-developer-guide-agent-loop.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/cron-internals](./docs-zh-Hans-developer-guide-cron-internals.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/gateway-internals](./docs-zh-Hans-developer-guide-gateway-internals.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/prompt-assembly](./docs-zh-Hans-developer-guide-prompt-assembly.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/session-storage](./docs-zh-Hans-developer-guide-session-storage.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/tools-runtime](./docs-zh-Hans-developer-guide-tools-runtime.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/trajectory-format](./docs-zh-Hans-developer-guide-trajectory-format.md)
