...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/software-development/software-development-rest-graphql-debug](https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/software-development/software-development-rest-graphql-debug)
...
## Core Principle[​](#core-principle "Direct link to Core Principle")
...
## 5-Minute Quickstart[​](#5-minute-quickstart "Direct link to 5-Minute Quickstart")

### REST via terminal[​](#rest-via-terminal "Direct link to REST via terminal")
...
### GraphQL via terminal[​](#graphql-via-terminal "Direct link to GraphQL via terminal")
...
### Python (requests) via execute\_code[​](#python-requests-via-execute_code "Direct link to Python (requests) via execute_code")
...
## Layered Debug Flow[​](#layered-debug-flow "Direct link to Layered Debug Flow")

### Step 1 — Connectivity[​](#step-1--connectivity "Direct link to Step 1 — Connectivity")
...
### Step 1.5 — Timeouts[​](#step-15--timeouts "Direct link to Step 1.5 — Timeouts")
...
### Step 2 — TLS/SSL[​](#step-2--tlsssl "Direct link to Step 2 — TLS/SSL")
...
### Step 3 — Authentication[​](#step-3--authentication "Direct link to Step 3 — Authentication")
...
### Step 4 — Request Format[​](#step-4--request-format "Direct link to Step 4 — Request Format")
...
### Step 5 — Response Parsing[​](#step-5--response-parsing "Direct link to Step 5 — Response Parsing")
...
### Step 6 — Semantic Validation[​](#step-6--semantic-validation "Direct link to Step 6 — Semantic Validation")
...
## HTTP Status Playbook[​](#http-status-playbook "Direct link to HTTP Status Playbook")

### 401 Unauthorized — credentials missing or invalid[​](#401-unauthorized--credentials-missing-or-invalid "Direct link to 401 Unauthorized — credentials missing or invalid")
...
### 403 Forbidden — authenticated but not authorized[​](#403-forbidden--authenticated-but-not-authorized "Direct link to 403 Forbidden — authenticated but not authorized")
...
### 404 Not Found — resource doesn't exist or URL is wrong[​](#404-not-found--resource-doesnt-exist-or-url-is-wrong "Direct link to 404 Not Found — resource doesn't exist or URL is wrong")
...
### 409 Conflict — state collision[​](#409-conflict--state-collision "Direct link to 409 Conflict — state collision")

1. Resource already exists (duplicate create)?
2. Stale `ETag` / `If-Match`?
3. Concurrent modification by another process?

### 422 Unprocessable Entity — valid JSON, invalid data[​](#422-unprocessable-entity--valid-json-invalid-data "Direct link to 422 Unprocessable Entity — valid JSON, invalid data")
...
### 429 Too Many Requests — rate limited[​](#429-too-many-requests--rate-limited "Direct link to 429 Too Many Requests — rate limited")
...
### 5xx — server-side, usually not your fault[​](#5xx--server-side-usually-not-your-fault "Direct link to 5xx — server-side, usually not your fault")
...
## Pagination & Idempotency[​](#pagination--idempotency "Direct link to Pagination & Idempotency")
...
## Correlation IDs[​](#correlation-ids "Direct link to Correlation IDs")
...
## Regression Test Template[​](#regression-test-template "Direct link to Regression Test Template")
...
### Token handling[​](#token-handling "Direct link to Token handling")
...
### Safe logging[​](#safe-logging "Direct link to Safe logging")
...
### Leak checklist[​](#leak-checklist "Direct link to Leak checklist")
...
## Hermes Tool Patterns[​](#hermes-tool-patterns "Direct link to Hermes Tool Patterns")

### terminal — for curl, dig, openssl[​](#terminal--for-curl-dig-openssl "Direct link to terminal — for curl, dig, openssl")
...
### execute\_code — for multi-step Python flows[​](#execute_code--for-multi-step-python-flows "Direct link to execute_code — for multi-step Python flows")
...
### web\_extract — for vendor API docs[​](#web_extract--for-vendor-api-docs "Direct link to web_extract — for vendor API docs")
...
### delegate\_task — for full CRUD test sweeps[​](#delegate_task--for-full-crud-test-sweeps "Direct link to delegate_task — for full CRUD test sweeps")
...
* [5-Minute Quickstart](#5-minute-quickstart)
  + [REST via terminal](#rest-via-terminal)
  + [GraphQL via terminal](#graphql-via-terminal)
  + [Python (requests) via execute\_code](#python-requests-via-execute_code)
* [Layered Debug Flow](#layered-debug-flow)
  + [Step 1 — Connectivity](#step-1--connectivity)
  + [Step 1.5 — Timeouts](#step-15--timeouts)
  + [Step 2 — TLS/SSL](#step-2--tlsssl)
  + [Step 3 — Authentication](#step-3--authentication)
  + [Step 4 — Request Format](#step-4--request-format)
  + [Step 5 — Response Parsing](#step-5--response-parsing)
  + [Step 6 — Semantic Validation](#step-6--semantic-validation)
* [HTTP Status Playbook](#http-status-playbook)
  + [401 Unauthorized — credentials missing or invalid](#401-unauthorized--credentials-missing-or-invalid)
  + [403 Forbidden — authenticated but not authorized](#403-forbidden--authenticated-but-not-authorized)
  + [404 Not Found — resource doesn't exist or URL is wrong](#404-not-found--resource-doesnt-exist-or-url-is-wrong)
  + [409 Conflict — state collision](#409-conflict--state-collision)
  + [422 Unprocessable Entity — valid JSON, invalid data](#422-unprocessable-entity--valid-json-invalid-data)
  + [429 Too Many Requests — rate limited](#429-too-many-requests--rate-limited)
  + [5xx — server-side, usually not your fault](#5xx--server-side-usually-not-your-fault)
* [Pagination & Idempotency](#pagination--idempotency)
* [Contract Validation](#contract-validation)
* [Correlation IDs](#correlation-ids)
* [Regression Test Template](#regression-test-template)
* [Security](#security)
  + [Token handling](#token-handling)
  + [Safe logging](#safe-logging)
  + [Leak checklist](#leak-checklist)
* [Hermes Tool Patterns](#hermes-tool-patterns)
  + [terminal — for curl, dig, openssl](#terminal--for-curl-dig-openssl)
  + [execute\_code — for multi-step Python flows](#execute_code--for-multi-step-python-flows)
  + [web\_extract — for vendor API docs](#web_extract--for-vendor-api-docs)
  + [delegate\_task — for full CRUD test sweeps](#delegate_task--for-full-crud-test-sweeps)
