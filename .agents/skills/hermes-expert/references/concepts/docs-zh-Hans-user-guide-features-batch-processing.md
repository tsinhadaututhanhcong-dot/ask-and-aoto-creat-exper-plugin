# 批量处理 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/batch-processing](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/batch-processing)
...
批量运行会启动大量并发 agent 会话，每个会话都会调用模型和工具。[Nous Portal](/docs/zh-Hans/user-guide/features/tool-gateway) 订阅将模型访问、网页搜索、图像生成、TTS 以及云端浏览器统一计费——当你希望在不同供应商账户间稳定控制每条轨迹成本、避免触碰速率限制时非常实用。使用 `hermes setup --portal` 完成配置，然后将 `--model` 指向 Nous 模型。

## 数据集格式[​](#数据集格式 "数据集格式的直接链接")
...
### 供应商路由（OpenRouter）[​](#供应商路由openrouter "供应商路由（OpenRouter）的直接链接")
...
### 按 Prompt 指定容器镜像[​](#按-prompt-指定容器镜像 "按 Prompt 指定容器镜像的直接链接")
...
* [数据集格式](#数据集格式)
* [配置选项](#配置选项)
  + [供应商路由（OpenRouter）](#供应商路由openrouter)
  + [推理控制](#推理控制)
  + [高级选项](#高级选项)
* [工具集分布](#工具集分布)
* [输出格式](#输出格式)
...
* [使用场景](#使用场景)
  + [训练数据生成](#训练数据生成)
  + [模型评估](#模型评估)
  + [按 Prompt 指定容器镜像](#按-prompt-指定容器镜像)
