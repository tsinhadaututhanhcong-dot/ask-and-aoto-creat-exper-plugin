# Cron 内部机制 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/cron-internals](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/cron-internals)
...
### 任务生命周期状态[​](#任务生命周期状态 "任务生命周期状态的直接链接")
...
### 向后兼容性[​](#向后兼容性 "向后兼容性的直接链接")

旧版任务可能使用单个 `skill` 字段而非 `skills` 数组。调度器在加载时会对此进行规范化——单个 `skill` 会被提升为 `skills: [skill]`。

## 调度器运行时[​](#调度器运行时 "调度器运行时的直接链接")

### Tick 周期[​](#tick-周期 "Tick 周期的直接链接")
...
### Gateway 集成[​](#gateway-集成 "Gateway 集成的直接链接")
...
* [Cron 功能指南](/docs/zh-Hans/user-guide/features/cron)
