# 使用 Webhook 自动发布 GitHub PR 评论 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/webhook-github-pr-review](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/webhook-github-pr-review)
...
如果你没有公网 URL，或只是想快速上手，请查看 [构建 GitHub PR 审查 Agent](/docs/zh-Hans/guides/github-pr-review-agent) —— 使用 cron 作业按计划轮询 PR，可在 NAT 和防火墙后运行。

参考文档

完整的 webhook 平台参考（所有配置选项、投递类型、动态订阅、安全模型），请参阅 [Webhooks](/docs/zh-Hans/user-guide/messaging/webhooks)。
...
## 第一步——启用 webhook 平台[​](#第一步启用-webhook-平台 "第一步——启用 webhook 平台的直接链接")
...
## 第二步——启动 gateway[​](#第二步启动-gateway "第二步——启动 gateway的直接链接")
...
## 第三步——在 GitHub 上注册 webhook[​](#第三步在-github-上注册-webhook "第三步——在 GitHub 上注册 webhook的直接链接")
...
## 第四步——打开一个测试 PR[​](#第四步打开一个测试-pr "第四步——打开一个测试 PR的直接链接")
...
## 使用 ngrok 进行本地测试[​](#使用-ngrok-进行本地测试 "使用 ngrok 进行本地测试的直接链接")
...
## 过滤特定 action[​](#过滤特定-action "过滤特定 action的直接链接")
...
## 使用 skill 保持一致的审查风格[​](#使用-skill-保持一致的审查风格 "使用 skill 保持一致的审查风格的直接链接")

加载一个 [Hermes skill](/docs/zh-Hans/user-guide/features/skills) 以赋予 agent 一致的审查风格。在 `config.yaml` 的 `platforms.webhook.extra.routes` 中，向你的路由添加 `skills`：
...
## 将响应发送到 Slack 或 Discord[​](#将响应发送到-slack-或-discord "将响应发送到 Slack 或 Discord的直接链接")
...
## GitLab 支持[​](#gitlab-支持 "GitLab 支持的直接链接")
...
* **[基于 Cron 的 PR 审查](/docs/zh-Hans/guides/github-pr-review-agent)** —— 按计划轮询 PR，无需公网端点
* **[Webhook 参考](/docs/zh-Hans/user-guide/messaging/webhooks)** —— webhook 平台的完整配置参考
* **[构建 Plugin](/docs/zh-Hans/developer-guide/plugins)** —— 将审查逻辑打包为可共享的 plugin
* **[Profiles](/docs/zh-Hans/user-guide/profiles)** —— 运行一个拥有独立内存和配置的专属审查者 profile

* [前提条件](#前提条件)
* [第一步——启用 webhook 平台](#第一步启用-webhook-平台)
* [第二步——启动 gateway](#第二步启动-gateway)
* [第三步——在 GitHub 上注册 webhook](#第三步在-github-上注册-webhook)
* [第四步——打开一个测试 PR](#第四步打开一个测试-pr)
* [使用 ngrok 进行本地测试](#使用-ngrok-进行本地测试)
* [过滤特定 action](#过滤特定-action)
* [使用 skill 保持一致的审查风格](#使用-skill-保持一致的审查风格)
* [将响应发送到 Slack 或 Discord](#将响应发送到-slack-或-discord)
* [GitLab 支持](#gitlab-支持)
* [安全说明](#安全说明)
* [故障排查](#故障排查)
* [完整配置参考](#完整配置参考)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/github-pr-review-agent](./docs-zh-Hans-guides-github-pr-review-agent.md)
