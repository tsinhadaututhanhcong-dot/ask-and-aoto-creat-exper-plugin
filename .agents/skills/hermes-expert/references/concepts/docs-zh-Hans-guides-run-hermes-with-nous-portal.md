# 通过 Nous Portal 运行 Hermes Agent | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/run-hermes-with-nous-portal](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/run-hermes-with-nous-portal)

本页总览

本指南带你从头到尾完成在 [Nous Portal](https://portal.nousresearch.com) 订阅下运行 Hermes Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [Nous Portal 集成页面](/docs/zh-Hans/integrations/nous-portal)。本页是操作步骤脚本。

## 前提条件[​](#前提条件 "前提条件的直接链接")

* 已安装 Hermes Agent（[快速入门](/docs/zh-Hans/getting-started/quickstart)）
* 在你正在配置的机器上有可用的浏览器（或 SSH 端口转发——参见 [OAuth over SSH](/docs/zh-Hans/guides/oauth-over-ssh)）
* 约 5 分钟时间

你**不需要**：OpenAI 密钥、Anthropic 密钥、Firecrawl 账号、FAL 账号、Browser Use 账号，或任何其他按供应商分配的凭证。这正是 Portal 的意义所在。

## 1. 获取订阅[​](#1-获取订阅 "1. 获取订阅的直接链接")

打开 [portal.nousresearch.com/manage-subscription](https://portal.nousresearch.com/manage-subscription)，注册并选择一个套餐。

已订阅？跳至第 2 步。

## 2. 运行一键配置[​](#2-运行一键配置 "2. 运行一键配置的直接链接")
...
### 如果我通过 SSH 连接到服务器怎么办？[​](#如果我通过-ssh-连接到服务器怎么办 "如果我通过 SSH 连接到服务器怎么办？的直接链接")
...
完整操作说明（包括 ProxyJump 链、mosh/tmux 和 ControlMaster 注意事项）请参阅 [OAuth over SSH / 远程主机](/docs/zh-Hans/guides/oauth-over-ssh)。

## 3. 验证配置是否成功[​](#3-验证配置是否成功 "3. 验证配置是否成功的直接链接")
...
## 4. 运行第一次对话[​](#4-运行第一次对话 "4. 运行第一次对话的直接链接")
...
## 5. 选择你实际需要的模型[​](#5-选择你实际需要的模型 "5. 选择你实际需要的模型的直接链接")
...
### 不要在 agent 任务中使用 Hermes-4[​](#不要在-agent-任务中使用-hermes-4 "不要在 agent 任务中使用 Hermes-4的直接链接")

Hermes-4-70B 和 Hermes-4-405B 在 Portal 上以大幅折扣提供，但它们是**对话/推理模型**，并非针对工具调用优化的模型。它们在多步骤 agent 循环中表现不佳。请通过 [Nous Chat](https://chat.nousresearch.com) 将它们用于对话/研究工作，或通过[订阅代理](/docs/zh-Hans/user-guide/features/subscription-proxy)从非 agent 工具中使用。对于 Hermes Agent 本身，请坚持使用上述前沿 agentic 模型。

Portal 的[信息页面](https://portal.nousresearch.com/info)也有此说明——这是 Nous 官方指导，并非仅代表 Hermes 一方的意见。

## 6. （可选）自定义 Tool Gateway 路由[​](#6-可选自定义-tool-gateway-路由 "6. （可选）自定义 Tool Gateway 路由的直接链接")
...
## 7. （可选）启用语音模式[​](#7-可选启用语音模式 "7. （可选）启用语音模式的直接链接")

由于 Tool Gateway 包含 OpenAI TTS，无需单独的 OpenAI 密钥即可使用[语音模式](/docs/zh-Hans/user-guide/features/voice-mode)：
...
## 8. （可选）Cron 定时任务与常驻工作流[​](#8-可选cron-定时任务与常驻工作流 "8. （可选）Cron 定时任务与常驻工作流的直接链接")

Portal 订阅对 [cron 定时任务](/docs/zh-Hans/user-guide/features/cron)和[批处理](/docs/zh-Hans/user-guide/features/batch-processing)的支持方式与交互式对话相同——OAuth refresh token 会自动复用。无需额外配置，直接安排 cron 任务，费用将计入你的订阅。
...
## Profiles 与多用户配置[​](#profiles-与多用户配置 "Profiles 与多用户配置的直接链接")

如果你使用 [Hermes profiles](/docs/zh-Hans/user-guide/profiles)（例如每个项目单独一套配置），Portal refresh token 会通过共享 token 存储自动在所有 profiles 之间共享。在任意 profile 上登录一次，其余 profiles 会自动获取。

对于多人共用一台机器的团队场景，每个人有自己的 Portal 账号 → 每个 home 目录保存各自的 `~/.hermes/auth.json` → 用户之间不共享 token。这是正确的边界划分。

## 故障排查[​](#故障排查 "故障排查的直接链接")

### 运行 `hermes setup --portal` 后，`hermes portal info` 显示"not logged in"[​](#运行-hermes-setup---portal-后hermes-portal-info-显示not-logged-in "运行-hermes-setup---portal-后hermes-portal-info-显示not-logged-in的直接链接")
...
如果浏览器未打开或回调失败，你可能在远程/无头主机上——参见 [OAuth over SSH](/docs/zh-Hans/guides/oauth-over-ssh) 了解端口转发的解决方案。

### "Model: currently openrouter"（或其他 provider）而非"using Nous as inference provider"[​](#model-currently-openrouter或其他-provider而非using-nous-as-inference-provider "\"Model: currently openrouter\"（或其他 provider）而非\"using Nous as inference provider\"的直接链接")
...
### Tool Gateway 工具显示合作方名称而非"via Nous Portal"[​](#tool-gateway-工具显示合作方名称而非via-nous-portal "Tool Gateway 工具显示合作方名称而非\"via Nous Portal\"的直接链接")
...
### 会话中途出现"Re-authentication required"[​](#会话中途出现re-authentication-required "会话中途出现\"Re-authentication required\"的直接链接")
...
### 我想要的模型不在 `/model` 选择器中[​](#我想要的模型不在-model-选择器中 "我想要的模型不在-model-选择器中的直接链接")
...
### 账单未出现在我的 Portal 账号中[​](#账单未出现在我的-portal-账号中 "账单未出现在我的 Portal 账号中的直接链接")
...
### 想要撤销并重新开始[​](#想要撤销并重新开始 "想要撤销并重新开始的直接链接")
...
## 用具体数字说明 Portal 的价值[​](#用具体数字说明-portal-的价值 "用具体数字说明 Portal 的价值的直接链接")
...
* **[Nous Portal 集成页面](/docs/zh-Hans/integrations/nous-portal)** — 订阅内容概述
* **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** — 每个 gateway 路由工具的完整说明
* **[订阅代理](/docs/zh-Hans/user-guide/features/subscription-proxy)** — 在非 Hermes 工具中使用你的 Portal 订阅
* **[语音模式](/docs/zh-Hans/user-guide/features/voice-mode)** — 在 Portal 订阅上配置语音对话
* **[OAuth over SSH](/docs/zh-Hans/guides/oauth-over-ssh)** — 远程/无头主机登录方案
* **[Profiles](/docs/zh-Hans/user-guide/profiles)** — 在多个 Hermes 配置之间共享一个 Portal 登录

* [前提条件](#前提条件)
* [1. 获取订阅](#1-获取订阅)
* [2. 运行一键配置](#2-运行一键配置)
  + [如果我通过 SSH 连接到服务器怎么办？](#如果我通过-ssh-连接到服务器怎么办)
* [3. 验证配置是否成功](#3-验证配置是否成功)
* [4. 运行第一次对话](#4-运行第一次对话)
* [5. 选择你实际需要的模型](#5-选择你实际需要的模型)
  + [不要在 agent 任务中使用 Hermes-4](#不要在-agent-任务中使用-hermes-4)
* [6. （可选）自定义 Tool Gateway 路由](#6-可选自定义-tool-gateway-路由)
* [7. （可选）启用语音模式](#7-可选启用语音模式)
* [8. （可选）Cron 定时任务与常驻工作流](#8-可选cron-定时任务与常驻工作流)
* [Profiles 与多用户配置](#profiles-与多用户配置)
* [故障排查](#故障排查)
  + [运行 `hermes setup --portal` 后，`hermes portal info` 显示"not logged in"](#运行-hermes-setup---portal-后hermes-portal-info-显示not-logged-in)
  + ["Model: currently openrouter"（或其他 provider）而非"using Nous as inference provider"](#model-currently-openrouter或其他-provider而非using-nous-as-inference-provider)
  + [Tool Gateway 工具显示合作方名称而非"via Nous Portal"](#tool-gateway-工具显示合作方名称而非via-nous-portal)
  + [会话中途出现"Re-authentication required"](#会话中途出现re-authentication-required)
  + [我想要的模型不在 `/model` 选择器中](#我想要的模型不在-model-选择器中)
  + [账单未出现在我的 Portal 账号中](#账单未出现在我的-portal-账号中)
  + [想要撤销并重新开始](#想要撤销并重新开始)
* [用具体数字说明 Portal 的价值](#用具体数字说明-portal-的价值)
* [另请参阅](#另请参阅)
