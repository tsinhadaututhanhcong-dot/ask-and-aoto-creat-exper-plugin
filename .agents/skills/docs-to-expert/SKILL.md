...
allowed-tools: run_command, view_file, write_to_file, read_url_content, find_by_name, grep_search
effort: high
---
...
Khác Claude Code, Antigravity 2.0 không có sẵn một thư mục "personal skills" toàn cục đã xác nhận — thư mục skill đích nằm ngay trong workspace hiện tại, dưới `.agents/skills/`:
```bash
skillDir=".agents/skills/<skill_name>"
mkdir -p "$skillDir/references"
```
(Dùng `$skillDir` làm ký hiệu tham chiếu xuyên suốt phần còn lại của skill này — đường dẫn thật là `.agents/skills/<skill_name>`.)
...
```bash
python ".agents/skills/docs-to-expert/scripts/fetch_llms_docs.py" --root "<root_url>" --output "$skillDir\references"
```
...
```bash
python ".agents/skills/docs-to-expert/scripts/extract_links.py" --url "<root_url>" --output "$skillDir\references\links.txt" --export-dir "$skillDir\references\concepts"
```
...
```bash
python ".agents/skills/docs-to-expert/scripts/fetch_js_rendered.py" --llms-txt "<url_llms.txt_da_tim_thay_o_Nhanh_A>" --output "$skillDir\references"
```
...
**Nhánh D (chỉ khi cả A, B, C đều thất bại - trang chống bot, cấu trúc quá đặc thù):** tự viết một script cào tùy biến bằng `write_to_file`, lưu vào một file tạm, chạy thử bằng `run_command`, sửa lỗi và lặp lại đến khi ra được `INDEX.md` + `concepts/` hoàn chỉnh theo đúng quy tắc "một trang gốc, một file". Không có sub-agent chuyên biệt nào để giao việc này - tự làm trực tiếp bằng `write_to_file` + `run_command`.
...
```bash
python ".agents/skills/docs-to-expert/scripts/tag_by_surface.py" --detect --index "$skillDir\references\INDEX.md"
```
...
```bash
python ".agents/skills/docs-to-expert/scripts/tag_by_surface.py" --apply --index "$skillDir\references\INDEX.md" --mapping "$skillDir\references\surface_mapping.json" --comparison-files "trang1.md,trang2.md"
```
...
Dùng `write_to_file` để tạo `$skillDir\SKILL.md`. Chọn đúng 1 trong 2 template dưới đây tùy Bước 5 có chạy hay không.
...
```bash
python ".agents/skills/docs-to-expert/scripts/fetch_llms_docs.py" --root "<root_url>" --output "$skillDir\references" --update
```
...
**Quy tắc tin cậy (BẮT BUỘC tuân thủ):** khác với trang docs (có thể tự tìm qua `search_web` + xác minh domain chính thức ở Bước 4 template phía trên), **repo GitHub KHÔNG BAO GIỜ được tự ý tìm/chọn** — chỉ ingest một repo khi người dùng chỉ định đích danh URL repo đó. Lý do: không có cách xác minh tự động "repo này đáng tin cậy" như cách xác minh domain chính thức; quyết định tin tưởng một repo cộng đồng cụ thể là quyết định của con người, không phải của AI tự suy đoán.

**Cách chạy:**

```bash
python ".agents/skills/docs-to-expert/scripts/fetch_repo_docs.py" --repo "<github_repo_url>" --output "$skillDir\references"
```
...
```bash
python ".agents/skills/docs-to-expert/scripts/fetch_repo_docs.py" --repo "<url>" --output "$skillDir\references" --max-files 120 --include-glob ".agents/agent/*.md" --include-glob ".agents/workflows/*.md" --include-glob ".agents/rules/*.md" --include-glob ".agents/skills/*/SKILL.md" --include-glob "AGENT_FLOW.md" --include-glob "CHANGELOG.md"
```
