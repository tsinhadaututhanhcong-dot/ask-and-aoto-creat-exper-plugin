...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/web-development/web-development-page-agent](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/web-development/web-development-page-agent)
...
alibaba/page-agent（[https://github.com/alibaba/page-agent，17k+](https://github.com/alibaba/page-agent%EF%BC%8C17k+) stars，MIT）是一个用 TypeScript 编写的页内 GUI agent。它运行在网页内部，以文本形式读取 DOM（无需截图，无需多模态 LLM），并对当前页面执行自然语言指令，如"点击登录按钮，然后将用户名填为 John"。纯客户端——宿主网站只需引入一个 script 并传入兼容 OpenAI 的 LLM 端点即可。
...
## 何时不应使用此 skill[​](#何时不应使用此-skill "何时不应使用此 skill的直接链接")
...
## 路径 1——通过 CDN 30 秒快速体验（无需安装）[​](#路径-1通过-cdn-30-秒快速体验无需安装 "路径 1——通过 CDN 30 秒快速体验（无需安装）的直接链接")
...
## 路径 2——npm 安装到你自己的 Web 应用（生产使用）[​](#路径-2npm-安装到你自己的-web-应用生产使用 "路径 2——npm 安装到你自己的 Web 应用（生产使用）的直接链接")
...
## 路径 3——克隆源码仓库（贡献代码，或深度定制）[​](#路径-3克隆源码仓库贡献代码或深度定制 "路径 3——克隆源码仓库（贡献代码，或深度定制）的直接链接")
...
## 仓库结构（路径 3）[​](#仓库结构路径-3 "仓库结构（路径 3）的直接链接")
...
* [何时不应使用此 skill](#何时不应使用此-skill)
* [前置条件](#前置条件)
* [路径 1——通过 CDN 30 秒快速体验（无需安装）](#路径-1通过-cdn-30-秒快速体验无需安装)
* [路径 2——npm 安装到你自己的 Web 应用（生产使用）](#路径-2npm-安装到你自己的-web-应用生产使用)
* [路径 3——克隆源码仓库（贡献代码，或深度定制）](#路径-3克隆源码仓库贡献代码或深度定制)
* [仓库结构（路径 3）](#仓库结构路径-3)
* [验证是否正常工作](#验证是否正常工作)
* [常见问题](#常见问题)
* [参考资料](#参考资料)