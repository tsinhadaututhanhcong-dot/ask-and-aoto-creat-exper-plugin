...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/discord](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/discord)
...
## Hermes 的行为方式[​](#hermes-的行为方式 "Hermes 的行为方式的直接链接")
...
### Discord Gateway（网关）模型[​](#discord-gateway网关模型 "Discord Gateway（网关）模型的直接链接")
...
### Discord 中的会话模型[​](#discord-中的会话模型 "Discord 中的会话模型的直接链接")
...
## 第一步：创建 Discord 应用[​](#第一步创建-discord-应用 "第一步：创建 Discord 应用的直接链接")
...
## 第二步：创建机器人[​](#第二步创建机器人 "第二步：创建机器人的直接链接")
...
## 第三步：启用特权网关 Intent（意图）[​](#第三步启用特权网关-intent意图 "第三步：启用特权网关 Intent（意图）的直接链接")
...
## 第四步：获取机器人 Token[​](#第四步获取机器人-token "第四步：获取机器人 Token的直接链接")
...
## 第五步：生成邀请 URL[​](#第五步生成邀请-url "第五步：生成邀请 URL的直接链接")

你需要一个 OAuth2 URL 来将机器人邀请到你的服务器。有两种方式：

### 方式 A：使用 Installation 标签页（推荐）[​](#方式-a使用-installation-标签页推荐 "方式 A：使用 Installation 标签页（推荐）的直接链接")
...
### 方式 B：手动 URL[​](#方式-b手动-url "方式 B：手动 URL的直接链接")
...
## 第六步：邀请到你的服务器[​](#第六步邀请到你的服务器 "第六步：邀请到你的服务器的直接链接")
...
## 第七步：找到你的 Discord 用户 ID[​](#第七步找到你的-discord-用户-id "第七步：找到你的 Discord 用户 ID的直接链接")
...
## 第八步：配置 Hermes Agent[​](#第八步配置-hermes-agent "第八步：配置 Hermes Agent的直接链接")
...
### 配置文件（`config.yaml`）[​](#配置文件configyaml "配置文件configyaml的直接链接")
...
#### `discord.require_mention`[​](#discordrequire_mention "discordrequire_mention的直接链接")

**类型：** 布尔值 — **默认值：** `true`

启用后，机器人仅在服务器频道中被直接 `@提及` 时响应。无论此设置如何，私信始终会得到响应。

#### `discord.thread_require_mention`[​](#discordthread_require_mention "discordthread_require_mention的直接链接")
...
#### `discord.free_response_channels`[​](#discordfree_response_channels "discordfree_response_channels的直接链接")
...
#### `discord.auto_thread`[​](#discordauto_thread "discordauto_thread的直接链接")
...
#### `discord.reactions`[​](#discordreactions "discordreactions的直接链接")
...
#### `discord.ignored_channels`[​](#discordignored_channels "discordignored_channels的直接链接")
...
#### `discord.no_thread_channels`[​](#discordno_thread_channels "discordno_thread_channels的直接链接")
...
#### `discord.channel_prompts`[​](#discordchannel_prompts "discordchannel_prompts的直接链接")
...
#### `discord.history_backfill`[​](#discordhistory_backfill "discordhistory_backfill的直接链接")
...
#### `discord.history_backfill_limit`[​](#discordhistory_backfill_limit "discordhistory_backfill_limit的直接链接")
...
#### `group_sessions_per_user`[​](#group_sessions_per_user "group_sessions_per_user的直接链接")
...
#### `display.tool_progress`[​](#displaytool_progress "displaytool_progress的直接链接")
...
#### `display.tool_progress_command`[​](#displaytool_progress_command "displaytool_progress_command的直接链接")
...
## 技能的原生斜杠命令[​](#技能的原生斜杠命令 "技能的原生斜杠命令的直接链接")
...
### 禁用斜杠命令注册[​](#禁用斜杠命令注册 "禁用斜杠命令注册的直接链接")
...
## 发送媒体（`send_message` + `MEDIA:` 标签）[​](#发送媒体send_message--media-标签 "发送媒体send_message--media-标签的直接链接")
...
## 接收任意文件类型[​](#接收任意文件类型 "接收任意文件类型的直接链接")
...
## 交互式提示（clarify）[​](#交互式提示clarify "交互式提示（clarify）的直接链接")
...
* [语音模式](/docs/zh-Hans/user-guide/features/voice-mode)
* [与 Hermes 使用语音模式](/docs/zh-Hans/guides/use-voice-mode-with-hermes)
...
### 基于角色的访问控制[​](#基于角色的访问控制 "基于角色的访问控制的直接链接")
...
有关保护 Hermes Agent 部署的更多信息，请参阅[安全指南](/docs/zh-Hans/user-guide/security)。

* [Hermes 的行为方式](#hermes-的行为方式)
  + [Discord Gateway（网关）模型](#discord-gateway网关模型)
  + [Discord 中的会话模型](#discord-中的会话模型)
  + [中断与并发](#中断与并发)
* [第一步：创建 Discord 应用](#第一步创建-discord-应用)
* [第二步：创建机器人](#第二步创建机器人)
* [第三步：启用特权网关 Intent（意图）](#第三步启用特权网关-intent意图)
* [第四步：获取机器人 Token](#第四步获取机器人-token)
* [第五步：生成邀请 URL](#第五步生成邀请-url)
  + [方式 A：使用 Installation 标签页（推荐）](#方式-a使用-installation-标签页推荐)
  + [方式 B：手动 URL](#方式-b手动-url)
  + [所需权限](#所需权限)
  + [推荐的附加权限](#推荐的附加权限)
  + [权限整数](#权限整数)
* [第六步：邀请到你的服务器](#第六步邀请到你的服务器)
* [第七步：找到你的 Discord 用户 ID](#第七步找到你的-discord-用户-id)
* [第八步：配置 Hermes Agent](#第八步配置-hermes-agent)
...
* [技能的原生斜杠命令](#技能的原生斜杠命令)
  + [禁用斜杠命令注册](#禁用斜杠命令注册)
* [发送媒体（`send_message` + `MEDIA:` 标签）](#发送媒体send_message--media-标签)
* [接收任意文件类型](#接收任意文件类型)
* [交互式提示（clarify）](#交互式提示clarify)
* [主频道](#主频道)
  + [使用斜杠命令](#使用斜杠命令)
  + [手动配置](#手动配置)
* [语音消息](#语音消息)
...
* [故障排除](#故障排除)
  ...
  + [启动时出现"Disallowed Intents"错误](#启动时出现disallowed-intents错误)
  + [机器人看不到特定频道中的消息](#机器人看不到特定频道中的消息)
  + [403 Forbidden 错误](#403-forbidden-错误)
  + [机器人离线](#机器人离线)
  + ["User not allowed" / 机器人忽略你](#user-not-allowed--机器人忽略你)
  + [同一频道中的用户意外共享上下文](#同一频道中的用户意外共享上下文)
* [安全](#安全)
  + [基于角色的访问控制](#基于角色的访问控制)
  + [提及控制](#提及控制)
