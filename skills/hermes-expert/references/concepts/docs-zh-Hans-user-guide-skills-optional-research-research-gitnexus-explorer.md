...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-gitnexus-explorer](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-gitnexus-explorer)
...
| 相关 skill | [`native-mcp`](/docs/zh-Hans/user-guide/skills/bundled/mcp/mcp-native-mcp), [`codebase-inspection`](/docs/zh-Hans/user-guide/skills/bundled/github/github-codebase-inspection) |
...
### 1. 克隆并构建 GitNexus（一次性设置）[​](#1-克隆并构建-gitnexus一次性设置 "1. 克隆并构建 GitNexus（一次性设置）的直接链接")
...
### 2. 为远程访问修补 Web UI[​](#2-为远程访问修补-web-ui "2. 为远程访问修补 Web UI的直接链接")
...
### 3. 为目标仓库建立索引[​](#3-为目标仓库建立索引 "3. 为目标仓库建立索引的直接链接")
...
### 4. 创建代理脚本[​](#4-创建代理脚本 "4. 创建代理脚本的直接链接")
...
### 6. 通过 Cloudflare 建立隧道（可选——用于远程访问）[​](#6-通过-cloudflare-建立隧道可选用于远程访问 "6. 通过 Cloudflare 建立隧道（可选——用于远程访问）的直接链接")
...
* [规模警告](#规模警告)
* [步骤](#步骤)
  + [1. 克隆并构建 GitNexus（一次性设置）](#1-克隆并构建-gitnexus一次性设置)
  + [2. 为远程访问修补 Web UI](#2-为远程访问修补-web-ui)
  + [3. 为目标仓库建立索引](#3-为目标仓库建立索引)
  + [4. 创建代理脚本](#4-创建代理脚本)
  + [5. 启动服务](#5-启动服务)
  + [6. 通过 Cloudflare 建立隧道（可选——用于远程访问）](#6-通过-cloudflare-建立隧道可选用于远程访问)
  + [7. 清理](#7-清理)
* [注意事项](#注意事项)
