# 持久目标 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/goals](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/goals)
...
## 目标进行中追加条件：`/subgoal`[​](#目标进行中追加条件subgoal "目标进行中追加条件subgoal的直接链接")
...
### 用户消息始终优先[​](#用户消息始终优先 "用户消息始终优先的直接链接")
...
### 运行中安全性（gateway）[​](#运行中安全性gateway "运行中安全性（gateway）的直接链接")
...
### 选择裁判模型[​](#选择裁判模型 "选择裁判模型的直接链接")

裁判使用 `goal_judge` 辅助任务。默认情况下，它解析为你的主模型（参见[辅助模型](/docs/zh-Hans/user-guide/configuration#auxiliary-models)）。若想将裁判路由到廉价快速的模型以降低成本，可添加覆盖配置：
...
* [快速开始](#快速开始)
* [命令](#命令)
* [目标进行中追加条件：`/subgoal`](#目标进行中追加条件subgoal)
* [行为细节](#行为细节)
  + [裁判](#裁判)
  + [失败开放语义](#失败开放语义)
  + [轮次预算](#轮次预算)
  + [用户消息始终优先](#用户消息始终优先)
  + [运行中安全性（gateway）](#运行中安全性gateway)
  + [持久化](#持久化)
  + [Prompt 缓存](#prompt-缓存)
* [配置](#配置)
  + [选择裁判模型](#选择裁判模型)
