...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard)
...
## Managing multiple profiles[​](#managing-multiple-profiles "Direct link to Managing multiple profiles")

The dashboard is a **machine-level** management surface: one server manages
every [profile](/docs/user-guide/profiles) on the machine. A profile switcher in the
...
### Connecting Hermes Desktop to a remote backend[​](#connecting-hermes-desktop-to-a-remote-backend "Direct link to Connecting Hermes Desktop to a remote backend")
...
#### Remote dashboard setup[​](#remote-dashboard-setup "Direct link to Remote dashboard setup")
...
Manage [MCP](/docs/integrations/mcp) servers without the CLI. The same `mcp_servers`
...
Manage dynamic [webhook subscriptions](/docs/user-guide/messaging/webhooks). The
...
### Pairing[​](#pairing "Direct link to Pairing")
...
### Admin endpoints[​](#admin-endpoints "Direct link to Admin endpoints")
...
## Authentication (gated mode)[​](#authentication-gated-mode "Direct link to Authentication (gated mode)")
...
### When the gate engages[​](#when-the-gate-engages "Direct link to When the gate engages")
...
### Fail-closed semantics[​](#fail-closed-semantics "Direct link to Fail-closed semantics")
...
### Default provider: Nous Research[​](#default-provider-nous-research "Direct link to Default provider: Nous Research")
...
#### Registering a dashboard[​](#registering-a-dashboard "Direct link to Registering a dashboard")
...
#### Worked example: Nous Research[​](#worked-example-nous-research "Direct link to Worked example: Nous Research")
...
### Username/password provider (no OAuth IDP)[​](#usernamepassword-provider-no-oauth-idp "Direct link to Username/password provider (no OAuth IDP)")
...
#### Worked example: username/password[​](#worked-example-usernamepassword "Direct link to Worked example: username/password")
...
#### Writing your own password provider[​](#writing-your-own-password-provider "Direct link to Writing your own password provider")
...
### Self-hosted OIDC provider[​](#self-hosted-oidc-provider "Direct link to Self-hosted OIDC provider")
...
#### What it verifies[​](#what-it-verifies "Direct link to What it verifies")
...
#### Worked example: Keycloak[​](#worked-example-keycloak "Direct link to Worked example: Keycloak")
...
### Public URL override[​](#public-url-override "Direct link to Public URL override")
...
### OAuth flow[​](#oauth-flow "Direct link to OAuth flow")
...
### Cookies set[​](#cookies-set "Direct link to Cookies set")
...
### Non-interactive (bearer-token) auth[​](#non-interactive-bearer-token-auth "Direct link to Non-interactive (bearer-token) auth")
...
### Verifying the gate is on[​](#verifying-the-gate-is-on "Direct link to Verifying the gate is on")
...
## Connecting Hermes Desktop to a remote backend[​](#connecting-hermes-desktop-to-a-remote-backend-1 "Direct link to Connecting Hermes Desktop to a remote backend")
...
### On the backend (the remote machine)[​](#on-the-backend-the-remote-machine "Direct link to On the backend (the remote machine)")
...
### In Hermes Desktop[​](#in-hermes-desktop "Direct link to In Hermes Desktop")
...
* [Pages](#pages)
  + [Status](#status)
  + [Chat](#chat)
  + [Connecting Hermes Desktop to a remote backend](#connecting-hermes-desktop-to-a-remote-backend)
  + [Config](#config)
  + [API Keys](#api-keys)
  + [Sessions](#sessions)
  + [Logs](#logs)
...
* [REST API](#rest-api)
  ...
  + [GET /api/sessions/{session\_id}/messages](#get-apisessionssession_idmessages)
  ...
  + [POST /api/cron/jobs/{job\_id}/pause](#post-apicronjobsjob_idpause)
  + [POST /api/cron/jobs/{job\_id}/resume](#post-apicronjobsjob_idresume)
  + [POST /api/cron/jobs/{job\_id}/trigger](#post-apicronjobsjob_idtrigger)
  + [DELETE /api/cron/jobs/{job\_id}](#delete-apicronjobsjob_id)
  + [GET /api/skills](#get-apiskills)
  + [PUT /api/skills/toggle](#put-apiskillstoggle)
  + [GET /api/tools/toolsets](#get-apitoolstoolsets)
  + [Admin endpoints](#admin-endpoints)
* [Authentication (gated mode)](#authentication-gated-mode)
  + [When the gate engages](#when-the-gate-engages)
  + [Fail-closed semantics](#fail-closed-semantics)
  + [Default provider: Nous Research](#default-provider-nous-research)
  + [Username/password provider (no OAuth IDP)](#usernamepassword-provider-no-oauth-idp)
  + [Self-hosted OIDC provider](#self-hosted-oidc-provider)
  ...
  + [Non-interactive (bearer-token) auth](#non-interactive-bearer-token-auth)
  + [Verifying the gate is on](#verifying-the-gate-is-on)
* [Connecting Hermes Desktop to a remote backend](#connecting-hermes-desktop-to-a-remote-backend-1)
  + [On the backend (the remote machine)](#on-the-backend-the-remote-machine)
  + [In Hermes Desktop](#in-hermes-desktop)
...
- [https://hermes-agent.nousresearch.com/docs/user-guide/features/extending-the-dashboard](./docs-user-guide-features-extending-the-dashboard.md)
