...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/devops/devops-pinggy-tunnel](https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/devops/devops-pinggy-tunnel)
...
## Procedure — Start a Tunnel and Get the URL[​](#procedure--start-a-tunnel-and-get-the-url "Direct link to Procedure — Start a Tunnel and Get the URL")
...
### 1. Confirm a local origin is up[​](#1-confirm-a-local-origin-is-up "Direct link to 1. Confirm a local origin is up")
...
### 2. Launch the tunnel as a background process[​](#2-launch-the-tunnel-as-a-background-process "Direct link to 2. Launch the tunnel as a background process")
...
### 3. Parse the URL out of the log[​](#3-parse-the-url-out-of-the-log "Direct link to 3. Parse the URL out of the log")
...
## Access Control via Username Keywords[​](#access-control-via-username-keywords "Direct link to Access Control via Username Keywords")
...
## Web Debugger (optional)[​](#web-debugger-optional "Direct link to Web Debugger (optional)")
...
### Recipe 1 — Receive a webhook callback[​](#recipe-1--receive-a-webhook-callback "Direct link to Recipe 1 — Receive a webhook callback")
...
### Recipe 2 — Expose an MCP server over HTTP/SSE[​](#recipe-2--expose-an-mcp-server-over-httpsse "Direct link to Recipe 2 — Expose an MCP server over HTTP/SSE")
...
### Recipe 3 — Expose a local LLM endpoint (Ollama / vLLM / llama.cpp)[​](#recipe-3--expose-a-local-llm-endpoint-ollama--vllm--llamacpp "Direct link to Recipe 3 — Expose a local LLM endpoint (Ollama / vLLM / llama.cpp)")
...
### Recipe 4 — Share a dev server with a one-shot password[​](#recipe-4--share-a-dev-server-with-a-one-shot-password "Direct link to Recipe 4 — Share a dev server with a one-shot password")
...
* [Procedure — Start a Tunnel and Get the URL](#procedure--start-a-tunnel-and-get-the-url)
  + [1. Confirm a local origin is up](#1-confirm-a-local-origin-is-up)
  + [2. Launch the tunnel as a background process](#2-launch-the-tunnel-as-a-background-process)
  + [3. Parse the URL out of the log](#3-parse-the-url-out-of-the-log)
  + [4. Verify](#4-verify)
  + [5. Teardown](#5-teardown)
* [Access Control via Username Keywords](#access-control-via-username-keywords)
* [Web Debugger (optional)](#web-debugger-optional)
* [Pitfalls](#pitfalls)
* [Recipes](#recipes)
  + [Recipe 1 — Receive a webhook callback](#recipe-1--receive-a-webhook-callback)
  + [Recipe 2 — Expose an MCP server over HTTP/SSE](#recipe-2--expose-an-mcp-server-over-httpsse)
  + [Recipe 3 — Expose a local LLM endpoint (Ollama / vLLM / llama.cpp)](#recipe-3--expose-a-local-llm-endpoint-ollama--vllm--llamacpp)
  + [Recipe 4 — Share a dev server with a one-shot password](#recipe-4--share-a-dev-server-with-a-one-shot-password)
