...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/health/health-neuroskill-bci](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/health/health-neuroskill-bci)
...
## CLI 参考：`npx neuroskill <command>`[​](#cli-参考npx-neuroskill-command "cli-参考npx-neuroskill-command的直接链接")
...
## 1. 检查当前状态[​](#1-检查当前状态 "1. 检查当前状态的直接链接")

### 获取实时指标[​](#获取实时指标 "获取实时指标的直接链接")
...
### 响应中的关键字段[​](#响应中的关键字段 "响应中的关键字段的直接链接")
...
## 2. 会话分析[​](#2-会话分析 "2. 会话分析的直接链接")

### 单次会话详情[​](#单次会话详情 "单次会话详情的直接链接")
...
## 3. 历史搜索[​](#3-历史搜索 "3. 历史搜索的直接链接")

### 神经相似性搜索[​](#神经相似性搜索 "神经相似性搜索的直接链接")
...
* **分析**：efficiency\_pct、onset\_latency\_min、rem\_latency\_min、bout 计数
...
## 7. 实时流式传输[​](#7-实时流式传输 "7. 实时流式传输的直接链接")
...
## 8. UMAP 可视化[​](#8-umap-可视化 "8. UMAP 可视化的直接链接")
...
## 9. 主动状态感知[​](#9-主动状态感知 "9. 主动状态感知的直接链接")

### 会话开始检查[​](#会话开始检查 "会话开始检查的直接链接")
...
### 何时主动提及状态[​](#何时主动提及状态 "何时主动提及状态的直接链接")
...
* **放松度 < 0.30，stress\_index 高** → 心脏相干性或 4-7-8 呼吸法
* **认知负荷 > 0.70 持续** → 认知负荷卸载（思维倾倒）
* **困倦度 > 0.60** → 超日节律重置或清醒重置
* **FAA < 0（负值）** → FAA 再平衡
* **心流状态（专注度 > 0.75，投入度 > 0.70）** → 切勿打断
* **高静止度 + headache\_index** → 颈部放松序列
* **低 RMSSD（< 25ms）** → 迷走神经调节

---

## 11. 附加工具[​](#11-附加工具 "11. 附加工具的直接链接")

### 专注计时器[​](#专注计时器 "专注计时器的直接链接")
...
### 原始 JSON 直通[​](#原始-json-直通 "原始 JSON 直通的直接链接")
...
* [前提条件](#前提条件)
  + [验证设置](#验证设置)
* [CLI 参考：`npx neuroskill <command>`](#cli-参考npx-neuroskill-command)
  + [全局标志](#全局标志)
* [1. 检查当前状态](#1-检查当前状态)
  + [获取实时指标](#获取实时指标)
  + [响应中的关键字段](#响应中的关键字段)
