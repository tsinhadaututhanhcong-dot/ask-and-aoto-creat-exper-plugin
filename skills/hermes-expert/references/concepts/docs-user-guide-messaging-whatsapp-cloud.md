...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud)
...
## Creating the Meta app[​](#creating-the-meta-app "Direct link to Creating the Meta app")
...
## Permanent token (production)[​](#permanent-token-production "Direct link to Permanent token (production)")
...
## Exposing Hermes to the internet[​](#exposing-hermes-to-the-internet "Direct link to Exposing Hermes to the internet")
...
### Cloudflare Tunnel (recommended)[​](#cloudflare-tunnel-recommended "Direct link to Cloudflare Tunnel (recommended)")
...
### Your own domain + reverse proxy[​](#your-own-domain--reverse-proxy "Direct link to Your own domain + reverse proxy")
...
## Configuring the webhook on Meta's side[​](#configuring-the-webhook-on-metas-side "Direct link to Configuring the webhook on Meta's side")
...
## Recipient whitelist (Meta-side)[​](#recipient-whitelist-meta-side "Direct link to Recipient whitelist (Meta-side)")
...
## Allowlist (Hermes-side)[​](#allowlist-hermes-side "Direct link to Allowlist (Hermes-side)")
...
## Polishing your bot's WhatsApp profile[​](#polishing-your-bots-whatsapp-profile "Direct link to Polishing your bot's WhatsApp profile")
...
### Interactive UX[​](#interactive-ux "Direct link to Interactive UX")
...
### Read receipts and typing indicator[​](#read-receipts-and-typing-indicator "Direct link to Read receipts and typing indicator")
...
### 24-hour conversation window[​](#24-hour-conversation-window "Direct link to 24-hour conversation window")
...
### Outbound rate limit[​](#outbound-rate-limit "Direct link to Outbound rate limit")
...
### Setup verification fails ("URL couldn't be validated") in Meta dashboard[​](#setup-verification-fails-url-couldnt-be-validated-in-meta-dashboard "Direct link to Setup verification fails (\"URL couldn't be validated\") in Meta dashboard")
...
### `graph error 100`: Object with ID '...' does not exist[​](#graph-error-100-object-with-id--does-not-exist "Direct link to graph-error-100-object-with-id--does-not-exist")
...
### `graph error 190`: Authentication Error[​](#graph-error-190-authentication-error "Direct link to graph-error-190-authentication-error")
...
### `graph error 131047`: Re-engagement message[​](#graph-error-131047-re-engagement-message "Direct link to graph-error-131047-re-engagement-message")
...
### Inbound message: `media metadata fetch failed (status=401)`[​](#inbound-message-media-metadata-fetch-failed-status401 "Direct link to inbound-message-media-metadata-fetch-failed-status401")

Same 401 root causes as outbound (`graph error 190`) — the access token is invalid or expired. Fix the token.

### Bot replies appear as raw JSON / tool-call leakage[​](#bot-replies-appear-as-raw-json--tool-call-leakage "Direct link to Bot replies appear as raw JSON / tool-call leakage")
...
### STT (voice note transcription) returns empty / "could not transcribe"[​](#stt-voice-note-transcription-returns-empty--could-not-transcribe "Direct link to STT (voice note transcription) returns empty / \"could not transcribe\"")
...
## Comparison to the Baileys bridge[​](#comparison-to-the-baileys-bridge "Direct link to Comparison to the Baileys bridge")
