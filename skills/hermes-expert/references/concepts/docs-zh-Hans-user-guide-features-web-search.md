# 网页搜索与提取 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/web-search](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/web-search)
...
如果您拥有付费 [Nous Portal](https://portal.nousresearch.com) 订阅，网页搜索和提取可通过 **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** 使用托管的 Firecrawl——无需 API 密钥。新安装可运行 `hermes setup --portal` 登录并一次性开启所有 gateway 工具；现有安装可通过 `hermes tools` 单独开启网页功能。

---

## `web_extract` 如何处理长页面[​](#web_extract-如何处理长页面 "web_extract-如何处理长页面的直接链接")
...
### 哪个模型负责摘要？[​](#哪个模型负责摘要 "哪个模型负责摘要？的直接链接")
...
完整参考和按任务覆盖模式，请参阅[辅助模型](/docs/zh-Hans/user-guide/configuration#auxiliary-models)。

### 摘要处理不适用的情况[​](#摘要处理不适用的情况 "摘要处理不适用的情况的直接链接")
...
### 通过 `hermes tools` 快速设置[​](#通过-hermes-tools-快速设置 "通过-hermes-tools-快速设置的直接链接")
...
### Firecrawl（默认）[​](#firecrawl默认 "Firecrawl（默认）的直接链接")
...
### SearXNG（免费，自托管）[​](#searxng免费自托管 "SearXNG（免费，自托管）的直接链接")

SearXNG 是一个注重隐私的开源元搜索引擎，聚合来自 70 多个搜索引擎的结果。**无需 API 密钥**——只需将 Hermes 指向一个运行中的 SearXNG 实例。

SearXNG 为**仅搜索**——`web_extract` 需要单独的提取提供商。

#### 方案 A — 使用 Docker 自托管（推荐）[​](#方案-a--使用-docker-自托管推荐 "方案 A — 使用 Docker 自托管（推荐）的直接链接")
...
#### 方案 B — 使用公共实例[​](#方案-b--使用公共实例 "方案 B — 使用公共实例的直接链接")
...
#### 将 SearXNG 与提取提供商配合使用[​](#将-searxng-与提取提供商配合使用 "将 SearXNG 与提取提供商配合使用的直接链接")
...
### Parallel[​](#parallel "Parallel的直接链接")
...
### xAI (Grok)[​](#xai-grok "xAI (Grok)的直接链接")

通过 Responses API 将 `web_search` 路由至 Grok 服务端的 [web\_search 工具](https://docs.x.ai/developers/tools/web-search)。Grok 执行实际搜索并以结构化 JSON 返回最佳结果。
...
### 按能力配置[​](#per-capability-configuration "按能力配置的直接链接")
...
### `web_search` 返回 `{"success": false}`[​](#web_search-返回-success-false "web_search-返回-success-false的直接链接")
...
### `web_extract` 提示"search-only backend"[​](#web_extract-提示search-only-backend "web_extract-提示search-only-backend的直接链接")
...
### SearXNG 返回 0 条结果[​](#searxng-返回-0-条结果 "SearXNG 返回 0 条结果的直接链接")
...
### 公共实例遭遇速率限制[​](#公共实例遭遇速率限制 "公共实例遭遇速率限制的直接链接")

切换到自托管实例（参见上方[方案 A](#option-a--self-host-with-docker-recommended)）。使用 Docker，您自己的实例没有速率限制。

### `web_extract` 返回截断内容并附有"summarization timed out"提示[​](#web_extract-返回截断内容并附有summarization-timed-out提示 "web_extract-返回截断内容并附有summarization-timed-out提示的直接链接")
...
## 可选技能：`searxng-search`[​](#可选技能searxng-search "可选技能searxng-search的直接链接")
...
* [后端](#后端)
* [`web_extract` 如何处理长页面](#web_extract-如何处理长页面)
  + [哪个模型负责摘要？](#哪个模型负责摘要)
  + [摘要处理不适用的情况](#摘要处理不适用的情况)
* [设置](#设置)
  + [通过 `hermes tools` 快速设置](#通过-hermes-tools-快速设置)
  + [Firecrawl（默认）](#firecrawl默认)
  + [SearXNG（免费，自托管）](#searxng免费自托管)
  + [Tavily](#tavily)
  + [Exa](#exa)
  + [Parallel](#parallel)
  + [xAI (Grok)](#xai-grok)
...
* [验证设置](#验证设置)
* [故障排查](#故障排查)
  + [`web_search` 返回 `{"success": false}`](#web_search-返回-success-false)
  + [`web_extract` 提示"search-only backend"](#web_extract-提示search-only-backend)
  + [SearXNG 返回 0 条结果](#searxng-返回-0-条结果)
  + [公共实例遭遇速率限制](#公共实例遭遇速率限制)
  + [`web_extract` 返回截断内容并附有"summarization timed out"提示](#web_extract-返回截断内容并附有summarization-timed-out提示)
* [可选技能：`searxng-search`](#可选技能searxng-search)
