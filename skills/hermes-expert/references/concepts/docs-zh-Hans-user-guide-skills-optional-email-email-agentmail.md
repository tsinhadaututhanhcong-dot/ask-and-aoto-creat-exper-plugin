# Agentmail — 通过 AgentMail 为 Agent 提供专属电子邮件收件箱 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/email/email-agentmail](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/email/email-agentmail)

本页总览

通过 AgentMail 为 Agent 提供专属电子邮件收件箱。使用 Agent 专属电子邮件地址（例如 [hermes-agent@agentmail.to](mailto:hermes-agent@agentmail.to)）自主发送、接收和管理电子邮件。
...
### 1. 获取 API 密钥[​](#1-获取-api-密钥 "1. 获取 API 密钥的直接链接")

* 访问 <https://console.agentmail.to>
* 创建账户并生成 API 密钥（以 `am_` 开头）

### 2. 配置 MCP 服务器[​](#2-配置-mcp-服务器 "2. 配置 MCP 服务器的直接链接")
...
### 3. 重启 Hermes[​](#3-重启-hermes "3. 重启 Hermes的直接链接")
...
## 可用工具（通过 MCP）[​](#可用工具通过-mcp "可用工具（通过 MCP）的直接链接")
...
### 创建收件箱并发送邮件[​](#创建收件箱并发送邮件 "创建收件箱并发送邮件的直接链接")
...
* [配置](#配置)
  ...
  + [2. 配置 MCP 服务器](#2-配置-mcp-服务器)
  + [3. 重启 Hermes](#3-重启-hermes)
* [可用工具（通过 MCP）](#可用工具通过-mcp)
* [操作流程](#操作流程)
  + [创建收件箱并发送邮件](#创建收件箱并发送邮件)
  + [检查传入邮件](#检查传入邮件)
  + [回复邮件](#回复邮件)
* [示例工作流](#示例工作流)
* [注意事项](#注意事项)
