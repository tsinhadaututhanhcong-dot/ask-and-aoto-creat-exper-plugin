# Nix & NixOS 安装配置 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/nix-setup](https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/nix-setup)
...
## 快速开始（任意 Nix 用户）[​](#快速开始任意-nix-用户 "快速开始（任意 Nix 用户）的直接链接")
...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](/docs/zh-Hans/getting-started/installation)完全相同——`hermes setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/`。
...
## NixOS 模块[​](#nixos-模块 "NixOS 模块的直接链接")
...
### 添加 Flake 输入[​](#添加-flake-输入 "添加 Flake 输入的直接链接")
...
### 容器感知 CLI[​](#容器感知-cli "容器感知 CLI的直接链接")
...
### 逃生舱：自带配置文件[​](#逃生舱自带配置文件 "逃生舱：自带配置文件的直接链接")
...
### sops-nix[​](#sops-nix "sops-nix的直接链接")
...
### OAuth / 认证预置[​](#oauth--认证预置 "OAuth / 认证预置的直接链接")
...
### stdio 传输（本地服务器）[​](#stdio-传输本地服务器 "stdio 传输（本地服务器）的直接链接")
...
### HTTP 传输（远程服务器）[​](#http-传输远程服务器 "HTTP 传输（远程服务器）的直接链接")
...
### 带 OAuth 的 HTTP 传输[​](#带-oauth-的-http-传输 "带 OAuth 的 HTTP 传输的直接链接")
...
### Sampling（服务器发起的 LLM 请求）[​](#sampling服务器发起的-llm-请求 "Sampling（服务器发起的 LLM 请求）的直接链接")
...
### GC Root 保护[​](#gc-root-保护 "GC Root 保护的直接链接")
...
### 目录插件（`extraPlugins`）[​](#目录插件extraplugins "目录插件extraplugins的直接链接")
...
### 入口点插件（`extraPythonPackages`）[​](#入口点插件extrapythonpackages "入口点插件extrapythonpackages的直接链接")
...
### 可选依赖组（`extraDependencyGroups`）[​](#可选依赖组extradependencygroups "可选依赖组extradependencygroups的直接链接")
...
### 使用 Overlay[​](#使用-overlay "使用 Overlay的直接链接")
...
### 开发 Shell[​](#开发-shell "开发 Shell的直接链接")
...
### direnv（推荐）[​](#direnv推荐 "direnv（推荐）的直接链接")
...
### Flake 检查[​](#flake-检查 "Flake 检查的直接链接")
...
### MCP 服务器[​](#mcp-服务器-1 "MCP 服务器的直接链接")
...
### GC Root 验证[​](#gc-root-验证 "GC Root 验证的直接链接")
...
* [前提条件](#前提条件)
* [快速开始（任意 Nix 用户）](#快速开始任意-nix-用户)
* [NixOS 模块](#nixos-模块)
  + [添加 Flake 输入](#添加-flake-输入)
  + [最小化配置](#最小化配置)
  + [容器感知 CLI](#容器感知-cli)
  + [验证运行状态](#验证运行状态)
  + [选择部署模式](#选择部署模式)
* [配置](#配置)
  + [声明式设置](#声明式设置)
  + [逃生舱：自带配置文件](#逃生舱自带配置文件)
  + [自定义速查表](#自定义速查表)
* [密钥管理](#密钥管理)
  + [sops-nix](#sops-nix)
  + [agenix](#agenix)
  + [OAuth / 认证预置](#oauth--认证预置)
* [文档](#文档)
* [MCP 服务器](#mcp-服务器)
  + [stdio 传输（本地服务器）](#stdio-传输本地服务器)
  + [HTTP 传输（远程服务器）](#http-传输远程服务器)
  + [带 OAuth 的 HTTP 传输](#带-oauth-的-http-传输)
  + [Sampling（服务器发起的 LLM 请求）](#sampling服务器发起的-llm-请求)
* [托管模式](#托管模式)
* [容器架构](#容器架构)
  + [各事件的持久性](#各事件的持久性)
  + [GC Root 保护](#gc-root-保护)
* [插件](#插件)
  + [目录插件（`extraPlugins`）](#目录插件extraplugins)
  + [入口点插件（`extraPythonPackages`）](#入口点插件extrapythonpackages)
  + [可选依赖组（`extraDependencyGroups`）](#可选依赖组extradependencygroups)
  + [组合使用](#组合使用)
  + [使用 Overlay](#使用-overlay)
  + [插件配置](#插件配置)
* [开发](#开发)
  + [开发 Shell](#开发-shell)
  + [direnv（推荐）](#direnv推荐)
