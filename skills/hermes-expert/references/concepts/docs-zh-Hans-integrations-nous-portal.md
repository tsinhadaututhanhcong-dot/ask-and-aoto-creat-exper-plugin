...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/nous-portal](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/nous-portal)
...
## 订阅包含的内容[​](#订阅包含的内容 "订阅包含的内容的直接链接")

### 300+ 前沿模型，统一账单[​](#300-前沿模型统一账单 "300+ 前沿模型，统一账单的直接链接")
...
同一订阅还解锁了 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)，将 Hermes Agent 的工具调用路由至 Nous 托管的基础设施。五个后端，一次登录：
...
| **文字转语音** | OpenAI TTS | 无需独立 OpenAI 密钥的高质量 TTS。在各消息平台上启用[语音模式](/docs/zh-Hans/user-guide/features/voice-mode)。 |
...
### Nous Chat[​](#nous-chat "Nous Chat的直接链接")
...
### 凭证不落入 dotfiles[​](#凭证不落入-dotfiles "凭证不落入 dotfiles的直接链接")
...
### 跨平台一致性[​](#跨平台一致性 "跨平台一致性的直接链接")

[原生 Windows](/docs/zh-Hans/user-guide/windows-native) 上，逐个配置 API 密钥是其最大痛点——在 Windows 上分别安装 Firecrawl 账号、FAL 账号、Browser Use 账号、OpenAI 密钥，是整个 agent 配置过程中摩擦最高的部分。Portal 订阅消除了这一问题：一次 OAuth 覆盖模型和所有 gateway 工具，Windows 用户无需手动配置四个后端，即可获得与 macOS/Linux 相同的体验。

## 关于 Hermes 4 的说明[​](#关于-hermes-4-的说明 "关于 Hermes 4 的说明的直接链接")
...
但**不建议在 Hermes Agent 内部使用它们**。Hermes 4 针对对话和推理进行了调优，而非 agent 所依赖的高频工具调用循环。请将它们用于 [Nous Chat](https://chat.nousresearch.com)、研究工作流，或通过[订阅代理](/docs/zh-Hans/user-guide/features/subscription-proxy)从其他工具调用——但在 agent 场景下，请从目录中选择前沿 agentic 模型：
...
### 全新安装——一条命令[​](#全新安装一条命令 "全新安装——一条命令的直接链接")
...
### 已有安装——在现有提供商旁添加 Portal[​](#已有安装在现有提供商旁添加-portal "已有安装——在现有提供商旁添加 Portal的直接链接")
...
### 无头环境 / SSH / 远程配置[​](#无头环境--ssh--远程配置 "无头环境 / SSH / 远程配置的直接链接")

OAuth 需要浏览器，但回调的 loopback 运行在 Hermes 所在的机器上。对于远程主机，请参阅 [OAuth over SSH / 远程主机](/docs/zh-Hans/guides/oauth-over-ssh)——与其他基于 OAuth 的提供商相同的方式同样适用于 Portal（`ssh -L` 端口转发）。

### Profile 配置[​](#profile-配置 "Profile 配置的直接链接")

如果你使用 [Hermes profiles（配置文件）](/docs/zh-Hans/user-guide/profiles)，Portal 的 refresh token 会通过共享令牌存储自动在所有 profile 间共享。在任意 profile 上登录一次，其余 profile 自动获取——无需为每个 profile 重复 OAuth 流程。

## 日常使用 Portal[​](#日常使用-portal "日常使用 Portal的直接链接")

### 查看当前配置状态[​](#查看当前配置状态 "查看当前配置状态的直接链接")
...
### 将 gateway 与自有后端混用[​](#将-gateway-与自有后端混用 "将 gateway 与自有后端混用的直接链接")
...
Tool Gateway 是按工具单独选择启用的，而非全部或全不。完整的每工具配置矩阵请参阅 [Tool Gateway 文档](/docs/zh-Hans/user-guide/features/tool-gateway)。
...
### `hermes portal info` 显示"not logged in"[​](#hermes-portal-info-显示not-logged-in "hermes-portal-info-显示not-logged-in的直接链接")
...
### 会话中途收到"需要重新认证"提示[​](#会话中途收到需要重新认证提示 "会话中途收到\"需要重新认证\"提示的直接链接")

你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `hermes auth add nous`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。

### 想使用 Portal 未暴露的特定提供商模型[​](#想使用-portal-未暴露的特定提供商模型 "想使用 Portal 未暴露的特定提供商模型的直接链接")
...
* **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** —— 每个 gateway 工具的完整详情、每工具配置及定价
* **[订阅代理](/docs/zh-Hans/user-guide/features/subscription-proxy)** —— 在非 Hermes 工具（其他 agent、脚本、第三方客户端）中使用你的 Portal 订阅
* **[语音模式](/docs/zh-Hans/user-guide/features/voice-mode)** —— 使用 Portal 的 OpenAI TTS 进行语音对话
* **[AI 提供商](/docs/zh-Hans/integrations/providers)** —— 完整提供商目录，供对比参考
* **[OAuth over SSH](/docs/zh-Hans/guides/oauth-over-ssh)** —— 从远程主机或纯浏览器环境登录
* **[Profiles](/docs/zh-Hans/user-guide/profiles)** —— 多个 Hermes 配置共享一个 Portal 登录

* [订阅包含的内容](#订阅包含的内容)
  + [300+ 前沿模型，统一账单](#300-前沿模型统一账单)
  + [Nous Tool Gateway](#nous-tool-gateway)
  + [Nous Chat](#nous-chat)
  + [凭证不落入 dotfiles](#凭证不落入-dotfiles)
  + [跨平台一致性](#跨平台一致性)
* [关于 Hermes 4 的说明](#关于-hermes-4-的说明)
* [配置](#配置)
  + [全新安装——一条命令](#全新安装一条命令)
  + [已有安装——在现有提供商旁添加 Portal](#已有安装在现有提供商旁添加-portal)
  + [无头环境 / SSH / 远程配置](#无头环境--ssh--远程配置)
  + [Profile 配置](#profile-配置)
* [日常使用 Portal](#日常使用-portal)
  + [查看当前配置状态](#查看当前配置状态)
  + [切换模型](#切换模型)
  + [将 gateway 与自有后端混用](#将-gateway-与自有后端混用)
  + [订阅管理](#订阅管理)
* [配置参考](#配置参考)
* [令牌处理](#令牌处理)
* [故障排查](#故障排查)
  + [`hermes portal info` 显示"not logged in"](#hermes-portal-info-显示not-logged-in)
  + [会话中途收到"需要重新认证"提示](#会话中途收到需要重新认证提示)
  + [想使用 Portal 未暴露的特定提供商模型](#想使用-portal-未暴露的特定提供商模型)
