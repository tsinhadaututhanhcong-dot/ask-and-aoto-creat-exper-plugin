# Spike — 在构建前验证想法的一次性实验 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-spike](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-spike)
...
| 相关 skill | [`sketch`](/docs/zh-Hans/user-guide/skills/bundled/creative/creative-sketch)、[`writing-plans`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-writing-plans)、[`subagent-driven-development`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-subagent-driven-development)、[`plan`](/docs/zh-Hans/user-guide/skills/bundled/software-development/software-development-plan) |
...
## 何时不使用此 skill[​](#何时不使用此-skill "何时不使用此 skill的直接链接")
...
### 1. 分解（Decompose）[​](#1-分解decompose "1. 分解（Decompose）的直接链接")
...
### 2. 对齐（Align，适用于多 spike 想法）[​](#2-对齐align适用于多-spike-想法 "2. 对齐（Align，适用于多 spike 想法）的直接链接")

展示 spike 表格。询问："按此顺序全部构建，还是需要调整？"在写任何代码之前，让用户删减、重排或重新定义。

### 3. 调研（Research，每个 spike 构建前）[​](#3-调研research每个-spike-构建前 "3. 调研（Research，每个 spike 构建前）的直接链接")
...
### 4. 构建（Build）[​](#4-构建build "4. 构建（Build）的直接链接")
...
### 5. 结论（Verdict）[​](#5-结论verdict "5. 结论（Verdict）的直接链接")
...
## 对比 spike[​](#对比-spike "对比 spike的直接链接")
...
## 前沿模式（决定下一步 spike 什么）[​](#前沿模式决定下一步-spike-什么 "前沿模式（决定下一步 spike 什么）的直接链接")
...
* [何时不使用此 skill](#何时不使用此-skill)
* [如果用户安装了完整的 GSD 系统](#如果用户安装了完整的-gsd-系统)
* [核心方法](#核心方法)
  + [1. 分解（Decompose）](#1-分解decompose)
  + [2. 对齐（Align，适用于多 spike 想法）](#2-对齐align适用于多-spike-想法)
  + [3. 调研（Research，每个 spike 构建前）](#3-调研research每个-spike-构建前)
  + [4. 构建（Build）](#4-构建build)
  + [5. 结论（Verdict）](#5-结论verdict)
* [对比 spike](#对比-spike)
* [前沿模式（决定下一步 spike 什么）](#前沿模式决定下一步-spike-什么)
* [输出](#输出)
* [致谢](#致谢)
