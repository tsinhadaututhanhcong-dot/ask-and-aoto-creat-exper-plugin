# 网页搜索提供商插件 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/web-search-provider-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/web-search-provider-plugin)
...
网页搜索是 Hermes 支持的多种**后端插件**之一。其他插件（各有其 ABC）包括：[图像生成提供商插件](/docs/zh-Hans/developer-guide/image-gen-provider-plugin)、[视频生成提供商插件](/docs/zh-Hans/developer-guide/video-gen-provider-plugin)、[记忆提供商插件](/docs/zh-Hans/developer-guide/memory-provider-plugin)、[上下文引擎插件](/docs/zh-Hans/developer-guide/context-engine-plugin)和[模型提供商插件](/docs/zh-Hans/developer-guide/model-provider-plugin)。通用工具/hook/CLI 插件请参阅[构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins)。
...
## WebSearchProvider ABC[​](#websearchprovider-abc "WebSearchProvider ABC的直接链接")
...
| `requires_env` | 在 `hermes plugins install` 期间进行交互式凭据提示（富格式说明参见[构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins#gate-on-environment-variables)） |

## ABC 参考[​](#abc-参考 "ABC 参考的直接链接")
...
## Hermes 如何将其接入工具[​](#hermes-如何将其接入工具 "Hermes 如何将其接入工具的直接链接")
...
## 懒加载可选依赖[​](#懒加载可选依赖 "懒加载可选依赖的直接链接")

如果你的提供商封装了第三方 SDK（如 DDGS 封装了 `ddgs` 包），请勿在模块顶层 `import`。在 `is_available()` 或 `search()` 内部使用 `tools.lazy_deps.ensure(...)` ——Hermes 将在首次使用时安装该包，并受 `security.allow_lazy_installs` 控制。安全模型详见[构建 Hermes 插件 → 懒加载](/docs/zh-Hans/developer-guide/plugins#lazy-install-optional-python-dependencies)。
...
`my_backend_web_package` 必须暴露顶层 `register` 函数。完整配置说明参见通用插件指南中的[通过 pip 分发](/docs/zh-Hans/developer-guide/plugins#distribute-via-pip)。

## 相关页面[​](#相关页面 "相关页面的直接链接")

* [网页搜索](/docs/zh-Hans/user-guide/features/web-search) — 面向用户的功能文档及各后端配置说明
* [插件概览](/docs/zh-Hans/user-guide/features/plugins) — 所有插件类型一览
* [构建 Hermes 插件](/docs/zh-Hans/developer-guide/plugins) — 通用工具/hook/斜杠命令指南

* [发现机制](#发现机制)
* [目录结构](#目录结构)
* [WebSearchProvider ABC](#websearchprovider-abc)
* [plugin.yaml](#pluginyaml)
* [ABC 参考](#abc-参考)
* [响应格式](#响应格式)
* [能力标志](#能力标志)
* [Hermes 如何将其接入工具](#hermes-如何将其接入工具)
* [懒加载可选依赖](#懒加载可选依赖)
* [参考实现](#参考实现)
* [通过 pip 分发](#通过-pip-分发)
* [相关页面](#相关页面)
