# Kanban（多 Agent 看板） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban)

本页总览

> **想要详细教程？** 请阅读 [Kanban 教程](/docs/zh-Hans/user-guide/features/kanban-tutorial) —— 包含四个用户故事（独立开发者、批量任务、带重试的角色流水线、熔断器），并附有各场景的仪表盘截图。本页是参考文档，教程是叙述性说明。
...
### 两个操作界面：模型通过工具交互，你通过 CLI 交互[​](#两个操作界面模型通过工具交互你通过-cli-交互 "两个操作界面：模型通过工具交互，你通过 CLI 交互的直接链接")
...
## Kanban 与 `delegate_task` 的对比[​](#kanban-与-delegate_task-的对比 "kanban-与-delegate_task-的对比的直接链接")
...
## 看板（多项目）[​](#boards-multi-project "看板（多项目）的直接链接")
...
### 通过 CLI 管理看板[​](#通过-cli-管理看板 "通过 CLI 管理看板的直接链接")
...
### 通过仪表盘管理看板[​](#通过仪表盘管理看板 "通过仪表盘管理看板的直接链接")
...
### Gateway 内嵌调度器（默认）[​](#gateway-内嵌调度器默认 "Gateway 内嵌调度器（默认）的直接链接")
...
### 幂等创建（用于自动化 / webhook）[​](#幂等创建用于自动化--webhook "幂等创建（用于自动化 / webhook）的直接链接")
...
### 批量 CLI 动词[​](#批量-cli-动词 "批量 CLI 动词的直接链接")
...
## Worker 如何与看板交互[​](#how-workers-interact-with-the-board "Worker 如何与看板交互的直接链接")
...
### 为什么使用工具而不是 shell 执行 `hermes kanban`[​](#为什么使用工具而不是-shell-执行-hermes-kanban "为什么使用工具而不是-shell-执行-hermes-kanban的直接链接")
...
### Worker 生命周期[​](#worker-生命周期 "Worker 生命周期的直接链接")
...
### 为特定任务固定额外 skill[​](#为特定任务固定额外-skill "为特定任务固定额外 skill的直接链接")
...
### 编排器的行为方式[​](#编排器的行为方式 "编排器的行为方式的直接链接")
...
## 仪表盘（GUI）[​](#仪表盘gui "仪表盘（GUI）的直接链接")

`/kanban` CLI 和斜杠命令足以无头运行看板，但可视化看板通常是人工介入的正确界面：分诊、跨配置文件监督、阅读评论线程以及在列之间拖动卡片。Hermes 将此作为**内置仪表盘插件**在 `plugins/kanban/` 中提供 —— 不是核心功能，不是单独的服务 —— 遵循[扩展仪表盘](/docs/zh-Hans/user-guide/features/extending-the-dashboard)中描述的模型。
...
### 自动与手动编排[​](#auto-vs-manual-orchestration "自动与手动编排的直接链接")
...
GUI 严格是一个**通过 DB 读取 + 通过 kanban\_db 写入**的层，没有自己的领域逻辑：
...
仪表盘的 HTTP 认证中间件[显式跳过 `/api/plugins/`](/docs/zh-Hans/user-guide/features/extending-the-dashboard#backend-api-routes) —— 插件路由在设计上是未认证的，因为仪表盘默认绑定到 localhost。这意味着 kanban REST 接口可以从主机上的任何进程访问。
...
插件使用标准的 Hermes 仪表盘插件契约 —— 完整的 manifest 参考、shell 槽、页面范围槽和 Plugin SDK，请参阅[扩展仪表盘](/docs/zh-Hans/user-guide/features/extending-the-dashboard)。额外的列、自定义卡片样式、租户过滤布局或完整的 `tab.override` 替换都可以表达，无需 fork 此插件。
...
## CLI 命令参考[​](#cli-命令参考 "CLI 命令参考的直接链接")
...
## `/kanban` 斜杠命令[​](#kanban-slash-command "kanban-slash-command的直接链接")
...
### 运行中使用：`/kanban` 绕过运行中 agent 保护[​](#运行中使用kanban-绕过运行中-agent-保护 "运行中使用kanban-绕过运行中-agent-保护的直接链接")
...
### `/kanban create` 时自动订阅（仅限 gateway）[​](#kanban-create-时自动订阅仅限-gateway "kanban-create-时自动订阅仅限-gateway的直接链接")
...
### 消息中的输出截断[​](#消息中的输出截断 "消息中的输出截断的直接链接")
...
## Gateway 通知[​](#gateway-通知 "Gateway 通知的直接链接")
...
## 运行记录 —— 每次尝试一行[​](#运行记录--每次尝试一行 "运行记录 —— 每次尝试一行的直接链接")
...
* [两个操作界面：模型通过工具交互，你通过 CLI 交互](#两个操作界面模型通过工具交互你通过-cli-交互)
* [Kanban 与 `delegate_task` 的对比](#kanban-与-delegate_task-的对比)
* [核心概念](#核心概念)
* [看板（多项目）](#boards-multi-project)
  + [通过 CLI 管理看板](#通过-cli-管理看板)
  + [通过仪表盘管理看板](#通过仪表盘管理看板)
* [快速开始](#快速开始)
  + [Gateway 内嵌调度器（默认）](#gateway-内嵌调度器默认)
  + [幂等创建（用于自动化 / webhook）](#幂等创建用于自动化--webhook)
  + [批量 CLI 动词](#批量-cli-动词)
* [Worker 如何与看板交互](#how-workers-interact-with-the-board)
  + [为什么使用工具而不是 shell 执行 `hermes kanban`](#为什么使用工具而不是-shell-执行-hermes-kanban)
  + [推荐的交接证据](#推荐的交接证据)
  + [Worker 生命周期](#worker-生命周期)
  + [为特定任务固定额外 skill](#为特定任务固定额外-skill)
  + [编排器的行为方式](#编排器的行为方式)
* [仪表盘（GUI）](#仪表盘gui)
  + [插件提供的功能](#插件提供的功能)
...
* [`/kanban` 斜杠命令](#kanban-slash-command)
  + [运行中使用：`/kanban` 绕过运行中 agent 保护](#运行中使用kanban-绕过运行中-agent-保护)
  + [`/kanban create` 时自动订阅（仅限 gateway）](#kanban-create-时自动订阅仅限-gateway)
  + [消息中的输出截断](#消息中的输出截断)
  + [自动补全](#自动补全)
* [协作模式](#协作模式)
* [多租户使用](#多租户使用)
* [Gateway 通知](#gateway-通知)
* [运行记录 —— 每次尝试一行](#运行记录--每次尝试一行)
  + [向前兼容性](#向前兼容性)
* [事件参考](#事件参考)
* [范围之外](#范围之外)
* [设计规范](#设计规范)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/extending-the-dashboard](./docs-zh-Hans-user-guide-features-extending-the-dashboard.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/kanban-tutorial](./docs-zh-Hans-user-guide-features-kanban-tutorial.md)
