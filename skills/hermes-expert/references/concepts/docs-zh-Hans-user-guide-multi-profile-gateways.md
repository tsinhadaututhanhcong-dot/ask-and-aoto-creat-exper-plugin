...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/multi-profile-gateways](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/multi-profile-gateways)

本页总览

Operate multiple [profiles](/docs/zh-Hans/user-guide/profiles.md) — each with its own bot tokens,
...
[Profiles](/docs/zh-Hans/user-guide/profiles.md) for the basics.

## When to use this[​](#when-to-use-this "When to use this的直接链接")
...
## Alternative: one gateway for all profiles (multiplexing)[​](#alternative-one-gateway-for-all-profiles-multiplexing "Alternative: one gateway for all profiles (multiplexing)的直接链接")
...
### When to prefer multiplexing[​](#when-to-prefer-multiplexing "When to prefer multiplexing的直接链接")
...
### How to opt in[​](#how-to-opt-in "How to opt in的直接链接")
...
### What changes when multiplexing is on[​](#what-changes-when-multiplexing-is-on "What changes when multiplexing is on的直接链接")

Enabling the flag changes how a few things behave. All of these revert the
moment the flag is off.

#### 1. Secondary profiles must not start their own gateway[​](#1-secondary-profiles-must-not-start-their-own-gateway "1. Secondary profiles must not start their own gateway的直接链接")
...
#### 2. HTTP-inbound platforms are reached via a `/p/<profile>/` URL prefix[​](#2-http-inbound-platforms-are-reached-via-a-pprofile-url-prefix "2-http-inbound-platforms-are-reached-via-a-pprofile-url-prefix的直接链接")
...
#### 3. Per-credential platforms still need their own token per profile[​](#3-per-credential-platforms-still-need-their-own-token-per-profile "3. Per-credential platforms still need their own token per profile的直接链接")
...
#### 4. Session keys are namespaced by profile[​](#4-session-keys-are-namespaced-by-profile "4. Session keys are namespaced by profile的直接链接")
...
#### 5. One PID/lock and one status surface[​](#5-one-pidlock-and-one-status-surface "5. One PID/lock and one status surface的直接链接")
...
#### What does **not** change[​](#what-does-not-change "what-does-not-change的直接链接")
...
## Start, stop, or restart all gateways at once[​](#start-stop-or-restart-all-gateways-at-once "Start, stop, or restart all gateways at once的直接链接")
...
## Manage one profile[​](#manage-one-profile "Manage one profile的直接链接")
...
## Service files[​](#service-files "Service files的直接链接")
...
## Viewing logs[​](#viewing-logs "Viewing logs的直接链接")
...
## Identify what's actually running[​](#identify-whats-actually-running "Identify what's actually running的直接链接")
...
## Editing configuration[​](#editing-configuration "Editing configuration的直接链接")
...
## Keeping the host awake[​](#keeping-the-host-awake "Keeping the host awake的直接链接")

The gateway process can run all day, but the operating system will still try
to sleep when idle. Two patterns:

### macOS — `caffeinate`[​](#macos--caffeinate "macos--caffeinate的直接链接")
...
### Linux — `systemd-inhibit` or `loginctl`[​](#linux--systemd-inhibit-or-loginctl "linux--systemd-inhibit-or-loginctl的直接链接")
...
## Token-conflict safety[​](#token-conflict-safety "Token-conflict safety的直接链接")
...
## Updating the code[​](#updating-the-code "Updating the code的直接链接")
...
### "Could not find service in domain for user gui: 501"[​](#could-not-find-service-in-domain-for-user-gui-501 "\"Could not find service in domain for user gui: 501\"的直接链接")
...
### Stale PID after a crash[​](#stale-pid-after-a-crash "Stale PID after a crash的直接链接")
...
### Forcing a hard reset of one service[​](#forcing-a-hard-reset-of-one-service "Forcing a hard reset of one service的直接链接")
...
### Health check[​](#health-check "Health check的直接链接")
...
* [Alternative: one gateway for all profiles (multiplexing)](#alternative-one-gateway-for-all-profiles-multiplexing)
  + [When to prefer multiplexing](#when-to-prefer-multiplexing)
  + [How to opt in](#how-to-opt-in)
  + [What changes when multiplexing is on](#what-changes-when-multiplexing-is-on)
* [Start, stop, or restart all gateways at once](#start-stop-or-restart-all-gateways-at-once)
* [Manage one profile](#manage-one-profile)
* [Service files](#service-files)
* [Viewing logs](#viewing-logs)
* [Identify what's actually running](#identify-whats-actually-running)
* [Editing configuration](#editing-configuration)
* [Keeping the host awake](#keeping-the-host-awake)
  + [macOS — `caffeinate`](#macos--caffeinate)
  + [Linux — `systemd-inhibit` or `loginctl`](#linux--systemd-inhibit-or-loginctl)
* [Token-conflict safety](#token-conflict-safety)
* [Updating the code](#updating-the-code)
* [Troubleshooting](#troubleshooting)
  + ["Could not find service in domain for user gui: 501"](#could-not-find-service-in-domain-for-user-gui-501)
  + [Stale PID after a crash](#stale-pid-after-a-crash)
  + [Forcing a hard reset of one service](#forcing-a-hard-reset-of-one-service)
