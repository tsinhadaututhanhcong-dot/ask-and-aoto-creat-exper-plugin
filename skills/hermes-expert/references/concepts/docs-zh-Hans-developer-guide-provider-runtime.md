# Provider 运行时解析 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/provider-runtime](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/provider-runtime)
...
如果你想添加一个新的一等推理 provider，请结合本页阅读 [添加 Provider](/docs/zh-Hans/developer-guide/adding-providers) 和 [Model Provider 插件指南](/docs/zh-Hans/developer-guide/model-provider-plugin)。

## 解析优先级[​](#解析优先级 "解析优先级的直接链接")
...
## Provider[​](#provider "Provider的直接链接")
...
## 运行时解析的输出[​](#运行时解析的输出 "运行时解析的输出的直接链接")
...
## OpenRouter 与自定义 OpenAI 兼容 base URL[​](#openrouter-与自定义-openai-兼容-base-url "OpenRouter 与自定义 OpenAI 兼容 base URL的直接链接")
...
## 原生 Anthropic 路径[​](#原生-anthropic-路径 "原生 Anthropic 路径的直接链接")
...
## OpenAI Codex 路径[​](#openai-codex-路径 "OpenAI Codex 路径的直接链接")
...
### 不支持回退的场景[​](#不支持回退的场景 "不支持回退的场景的直接链接")

* **子代理委托**（`tools/delegate_tool.py`）：子代理继承父代理的 provider，但不继承回退配置
* **辅助任务**：使用各自独立的 provider 自动检测链（见上方辅助模型路由）

Cron 任务**支持**回退：`run_job()` 从 `config.yaml` 读取 `fallback_providers`（或旧版 `fallback_model`）并传递给 `AIAgent(fallback_model=...)`，与 gateway 的 `_load_fallback_model()` 模式一致。参见 [Cron 内部机制](/docs/zh-Hans/developer-guide/cron-internals)。
...
* [上下文压缩与 Prompt 缓存](/docs/zh-Hans/developer-guide/context-compression-and-caching)

* [解析优先级](#解析优先级)
* [Provider](#provider)
* [运行时解析的输出](#运行时解析的输出)
* [为什么这很重要](#为什么这很重要)
* [OpenRouter 与自定义 OpenAI 兼容 base URL](#openrouter-与自定义-openai-兼容-base-url)
* [原生 Anthropic 路径](#原生-anthropic-路径)
* [OpenAI Codex 路径](#openai-codex-路径)
* [辅助模型路由](#辅助模型路由)
* [回退模型](#回退模型)
  + [内部工作原理](#内部工作原理)
  + [不支持回退的场景](#不支持回退的场景)
