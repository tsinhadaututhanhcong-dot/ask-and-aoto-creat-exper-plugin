# Secret Source Plugins | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/secret-source-plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/secret-source-plugin)
...
The bundled set is deliberately closed, same policy as [memory providers](/docs/zh-Hans/developer-guide/memory-provider-plugin): PRs adding new vault backends under `agent/secret_sources/` are closed with a pointer to this guide. Publish your backend as a standalone plugin repo and share it in the Nous Research Discord (`#plugins-skills-and-skins`).

## What the framework owns vs. what you own[​](#what-the-framework-owns-vs-what-you-own "What the framework owns vs. what you own的直接链接")
...
## The SecretSource ABC[​](#the-secretsource-abc "The SecretSource ABC的直接链接")
...
### Contract rules (enforced, not suggestions)[​](#contract-rules-enforced-not-suggestions "Contract rules (enforced, not suggestions)的直接链接")
...
### Choosing your `shape`[​](#choosing-your-shape "choosing-your-shape的直接链接")
...
### Optional hooks[​](#optional-hooks "Optional hooks的直接链接")
...
## Subprocess safety: use `run_secret_cli()`[​](#subprocess-safety-use-run_secret_cli "subprocess-safety-use-run_secret_cli的直接链接")
...
## Registering[​](#registering "Registering的直接链接")
...
## Users configure it like any other source[​](#users-configure-it-like-any-other-source "Users configure it like any other source的直接链接")
...
Multi-source precedence, conflict warnings, and `(from My Vault)` provenance labels all work automatically — see the [user-facing secrets docs](/docs/zh-Hans/user-guide/secrets/) for the precedence ladder.

## Validate with the conformance kit[​](#validate-with-the-conformance-kit "Validate with the conformance kit的直接链接")
...
## ErrorKind reference[​](#errorkind-reference "ErrorKind reference的直接链接")
...
* [What the framework owns vs. what you own](#what-the-framework-owns-vs-what-you-own)
* [Directory structure](#directory-structure)
* [The SecretSource ABC](#the-secretsource-abc)
  + [Contract rules (enforced, not suggestions)](#contract-rules-enforced-not-suggestions)
  + [Choosing your `shape`](#choosing-your-shape)
  + [Optional hooks](#optional-hooks)
* [Subprocess safety: use `run_secret_cli()`](#subprocess-safety-use-run_secret_cli)
* [Registering](#registering)
* [Users configure it like any other source](#users-configure-it-like-any-other-source)
* [Validate with the conformance kit](#validate-with-the-conformance-kit)
* [ErrorKind reference](#errorkind-reference)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/secrets/](./docs-zh-Hans-user-guide-secrets.md)
