# 构建 Hermes 插件 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/plugins/](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/plugins/)
...
| **LLM / 推理后端**（新提供商） | [模型提供商插件](/docs/zh-Hans/developer-guide/model-provider-plugin) |
| **网关频道**（Discord/Telegram/IRC/Teams 等） | [添加平台适配器](/docs/zh-Hans/developer-guide/adding-platform-adapters) |
| **记忆后端**（Honcho/Mem0/Supermemory 等） | [记忆提供商插件](/docs/zh-Hans/developer-guide/memory-provider-plugin) |
| **上下文压缩引擎** | [上下文引擎插件](/docs/zh-Hans/developer-guide/context-engine-plugin) |
| **图像生成后端** | [图像生成提供商插件](/docs/zh-Hans/developer-guide/image-gen-provider-plugin) |
| **视频生成后端** | [视频生成提供商插件](/docs/zh-Hans/developer-guide/video-gen-provider-plugin) |
| **网页搜索/提取后端** | [网页搜索提供商插件](/docs/zh-Hans/developer-guide/web-search-provider-plugin) |
| **云浏览器后端**（Browserbase 类 CDP 会话提供商） | [浏览器提供商插件](/docs/zh-Hans/developer-guide/browser-provider-plugin) |
| **密钥管理器后端**（保险库 / 密码管理器 / 系统钥匙串） | [密钥源插件](/docs/zh-Hans/developer-guide/secret-source-plugin) |
| **仪表盘 OIDC/认证提供商** | [Web 仪表盘 — 自定义提供商](/docs/zh-Hans/user-guide/features/web-dashboard#custom-providers) — `ctx.register_dashboard_auth_provider()` |
| **TTS 后端**（任意 CLI——Piper、VoxCPM、Kokoro、声音克隆等） | [TTS 自定义命令提供商](/docs/zh-Hans/user-guide/features/tts#custom-command-providers)——配置驱动，无需 Python |
| **STT 后端**（自定义 whisper / ASR CLI） | [语音消息转录](/docs/zh-Hans/user-guide/features/tts#voice-message-transcription-stt)——将 `HERMES_LOCAL_STT_COMMAND` 设置为 shell 模板 |
| **通过 MCP 接入外部工具**（文件系统、GitHub、Linear、任意 MCP 服务器） | [MCP](/docs/zh-Hans/user-guide/features/mcp)——在 `config.yaml` 中声明 `mcp_servers.<name>` |
| **网关事件钩子**（在启动、会话事件、命令时触发） | [事件钩子](/docs/zh-Hans/user-guide/features/hooks#gateway-event-hooks)——将 `HOOK.yaml` + `handler.py` 放入 `~/.hermes/hooks/<name>/` |
| **Shell 钩子**（在事件发生时运行 shell 命令） | [Shell 钩子](/docs/zh-Hans/user-guide/features/hooks#shell-hooks)——在 `config.yaml` 的 `hooks:` 下声明 |
| **额外技能来源**（自定义 GitHub 仓库、私有技能索引） | [技能](/docs/zh-Hans/user-guide/features/skills)——`hermes skills tap add <repo>` · [发布 tap](/docs/zh-Hans/user-guide/features/skills#publishing-a-custom-skill-tap) |
| 一流的**核心**推理提供商（非插件） | [添加提供商](/docs/zh-Hans/developer-guide/adding-providers) |

查看完整的[可插拔接口表](/docs/zh-Hans/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each)，获取每种扩展接口的汇总视图，包括配置驱动（TTS、STT、MCP、shell 钩子）和放入目录（网关钩子）两种方式。

## 你将构建什么[​](#你将构建什么 "你将构建什么的直接链接")
...
## 第一步：创建插件目录[​](#第一步创建插件目录 "第一步：创建插件目录的直接链接")
...
## 第二步：编写清单文件[​](#第二步编写清单文件 "第二步：编写清单文件的直接链接")
...
## 第三步：编写工具 schema[​](#第三步编写工具-schema "第三步：编写工具 schema的直接链接")
...
## 第四步：编写工具处理器[​](#第四步编写工具处理器 "第四步：编写工具处理器的直接链接")
...
## 第五步：编写注册代码[​](#第五步编写注册代码 "第五步：编写注册代码的直接链接")
...
* `ctx.dispatch_tool(name, arguments)` ——以父代理的上下文（审批、凭证、task\_id 自动连接）调用任意其他工具（内置或来自其他插件）。适用于需要直接调用 `terminal`、`read_file` 或其他工具的斜杠命令处理器，效果等同于模型直接调用。
...
### 调试插件发现问题[​](#调试插件发现问题 "调试插件发现问题的直接链接")
...
## 插件还能做什么？[​](#插件还能做什么 "插件还能做什么？的直接链接")

### 随附数据文件[​](#随附数据文件 "随附数据文件的直接链接")
...
### 根据环境变量决定是否启用[​](#根据环境变量决定是否启用 "根据环境变量决定是否启用的直接链接")
...
如果 `WEATHER_API_KEY` 未设置，插件将被禁用并显示清晰的提示信息。不会崩溃，代理中也不会报错——只会显示"Plugin weather disabled (missing: WEATHER\_API\_KEY)"。
...
### 懒加载可选 Python 依赖[​](#懒加载可选-python-依赖 "懒加载可选 Python 依赖的直接链接")
...
每个钩子的完整文档见\*\*[事件钩子参考](/docs/zh-Hans/user-guide/features/hooks#plugin-hooks)\*\*——回调签名、参数表、触发时机和示例。以下是摘要：

| 钩子 | 触发时机 | 回调签名 | 返回值 |
| --- | --- | --- | --- |
| [`pre_tool_call`](/docs/zh-Hans/user-guide/features/hooks#pre_tool_call) | 任意工具执行前 | `tool_name: str, args: dict, task_id: str` | 忽略 |
| [`post_tool_call`](/docs/zh-Hans/user-guide/features/hooks#post_tool_call) | 任意工具返回后 | `tool_name: str, args: dict, result: str, task_id: str, duration_ms: int` | 忽略 |
| [`pre_llm_call`](/docs/zh-Hans/user-guide/features/hooks#pre_llm_call) | 每轮一次，工具调用循环前 | `session_id: str, user_message: str, conversation_history: list, is_first_turn: bool, model: str, platform: str` | [上下文注入](#pre_llm_call-context-injection) |
| [`post_llm_call`](/docs/zh-Hans/user-guide/features/hooks#post_llm_call) | 每轮一次，工具调用循环后（仅成功轮次） | `session_id: str, user_message: str, assistant_response: str, conversation_history: list, model: str, platform: str` | 忽略 |
| [`on_session_start`](/docs/zh-Hans/user-guide/features/hooks#on_session_start) | 新会话创建（仅第一轮） | `session_id: str, model: str, platform: str` | 忽略 |
| [`on_session_end`](/docs/zh-Hans/user-guide/features/hooks#on_session_end) | 每次 `run_conversation` 调用结束 + CLI 退出 | `session_id: str, completed: bool, interrupted: bool, model: str, platform: str` | 忽略 |
| [`on_session_finalize`](/docs/zh-Hans/user-guide/features/hooks#on_session_finalize) | CLI/网关销毁活跃会话 | `session_id: str | None, platform: str` | 忽略 |
| [`on_session_reset`](/docs/zh-Hans/user-guide/features/hooks#on_session_reset) | 网关切换新会话键（`/new`、`/reset`） | `session_id: str, platform: str` | 忽略 |

大多数钩子是即发即忘的观察者——其返回值被忽略。例外是 `pre_llm_call`，它可以向对话中注入上下文。

所有回调都应接受 `**kwargs` 以保持向前兼容性。如果钩子回调崩溃，会被记录日志并跳过。其他钩子和代理继续正常运行。

### `pre_llm_call` 上下文注入[​](#pre_llm_call-上下文注入 "pre_llm_call-上下文注入的直接链接")
...
#### 示例：记忆召回插件[​](#示例记忆召回插件 "示例：记忆召回插件的直接链接")
...
#### 示例：仅观察钩子（不注入）[​](#示例仅观察钩子不注入 "示例：仅观察钩子（不注入）的直接链接")
...
**记忆提供商插件**使用基于约定的方式：在插件的 `cli.py` 文件中添加 `register_cli(subparser)` 函数。记忆插件发现系统会自动找到它——无需调用 `ctx.register_cli_command()`。详见[记忆提供商插件指南](/docs/zh-Hans/developer-guide/memory-provider-plugin#adding-cli-commands)。

**活跃提供商限制：** 记忆插件 CLI 命令仅在其提供商是配置中活跃的 `memory.provider` 时才会出现。如果用户尚未设置你的提供商，你的 CLI 命令不会出现在帮助输出中。

### 注册斜杠命令[​](#注册斜杠命令 "注册斜杠命令的直接链接")
...
### 从斜杠命令分发工具[​](#从斜杠命令分发工具 "从斜杠命令分发工具的直接链接")
...
### 模型提供商插件——添加 LLM 后端[​](#模型提供商插件添加-llm-后端 "模型提供商插件——添加 LLM 后端的直接链接")
...
在任何调用 `get_provider_profile()` 或 `list_providers()` 的地方首次使用时懒加载发现——`auth.py`、`config.py`、`doctor.py`、`models.py`、`runtime_provider.py` 和 chat\_completions 传输层会自动连接。用户插件按名称覆盖内置插件。

**完整指南：** [模型提供商插件](/docs/zh-Hans/developer-guide/model-provider-plugin)——字段参考、可覆盖钩子（`prepare_messages`、`build_extra_body`、`build_api_kwargs_extras`、`fetch_models`）、api\_mode 选择、认证类型、测试。

### 平台插件——添加网关频道[​](#平台插件添加网关频道 "平台插件——添加网关频道的直接链接")
...
**完整指南：** [添加平台适配器](/docs/zh-Hans/developer-guide/adding-platform-adapters)——完整的 `BasePlatformAdapter` 约定、消息路由、认证限制、设置向导集成。参考 `plugins/platforms/irc/` 获取仅使用标准库的可用示例。

### 记忆提供商插件——添加跨会话知识后端[​](#记忆提供商插件添加跨会话知识后端 "记忆提供商插件——添加跨会话知识后端的直接链接")
...
**完整指南：** [记忆提供商插件](/docs/zh-Hans/developer-guide/memory-provider-plugin)——完整的 `MemoryProvider` ABC、线程约定、配置文件隔离、通过 `cli.py` 注册 CLI 命令。

### 上下文引擎插件——替换上下文压缩器[​](#上下文引擎插件替换上下文压缩器 "上下文引擎插件——替换上下文压缩器的直接链接")
...
**完整指南：** [上下文引擎插件](/docs/zh-Hans/developer-guide/context-engine-plugin)。

### 图像生成后端[​](#图像生成后端 "图像生成后端的直接链接")
...
**完整指南：** [图像生成提供商插件](/docs/zh-Hans/developer-guide/image-gen-provider-plugin)——完整的 `ImageGenProvider` ABC、`list_models()` / `get_setup_schema()` 元数据、`success_response()`/`error_response()` 辅助函数、base64 与 URL 输出、用户覆盖、pip 分发。
...
## 非 Python 扩展接口[​](#非-python-扩展接口 "非 Python 扩展接口的直接链接")

Hermes 也接受完全不是 Python 插件的扩展。这些在[可插拔接口表](/docs/zh-Hans/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each)中有所展示；以下各节简要介绍每种编写方式。

### MCP 服务器——注册外部工具[​](#mcp-服务器注册外部工具 "MCP 服务器——注册外部工具的直接链接")
...
Hermes 在启动时连接到每个服务器，列出其工具，并与内置工具一起注册。LLM 看到它们的方式与其他工具完全相同。**完整指南：** [MCP](/docs/zh-Hans/user-guide/features/mcp)。

### 网关事件钩子——在生命周期事件时触发[​](#网关事件钩子在生命周期事件时触发 "网关事件钩子——在生命周期事件时触发的直接链接")
...
**完整指南：** [网关事件钩子](/docs/zh-Hans/user-guide/features/hooks#gateway-event-hooks)。

### Shell 钩子——在工具调用时运行 shell 命令[​](#shell-钩子在工具调用时运行-shell-命令 "Shell 钩子——在工具调用时运行 shell 命令的直接链接")
...
**完整指南：** [Shell 钩子](/docs/zh-Hans/user-guide/features/hooks#shell-hooks)。

### 技能来源——添加自定义技能注册表[​](#技能来源添加自定义技能注册表 "技能来源——添加自定义技能注册表的直接链接")
...
**完整指南：** [技能中心](/docs/zh-Hans/user-guide/features/skills#skills-hub) · [发布自定义 tap](/docs/zh-Hans/user-guide/features/skills#publishing-a-custom-skill-tap)（仓库结构、最小示例、非默认路径、信任级别）。

### 通过命令模板接入 TTS / STT[​](#通过命令模板接入-tts--stt "通过命令模板接入 TTS / STT的直接链接")
...
**完整指南：** [TTS 自定义命令提供商](/docs/zh-Hans/user-guide/features/tts#custom-command-providers) · [STT](/docs/zh-Hans/user-guide/features/tts#voice-message-transcription-stt)。

## 通过 pip 分发[​](#通过-pip-分发 "通过 pip 分发的直接链接")
...
## 为 NixOS 分发[​](#为-nixos-分发 "为 NixOS 分发的直接链接")
...
完整文档（包括 overlay 用法和冲突检查）见 [Nix 设置指南](/docs/zh-Hans/getting-started/nix-setup#plugins)。
...
* [你将构建什么](#你将构建什么)
* [第一步：创建插件目录](#第一步创建插件目录)
* [第二步：编写清单文件](#第二步编写清单文件)
* [第三步：编写工具 schema](#第三步编写工具-schema)
* [第四步：编写工具处理器](#第四步编写工具处理器)
* [第五步：编写注册代码](#第五步编写注册代码)
* [第六步：测试](#第六步测试)
  + [调试插件发现问题](#调试插件发现问题)
* [插件的最终结构](#插件的最终结构)
* [插件还能做什么？](#插件还能做什么)
  + [随附数据文件](#随附数据文件)
  + [捆绑技能](#捆绑技能)
  + [根据环境变量决定是否启用](#根据环境变量决定是否启用)
  + [懒加载可选 Python 依赖](#懒加载可选-python-依赖)
  + [条件工具可用性](#条件工具可用性)
  + [覆盖内置工具](#覆盖内置工具)
  + [注册多个钩子](#注册多个钩子)
  + [钩子参考](#钩子参考)
  + [`pre_llm_call` 上下文注入](#pre_llm_call-上下文注入)
  + [注册 CLI 命令](#注册-cli-命令)
  + [注册斜杠命令](#注册斜杠命令)
  + [从斜杠命令分发工具](#从斜杠命令分发工具)
* [专用插件类型](#专用插件类型)
  + [模型提供商插件——添加 LLM 后端](#模型提供商插件添加-llm-后端)
  + [平台插件——添加网关频道](#平台插件添加网关频道)
  + [记忆提供商插件——添加跨会话知识后端](#记忆提供商插件添加跨会话知识后端)
  + [上下文引擎插件——替换上下文压缩器](#上下文引擎插件替换上下文压缩器)
  + [图像生成后端](#图像生成后端)
* [非 Python 扩展接口](#非-python-扩展接口)
  + [MCP 服务器——注册外部工具](#mcp-服务器注册外部工具)
  + [网关事件钩子——在生命周期事件时触发](#网关事件钩子在生命周期事件时触发)
  + [Shell 钩子——在工具调用时运行 shell 命令](#shell-钩子在工具调用时运行-shell-命令)
  + [技能来源——添加自定义技能注册表](#技能来源添加自定义技能注册表)
  + [通过命令模板接入 TTS / STT](#通过命令模板接入-tts--stt)
* [通过 pip 分发](#通过-pip-分发)
* [为 NixOS 分发](#为-nixos-分发)
* [常见错误](#常见错误)

## Related Files
> **LLM Navigation:** Các tệp dưới đây được liên kết trực tiếp từ tài liệu này. Hãy đọc chúng nếu cần thêm ngữ cảnh.

- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/adding-platform-adapters](./docs-zh-Hans-developer-guide-adding-platform-adapters.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/adding-providers](./docs-zh-Hans-developer-guide-adding-providers.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/browser-provider-plugin](./docs-zh-Hans-developer-guide-browser-provider-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/context-engine-plugin](./docs-zh-Hans-developer-guide-context-engine-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/image-gen-provider-plugin](./docs-zh-Hans-developer-guide-image-gen-provider-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/memory-provider-plugin](./docs-zh-Hans-developer-guide-memory-provider-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/model-provider-plugin](./docs-zh-Hans-developer-guide-model-provider-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/secret-source-plugin](./docs-zh-Hans-developer-guide-secret-source-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin](./docs-zh-Hans-developer-guide-video-gen-provider-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/web-search-provider-plugin](./docs-zh-Hans-developer-guide-web-search-provider-plugin.md)
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/nix-setup](./docs-zh-Hans-getting-started-nix-setup.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/web-dashboard](./docs-zh-Hans-user-guide-features-web-dashboard.md)
