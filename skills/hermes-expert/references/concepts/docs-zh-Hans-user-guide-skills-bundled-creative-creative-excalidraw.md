# Excalidraw — 手绘风格 Excalidraw JSON 图表（架构图、流程图、时序图） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-excalidraw](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-excalidraw)
...
### 上传以获取可分享链接[​](#上传以获取可分享链接 "上传以获取可分享链接的直接链接")
...
## 元素格式参考[​](#元素格式参考 "元素格式参考的直接链接")

### 必填字段（所有元素）[​](#必填字段所有元素 "必填字段（所有元素）的直接链接")

`type`、`id`（唯一字符串）、`x`、`y`、`width`、`height`

### 默认值（可省略——会自动应用）[​](#默认值可省略会自动应用 "默认值（可省略——会自动应用）的直接链接")
...
### 箭头绑定（将箭头连接到形状）[​](#箭头绑定将箭头连接到形状 "箭头绑定（将箭头连接到形状）的直接链接")
...
### 绘制顺序（z 轴顺序）[​](#绘制顺序z-轴顺序 "绘制顺序（z 轴顺序）的直接链接")

* 数组顺序 = z 轴顺序（第一个 = 最底层，最后一个 = 最顶层）
* 按顺序逐步输出：背景区域 → 形状 → 其绑定文本 → 其箭头 → 下一个形状
* 错误做法：所有矩形，然后所有文本，然后所有箭头
* 正确做法：bg\_zone → shape1 → text\_for\_shape1 → arrow1 → arrow\_label\_text → shape2 → text\_for\_shape2 → ...
* 始终将绑定文本元素紧接在其容器形状之后
...
* [工作流程](#工作流程)
  + [保存图表](#保存图表)
  + [上传以获取可分享链接](#上传以获取可分享链接)
* [元素格式参考](#元素格式参考)
  + [必填字段（所有元素）](#必填字段所有元素)
  + [默认值（可省略——会自动应用）](#默认值可省略会自动应用)
  + [元素类型](#元素类型)
  + [箭头绑定（将箭头连接到形状）](#箭头绑定将箭头连接到形状)
  + [绘制顺序（z 轴顺序）](#绘制顺序z-轴顺序)
  + [尺寸规范](#尺寸规范)
  + [颜色调色板](#颜色调色板)
  + [使用技巧](#使用技巧)