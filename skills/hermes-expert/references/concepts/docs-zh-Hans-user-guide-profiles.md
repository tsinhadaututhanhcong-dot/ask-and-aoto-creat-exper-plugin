# Profiles：运行多个 Agent | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/profiles](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/profiles)

本页总览

在同一台机器上运行多个独立的 Hermes agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。

## 什么是 profile？[​](#什么是-profile "什么是 profile？的直接链接")
...
## 创建 profile[​](#创建-profile "创建 profile的直接链接")

### 空白 profile[​](#空白-profile "空白 profile的直接链接")
...
你也可以稍后通过 `hermes profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](/docs/zh-Hans/user-guide/features/kanban#auto-vs-manual-orchestration)。

### 仅克隆配置（`--clone`）[​](#仅克隆配置--clone "仅克隆配置--clone的直接链接")
...
### 克隆全部内容（`--clone-all`）[​](#克隆全部内容--clone-all "克隆全部内容--clone-all的直接链接")
...
### 从指定 profile 克隆[​](#从指定-profile-克隆 "从指定 profile 克隆的直接链接")
...
启用 Honcho 后，克隆操作会自动为新 profile 创建专属 AI 对等体，同时共享同一用户工作区。每个 profile 构建各自的观察记录和身份标识。详见 [Honcho——多 agent / Profiles](/docs/zh-Hans/user-guide/features/memory-providers#honcho)。

## 使用 profile[​](#使用-profile "使用 profile的直接链接")
...
### 粘性默认值（`hermes profile use`）[​](#粘性默认值hermes-profile-use "粘性默认值hermes-profile-use的直接链接")
...
### 了解当前所在 profile[​](#了解当前所在-profile "了解当前所在 profile的直接链接")
...
## Profile vs 工作区 vs 沙箱[​](#profile-vs-工作区-vs-沙箱 "Profile vs 工作区 vs 沙箱的直接链接")
...
## 运行 gateway[​](#运行-gateway "运行 gateway的直接链接")
...
### 不同的 bot token[​](#不同的-bot-token "不同的 bot token的直接链接")
...
## 管理 profile[​](#管理-profile "管理 profile的直接链接")
...
## 删除 profile[​](#删除-profile "删除 profile的直接链接")
...
## 将 profile 作为发行版共享[​](#将-profile-作为发行版共享 "将 profile 作为发行版共享的直接链接")
...
完整指南请参阅 **[Profile 发行版：共享完整 Agent](/docs/zh-Hans/user-guide/profile-distributions)**——包括编写、发布、更新语义、安全模型和使用场景。

* [什么是 profile？](#什么是-profile)
* [快速开始](#快速开始)
* [创建 profile](#创建-profile)
  + [空白 profile](#空白-profile)
  + [仅克隆配置（`--clone`）](#仅克隆配置--clone)
  + [克隆全部内容（`--clone-all`）](#克隆全部内容--clone-all)
  + [从指定 profile 克隆](#从指定-profile-克隆)
* [使用 profile](#使用-profile)
  + [命令别名](#命令别名)
  + [`-p` 标志](#-p-标志)
  + [粘性默认值（`hermes profile use`）](#粘性默认值hermes-profile-use)
  + [了解当前所在 profile](#了解当前所在-profile)
* [Profile vs 工作区 vs 沙箱](#profile-vs-工作区-vs-沙箱)
* [运行 gateway](#运行-gateway)
  + [不同的 bot token](#不同的-bot-token)
  + [安全性：token 锁](#安全性token-锁)
  + [持久化服务](#持久化服务)
...
* [Tab 补全](#tab-补全)
* [工作原理](#工作原理)
* [将 profile 作为发行版共享](#将-profile-作为发行版共享)

## Related Files
> **LLM Navigation:** Các tệp dưới đây được liên kết trực tiếp từ tài liệu này. Hãy đọc chúng nếu cần thêm ngữ cảnh.

- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/docker](./docs-zh-Hans-user-guide-docker.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/profile-distributions](./docs-zh-Hans-user-guide-profile-distributions.md)
