# Pinggy Tunnel — 通过 Pinggy 实现零安装 SSH localhost 隧道 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/devops/devops-pinggy-tunnel](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/devops/devops-pinggy-tunnel)
...
| 相关 skill | `cloudflared-quick-tunnel`, [`webhook-subscriptions`](/docs/zh-Hans/user-guide/skills/bundled/devops/devops-webhook-subscriptions) |
...
## 操作流程——启动隧道并获取 URL[​](#操作流程启动隧道并获取-url "操作流程——启动隧道并获取 URL的直接链接")

模型应使用 `terminal` 工具。隧道在共享期间必须保持存活，因此以后台进程方式运行，并从 stdout 解析公共 URL。

### 1. 确认本地源服务已启动[​](#1-确认本地源服务已启动 "1. 确认本地源服务已启动的直接链接")
...
### 2. 以后台进程方式启动隧道[​](#2-以后台进程方式启动隧道 "2. 以后台进程方式启动隧道的直接链接")
...
### 3. 从日志中解析 URL[​](#3-从日志中解析-url "3. 从日志中解析 URL的直接链接")
...
如果有来自 `terminal(background=True)` 的 session\_id，优先使用 `process(action='kill', session_id=...)`。

## 通过用户名关键字进行访问控制[​](#通过用户名关键字进行访问控制 "通过用户名关键字进行访问控制的直接链接")
...
## Web 调试器（可选）[​](#web-调试器可选 "Web 调试器（可选）的直接链接")
...
### 配方 1——接收 webhook 回调[​](#配方-1接收-webhook-回调 "配方 1——接收 webhook 回调的直接链接")
...
### 配方 2——通过 HTTP/SSE 暴露 MCP 服务器[​](#配方-2通过-httpsse-暴露-mcp-服务器 "配方 2——通过 HTTP/SSE 暴露 MCP 服务器的直接链接")
...
### 配方 3——暴露本地 LLM 端点（Ollama / vLLM / llama.cpp）[​](#配方-3暴露本地-llm-端点ollama--vllm--llamacpp "配方 3——暴露本地 LLM 端点（Ollama / vLLM / llama.cpp）的直接链接")
...
### 配方 4——用一次性密码共享开发服务器[​](#配方-4用一次性密码共享开发服务器 "配方 4——用一次性密码共享开发服务器的直接链接")
...
* [前提条件](#前提条件)
* [快速参考](#快速参考)
* [操作流程——启动隧道并获取 URL](#操作流程启动隧道并获取-url)
  + [1. 确认本地源服务已启动](#1-确认本地源服务已启动)
  + [2. 以后台进程方式启动隧道](#2-以后台进程方式启动隧道)
  + [3. 从日志中解析 URL](#3-从日志中解析-url)
  + [4. 验证](#4-验证)
  + [5. 关闭隧道](#5-关闭隧道)
* [通过用户名关键字进行访问控制](#通过用户名关键字进行访问控制)
* [Web 调试器（可选）](#web-调试器可选)
* [注意事项](#注意事项)
* [示例配方](#示例配方)
  + [配方 1——接收 webhook 回调](#配方-1接收-webhook-回调)
  + [配方 2——通过 HTTP/SSE 暴露 MCP 服务器](#配方-2通过-httpsse-暴露-mcp-服务器)
  + [配方 3——暴露本地 LLM 端点（Ollama / vLLM / llama.cpp）](#配方-3暴露本地-llm-端点ollama--vllm--llamacpp)
  + [配方 4——用一次性密码共享开发服务器](#配方-4用一次性密码共享开发服务器)
* [验证](#验证)