# 斜杠命令参考 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/slash-commands](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/slash-commands)
...
## 权限与管理员/用户分级[​](#权限与管理员用户分级 "权限与管理员/用户分级的直接链接")
...
* [Telegram](/docs/zh-Hans/user-guide/messaging/telegram#slash-command-access-control)
* [Discord](/docs/zh-Hans/user-guide/messaging/discord)
* [Slack](/docs/zh-Hans/user-guide/messaging/slack)
* [Matrix](/docs/zh-Hans/user-guide/messaging/matrix)
* [Mattermost](/docs/zh-Hans/user-guide/messaging/mattermost)
* [Signal](/docs/zh-Hans/user-guide/messaging/signal)

如果某个作用域未设置 `allow_admin_from`，该作用域将保持不受限的向后兼容模式——所有允许的用户均可运行所有命令。

## 交互式 CLI 斜杠命令[​](#交互式-cli-斜杠命令 "交互式 CLI 斜杠命令的直接链接")
...
| `/goal <text>` | 设置一个持续目标，Hermes 将跨轮次持续推进——这是我们对 Ralph loop 的实现。每轮结束后，辅助裁判模型会判断目标是否完成；若未完成，Hermes 自动继续。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。预算默认为 20 轮（`goals.max_turns`）；任何真实用户消息都会抢占继续循环，状态在 `/resume` 后保留。完整说明见 [持续目标](/docs/zh-Hans/user-guide/features/goals)。 |
...
| `/background <prompt>`（别名：`/bg`、`/btw`） | 在独立的后台会话中运行 prompt。agent 独立处理你的 prompt——当前会话保持空闲可继续其他工作。任务完成后结果以面板形式显示。见 [CLI 后台会话](/docs/zh-Hans/user-guide/cli#background-sessions)。 |
| `/branch [name]`（别名：`/fork`） | 分支当前会话（探索不同路径） |
| `/handoff <platform>` | **仅限 CLI。** 将当前会话移交给消息平台（Telegram、Discord、Slack、WhatsApp、Signal、Matrix）。gateway 立即接管，在支持线程的平台上创建新线程（Telegram 话题、Discord 文字频道线程、Slack 消息锚定线程），将目标重新绑定到你的 CLI session\_id 以重放完整的角色感知转录，并伪造一条合成用户轮次让 agent 确认已在新位置工作。成功后 CLI 干净退出并提示 `/resume`；随时可用 `/resume <title>` 在本地恢复。轮次进行中拒绝执行。需要 gateway 正在运行且目标平台已配置 home 频道（从目标聊天中执行 `/sethome`）。见 [跨平台移交](/docs/zh-Hans/user-guide/sessions#cross-platform-handoff)。 |
...
| `/codex-runtime [auto|codex_app_server|on|off]` | 切换 OpenAI/Codex 模型的可选 [Codex app-server runtime](/docs/zh-Hans/user-guide/features/codex-app-server-runtime)。`auto`（默认）使用 Hermes 标准 chat completions；`codex_app_server` 将轮次交给 `codex app-server` 子进程，支持原生 shell、apply\_patch、ChatGPT 订阅认证和迁移的 Codex 插件。下次会话生效。 |
...
| `/memory [pending|approve|reject|approval]` | 审核由写入审批门控（`memory.write_approval`）暂存的待处理 memory 写入，并切换该门控。见 [Memory 功能](/docs/zh-Hans/user-guide/features/memory)。 |
| `/bundles` | 列出已配置的 skill bundle——即一次预加载多个 skill 的 `/<name>` 斜杠别名。在 `~/.hermes/config.yaml` 的 `bundles:` 下配置。见 [Skills 功能](/docs/zh-Hans/user-guide/features/skills)。 |
...
### 动态 CLI 斜杠命令[​](#动态-cli-斜杠命令 "动态 CLI 斜杠命令的直接链接")
...
## 消息平台斜杠命令[​](#消息平台斜杠命令 "消息平台斜杠命令的直接链接")
...
| `/codex-runtime [auto|codex_app_server|on|off]` | 切换可选的 [Codex app-server runtime](/docs/zh-Hans/user-guide/features/codex-app-server-runtime)。持久化到 config.yaml 中的 `model.openai_runtime` 并驱逐缓存的 agent，使下一条消息使用新 runtime。下次会话生效。 |
...
| `/topic [off|help|session-id]` | **仅限 Telegram DM。** 管理用户自主的多会话话题模式。`/topic` 启用或显示状态；`/topic off` 禁用并清除绑定；`/topic help` 显示用法；在话题中执行 `/topic <session-id>` 可恢复之前的会话。见 [多会话 DM 模式](/docs/zh-Hans/user-guide/messaging/telegram#multi-session-dm-mode-topic)。 |
...
| `/background <prompt>` | 在独立的后台会话中运行 prompt。任务完成后结果投递回同一聊天。见 [消息平台后台会话](/docs/zh-Hans/user-guide/messaging/#background-sessions)。 |
| `/queue <prompt>`（别名：`/q`） | 将 prompt 加入队列等待下一轮处理，不中断当前轮次。 |
| `/steer <prompt>` | 在下一次工具调用后注入一条消息，不中断——模型在下一次迭代时获取，而非作为新轮次。 |
| `/goal <text>` | 设置一个持续目标，Hermes 将跨轮次持续推进——这是我们对 Ralph loop 的实现。裁判模型在每轮后检查；若未完成，Hermes 自动继续，直到完成、你暂停/清除，或达到轮次预算（默认 20）。子命令：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。agent 运行中可安全执行 status/pause/clear；设置新目标需先执行 `/stop`。见 [持续目标](/docs/zh-Hans/user-guide/features/goals)。 |
...
| `/memory [pending|approve|reject|approval]` | 审核由写入审批门控（`memory.write_approval`）暂存的待处理 memory 写入——可直接在聊天中批准或拒绝——并通过 `/memory approval on|off` 切换门控。见 [Memory 功能](/docs/zh-Hans/user-guide/features/memory)。 |
...
| `/kanban <action>` | 从聊天中操作多 profile、多项目协作看板——参数与 CLI 完全一致。绕过运行中 agent 的保护，因此 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等均可在轮次进行中使用。`/kanban create …` 会自动将发起聊天订阅到新任务的终态事件。见 [Kanban 斜杠命令](/docs/zh-Hans/user-guide/features/kanban#kanban-slash-command)。 |
...
## 破坏性命令的确认提示[​](#破坏性命令的确认提示 "破坏性命令的确认提示的直接链接")
...
在 `~/.hermes/config.yaml` 中设置 `approvals.destructive_slash_confirm: false` 可全局禁用提示；设置回 `true` 可重新启用。背景说明见 [安全——破坏性斜杠命令确认](/docs/zh-Hans/user-guide/security#dangerous-command-approval)。

* [权限与管理员/用户分级](#权限与管理员用户分级)
* [交互式 CLI 斜杠命令](#交互式-cli-斜杠命令)
  + [会话](#会话)
  + [配置](#配置)
  + [工具与 Skill](#工具与-skill)
  + [信息](#信息)
  + [退出](#退出)
  + [动态 CLI 斜杠命令](#动态-cli-斜杠命令)
  + [快捷命令](#快捷命令)
  + [自定义模型别名](#自定义模型别名)
  + [别名解析](#别名解析)
* [消息平台斜杠命令](#消息平台斜杠命令)
* [注意事项](#注意事项)
* [破坏性命令的确认提示](#破坏性命令的确认提示)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/goals](./docs-zh-Hans-user-guide-features-goals.md)
