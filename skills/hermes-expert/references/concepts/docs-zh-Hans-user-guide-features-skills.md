# Skills 系统 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/skills](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/skills)
...
* [捆绑 Skills 目录](/docs/zh-Hans/reference/skills-catalog)
* [官方可选 Skills 目录](/docs/zh-Hans/reference/optional-skills-catalog)

## 使用 Skills[​](#使用-skills "使用 Skills的直接链接")
...
## SKILL.md 格式[​](#skillmd-格式 "SKILL.md 格式的直接链接")
...
### 平台特定 Skills[​](#平台特定-skills "平台特定 Skills的直接链接")
...
## Skill 输出与媒体传递[​](#skill-输出与媒体传递 "Skill 输出与媒体传递的直接链接")
...
### 强制文档式传递：`[[as_document]]`[​](#强制文档式传递as_document "强制文档式传递as_document的直接链接")
...
### 条件激活（Fallback Skills）[​](#条件激活fallback-skills "条件激活（Fallback Skills）的直接链接")
...
## 加载时的安全设置[​](#加载时的安全设置 "加载时的安全设置的直接链接")
...
一旦设置，声明的环境变量会**自动传递**到 `execute_code` 和 `terminal` 沙箱——skill 的脚本可以直接使用 `$TENOR_API_KEY`。对于非 skill 的环境变量，使用 `terminal.env_passthrough` 配置选项。详情参见[环境变量传递](/docs/zh-Hans/user-guide/security#environment-variable-passthrough)。

### Skill 配置设置[​](#skill-配置设置 "Skill 配置设置的直接链接")
...
详情参见 [Skill 设置](/docs/zh-Hans/user-guide/configuration#skill-settings) 和[创建 Skills——配置设置](/docs/zh-Hans/developer-guide/creating-skills#config-settings-configyaml)。

## Skill 目录结构[​](#skill-目录结构 "Skill 目录结构的直接链接")
...
## 外部 Skill 目录[​](#外部-skill-目录 "外部 Skill 目录的直接链接")
...
## Skill 捆绑包[​](#skill-捆绑包 "Skill 捆绑包的直接链接")
...
### 捆绑包优于逐个手动安装 skill 的场景[​](#捆绑包优于逐个手动安装-skill-的场景 "捆绑包优于逐个手动安装 skill 的场景的直接链接")
...
## Agent 管理的 Skills（skill\_manage 工具）[​](#agent-管理的-skillsskill_manage-工具 "Agent 管理的 Skills（skill_manage 工具）的直接链接")

agent 可以通过 `skill_manage` 工具创建、更新和删除自己的 skills。这是 agent 的**程序性记忆**——当它找到一个非平凡的工作流时，它会将该方法保存为 skill 以供将来复用。

### Agent 创建 Skills 的时机[​](#agent-创建-skills-的时机 "Agent 创建 Skills 的时机的直接链接")
...
## Skills Hub[​](#skills-hub "Skills Hub的直接链接")
...
### 支持的 hub 来源[​](#支持的-hub-来源 "支持的 hub 来源的直接链接")
...
### 集成的 hub 和注册表[​](#集成的-hub-和注册表 "集成的 hub 和注册表的直接链接")

Hermes 目前与以下 skills 生态系统和发现来源集成：

#### 1. 官方可选 skills（`official`）[​](#1-官方可选-skillsofficial "1-官方可选-skillsofficial的直接链接")

这些 skills 在 Hermes 仓库中维护，以内置信任级别安装。

* 目录：[官方可选 Skills 目录](/docs/zh-Hans/reference/optional-skills-catalog)
...
#### 2. skills.sh（`skills-sh`）[​](#2-skillsshskills-sh "2-skillsshskills-sh的直接链接")
...
#### 3. Well-known skill 端点（`well-known`）[​](#3-well-known-skill-端点well-known "3-well-known-skill-端点well-known的直接链接")
...
#### 4. 直接 GitHub skills（`github`）[​](#4-直接-github-skillsgithub "4-直接-github-skillsgithub的直接链接")
...
#### 5. ClawHub（`clawhub`）[​](#5-clawhubclawhub "5-clawhubclawhub的直接链接")
...
#### 6. Claude 市场式仓库（`claude-marketplace`）[​](#6-claude-市场式仓库claude-marketplace "6-claude-市场式仓库claude-marketplace的直接链接")
...
#### 7. LobeHub（`lobehub`）[​](#7-lobehublobehub "7-lobehublobehub的直接链接")
...
#### 8. browse.sh（`browse-sh`）[​](#8-browseshbrowse-sh "8-browseshbrowse-sh的直接链接")
...
#### 9. 直接 URL（`url`）[​](#9-直接-urlurl "9-直接-urlurl的直接链接")
...
### 安全扫描与 `--force`[​](#安全扫描与---force "安全扫描与---force的直接链接")
...
### 发布自定义 skill tap[​](#发布自定义-skill-tap "发布自定义 skill tap的直接链接")
...
#### 最小 tap 示例[​](#最小-tap-示例 "最小 tap 示例的直接链接")
...
#### 直接安装单个 skills（无需添加 tap）[​](#直接安装单个-skills无需添加-tap "直接安装单个 skills（无需添加 tap）的直接链接")
...
#### tap 的信任级别[​](#tap-的信任级别 "tap 的信任级别的直接链接")
...
## 捆绑 skill 更新（`hermes skills reset`）[​](#捆绑-skill-更新hermes-skills-reset "捆绑-skill-更新hermes-skills-reset的直接链接")
...
### 斜杠命令（在聊天中）[​](#斜杠命令在聊天中 "斜杠命令（在聊天中）的直接链接")
...
* [SKILL.md 格式](#skillmd-格式)
  + [平台特定 Skills](#平台特定-skills)
* [Skill 输出与媒体传递](#skill-输出与媒体传递)
  + [强制文档式传递：`[[as_document]]`](#强制文档式传递as_document)
  + [条件激活（Fallback Skills）](#条件激活fallback-skills)
* [加载时的安全设置](#加载时的安全设置)
  + [Skill 配置设置](#skill-配置设置)
...
* [Skill 捆绑包](#skill-捆绑包)
  ...
  + [管理捆绑包](#管理捆绑包)
  + [行为](#行为)
  + [捆绑包优于逐个手动安装 skill 的场景](#捆绑包优于逐个手动安装-skill-的场景)
* [Agent 管理的 Skills（skill\_manage 工具）](#agent-管理的-skillsskill_manage-工具)
  + [Agent 创建 Skills 的时机](#agent-创建-skills-的时机)
  + [操作](#操作)
* [Skills Hub](#skills-hub)
  + [常用命令](#常用命令)
  + [支持的 hub 来源](#支持的-hub-来源)
  + [集成的 hub 和注册表](#集成的-hub-和注册表)
  + [安全扫描与 `--force`](#安全扫描与---force)
  + [信任级别](#信任级别)
  + [更新生命周期](#更新生命周期)
  + [发布自定义 skill tap](#发布自定义-skill-tap)
* [捆绑 skill 更新（`hermes skills reset`）](#捆绑-skill-更新hermes-skills-reset)
  + [斜杠命令（在聊天中）](#斜杠命令在聊天中)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/optional-skills-catalog](./docs-zh-Hans-reference-optional-skills-catalog.md)
