# CLI 界面 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/cli](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/cli)
...
Hermes 还提供了一个现代 TUI，支持模态覆盖层、鼠标选择和非阻塞输入。使用 `hermes --tui` 启动——参见 [TUI](/docs/zh-Hans/user-guide/tui) 指南。

## 运行 CLI[​](#运行-cli "运行 CLI的直接链接")
...
![Hermes CLI 布局的风格化预览，展示了横幅、对话区域和固定输入提示符。](/img/docs/cli-layout.svg)
...
### 会话恢复显示[​](#会话恢复显示 "会话恢复显示的直接链接")

恢复之前的会话时（`hermes -c` 或 `hermes --resume <id>`），横幅与输入提示符之间会出现一个"Previous Conversation"面板，显示对话历史的简洁摘要。详情及配置说明参见[会话——恢复时的对话摘要](/docs/zh-Hans/user-guide/sessions#conversation-recap-on-resume)。
...
完整的内置 CLI 和消息列表，参见[斜杠命令参考](/docs/zh-Hans/reference/slash-commands)。

语音模式的设置、提供商、静音调节以及消息/Discord 语音用法，参见[语音模式](/docs/zh-Hans/user-guide/features/voice-mode)。
...
然后在任意聊天中输入 `/status`、`/gpu` 或 `/restart`。更多示例参见[配置指南](/docs/zh-Hans/user-guide/configuration#quick-commands)。

## 启动时预加载 Skill[​](#启动时预加载-skill "启动时预加载 Skill的直接链接")
...
## Skill 斜杠命令[​](#skill-斜杠命令 "Skill 斜杠命令的直接链接")
...
### Shift+Enter 兼容性[​](#shiftenter-兼容性 "Shift+Enter 兼容性的直接链接")
...
使用 `/verbose` 循环切换显示模式：`off → new → all → verbose`。该命令也可为消息平台启用——参见[配置](/docs/zh-Hans/user-guide/configuration#display-settings)。

### 工具预览长度[​](#工具预览长度 "工具预览长度的直接链接")
...
* [斜杠命令](#斜杠命令)
* [快捷命令](#快捷命令)
* [启动时预加载 Skill](#启动时预加载-skill)
* [Skill 斜杠命令](#skill-斜杠命令)
* [人格设定](#人格设定)
* [多行输入](#多行输入)
  + [Shift+Enter 兼容性](#shiftenter-兼容性)
* [中断 Agent](#中断-agent)
  + [繁忙输入模式](#繁忙输入模式)
  + [挂起到后台](#挂起到后台)
* [工具进度显示](#工具进度显示)
