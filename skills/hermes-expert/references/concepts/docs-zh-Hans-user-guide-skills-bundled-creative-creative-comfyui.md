...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-comfyui](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-comfyui)
...
| 相关 skill | [`stable-diffusion-image-generation`](/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-stable-diffusion), `image_gen` |
...
## 此 skill 包含的内容[​](#此-skill-包含的内容 "此 skill 包含的内容的直接链接")
...
| `auto_fix_deps.py` | 运行 check\_deps 然后执行 `comfy node install` / `comfy model download` |
...
| `fetch_logs.py` | 拉取指定 prompt\_id 的 traceback / 状态消息 |
...
## 核心工作流[​](#核心工作流 "核心工作流的直接链接")

### 第一步：获取 API 格式的工作流 JSON[​](#第一步获取-api-格式的工作流-json "第一步：获取 API 格式的工作流 JSON的直接链接")
...
### 第二步：查看可控内容[​](#第二步查看可控内容 "第二步：查看可控内容的直接链接")
...
### 第三步：带参数运行[​](#第三步带参数运行 "第三步：带参数运行的直接链接")
...
### 第四步：呈现结果[​](#第四步呈现结果 "第四步：呈现结果的直接链接")
...
### 第零步：询问本地还是云端（始终优先）[​](#第零步询问本地还是云端始终优先 "第零步：询问本地还是云端（始终优先）的直接链接")
...
### 第一步：验证硬件（仅当用户选择本地时）[​](#第一步验证硬件仅当用户选择本地时 "第一步：验证硬件（仅当用户选择本地时）的直接链接")
...
脚本还会显示 `wsl: true`（带 NVIDIA 直通的 WSL2）和 `rosetta: true`（Apple Silicon 上的 x86\_64 Python——必须重新安装为 ARM64）。

如果结果为 `cloud` 但用户想要本地，不要静默继续。逐字显示 `notes` 数组，并询问他们是否要（a）切换至 Cloud 或（b）强制本地安装（在现代模型上会 OOM 或极慢）。

### 选择安装路径[​](#选择安装路径 "选择安装路径的直接链接")
...
### 路径 A：Comfy Cloud（无需本地安装）[​](#路径-acomfy-cloud无需本地安装 "路径 A：Comfy Cloud（无需本地安装）的直接链接")
...
### 路径 B：ComfyUI Desktop（Windows / macOS）[​](#路径-bcomfyui-desktopwindows--macos "路径 B：ComfyUI Desktop（Windows / macOS）的直接链接")
...
### 路径 C：ComfyUI Portable（仅 Windows）[​](#路径-ccomfyui-portable仅-windows "路径 C：ComfyUI Portable（仅 Windows）的直接链接")
...
### 路径 D：comfy-cli（全平台——推荐用于 Agent）[​](#路径-dcomfy-cli全平台推荐用于-agent "路径 D：comfy-cli（全平台——推荐用于 Agent）的直接链接")

官方 CLI 是无头/自动化安装的最佳路径。

**文档：** <https://docs.comfy.org/comfy-cli/getting-started>

#### 安装 comfy-cli[​](#安装-comfy-cli "安装 comfy-cli的直接链接")
...
#### 安装 ComfyUI[​](#安装-comfyui "安装 ComfyUI的直接链接")
...
### 路径 E：手动安装（高级 / 不支持的硬件）[​](#路径-e手动安装高级--不支持的硬件 "路径 E：手动安装（高级 / 不支持的硬件）的直接链接")
...
### 安装后：下载模型[​](#安装后下载模型 "安装后：下载模型的直接链接")
...
### 安装后：安装自定义节点[​](#安装后安装自定义节点 "安装后：安装自定义节点的直接链接")
...
## 图像上传（img2img / Inpainting）[​](#图像上传img2img--inpainting "图像上传（img2img / Inpainting）的直接链接")
...
4. **缺少自定义节点** — "class\_type not found" 表示所需节点未安装。`check_deps.py` 会报告需要安装哪个包；`auto_fix_deps.py` 会自动执行安装。
...
* [快速开始](#快速开始)
  ...
  + [一行健康检查](#一行健康检查)
* [核心工作流](#核心工作流)
  + [第一步：获取 API 格式的工作流 JSON](#第一步获取-api-格式的工作流-json)
  + [第二步：查看可控内容](#第二步查看可控内容)
  + [第三步：带参数运行](#第三步带参数运行)
  + [第四步：呈现结果](#第四步呈现结果)
* [决策树](#决策树)
* [安装与引导](#安装与引导)
  + [第零步：询问本地还是云端（始终优先）](#第零步询问本地还是云端始终优先)
  + [第一步：验证硬件（仅当用户选择本地时）](#第一步验证硬件仅当用户选择本地时)
  + [选择安装路径](#选择安装路径)
  + [路径 A：Comfy Cloud（无需本地安装）](#路径-acomfy-cloud无需本地安装)
  + [路径 B：ComfyUI Desktop（Windows / macOS）](#路径-bcomfyui-desktopwindows--macos)
  + [路径 C：ComfyUI Portable（仅 Windows）](#路径-ccomfyui-portable仅-windows)
  + [路径 D：comfy-cli（全平台——推荐用于 Agent）](#路径-dcomfy-cli全平台推荐用于-agent)
  + [路径 E：手动安装（高级 / 不支持的硬件）](#路径-e手动安装高级--不支持的硬件)
  + [安装后：下载模型](#安装后下载模型)
  + [安装后：安装自定义节点](#安装后安装自定义节点)
  + [安装后：验证](#安装后验证)
* [图像上传（img2img / Inpainting）](#图像上传img2img--inpainting)
* [云端特性](#云端特性)
* [队列与系统管理](#队列与系统管理)
* [常见问题](#常见问题)
* [验证清单](#验证清单)
