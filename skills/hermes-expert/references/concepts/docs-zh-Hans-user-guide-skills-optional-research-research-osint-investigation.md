...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-osint-investigation](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-osint-investigation)
...
| 相关 skill | [`domain-intel`](/docs/zh-Hans/user-guide/skills/optional/research/research-domain-intel), [`arxiv`](/docs/zh-Hans/user-guide/skills/bundled/research/research-arxiv) |
...
* 美国**联邦**竞选财务 — FEC 在此处有意不覆盖（免费 DEMO\_KEY 层级的 API 对临时贡献者姓名查询不可靠）。联邦捐款请直接引导用户访问 [https://www.fec.gov/data/。](https://www.fec.gov/data/%E3%80%82)

## 工作流程[​](#工作流程 "工作流程的直接链接")

Agent 通过 `terminal` 工具运行脚本。`SKILL_DIR` 是存放此 SKILL.md 的目录。

### 1. 确定适用的数据来源[​](#1-确定适用的数据来源 "1. 确定适用的数据来源的直接链接")
...
### 3. 跨来源实体解析[​](#3-跨来源实体解析 "3. 跨来源实体解析的直接链接")
...
### 4. 统计时序关联（可选）[​](#4-统计时序关联可选 "4. 统计时序关联（可选）的直接链接")
...
### 5. 构建调查结果 JSON（证据链）[​](#5-构建调查结果-json证据链 "5. 构建调查结果 JSON（证据链）的直接链接")
...
## 置信度与证据规范[​](#置信度与证据规范 "置信度与证据规范的直接链接")
...
* [工作流程](#工作流程)
  + [1. 确定适用的数据来源](#1-确定适用的数据来源)
  + [2. 获取数据](#2-获取数据)
  + [3. 跨来源实体解析](#3-跨来源实体解析)
  + [4. 统计时序关联（可选）](#4-统计时序关联可选)
  + [5. 构建调查结果 JSON（证据链）](#5-构建调查结果-json证据链)
* [置信度与证据规范](#置信度与证据规范)
* [添加新数据来源](#添加新数据来源)
* [工具及其限制](#工具及其限制)
* [法律说明](#法律说明)
