# 上下文压缩与缓存 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/context-compression-and-caching](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/context-compression-and-caching)
...
## 可插拔上下文引擎[​](#可插拔上下文引擎 "可插拔上下文引擎的直接链接")
...
关于构建上下文引擎插件，请参阅 [Context Engine 插件](/docs/zh-Hans/developer-guide/context-engine-plugin)。

## 双重压缩系统[​](#双重压缩系统 "双重压缩系统的直接链接")
...
### 1. Gateway 会话清理（85% 阈值）[​](#1-gateway-会话清理85-阈值 "1. Gateway 会话清理（85% 阈值）的直接链接")
...
### 2. Agent ContextCompressor（50% 阈值，可配置）[​](#2-agent-contextcompressor50-阈值可配置 "2. Agent ContextCompressor（50% 阈值，可配置）的直接链接")
...
### 计算值（200K 上下文模型，默认参数）[​](#计算值200k-上下文模型默认参数 "计算值（200K 上下文模型，默认参数）的直接链接")
...
### 阶段 1：清除旧工具结果（廉价，无需 LLM 调用）[​](#阶段-1清除旧工具结果廉价无需-llm-调用 "阶段 1：清除旧工具结果（廉价，无需 LLM 调用）的直接链接")
...
### 阶段 2：确定边界[​](#阶段-2确定边界 "阶段 2：确定边界的直接链接")
...
边界对齐以避免拆分 tool\_call/tool\_result 组。`_align_boundary_backward()` 方法会跳过连续的工具结果，找到父级 assistant 消息，保持组的完整性。

### 阶段 3：生成结构化摘要[​](#阶段-3生成结构化摘要 "阶段 3：生成结构化摘要的直接链接")
...
### 阶段 4：组装压缩后的消息[​](#阶段-4组装压缩后的消息 "阶段 4：组装压缩后的消息的直接链接")
...
`_sanitize_tool_pairs()` 清理孤立的 tool\_call/tool\_result 对：

* 引用已删除调用的工具结果 → 删除
* 结果已被删除的工具调用 → 注入存根结果

### 迭代重压缩[​](#迭代重压缩 "迭代重压缩的直接链接")
...
## 压缩前后示例[​](#压缩前后示例 "压缩前后示例的直接链接")

### 压缩前（45 条消息，约 95K token）[​](#压缩前45-条消息约-95k-token "压缩前（45 条消息，约 95K token）的直接链接")
...
### 压缩后（25 条消息，约 45K token）[​](#压缩后25-条消息约-45k-token "压缩后（25 条消息，约 45K token）的直接链接")
...
## Prompt 缓存（Anthropic）[​](#prompt-缓存anthropic "Prompt 缓存（Anthropic）的直接链接")

来源：`agent/prompt_caching.py`

通过缓存对话前缀，在多轮对话中将输入 token 成本降低约 75%。使用 Anthropic 的 `cache_control` 断点。

### 策略：system\_and\_3[​](#策略system_and_3 "策略：system_and_3的直接链接")

Anthropic 每次请求最多允许 4 个 `cache_control` 断点。Hermes 使用"system\_and\_3"策略：
...
### 缓存感知设计模式[​](#缓存感知设计模式 "缓存感知设计模式的直接链接")
...
### 启用 Prompt 缓存[​](#启用-prompt-缓存 "启用 Prompt 缓存的直接链接")
...
* [可插拔上下文引擎](#可插拔上下文引擎)
* [双重压缩系统](#双重压缩系统)
  + [1. Gateway 会话清理（85% 阈值）](#1-gateway-会话清理85-阈值)
  + [2. Agent ContextCompressor（50% 阈值，可配置）](#2-agent-contextcompressor50-阈值可配置)
* [配置](#配置)
  + [参数详情](#参数详情)
  + [计算值（200K 上下文模型，默认参数）](#计算值200k-上下文模型默认参数)
* [压缩算法](#压缩算法)
  + [阶段 1：清除旧工具结果（廉价，无需 LLM 调用）](#阶段-1清除旧工具结果廉价无需-llm-调用)
  + [阶段 2：确定边界](#阶段-2确定边界)
  + [阶段 3：生成结构化摘要](#阶段-3生成结构化摘要)
  + [阶段 4：组装压缩后的消息](#阶段-4组装压缩后的消息)
  + [迭代重压缩](#迭代重压缩)
* [压缩前后示例](#压缩前后示例)
  + [压缩前（45 条消息，约 95K token）](#压缩前45-条消息约-95k-token)
  + [压缩后（25 条消息，约 45K token）](#压缩后25-条消息约-45k-token)
* [Prompt 缓存（Anthropic）](#prompt-缓存anthropic)
  + [策略：system\_and\_3](#策略system_and_3)
  + [工作原理](#工作原理)
  + [缓存感知设计模式](#缓存感知设计模式)
  + [启用 Prompt 缓存](#启用-prompt-缓存)
* [上下文压力警告](#上下文压力警告)
