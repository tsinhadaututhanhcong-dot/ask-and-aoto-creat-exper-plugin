# 将脚本输出推送到消息平台 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/pipe-script-output](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/pipe-script-output)
...
## 实际使用示例[​](#实际使用示例 "实际使用示例的直接链接")

### 监控：内存 / 磁盘告警[​](#监控内存--磁盘告警 "监控：内存 / 磁盘告警的直接链接")
...
### CI / CD：构建与测试结果[​](#ci--cd构建与测试结果 "CI / CD：构建与测试结果的直接链接")
...
### Cron：每日报告[​](#cron每日报告 "Cron：每日报告的直接链接")
...
### 长时任务：完成后推送通知[​](#长时任务完成后推送通知 "长时任务：完成后推送通知的直接链接")
...
### 脚本中使用 `--json` 与 `--quiet`[​](#脚本中使用---json-与---quiet "脚本中使用---json-与---quiet的直接链接")
...
## `hermes send` 需要 gateway 运行吗？[​](#hermes-send-需要-gateway-运行吗 "hermes-send-需要-gateway-运行吗的直接链接")
...
* [用 Cron 自动化一切](/docs/zh-Hans/guides/automate-with-cron) —
  输出自动投递到任意平台的定时任务。
* [Gateway 内部机制](/docs/zh-Hans/developer-guide/gateway-internals) —
  `hermes send` 与 cron 投递共享的投递路由器。
* [消息平台配置](/docs/zh-Hans/user-guide/messaging/) —
  各平台的一次性配置说明。
...
* [消息体解析顺序](#消息体解析顺序)
* [实际使用示例](#实际使用示例)
  + [监控：内存 / 磁盘告警](#监控内存--磁盘告警)
  + [CI / CD：构建与测试结果](#ci--cd构建与测试结果)
  + [Cron：每日报告](#cron每日报告)
  + [长时任务：完成后推送通知](#长时任务完成后推送通知)
  + [脚本中使用 `--json` 与 `--quiet`](#脚本中使用---json-与---quiet)
* [`hermes send` 需要 gateway 运行吗？](#hermes-send-需要-gateway-运行吗)
* [列出与发现目标](#列出与发现目标)
* [与其他方案的对比](#与其他方案的对比)
* [相关文档](#相关文档)
