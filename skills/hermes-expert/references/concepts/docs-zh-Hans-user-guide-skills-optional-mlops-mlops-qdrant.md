# Qdrant Vector Search — 用于 RAG 和语义搜索的高性能向量相似度搜索引擎 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-qdrant](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-qdrant)
...
## 何时使用 Qdrant[​](#何时使用-qdrant "何时使用 Qdrant的直接链接")
...
### Points（点）— 基本数据单元[​](#points点-基本数据单元 "Points（点）— 基本数据单元的直接链接")
...
### Collections（集合）— 向量容器[​](#collections集合-向量容器 "Collections（集合）— 向量容器的直接链接")
...
## RAG 集成[​](#rag-集成 "RAG 集成的直接链接")

### 与 sentence-transformers 集成[​](#与-sentence-transformers-集成 "与 sentence-transformers 集成的直接链接")
...
### 与 LangChain 集成[​](#与-langchain-集成 "与 LangChain 集成的直接链接")
...
### 与 LlamaIndex 集成[​](#与-llamaindex-集成 "与 LlamaIndex 集成的直接链接")
...
## 多向量支持[​](#多向量支持 "多向量支持的直接链接")

### 命名向量（不同 embedding 模型）[​](#命名向量不同-embedding-模型 "命名向量（不同 embedding 模型）的直接链接")
...
### 稀疏向量（BM25、SPLADE）[​](#稀疏向量bm25splade "稀疏向量（BM25、SPLADE）的直接链接")
...
## Payload 索引[​](#payload-索引 "Payload 索引的直接链接")
...
### Qdrant Cloud[​](#qdrant-cloud "Qdrant Cloud的直接链接")
...
* **GitHub**：[https://github.com/qdrant/qdrant（22k+](https://github.com/qdrant/qdrant%EF%BC%8822k+) stars）
...
* [何时使用 Qdrant](#何时使用-qdrant)
* [快速开始](#快速开始)
  + [安装](#安装)
  + [基本用法](#基本用法)
* [核心概念](#核心概念)
  + [Points（点）— 基本数据单元](#points点-基本数据单元)
  + [Collections（集合）— 向量容器](#collections集合-向量容器)
  + [距离度量](#距离度量)
* [搜索操作](#搜索操作)
  + [基本搜索](#基本搜索)
  + [带过滤的搜索](#带过滤的搜索)
  + [批量搜索](#批量搜索)
* [RAG 集成](#rag-集成)
  + [与 sentence-transformers 集成](#与-sentence-transformers-集成)
  + [与 LangChain 集成](#与-langchain-集成)
  + [与 LlamaIndex 集成](#与-llamaindex-集成)
* [多向量支持](#多向量支持)
  + [命名向量（不同 embedding 模型）](#命名向量不同-embedding-模型)
  + [稀疏向量（BM25、SPLADE）](#稀疏向量bm25splade)
* [量化（内存优化）](#量化内存优化)
* [Payload 索引](#payload-索引)
* [生产部署](#生产部署)
