# 从 OpenClaw 迁移 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/migrate-from-openclaw](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/migrate-from-openclaw)
...
### Persona（角色设定）、记忆与指令[​](#persona角色设定记忆与指令 "Persona（角色设定）、记忆与指令的直接链接")
...
### Skills（技能，4 个来源）[​](#skills技能4-个来源 "Skills（技能，4 个来源）的直接链接")
...
### 模型与 Provider 配置[​](#模型与-provider-配置 "模型与 Provider 配置的直接链接")
...
### TTS（文字转语音）[​](#tts文字转语音 "TTS（文字转语音）的直接链接")
...
### 已归档（无对应 Hermes 等效项）[​](#已归档无对应-hermes-等效项 "已归档（无对应 Hermes 等效项）的直接链接")
...
| 插件 | `archive/plugins-config.json` | 参见 [插件指南](/docs/zh-Hans/user-guide/features/hooks) |
...
## API 密钥解析[​](#api-密钥解析 "API 密钥解析的直接链接")
...
### 支持的密钥目标[​](#支持的密钥目标 "支持的密钥目标的直接链接")

`OPENROUTER_API_KEY`、`OPENAI_API_KEY`、`ANTHROPIC_API_KEY`、`DEEPSEEK_API_KEY`、`GEMINI_API_KEY`、`ZAI_API_KEY`、`MINIMAX_API_KEY`、`ELEVENLABS_API_KEY`、`TELEGRAM_BOT_TOKEN`、`VOICE_TOOLS_OPENAI_KEY`

不在此白名单中的密钥一律不会被复制。

## SecretRef 处理[​](#secretref-处理 "SecretRef 处理的直接链接")
...
### "OpenClaw directory not found"[​](#openclaw-directory-not-found "\"OpenClaw directory not found\"的直接链接")

迁移依次检查 `~/.openclaw/`、`~/.clawdbot/`、`~/.moltbot/`。若你的安装路径不同，请使用 `--source /path/to/your/openclaw`。

### "No provider API keys found"[​](#no-provider-api-keys-found "\"No provider API keys found\"的直接链接")
...
### 迁移后 skills 未出现[​](#迁移后-skills-未出现 "迁移后 skills 未出现的直接链接")

导入的 skills 位于 `~/.hermes/skills/openclaw-imports/`。开启新会话后生效，或运行 `/skills` 验证是否已加载。

### TTS 语音未迁移[​](#tts-语音未迁移 "TTS 语音未迁移的直接链接")
...
* [选项](#选项)
* [迁移内容](#迁移内容)
  + [Persona（角色设定）、记忆与指令](#persona角色设定记忆与指令)
  + [Skills（技能，4 个来源）](#skills技能4-个来源)
