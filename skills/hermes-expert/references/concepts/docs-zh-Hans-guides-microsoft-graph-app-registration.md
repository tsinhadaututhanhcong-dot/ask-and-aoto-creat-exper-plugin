# 注册 Microsoft Graph 应用程序 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/microsoft-graph-app-registration](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/microsoft-graph-app-registration)
...
## 步骤 1：创建应用注册[​](#步骤-1创建应用注册 "步骤 1：创建应用注册的直接链接")
...
## 步骤 2：创建客户端密钥[​](#步骤-2创建客户端密钥 "步骤 2：创建客户端密钥的直接链接")
...
## 步骤 3：授予 Graph API 权限[​](#步骤-3授予-graph-api-权限 "步骤 3：授予 Graph API 权限的直接链接")
...
### 转录优先摘要所需权限[​](#转录优先摘要所需权限 "转录优先摘要所需权限的直接链接")
...
### 录制回退所需权限（当转录不可用时）[​](#录制回退所需权限当转录不可用时 "录制回退所需权限（当转录不可用时）的直接链接")
...
### 出站摘要投递所需权限（仅限 Graph 模式）[​](#出站摘要投递所需权限仅限-graph-模式 "出站摘要投递所需权限（仅限 Graph 模式）的直接链接")
...
## 步骤 5：将凭据写入环境文件[​](#步骤-5将凭据写入环境文件 "步骤 5：将凭据写入环境文件的直接链接")
...
## 步骤 6：验证令牌流程[​](#步骤-6验证令牌流程 "步骤 6：验证令牌流程的直接链接")
...
* [前提条件](#前提条件)
* [步骤 1：创建应用注册](#步骤-1创建应用注册)
* [步骤 2：创建客户端密钥](#步骤-2创建客户端密钥)
* [步骤 3：授予 Graph API 权限](#步骤-3授予-graph-api-权限)
  + [转录优先摘要所需权限](#转录优先摘要所需权限)
  + [录制回退所需权限（当转录不可用时）](#录制回退所需权限当转录不可用时)
  + [出站摘要投递所需权限（仅限 Graph 模式）](#出站摘要投递所需权限仅限-graph-模式)
  + [不推荐的权限](#不推荐的权限)
* [步骤 4：（推荐）通过应用程序访问策略限定应用范围](#步骤-4推荐通过应用程序访问策略限定应用范围)
* [步骤 5：将凭据写入环境文件](#步骤-5将凭据写入环境文件)
* [步骤 6：验证令牌流程](#步骤-6验证令牌流程)
* [轮换客户端密钥](#轮换客户端密钥)
* [后续步骤](#后续步骤)