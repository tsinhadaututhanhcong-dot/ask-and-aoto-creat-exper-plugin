# 委托与并行工作 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/delegation-patterns](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/delegation-patterns)

本页总览

Hermes 可以生成隔离的子代理来并行处理任务。每个子代理拥有独立的对话、终端会话和工具集。只有最终摘要会返回——中间工具调用不会进入你的上下文窗口。

完整功能参考，请参阅[子代理委托](/docs/zh-Hans/user-guide/features/delegation)。
...
## 模式：比较备选方案[​](#模式比较备选方案 "模式：比较备选方案的直接链接")
...
## 模式：多文件重构[​](#模式多文件重构 "模式：多文件重构的直接链接")
...
## 模式：先收集后分析[​](#模式先收集后分析 "模式：先收集后分析的直接链接")
...
## 工具集选择[​](#工具集选择 "工具集选择的直接链接")

根据子代理的需求选择工具集：

| 任务类型 | 工具集 | 原因 |
| --- | --- | --- |
| 网络研究 | `["web"]` | 仅 web\_search + web\_extract |
...
### 调整并发数与深度[​](#调整并发数与深度 "调整并发数与深度的直接链接")
...
**目标要具体。** "修复 bug"过于模糊。"修复 api/handlers.py 第 47 行的 TypeError，该错误由 parse\_body() 向 process\_request() 返回 None 引起"才能给子代理足够的信息。
...
*完整的委托参考——所有参数、ACP 集成和高级配置——请参阅[子代理委托](/docs/zh-Hans/user-guide/features/delegation)。*
