# 对抗性 UX 测试 — 扮演产品最难搞的技术抵触用户 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/dogfood/dogfood-adversarial-ux-test](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/dogfood/dogfood-adversarial-ux-test)
...
| 相关 skill | [`dogfood`](/docs/zh-Hans/user-guide/skills/bundled/dogfood/dogfood-dogfood) |
...
## 第一步：定义 Persona[​](#第一步定义-persona "第一步：定义 Persona的直接链接")
...
### 好的 Persona 示例[​](#好的-persona-示例 "好的 Persona 示例的直接链接")

> **"大迈克"麦卡利斯特** — 58 岁的力量与体能教练。只用 WhatsApp，仅此而已。他的"电子表格"是一本纸质笔记本。"如果我 10 秒内搞不明白，我就回去用我的笔记本。"需要记录 25 名球员的训练结果。讨厌小字、术语和密码。

### 差的 Persona 示例[​](#差的-persona-示例 "差的 Persona 示例的直接链接")

> "一个不喜欢这个应用的用户"——太模糊，没有约束，没有声音。

Persona 必须**足够具体，能在 20 分钟的测试中保持角色一致性**。

## 第二步：成为那个混蛋（以 Persona 身份浏览）[​](#第二步成为那个混蛋以-persona-身份浏览 "第二步：成为那个混蛋（以 Persona 身份浏览）的直接链接")
...
## 第三步：发泄（以角色身份写反馈）[​](#第三步发泄以角色身份写反馈 "第三步：发泄（以角色身份写反馈）的直接链接")
...
## 第四步：实用主义过滤器（关键——不可跳过）[​](#第四步实用主义过滤器关键不可跳过 "第四步：实用主义过滤器（关键——不可跳过）的直接链接")
...
## 第五步：创建工单[​](#第五步创建工单 "第五步：创建工单的直接链接")
...
## 各行业 Persona 示例[​](#各行业-persona-示例 "各行业 Persona 示例的直接链接")
...
* [为什么有效](#为什么有效)
* [使用方法](#使用方法)
* [第一步：定义 Persona](#第一步定义-persona)
  + [好的 Persona 示例](#好的-persona-示例)
  + [差的 Persona 示例](#差的-persona-示例)
* [第二步：成为那个混蛋（以 Persona 身份浏览）](#第二步成为那个混蛋以-persona-身份浏览)
* [第三步：发泄（以角色身份写反馈）](#第三步发泄以角色身份写反馈)
* [第四步：实用主义过滤器（关键——不可跳过）](#第四步实用主义过滤器关键不可跳过)
  + [过滤标准](#过滤标准)
* [第五步：创建工单](#第五步创建工单)
* [第六步：报告](#第六步报告)
* [技巧](#技巧)
* [各行业 Persona 示例](#各行业-persona-示例)
* [规则](#规则)
