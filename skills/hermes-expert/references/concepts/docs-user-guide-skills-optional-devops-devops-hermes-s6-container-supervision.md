...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/devops/devops-hermes-s6-container-supervision](https://hermes-agent.nousresearch.com/docs/user-guide/skills/optional/devops/devops-hermes-s6-container-supervision)
...
## Why Architecture B (CMD as main program, not s6-supervised)[​](#why-architecture-b-cmd-as-main-program-not-s6-supervised "Direct link to Why Architecture B (CMD as main program, not s6-supervised)")
...
## Quick recipes[​](#quick-recipes "Direct link to Quick recipes")

### Verify s6 is PID 1 in a running container[​](#verify-s6-is-pid-1-in-a-running-container "Direct link to Verify s6 is PID 1 in a running container")
...
### Inspect a profile gateway service[​](#inspect-a-profile-gateway-service "Direct link to Inspect a profile gateway service")
...
### Bring a service up/down manually[​](#bring-a-service-updown-manually "Direct link to Bring a service up/down manually")
...
### Watch the cont-init reconciler log[​](#watch-the-cont-init-reconciler-log "Direct link to Watch the cont-init reconciler log")
...
### Add a new static service[​](#add-a-new-static-service "Direct link to Add a new static service")
...
### Change the per-profile gateway run command[​](#change-the-per-profile-gateway-run-command "Direct link to Change the per-profile gateway run command")
...
### Run the docker test harness[​](#run-the-docker-test-harness "Direct link to Run the docker test harness")
...
### "command not found" via `docker exec`[​](#command-not-found-via-docker-exec "Direct link to command-not-found-via-docker-exec")
...
### Profile directory ownership[​](#profile-directory-ownership "Direct link to Profile directory ownership")
...
### Files written by `docker exec` are root-owned[​](#files-written-by-docker-exec-are-root-owned "Direct link to files-written-by-docker-exec-are-root-owned")
...
### Service slot exists but s6-svstat says "s6-supervise not running"[​](#service-slot-exists-but-s6-svstat-says-s6-supervise-not-running "Direct link to Service slot exists but s6-svstat says \"s6-supervise not running\"")
...
### Gateway starts then immediately exits (`down (exitcode 1)` in svstat)[​](#gateway-starts-then-immediately-exits-down-exitcode-1-in-svstat "Direct link to gateway-starts-then-immediately-exits-down-exitcode-1-in-svstat")
...
### Reconciler skipped a profile[​](#reconciler-skipped-a-profile "Direct link to Reconciler skipped a profile")
...
### "Help, the container exits 143!"[​](#help-the-container-exits-143 "Direct link to \"Help, the container exits 143!\"")
