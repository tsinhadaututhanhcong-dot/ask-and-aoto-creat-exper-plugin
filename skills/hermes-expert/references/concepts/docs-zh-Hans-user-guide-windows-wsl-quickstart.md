# Windows (WSL2) 指南 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/windows-wsl-quickstart](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/windows-wsl-quickstart)

本页总览

Hermes Agent 现已同时支持原生 Windows 和 WSL2。本页介绍 WSL2 路径；如需原生 PowerShell 安装方式，请参阅专属的 **[Windows（原生）指南](/docs/zh-Hans/user-guide/windows-native)**。
...
## 为什么选择 WSL2（而非原生 Windows）[​](#为什么选择-wsl2而非原生-windows "为什么选择 WSL2（而非原生 Windows）的直接链接")
...
### 发行版选择[​](#发行版选择 "发行版选择的直接链接")

我们以 Ubuntu（LTS）为测试基准。Debian 同样可用。Arch 和 NixOS 也有人在用，但一键安装脚本假设使用基于 Debian 的 `apt` 系统 —— 如需其他路径，请参阅 [Nix 安装指南](/docs/zh-Hans/getting-started/nix-setup)。

### 启用 systemd（推荐）[​](#启用-systemd推荐 "启用 systemd（推荐）的直接链接")
...
### 在 WSL 内安装 Hermes[​](#在-wsl-内安装-hermes "在 WSL 内安装 Hermes的直接链接")
...
安装程序将 WSL2 视为普通 Linux —— 无需任何 WSL 专属配置。完整目录结构请参阅[安装说明](/docs/zh-Hans/getting-started/installation)。

## 文件系统：跨越 Windows ↔ WSL2 边界[​](#文件系统跨越-windows--wsl2-边界 "文件系统：跨越 Windows ↔ WSL2 边界的直接链接")
...
### Hermes 和项目应放在哪里[​](#hermes-和项目应放在哪里 "Hermes 和项目应放在哪里的直接链接")
...
### 在两侧之间传输文件[​](#在两侧之间传输文件 "在两侧之间传输文件的直接链接")
...
### 行尾符、BOM 与 git[​](#行尾符bom-与-git "行尾符、BOM 与 git的直接链接")
...
### "在 WSL 内 clone 还是在 `/mnt/c` 上 clone？"[​](#在-wsl-内-clone-还是在-mntc-上-clone "在-wsl-内-clone-还是在-mntc-上-clone的直接链接")
...
一个例外：**启动 Windows 二进制文件的 MCP bridge。** 如果你通过 `cmd.exe` 使用 `chrome-devtools-mcp`（参见 [MCP 指南：WSL → Windows Chrome](/docs/zh-Hans/guides/use-mcp-with-hermes#wsl2-bridge-hermes-in-wsl-to-windows-chrome)），当 Hermes 的当前工作目录是 `~` 时，Windows 可能会报 `UNC` 警告。此时请从 `/mnt/c/` 下的某个目录启动 Hermes，以便 Windows 进程拥有一个带盘符的工作目录。

## 网络：WSL ↔ Windows[​](#网络wsl--windows "网络：WSL ↔ Windows的直接链接")

WSL2 在轻量级虚拟机中运行，拥有独立的网络栈。这意味着 WSL 内的 `localhost` 与 Windows 上的 `localhost` **并不相同** —— 从网络角度看，它们是两台独立的主机。对于每个服务，你需要确定流量方向，并选择正确的桥接方式。

以下两种情况最为常见。

### 情况一 —— WSL 中的 Hermes 访问 Windows 上的服务[​](#情况一--wsl-中的-hermes-访问-windows-上的服务 "情况一 —— WSL 中的 Hermes 访问 Windows 上的服务的直接链接")

最常见的场景：你在 **Windows 上运行 Ollama、LM Studio 或 llama-server**，而 WSL 内的 Hermes 需要访问它。

此场景的权威说明在 providers 指南中：**[WSL2 本地模型网络配置 →](/docs/zh-Hans/integrations/providers#wsl2-networking-windows-users)**
...
### 情况二 —— Windows（或局域网）上的设备访问 WSL 中的 Hermes[​](#情况二--windows或局域网上的设备访问-wsl-中的-hermes "情况二 —— Windows（或局域网）上的设备访问 WSL 中的 Hermes的直接链接")

这是反向情况，其他地方较少记录，但以下场景需要用到：

* 从 Windows 浏览器使用 Hermes **Web Dashboard**。
* 从 Windows 侧工具使用 **OpenAI 兼容 API 服务器**（当 `API_SERVER_ENABLED=true` 时由 `hermes gateway` 暴露）。参见 [API Server 功能页](/docs/zh-Hans/user-guide/features/api-server)。
* 测试**消息 gateway**（Telegram、Discord 等），平台会向本地 webhook URL 发送请求 —— 通常建议使用 `cloudflared`/`ngrok` 而非原始端口转发。

#### 子情况 2a：从 Windows 宿主机本身访问[​](#子情况-2a从-windows-宿主机本身访问 "子情况 2a：从 Windows 宿主机本身访问的直接链接")
...
#### 子情况 2b：从局域网中的其他设备访问（手机、平板、另一台 PC）[​](#子情况-2b从局域网中的其他设备访问手机平板另一台-pc "子情况 2b：从局域网中的其他设备访问（手机、平板、另一台 PC）的直接链接")
...
对于来自云端消息服务商的 webhook（Telegram `setWebhook`、Slack 事件等），不建议折腾端口转发 —— 请使用 `cloudflared` 隧道。参见 [webhook 指南](/docs/zh-Hans/user-guide/messaging/webhooks)。

## 在 Windows 上长期运行 Hermes 服务[​](#在-windows-上长期运行-hermes-服务 "在 Windows 上长期运行 Hermes 服务的直接链接")

Hermes 的 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway) 和 API 服务器都是长期运行的进程。在 WSL2 中，有以下几种方式保持它们持续运行。

### 在 WSL 内使用 systemd（推荐）[​](#在-wsl-内使用-systemd推荐 "在 WSL 内使用 systemd（推荐）的直接链接")
...
### 让 WSL 在 Windows 登录时自动启动[​](#让-wsl-在-windows-登录时自动启动 "让 WSL 在 Windows 登录时自动启动的直接链接")
...
## GPU 直通（本地模型）[​](#gpu-直通本地模型 "GPU 直通（本地模型）的直接链接")
...
参见 [WSL2 网络配置](/docs/zh-Hans/integrations/providers#wsl2-networking-windows-users)。九成情况是服务绑定在 `127.0.0.1` 上，需要改为 `0.0.0.0`（Ollama：`OLLAMA_HOST=0.0.0.0`），或者缺少防火墙规则。
...
* **[安装说明](/docs/zh-Hans/getting-started/installation)** —— 实际安装步骤（Linux/WSL2/Termux 均使用同一安装程序）。
* **[集成 → Providers → WSL2 网络配置](/docs/zh-Hans/integrations/providers#wsl2-networking-windows-users)** —— 本地模型服务器网络配置的权威深度说明。
* **[MCP 指南 → WSL → Windows Chrome](/docs/zh-Hans/guides/use-mcp-with-hermes#wsl2-bridge-hermes-in-wsl-to-windows-chrome)** —— 从 WSL 中的 Hermes 控制你已登录的 Windows Chrome。
* **[Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)** 和 **[Web Dashboard](/docs/zh-Hans/user-guide/features/web-dashboard)** —— 你最常需要从 WSL 暴露到网络其他部分的长期运行服务。

* [为什么选择 WSL2（而非原生 Windows）](#为什么选择-wsl2而非原生-windows)
* [安装 WSL2](#安装-wsl2)
  + [发行版选择](#发行版选择)
  + [启用 systemd（推荐）](#启用-systemd推荐)
  + [在 WSL 内安装 Hermes](#在-wsl-内安装-hermes)
* [文件系统：跨越 Windows ↔ WSL2 边界](#文件系统跨越-windows--wsl2-边界)
  + [两个方向](#两个方向)
  + [Hermes 和项目应放在哪里](#hermes-和项目应放在哪里)
  + [在两侧之间传输文件](#在两侧之间传输文件)
  + [行尾符、BOM 与 git](#行尾符bom-与-git)
  + ["在 WSL 内 clone 还是在 `/mnt/c` 上 clone？"](#在-wsl-内-clone-还是在-mntc-上-clone)
* [网络：WSL ↔ Windows](#网络wsl--windows)
  + [情况一 —— WSL 中的 Hermes 访问 Windows 上的服务](#情况一--wsl-中的-hermes-访问-windows-上的服务)
  + [情况二 —— Windows（或局域网）上的设备访问 WSL 中的 Hermes](#情况二--windows或局域网上的设备访问-wsl-中的-hermes)
* [在 Windows 上长期运行 Hermes 服务](#在-windows-上长期运行-hermes-服务)
  + [在 WSL 内使用 systemd（推荐）](#在-wsl-内使用-systemd推荐)
  + [让 WSL 在 Windows 登录时自动启动](#让-wsl-在-windows-登录时自动启动)
* [GPU 直通（本地模型）](#gpu-直通本地模型)
* [常见问题](#常见问题)
* [下一步](#下一步)
