# Matrix | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/matrix](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/matrix)
...
### Matrix 中的会话模型[​](#matrix-中的会话模型 "Matrix 中的会话模型的直接链接")
...
## 第一步：创建机器人账户[​](#第一步创建机器人账户 "第一步：创建机器人账户的直接链接")

你需要为机器人准备一个 Matrix 用户账户。有以下几种方式：

### 方式 A：在你的 Homeserver 上注册（推荐）[​](#方式-a在你的-homeserver-上注册推荐 "方式 A：在你的 Homeserver 上注册（推荐）的直接链接")
...
### 方式 B：使用 matrix.org 或其他公共 Homeserver[​](#方式-b使用-matrixorg-或其他公共-homeserver "方式 B：使用 matrix.org 或其他公共 Homeserver的直接链接")

1. 前往 [Element Web](https://app.element.io) 创建新账户。
2. 为机器人选择一个用户名（例如 `hermes-bot`）。

### 方式 C：使用你自己的账户[​](#方式-c使用你自己的账户 "方式 C：使用你自己的账户的直接链接")

你也可以以自己的用户身份运行 Hermes。这意味着机器人以你的名义发帖——适合个人助手场景。

## 第二步：获取访问令牌[​](#第二步获取访问令牌 "第二步：获取访问令牌的直接链接")

Hermes 需要访问令牌（access token）来向 homeserver 进行身份验证。有两种方式：

### 方式 A：访问令牌（推荐）[​](#方式-a访问令牌推荐 "方式 A：访问令牌（推荐）的直接链接")
...
### 方式 B：密码登录[​](#方式-b密码登录 "方式 B：密码登录的直接链接")
...
## 第三步：找到你的 Matrix 用户 ID[​](#第三步找到你的-matrix-用户-id "第三步：找到你的 Matrix 用户 ID的直接链接")
...
## 第四步：配置 Hermes Agent[​](#第四步配置-hermes-agent "第四步：配置 Hermes Agent的直接链接")
...
## 端对端加密（E2EE）[​](#端对端加密e2ee "端对端加密（E2EE）的直接链接")
...
### 交叉签名验证（推荐）[​](#交叉签名验证推荐 "交叉签名验证（推荐）的直接链接")
...
## 房间白名单（`allowed_rooms`）[​](#房间白名单allowed_rooms "房间白名单allowed_rooms的直接链接")
...
另请参阅：[管理员/用户斜杠命令分离](/docs/zh-Hans/reference/slash-commands#permissions-and-adminuser-split)。
...
### 机器人不响应消息[​](#机器人不响应消息 "机器人不响应消息的直接链接")
...
### 机器人加入房间但静默丢弃所有消息（时钟偏差）[​](#机器人加入房间但静默丢弃所有消息时钟偏差 "机器人加入房间但静默丢弃所有消息（时钟偏差）的直接链接")
...
### 启动时出现"身份验证失败"/"whoami 失败"[​](#启动时出现身份验证失败whoami-失败 "启动时出现\"身份验证失败\"/\"whoami 失败\"的直接链接")
...
### "mautrix 未安装"错误[​](#mautrix-未安装错误 "\"mautrix 未安装\"错误的直接链接")
...
### 加密错误/"无法解密事件"[​](#加密错误无法解密事件 "加密错误/\"无法解密事件\"的直接链接")
...
### 从带有 E2EE 的旧版本升级[​](#从带有-e2ee-的旧版本升级 "从带有 E2EE 的旧版本升级的直接链接")
...
## 代理模式（macOS 上的 E2EE）[​](#代理模式macos-上的-e2ee "代理模式（macOS 上的 E2EE）的直接链接")
...
### 第一步：配置主机（macOS）[​](#第一步配置主机macos "第一步：配置主机（macOS）的直接链接")
...
### 第二步：配置 Docker 容器（Linux 虚拟机）[​](#第二步配置-docker-容器linux-虚拟机 "第二步：配置 Docker 容器（Linux 虚拟机）的直接链接")
...
### 第三步：同时启动[​](#第三步同时启动 "第三步：同时启动的直接链接")
...
### 同步问题/机器人落后[​](#同步问题机器人落后 "同步问题/机器人落后的直接链接")
...
### "用户不被允许"/机器人忽略你[​](#用户不被允许机器人忽略你 "\"用户不被允许\"/机器人忽略你的直接链接")
...
* [Hermes 的行为方式](#hermes-的行为方式)
  + [Matrix 中的会话模型](#matrix-中的会话模型)
  + [提及与线程配置](#提及与线程配置)
* [第一步：创建机器人账户](#第一步创建机器人账户)
  + [方式 A：在你的 Homeserver 上注册（推荐）](#方式-a在你的-homeserver-上注册推荐)
  + [方式 B：使用 matrix.org 或其他公共 Homeserver](#方式-b使用-matrixorg-或其他公共-homeserver)
  + [方式 C：使用你自己的账户](#方式-c使用你自己的账户)
* [第二步：获取访问令牌](#第二步获取访问令牌)
  + [方式 A：访问令牌（推荐）](#方式-a访问令牌推荐)
  + [方式 B：密码登录](#方式-b密码登录)
* [第三步：找到你的 Matrix 用户 ID](#第三步找到你的-matrix-用户-id)
* [第四步：配置 Hermes Agent](#第四步配置-hermes-agent)
  + [方式 A：交互式设置（推荐）](#方式-a交互式设置推荐)
  + [方式 B：手动配置](#方式-b手动配置)
  + [启动 Gateway](#启动-gateway)
* [端对端加密（E2EE）](#端对端加密e2ee)
  + [前提条件](#前提条件)
  + [启用 E2EE](#启用-e2ee)
  + [交叉签名验证（推荐）](#交叉签名验证推荐)
* [主房间](#主房间)
  + [使用斜杠命令](#使用斜杠命令)
  + [手动配置](#手动配置)
* [房间白名单（`allowed_rooms`）](#房间白名单allowed_rooms)
* [故障排查](#故障排查)
  + [机器人不响应消息](#机器人不响应消息)
  + [机器人加入房间但静默丢弃所有消息（时钟偏差）](#机器人加入房间但静默丢弃所有消息时钟偏差)
  + [启动时出现"身份验证失败"/"whoami 失败"](#启动时出现身份验证失败whoami-失败)
  + ["mautrix 未安装"错误](#mautrix-未安装错误)
  + [加密错误/"无法解密事件"](#加密错误无法解密事件)
  + [从带有 E2EE 的旧版本升级](#从带有-e2ee-的旧版本升级)
* [代理模式（macOS 上的 E2EE）](#代理模式macos-上的-e2ee)
  + [工作原理](#工作原理)
  + [第一步：配置主机（macOS）](#第一步配置主机macos)
  + [第二步：配置 Docker 容器（Linux 虚拟机）](#第二步配置-docker-容器linux-虚拟机)
  + [第三步：同时启动](#第三步同时启动)
  + [配置参考](#配置参考)
  + [适用于任何平台](#适用于任何平台)
  + [同步问题/机器人落后](#同步问题机器人落后)
  + [机器人离线](#机器人离线)
  + ["用户不被允许"/机器人忽略你](#用户不被允许机器人忽略你)
* [安全](#安全)
* [注意事项](#注意事项)
