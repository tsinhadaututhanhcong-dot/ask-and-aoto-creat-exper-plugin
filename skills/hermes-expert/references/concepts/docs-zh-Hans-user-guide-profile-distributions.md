# Profile 分发：共享完整 Agent | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/profile-distributions](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/profile-distributions)
...
如果说 [profile](/docs/zh-Hans/user-guide/profiles) 是本地 agent，那么分发就是让该 agent 可共享的形式。

## 这意味着什么[​](#这意味着什么 "这意味着什么的直接链接")
...
## 为什么选择 git？[​](#为什么选择-git "为什么选择 git？的直接链接")
...
## 什么时候应该使用分发？[​](#什么时候应该使用分发 "什么时候应该使用分发？的直接链接")
...
* **你只想在自己的机器上备份一个 profile。** 使用 [`hermes profile export` / `import`](/docs/zh-Hans/reference/profile-commands#hermes-profile-export)——那正是这两个命令的用途。
* **你想随 agent 一起共享 API 密钥。** `auth.json` 和 `.env` 被刻意排除在分发之外。每个安装者使用自己的凭据。
* **你想共享记忆 / 会话 / 对话历史。** 这些是用户数据，不是分发内容，永远不会被发送。

## 生命周期：从作者到安装者再到更新[​](#生命周期从作者到安装者再到更新 "生命周期：从作者到安装者再到更新的直接链接")

以下是完整的端到端流程，选择你关心的一侧阅读。

---

## 作者篇：发布分发[​](#作者篇发布分发 "作者篇：发布分发的直接链接")

### 第一步——从一个可用的 profile 开始[​](#第一步从一个可用的-profile-开始 "第一步——从一个可用的 profile 开始的直接链接")
...
### 第二步——添加 `distribution.yaml`[​](#第二步添加-distributionyaml "第二步添加-distributionyaml的直接链接")
...
### 第三步——推送到 git 仓库[​](#第三步推送到-git-仓库 "第三步——推送到 git 仓库的直接链接")
...
### 第四步——为版本发布打标签[​](#第四步为版本发布打标签 "第四步——为版本发布打标签的直接链接")
...
### 分发所有权 vs 用户所有权[​](#分发所有权-vs-用户所有权 "分发所有权 vs 用户所有权的直接链接")
...
## 安装者篇：使用分发[​](#安装者篇使用分发 "安装者篇：使用分发的直接链接")
...
### 覆盖 profile 名称[​](#覆盖-profile-名称 "覆盖 profile 名称的直接链接")
...
## 使用场景与模式[​](#使用场景与模式 "使用场景与模式的直接链接")

### 个人：跨机器同步同一个 agent[​](#个人跨机器同步同一个-agent "个人：跨机器同步同一个 agent的直接链接")
...
### 团队：发布经过审核的内部 agent[​](#团队发布经过审核的内部-agent "团队：发布经过审核的内部 agent的直接链接")
...
### 社区：发布公开 agent[​](#社区发布公开-agent "社区：发布公开 agent的直接链接")
...
### 产品：发布有主见的 agent[​](#产品发布有主见的-agent "产品：发布有主见的 agent的直接链接")
...
### 临时：在共享基础设施上运行一次性脚本[​](#临时在共享基础设施上运行一次性脚本 "临时：在共享基础设施上运行一次性脚本的直接链接")
...
### 在更新时保留本地配置自定义[​](#在更新时保留本地配置自定义 "在更新时保留本地配置自定义的直接链接")
...
### Fork 并自定义[​](#fork-并自定义 "Fork 并自定义的直接链接")
...
## 分发中永远不包含的内容[​](#分发中永远不包含的内容 "分发中永远不包含的内容的直接链接")
...
有关实现细节、精确的 CLI 行为和所有标志，请参阅 [Profile 命令参考](/docs/zh-Hans/reference/profile-commands#distribution-commands)。
...
* [Profiles：运行多个 Agent](/docs/zh-Hans/user-guide/profiles) — 基础概念
* [Profile 命令参考](/docs/zh-Hans/reference/profile-commands) — 每个标志、每个选项
* [`hermes profile export` / `import`](/docs/zh-Hans/reference/profile-commands#hermes-profile-export) — 本地备份 / 恢复（非分发）
* [在 Hermes 中使用 SOUL](/docs/zh-Hans/guides/use-soul-with-hermes) — 编写个性
* [个性与 SOUL](/docs/zh-Hans/user-guide/features/personality) — SOUL 在 agent 中的作用
* [技能目录](/docs/zh-Hans/reference/skills-catalog) — 可打包的技能

* [这意味着什么](#这意味着什么)
* [为什么选择 git？](#为什么选择-git)
* [什么时候应该使用分发？](#什么时候应该使用分发)
* [生命周期：从作者到安装者再到更新](#生命周期从作者到安装者再到更新)
* [作者篇：发布分发](#作者篇发布分发)
  + [第一步——从一个可用的 profile 开始](#第一步从一个可用的-profile-开始)
  + [第二步——添加 `distribution.yaml`](#第二步添加-distributionyaml)
  + [第三步——推送到 git 仓库](#第三步推送到-git-仓库)
  + [第四步——为版本发布打标签](#第四步为版本发布打标签)
  + [仓库结构示例](#仓库结构示例)
  + [分发所有权 vs 用户所有权](#分发所有权-vs-用户所有权)
* [安装者篇：使用分发](#安装者篇使用分发)
  + [安装](#安装)
  + [来源类型](#来源类型)
  + [覆盖 profile 名称](#覆盖-profile-名称)
  + [填写环境变量](#填写环境变量)
  + [查看已安装内容](#查看已安装内容)
  + [更新](#更新)
  + [删除](#删除)
* [使用场景与模式](#使用场景与模式)
  + [个人：跨机器同步同一个 agent](#个人跨机器同步同一个-agent)
  + [团队：发布经过审核的内部 agent](#团队发布经过审核的内部-agent)
  + [社区：发布公开 agent](#社区发布公开-agent)
  + [产品：发布有主见的 agent](#产品发布有主见的-agent)
  + [临时：在共享基础设施上运行一次性脚本](#临时在共享基础设施上运行一次性脚本)
* [实用技巧](#实用技巧)
  + [固定到特定版本](#固定到特定版本)
  + [查看当前版本与最新版本](#查看当前版本与最新版本)
  + [在更新时保留本地配置自定义](#在更新时保留本地配置自定义)
  + [强制全新重装](#强制全新重装)
  + [Fork 并自定义](#fork-并自定义)
  + [推送前测试分发](#推送前测试分发)
* [分发中永远不包含的内容](#分发中永远不包含的内容)
* [安全与信任](#安全与信任)
* [底层实现](#底层实现)
* [另请参阅](#另请参阅)
