# 快速入门 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/quickstart](https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/quickstart)

本页总览

本指南带你从零开始搭建一个能够应对实际使用的 Hermes 环境。完成安装、选择 provider（服务提供商）、验证对话正常运行，并了解出现问题时的处理方法。

## 更喜欢看视频？[​](#更喜欢看视频 "更喜欢看视频？的直接链接")
...
## 1. 安装 Hermes Agent[​](#1-安装-hermes-agent "1. 安装 Hermes Agent的直接链接")

### 在 macOS 或 Windows 上使用 Hermes Desktop 安装器（推荐）[​](#在-macos-或-windows-上使用-hermes-desktop-安装器推荐 "在 macOS 或 Windows 上使用 Hermes Desktop 安装器（推荐）的直接链接")

如需同时安装命令行与桌面应用，请从我们的官网[下载 Hermes Desktop 安装器](https://hermes-agent.nousresearch.com/)并运行。

### 不使用 Hermes Desktop：[​](#不使用-hermes-desktop "不使用 Hermes Desktop：的直接链接")
...
如果你在手机上安装，请参阅专门的 [Termux 指南](/docs/zh-Hans/getting-started/termux)，其中包含经过测试的手动安装步骤、支持的扩展功能以及当前 Android 特有的限制。
...
详细的安装选项、前置条件和故障排查，请参阅 [安装指南](/docs/zh-Hans/getting-started/installation)。

## 2. 选择 Provider[​](#2-选择-provider "2. 选择 Provider的直接链接")
...
一个订阅涵盖 300+ 个模型，以及 [Tool Gateway](/docs/zh-Hans/user-guide/features/tool-gateway)（网页搜索、图像生成、TTS、云端浏览器）。全新安装时：
...
| **AWS Bedrock** | 通过原生 Converse API 使用 Claude、Nova、Llama、DeepSeek | IAM 角色或 `aws configure`（[指南](/docs/zh-Hans/guides/aws-bedrock)） |
...
对于大多数初次使用的用户：选择一个 provider，接受默认值（除非你明确知道为何要修改）。完整的 provider 目录及环境变量和配置步骤请参阅 [Providers](/docs/zh-Hans/integrations/providers) 页面。
...
你可以随时通过 `hermes model` 切换 provider——没有锁定。所有支持的 provider 完整列表及配置详情，请参阅 [AI Providers](/docs/zh-Hans/integrations/providers)。

### 配置的存储方式[​](#配置的存储方式 "配置的存储方式的直接链接")
...
## 3. 运行第一次对话[​](#3-运行第一次对话 "3. 运行第一次对话的直接链接")
...
Hermes 提供两种终端界面：经典的 `prompt_toolkit` CLI，以及更新的 [TUI](/docs/zh-Hans/user-guide/tui)（支持模态覆盖层、鼠标选择和非阻塞输入）。两者共享相同的会话、斜杠命令和配置——分别用 `hermes` 和 `hermes --tui` 试试看。
...
## 4. 验证会话功能[​](#4-验证会话功能 "4. 验证会话功能的直接链接")
...
## 5. 尝试核心功能[​](#5-尝试核心功能 "5. 尝试核心功能的直接链接")
...
## 6. 添加下一层功能[​](#6-添加下一层功能 "6. 添加下一层功能的直接链接")

仅在基础对话正常后进行。按需选择：

### 机器人或共享助手[​](#机器人或共享助手 "机器人或共享助手的直接链接")

```
hermes gateway setup    # 交互式平台配置
```

接入 [Telegram](/docs/zh-Hans/user-guide/messaging/telegram)、[Discord](/docs/zh-Hans/user-guide/messaging/discord)、[Slack](/docs/zh-Hans/user-guide/messaging/slack)、[WhatsApp](/docs/zh-Hans/user-guide/messaging/whatsapp)、[Signal](/docs/zh-Hans/user-guide/messaging/signal)、[Email](/docs/zh-Hans/user-guide/messaging/email)、[Home Assistant](/docs/zh-Hans/user-guide/messaging/homeassistant) 或 [Microsoft Teams](/docs/zh-Hans/user-guide/messaging/teams)。

### 自动化与工具[​](#自动化与工具 "自动化与工具的直接链接")
...
然后在 CLI 中输入：`/voice on`。按 `Ctrl+B` 开始录音。参阅 [语音模式](/docs/zh-Hans/user-guide/features/voice-mode)。

### Skills[​](#skills "Skills的直接链接")
...
### 编辑器集成（ACP）[​](#编辑器集成acp "编辑器集成（ACP）的直接链接")
...
参阅 [ACP 编辑器集成](/docs/zh-Hans/user-guide/features/acp)。
...
* **[CLI 指南](/docs/zh-Hans/user-guide/cli)** — 掌握终端界面
* **[配置](/docs/zh-Hans/user-guide/configuration)** — 自定义你的配置
* **[消息 Gateway](/docs/zh-Hans/user-guide/messaging/)** — 接入 Telegram、Discord、Slack、WhatsApp、Signal、Email、Home Assistant、Teams 等
* **[工具与工具集](/docs/zh-Hans/user-guide/features/tools)** — 探索可用功能
* **[AI Providers](/docs/zh-Hans/integrations/providers)** — 完整 provider 列表及配置详情
* **[Skills 系统](/docs/zh-Hans/user-guide/features/skills)** — 可复用的工作流与知识
* **[技巧与最佳实践](/docs/zh-Hans/guides/tips)** — 高级用户技巧

* [更喜欢看视频？](#更喜欢看视频)
* [适用人群](#适用人群)
* [最快路径](#最快路径)
* [1. 安装 Hermes Agent](#1-安装-hermes-agent)
  + [在 macOS 或 Windows 上使用 Hermes Desktop 安装器（推荐）](#在-macos-或-windows-上使用-hermes-desktop-安装器推荐)
  + [不使用 Hermes Desktop：](#不使用-hermes-desktop)
