# Docker | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/docker](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/docker)
...
2. **Docker 作为终端后端** — agent 在宿主机上运行，但将每条命令在单个持久化 Docker 沙箱容器中执行，该容器在工具调用、`/new` 和子 agent 之间保持存活，直至 Hermes 进程结束（参见 [配置 → Docker 后端](/docs/zh-Hans/user-guide/configuration#docker-backend)）
...
## 以 gateway 模式运行[​](#以-gateway-模式运行 "以 gateway 模式运行的直接链接")
...
端口 8642 暴露 gateway 的 [OpenAI 兼容 API 服务器](/docs/zh-Hans/user-guide/features/api-server)和健康检查端点。如果你只使用聊天平台（Telegram、Discord 等），该端口是可选的；但如果你希望 dashboard 或外部工具访问 gateway，则必须开放。
...
## 运行 dashboard[​](#运行-dashboard "运行 dashboard的直接链接")
...
无论选择哪种，调用方在访问受保护路由前都会先被重定向到登录页。完整说明见 [Web Dashboard → 鉴权](/docs/zh-Hans/user-guide/features/web-dashboard)。
...
## 交互式运行（CLI 聊天）[​](#交互式运行cli-聊天 "交互式运行（CLI 聊天）的直接链接")
...
某些 skill CLI 会把凭据写到 `~` 下，因此在官方 Docker 布局里要针对子进程 HOME 初始化，而不是只针对数据卷根目录。例如 [xurl skill](/docs/zh-Hans/user-guide/skills/bundled/social-media/social-media-xurl) 会把 OAuth 状态存到 `~/.xurl`；在容器里这对应 `/opt/data/home/.xurl`，因此手动认证时应使用 `HOME=/opt/data/home xurl auth status` 之类的调用。

注意

切勿同时对同一数据目录运行两个 Hermes **gateway** 容器——会话文件和记忆存储不支持并发写入。

## 多 profile 支持[​](#多-profile-支持 "多 profile 支持的直接链接")

Hermes 支持[多个 profile](/docs/zh-Hans/reference/profile-commands)——独立的 `~/.hermes/` 子目录，让你可以从单个安装运行独立的 agent（不同的 SOUL、skills、memory、sessions、credentials）。**在官方 Docker 镜像内，s6 监管树把每个 profile 当作一等受监管服务**，因此推荐部署方式是：**一个容器承载多个 profile**。
...
若第二个 profile 也要暴露 OpenAI 兼容 API server，请在**该 profile 自己的** `.env` 中设置不同的 `API_SERVER_PORT`，然后重启该 profile 的 gateway；不要把端口放进容器级 `environment:`，否则所有 profile 都会争抢同一个端口。更底层的监管细节见后文的 [Per-profile gateway 监管](#per-profile-gateway-%E7%9B%91%E7%AE%A1)。

## 环境变量转发[​](#环境变量转发 "环境变量转发的直接链接")
...
使用 `docker compose up -d` 启动，使用 `docker compose logs -f` 查看日志。Dashboard 的 stdout/stderr 会直接出现在这里；gateway 主日志则写入每个 profile 的 s6 日志文件，见下方的 [Per-profile gateway 监管](#per-profile-gateway-%E7%9B%91%E7%AE%A1)。
...
## Dockerfile 说明[​](#dockerfile-说明 "Dockerfile 说明的直接链接")
...
* **`openssh-client`** — 从容器内启用 [SSH 终端后端](/docs/zh-Hans/user-guide/configuration#ssh-backend)。SSH 后端调用系统 `ssh` 二进制文件；若缺少此组件，在容器化安装中会静默失败
...
### Per-profile gateway 监管[​](#per-profile-gateway-监管 "Per-profile gateway 监管的直接链接")
...
## 技能与凭据文件[​](#技能与凭据文件 "技能与凭据文件的直接链接")

当使用 Docker 作为执行环境时（不是上述方法，而是 agent 在 Docker 沙箱内运行命令——参见 [配置 → Docker 后端](/docs/zh-Hans/user-guide/configuration#docker-backend)），Hermes 为所有工具调用复用单个长期运行的容器，并自动将技能目录（`~/.hermes/skills/`）和技能声明的所有凭据文件以只读卷的形式绑定挂载到该容器中。技能脚本、模板和引用在沙箱内无需手动配置即可使用，由于容器在 Hermes 进程的整个生命周期内持续存在，你安装的任何依赖或写入的文件都会在下次工具调用时保留。

SSH 和 Modal 后端也会进行相同的同步——技能和凭据文件在每次命令执行前通过 rsync 或 Modal mount API 上传。

## 在容器中安装更多工具[​](#在容器中安装更多工具 "在容器中安装更多工具的直接链接")

官方镜像预装了一套精选工具（参见 [Dockerfile 说明](#what-the-dockerfile-does)），但并非 agent 可能需要的每个工具都已预装。以下是五种推荐方式，按工作量和持久性递增排列。

### npm 或 Python 工具——使用 `npx` 或 `uvx`[​](#npm-或-python-工具使用-npx-或-uvx "npm-或-python-工具使用-npx-或-uvx的直接链接")
...
### 其他工具（apt 包、二进制文件）——安装并记住[​](#其他工具apt-包二进制文件安装并记住 "其他工具（apt 包、二进制文件）——安装并记住的直接链接")
...
### 持久安装——构建派生镜像[​](#持久安装构建派生镜像 "持久安装——构建派生镜像的直接链接")
...
### 复杂工具或多服务栈——运行 sidecar 容器[​](#复杂工具或多服务栈运行-sidecar-容器 "复杂工具或多服务栈——运行 sidecar 容器的直接链接")
...
### 广泛有用的工具——提交 issue 或 pull request[​](#广泛有用的工具提交-issue-或-pull-request "广泛有用的工具——提交 issue 或 pull request的直接链接")
...
## 连接本地推理服务器（vLLM、Ollama 等）[​](#连接本地推理服务器vllmollama-等 "连接本地推理服务器（vLLM、Ollama 等）的直接链接")

在 Docker 中运行 Hermes 且推理服务器（vLLM、Ollama、text-generation-inference 等）也在宿主机或另一个容器中运行时，网络配置需要额外注意。

### Docker Compose（推荐）[​](#docker-compose推荐 "Docker Compose（推荐）的直接链接")
...
### 独立 Docker run（无 Compose）[​](#独立-docker-run无-compose "独立 Docker run（无 Compose）的直接链接")
...
使用 `--network host` 时，`-p` 标志会被忽略——所有容器端口直接暴露在宿主机上。

### 验证连通性[​](#验证连通性 "验证连通性的直接链接")
...
### "Permission denied" 错误[​](#permission-denied-错误 "\"Permission denied\" 错误的直接链接")
...
### 浏览器工具无法使用[​](#浏览器工具无法使用 "浏览器工具无法使用的直接链接")
...
### 网络问题后 gateway 无法重连[​](#网络问题后-gateway-无法重连 "网络问题后 gateway 无法重连的直接链接")
...
### 检查容器健康状态[​](#检查容器健康状态 "检查容器健康状态的直接链接")
...
* [快速开始](#快速开始)
* [以 gateway 模式运行](#以-gateway-模式运行)
* [运行 dashboard](#运行-dashboard)
* [交互式运行（CLI 聊天）](#交互式运行cli-聊天)
* [持久化卷](#持久化卷)
  + [不可变安装树](#不可变安装树)
* [多 profile 支持](#多-profile-支持)
* [环境变量转发](#环境变量转发)
* [Docker Compose 示例](#docker-compose-示例)
* [资源限制](#资源限制)
* [Dockerfile 说明](#dockerfile-说明)
  + [Per-profile gateway 监管](#per-profile-gateway-监管)
* [升级](#升级)
* [技能与凭据文件](#技能与凭据文件)
* [在容器中安装更多工具](#在容器中安装更多工具)
  + [npm 或 Python 工具——使用 `npx` 或 `uvx`](#npm-或-python-工具使用-npx-或-uvx)
  + [其他工具（apt 包、二进制文件）——安装并记住](#其他工具apt-包二进制文件安装并记住)
  + [持久安装——构建派生镜像](#持久安装构建派生镜像)
  + [复杂工具或多服务栈——运行 sidecar 容器](#复杂工具或多服务栈运行-sidecar-容器)
  + [广泛有用的工具——提交 issue 或 pull request](#广泛有用的工具提交-issue-或-pull-request)
* [连接本地推理服务器（vLLM、Ollama 等）](#连接本地推理服务器vllmollama-等)
  + [Docker Compose（推荐）](#docker-compose推荐)
  + [独立 Docker run（无 Compose）](#独立-docker-run无-compose)
  + [验证连通性](#验证连通性)
  + [Ollama](#ollama)
* [故障排查](#故障排查)
  + [容器立即退出](#容器立即退出)
  + ["Permission denied" 错误](#permission-denied-错误)
  + [浏览器工具无法使用](#浏览器工具无法使用)
  + [网络问题后 gateway 无法重连](#网络问题后-gateway-无法重连)
  + [检查容器健康状态](#检查容器健康状态)

## Related Files
> **LLM Navigation:** Các tệp dưới đây được liên kết trực tiếp từ tài liệu này. Hãy đọc chúng nếu cần thêm ngữ cảnh.

- [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/profile-commands](./docs-zh-Hans-reference-profile-commands.md)
