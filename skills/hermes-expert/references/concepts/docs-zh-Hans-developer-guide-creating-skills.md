# 创建 Skill | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/creating-skills](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/creating-skills)

本页总览

Skill 是为 Hermes Agent 添加新能力的首选方式。与 tool 相比，skill 更易于创建，无需修改 agent 代码，且可与社区共享。

## 应该创建 Skill 还是 Tool？[​](#应该创建-skill-还是-tool "应该创建 Skill 还是 Tool？的直接链接")
...
### 平台专属 Skill[​](#平台专属-skill "平台专属 Skill的直接链接")
...
### 条件式 Skill 激活[​](#条件式-skill-激活 "条件式 Skill 激活的直接链接")
...
### 环境变量要求[​](#环境变量要求 "环境变量要求的直接链接")

Skill 可声明所需的环境变量。当通过 `skill_view` 加载 skill 时，其所需变量会自动注册，以便透传（passthrough）到沙箱执行环境（terminal、execute\_code）中。
...
## 加载时的安全配置[​](#加载时的安全配置 "加载时的安全配置的直接链接")
...
加载 skill 时，已设置的 `required_environment_variables` 会**自动透传**到 `execute_code` 和 `terminal` 沙箱——包括 Docker 和 Modal 等远程后端。Skill 的脚本无需用户额外配置即可访问 `$TENOR_API_KEY`（或 Python 中的 `os.environ["TENOR_API_KEY"]`）。详见 [环境变量透传](/docs/zh-Hans/user-guide/security#environment-variable-passthrough)。

旧版 `prerequisites.env_vars` 作为向后兼容的别名仍受支持。

### Config 配置项（config.yaml）[​](#config-配置项configyaml "Config 配置项（config.yaml）的直接链接")
...
### 凭证文件要求（OAuth token 等）[​](#凭证文件要求oauth-token-等 "凭证文件要求（OAuth token 等）的直接链接")
...
## Skill 规范[​](#skill-规范 "Skill 规范的直接链接")

### 无外部依赖[​](#无外部依赖 "无外部依赖的直接链接")
...
### 包含辅助脚本[​](#包含辅助脚本 "包含辅助脚本的直接链接")

对于 XML/JSON 解析或复杂逻辑，请在 `scripts/` 中包含辅助脚本——不要每次都期望 LLM 内联编写解析器。

### 以文档形式传递媒体（`[[as_document]]`）[​](#以文档形式传递媒体as_document "以文档形式传递媒体as_document的直接链接")

如果 skill 生成高分辨率截图、图表或任何有损预览压缩会造成损失的图片，请在响应中某处（通常是最后一行）输出字面指令 `[[as_document]]`。Gateway 会去除该指令，并将该响应中所有提取的媒体路径以可下载文件附件的形式传递，而非内联图片气泡。完整语义请参见 [Skill 输出与媒体传递](/docs/zh-Hans/user-guide/features/skills#skill-output-and-media-delivery)。

#### 在 SKILL.md 中引用内置脚本[​](#在-skillmd-中引用内置脚本 "在 SKILL.md 中引用内置脚本的直接链接")
...
#### 内联 shell 片段（需手动开启）[​](#内联-shell-片段需手动开启 "内联 shell 片段（需手动开启）的直接链接")
...
## Skill 应放在哪里？[​](#skill-应放在哪里 "Skill 应放在哪里？的直接链接")
...
## 发布 Skill[​](#发布-skill "发布 Skill的直接链接")

### 发布到 Skills Hub[​](#发布到-skills-hub "发布到 Skills Hub的直接链接")

```
hermes skills publish skills/my-skill --to github --repo owner/repo
```

### 发布到自定义仓库[​](#发布到自定义仓库 "发布到自定义仓库的直接链接")
...
* [应该创建 Skill 还是 Tool？](#应该创建-skill-还是-tool)
* [Skill 目录结构](#skill-目录结构)
* [SKILL.md 格式](#skillmd-格式)
  + [平台专属 Skill](#平台专属-skill)
  + [条件式 Skill 激活](#条件式-skill-激活)
  + [环境变量要求](#环境变量要求)
* [加载时的安全配置](#加载时的安全配置)
  + [Config 配置项（config.yaml）](#config-配置项configyaml)
  + [凭证文件要求（OAuth token 等）](#凭证文件要求oauth-token-等)
* [Skill 规范](#skill-规范)
  + [无外部依赖](#无外部依赖)
  + [渐进式披露](#渐进式披露)
  + [包含辅助脚本](#包含辅助脚本)
  + [以文档形式传递媒体（`[[as_document]]`）](#以文档形式传递媒体as_document)
  + [测试](#测试)
* [Skill 应放在哪里？](#skill-应放在哪里)
* [发布 Skill](#发布-skill)
  + [发布到 Skills Hub](#发布到-skills-hub)
  + [发布到自定义仓库](#发布到自定义仓库)
* [安全扫描](#安全扫描)
