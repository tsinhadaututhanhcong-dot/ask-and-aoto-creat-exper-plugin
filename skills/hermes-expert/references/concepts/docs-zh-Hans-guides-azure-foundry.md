...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/azure-foundry](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/azure-foundry)
...
## Microsoft Entra ID（无密钥，RBAC）——推荐[​](#microsoft-entra-id无密钥rbac推荐 "Microsoft Entra ID（无密钥，RBAC）——推荐的直接链接")
...
### 为什么使用 Entra ID？[​](#为什么使用-entra-id "为什么使用 Entra ID？的直接链接")
...
### 一次性设置（Azure 侧）[​](#一次性设置azure-侧 "一次性设置（Azure 侧）的直接链接")
...
### 一次性设置（Hermes 侧）[​](#一次性设置hermes-侧 "一次性设置（Hermes 侧）的直接链接")
...
### 写入 `config.yaml` 的配置[​](#写入-configyaml-的配置 "写入-configyaml-的配置的直接链接")
...
#### 主权云（政府云、中国云）[​](#主权云政府云中国云 "主权云（政府云、中国云）的直接链接")
...
## 配置（写入 `config.yaml`）[​](#配置写入-configyaml "配置写入-configyaml的直接链接")
...
## OpenAI 风格端点（GPT、Llama 等）[​](#openai-风格端点gptllama-等 "OpenAI 风格端点（GPT、Llama 等）的直接链接")
...
## Anthropic 风格端点（通过 Microsoft Foundry 使用 Claude）[​](#anthropic-风格端点通过-microsoft-foundry-使用-claude "Anthropic 风格端点（通过 Microsoft Foundry 使用 Claude）的直接链接")
...
## 替代方案：`provider: anthropic` + Azure base URL[​](#替代方案provider-anthropic--azure-base-url "替代方案provider-anthropic--azure-base-url的直接链接")
...
`key_env` 是规范的 snake\_case 字段名；`api_key_env`（以及驼峰式 `keyEnv` / `apiKeyEnv`）作为别名被接受。如果同时设置了 `key_env` 和 `AZURE_ANTHROPIC_KEY`/`ANTHROPIC_API_KEY`，`key_env` 指定的环境变量优先。
...
| 变量 | 用途 |
| --- | --- |
| `AZURE_FOUNDRY_API_KEY` | Microsoft Foundry / Azure OpenAI 的主 API 密钥（api\_key 模式） |
...
* [AWS Bedrock](/docs/zh-Hans/guides/aws-bedrock) — 另一个主要的云 provider 集成
...
* [前提条件](#前提条件)
* [快速开始](#快速开始)
* [Microsoft Entra ID（无密钥，RBAC）——推荐](#microsoft-entra-id无密钥rbac推荐)
  + [为什么使用 Entra ID？](#为什么使用-entra-id)
  + [一次性设置（Azure 侧）](#一次性设置azure-侧)
  + [一次性设置（Hermes 侧）](#一次性设置hermes-侧)
  + [写入 `config.yaml` 的配置](#写入-configyaml-的配置)
  + [凭据解析顺序](#凭据解析顺序)
  + [部署模式](#部署模式)
  + [健康检查](#健康检查)
  + [限制](#限制)
* [配置（写入 `config.yaml`）](#配置写入-configyaml)
* [OpenAI 风格端点（GPT、Llama 等）](#openai-风格端点gptllama-等)
* [Anthropic 风格端点（通过 Microsoft Foundry 使用 Claude）](#anthropic-风格端点通过-microsoft-foundry-使用-claude)
* [替代方案：`provider: anthropic` + Azure base URL](#替代方案provider-anthropic--azure-base-url)
* [模型发现](#模型发现)
* [环境变量](#环境变量)
* [故障排查](#故障排查)
* [相关链接](#相关链接)
