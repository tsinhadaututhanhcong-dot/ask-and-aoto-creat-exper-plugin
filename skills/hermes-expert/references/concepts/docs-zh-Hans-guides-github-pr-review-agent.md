# 教程：GitHub PR 审查 Agent | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/github-pr-review-agent](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/github-pr-review-agent)
...
如果你有可用的公开端点，请查看[使用 Webhook 自动化 GitHub PR 评论](/docs/zh-Hans/guides/webhook-github-pr-review)——GitHub 会在 PR 被打开或更新时立即向 Hermes 推送事件。
...
* **已安装并认证 GitHub CLI（`gh`）**：
  ...
  gh auth login
  ```
* **已配置消息通知**（可选）— [Telegram](/docs/zh-Hans/user-guide/messaging/telegram) 或 [Discord](/docs/zh-Hans/user-guide/messaging/discord)
...
## 第一步：验证配置[​](#第一步验证配置 "第一步：验证配置的直接链接")
...
## 第二步：手动试审一个 PR[​](#第二步手动试审一个-pr "第二步：手动试审一个 PR的直接链接")
...
## 第三步：创建审查 Skill[​](#第三步创建审查-skill "第三步：创建审查 Skill的直接链接")
...
## 第四步：教会它你的团队规范[​](#第四步教会它你的团队规范 "第四步：教会它你的团队规范的直接链接")
...
## 第五步：创建自动化 Cron 任务[​](#第五步创建自动化-cron-任务 "第五步：创建自动化 Cron 任务的直接链接")
...
### 其他常用调度计划[​](#其他常用调度计划 "其他常用调度计划的直接链接")
...
## 第六步：按需手动触发[​](#第六步按需手动触发 "第六步：按需手动触发的直接链接")
...
### 直接在 GitHub 上发布审查评论[​](#直接在-github-上发布审查评论 "直接在 GitHub 上发布审查评论的直接链接")
...
### 每周 PR 看板[​](#每周-pr-看板 "每周 PR 看板的直接链接")
...
### "gh: command not found"[​](#gh-command-not-found "\"gh: command not found\"的直接链接")

Gateway 在精简环境中运行。请确保 `gh` 在系统 PATH 中，然后重启 gateway。

### 审查结果过于泛泛[​](#审查结果过于泛泛 "审查结果过于泛泛的直接链接")

1. 添加 `code-review` skill（第三步）
2. 通过 memory（记忆）向 Hermes 传授你的团队规范（第四步）
3. 它对你的技术栈了解越多，审查质量越好

### Cron 任务未运行[​](#cron-任务未运行 "Cron 任务未运行的直接链接")
...
* **[基于 Webhook 的 PR 审查](/docs/zh-Hans/guides/webhook-github-pr-review)** — 在 PR 被打开时立即获得审查（需要公开端点）
* **[每日简报 Bot](/docs/zh-Hans/guides/daily-briefing-bot)** — 将 PR 审查与你的晨间资讯摘要结合
* **[构建 Plugin](/docs/zh-Hans/developer-guide/plugins)** — 将审查逻辑封装为可共享的 plugin
* **[Profiles](/docs/zh-Hans/user-guide/profiles)** — 运行一个专属审查器 profile，拥有独立的 memory 和配置
* **[Fallback Providers](/docs/zh-Hans/user-guide/features/fallback-providers)** — 确保在某个 provider 不可用时审查任务仍能正常运行

* [前提条件](#前提条件)
* [第一步：验证配置](#第一步验证配置)
* [第二步：手动试审一个 PR](#第二步手动试审一个-pr)
* [第三步：创建审查 Skill](#第三步创建审查-skill)
* [第四步：教会它你的团队规范](#第四步教会它你的团队规范)
* [第五步：创建自动化 Cron 任务](#第五步创建自动化-cron-任务)
  + [其他常用调度计划](#其他常用调度计划)
* [第六步：按需手动触发](#第六步按需手动触发)
* [进阶用法](#进阶用法)
  + [直接在 GitHub 上发布审查评论](#直接在-github-上发布审查评论)
  + [每周 PR 看板](#每周-pr-看板)
  + [多仓库监控](#多仓库监控)
* [故障排查](#故障排查)
  + ["gh: command not found"](#gh-command-not-found)
  + [审查结果过于泛泛](#审查结果过于泛泛)
  + [Cron 任务未运行](#cron-任务未运行)
  + [速率限制](#速率限制)
* [下一步](#下一步)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/webhook-github-pr-review](./docs-zh-Hans-guides-webhook-github-pr-review.md)
