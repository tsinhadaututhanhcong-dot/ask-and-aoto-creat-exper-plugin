# Airtable — 通过 curl 调用 Airtable REST API | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-airtable](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-airtable)
...
## 字段类型（请求体格式）[​](#字段类型请求体格式 "字段类型（请求体格式）的直接链接")
...
### 列出令牌可访问的 bases[​](#列出令牌可访问的-bases "列出令牌可访问的 bases的直接链接")
...
### 列出某个 base 的 tables 及 schema[​](#列出某个-base-的-tables-及-schema "列出某个 base 的 tables 及 schema的直接链接")
...
### 列出记录（前 10 条）[​](#列出记录前-10-条 "列出记录（前 10 条）的直接链接")
...
### 过滤记录（filterByFormula）[​](#过滤记录filterbyformula "过滤记录（filterByFormula）的直接链接")
...
### 排序并选择特定字段[​](#排序并选择特定字段 "排序并选择特定字段的直接链接")
...
## 常用变更操作[​](#常用变更操作 "常用变更操作的直接链接")

### 创建单条记录[​](#创建单条记录 "创建单条记录的直接链接")
...
### 单次调用最多创建 10 条记录[​](#单次调用最多创建-10-条记录 "单次调用最多创建 10 条记录的直接链接")
...
### 更新记录（PATCH——合并更新，保留未修改字段）[​](#更新记录patch合并更新保留未修改字段 "更新记录（PATCH——合并更新，保留未修改字段）的直接链接")
...
### 按合并字段 upsert（无需 ID）[​](#按合并字段-upsert无需-id "按合并字段 upsert（无需 ID）的直接链接")
...
### 单次调用最多删除 10 条记录[​](#单次调用最多删除-10-条记录 "单次调用最多删除 10 条记录的直接链接")
...
## Hermes 典型工作流[​](#hermes-典型工作流 "Hermes 典型工作流的直接链接")
...
## Hermes 重要说明[​](#hermes-重要说明 "Hermes 重要说明的直接链接")
...
* [前置条件](#前置条件)
* [API 基础](#api-基础)
* [字段类型（请求体格式）](#字段类型请求体格式)
* [常用查询](#常用查询)
  + [列出令牌可访问的 bases](#列出令牌可访问的-bases)
  + [列出某个 base 的 tables 及 schema](#列出某个-base-的-tables-及-schema)
  + [列出记录（前 10 条）](#列出记录前-10-条)
  + [获取单条记录](#获取单条记录)
  + [过滤记录（filterByFormula）](#过滤记录filterbyformula)
  + [排序并选择特定字段](#排序并选择特定字段)
  + [使用命名视图](#使用命名视图)
* [常用变更操作](#常用变更操作)
  + [创建单条记录](#创建单条记录)
  + [单次调用最多创建 10 条记录](#单次调用最多创建-10-条记录)
  + [更新记录（PATCH——合并更新，保留未修改字段）](#更新记录patch合并更新保留未修改字段)
  + [按合并字段 upsert（无需 ID）](#按合并字段-upsert无需-id)
  + [删除单条记录](#删除单条记录)
  + [单次调用最多删除 10 条记录](#单次调用最多删除-10-条记录)
* [分页](#分页)
* [Hermes 典型工作流](#hermes-典型工作流)
* [注意事项](#注意事项)
* [Hermes 重要说明](#hermes-重要说明)