...
**Source:** [https://hermes-agent.nousresearch.com/docs/user-guide/skills/bundled/productivity/productivity-notion](https://hermes-agent.nousresearch.com/docs/user-guide/skills/bundled/productivity/productivity-notion)
...
### 1. Get an integration token (required for both paths)[​](#1-get-an-integration-token-required-for-both-paths "Direct link to 1. Get an integration token (required for both paths)")
...
### 2. Install `ntn` (preferred path on macOS / Linux)[​](#2-install-ntn-preferred-path-on-macos--linux "Direct link to 2-install-ntn-preferred-path-on-macos--linux")
...
### 3. Choose path at runtime[​](#3-choose-path-at-runtime "Direct link to 3. Choose path at runtime")
...
## Path A — `ntn` CLI (preferred, macOS / Linux)[​](#path-a--ntn-cli-preferred-macos--linux "Direct link to path-a--ntn-cli-preferred-macos--linux")

### Raw API calls (shorthand for curl)[​](#raw-api-calls-shorthand-for-curl "Direct link to Raw API calls (shorthand for curl)")
...
### Read page metadata[​](#read-page-metadata "Direct link to Read page metadata")

```
ntn api v1/pages/{page_id}
```

### Read page as Markdown (agent-friendly)[​](#read-page-as-markdown-agent-friendly "Direct link to Read page as Markdown (agent-friendly)")

```
ntn api v1/pages/{page_id}/markdown
```

### Read page content as blocks[​](#read-page-content-as-blocks "Direct link to Read page content as blocks")

```
ntn api v1/blocks/{page_id}/children
```

### Create page from Markdown[​](#create-page-from-markdown "Direct link to Create page from Markdown")
...
### Patch a page with Markdown[​](#patch-a-page-with-markdown "Direct link to Patch a page with Markdown")
...
### Query a database (data source)[​](#query-a-database-data-source "Direct link to Query a database (data source)")
...
### File uploads (one-liner — biggest CLI win)[​](#file-uploads-one-liner--biggest-cli-win "Direct link to File uploads (one-liner — biggest CLI win)")
...
### Useful env vars[​](#useful-env-vars "Direct link to Useful env vars")
...
## Path B — HTTP + curl (cross-platform, default on Windows)[​](#path-b--http--curl-cross-platform-default-on-windows "Direct link to Path B — HTTP + curl (cross-platform, default on Windows)")
...
### Search[​](#search-1 "Direct link to Search")
...
### Read page metadata[​](#read-page-metadata-1 "Direct link to Read page metadata")
...
### Read page as Markdown (agent-friendly)[​](#read-page-as-markdown-agent-friendly-1 "Direct link to Read page as Markdown (agent-friendly)")
...
### Read page content as blocks (when you need structure)[​](#read-page-content-as-blocks-when-you-need-structure "Direct link to Read page content as blocks (when you need structure)")
...
### Create page from Markdown[​](#create-page-from-markdown-1 "Direct link to Create page from Markdown")
...
### Patch a page with Markdown[​](#patch-a-page-with-markdown-1 "Direct link to Patch a page with Markdown")
...
### Create page in a database (typed properties)[​](#create-page-in-a-database-typed-properties "Direct link to Create page in a database (typed properties)")
...
### Query a database (data source)[​](#query-a-database-data-source-1 "Direct link to Query a database (data source)")
...
### Append blocks to a page[​](#append-blocks-to-a-page "Direct link to Append blocks to a page")
...
### File uploads (3-step flow)[​](#file-uploads-3-step-flow "Direct link to File uploads (3-step flow)")
...
## API Version 2025-09-03 — Databases vs Data Sources[​](#api-version-2025-09-03--databases-vs-data-sources "Direct link to API Version 2025-09-03 — Databases vs Data Sources")
...
## Notion Workers (advanced, requires `ntn`)[​](#notion-workers-advanced-requires-ntn "Direct link to notion-workers-advanced-requires-ntn")
...
### Minimal Worker[​](#minimal-worker "Direct link to Minimal Worker")
...
### Webhook capability[​](#webhook-capability "Direct link to Webhook capability")
...
### Worker lifecycle commands[​](#worker-lifecycle-commands "Direct link to Worker lifecycle commands")
...
## Notion-Flavored Markdown (used by `/markdown` endpoints)[​](#notion-flavored-markdown-used-by-markdown-endpoints "Direct link to notion-flavored-markdown-used-by-markdown-endpoints")
...
## Choosing the Right Path[​](#choosing-the-right-path "Direct link to Choosing the Right Path")
...
* [Setup](#setup)
  + [1. Get an integration token (required for both paths)](#1-get-an-integration-token-required-for-both-paths)
  + [2. Install `ntn` (preferred path on macOS / Linux)](#2-install-ntn-preferred-path-on-macos--linux)
  + [3. Choose path at runtime](#3-choose-path-at-runtime)
* [API Basics](#api-basics)
* [Path A — `ntn` CLI (preferred, macOS / Linux)](#path-a--ntn-cli-preferred-macos--linux)
  + [Raw API calls (shorthand for curl)](#raw-api-calls-shorthand-for-curl)
  + [Search](#search)
  + [Read page metadata](#read-page-metadata)
  + [Read page as Markdown (agent-friendly)](#read-page-as-markdown-agent-friendly)
  + [Read page content as blocks](#read-page-content-as-blocks)
  + [Create page from Markdown](#create-page-from-markdown)
  + [Patch a page with Markdown](#patch-a-page-with-markdown)
  + [Query a database (data source)](#query-a-database-data-source)
  + [File uploads (one-liner — biggest CLI win)](#file-uploads-one-liner--biggest-cli-win)
  + [Useful env vars](#useful-env-vars)
* [Path B — HTTP + curl (cross-platform, default on Windows)](#path-b--http--curl-cross-platform-default-on-windows)
  + [Search](#search-1)
  + [Read page metadata](#read-page-metadata-1)
  + [Read page as Markdown (agent-friendly)](#read-page-as-markdown-agent-friendly-1)
  + [Read page content as blocks (when you need structure)](#read-page-content-as-blocks-when-you-need-structure)
  + [Create page from Markdown](#create-page-from-markdown-1)
  + [Patch a page with Markdown](#patch-a-page-with-markdown-1)
  + [Create page in a database (typed properties)](#create-page-in-a-database-typed-properties)
  + [Query a database (data source)](#query-a-database-data-source-1)
  + [Create a database](#create-a-database)
  + [Update page properties](#update-page-properties)
  + [Append blocks to a page](#append-blocks-to-a-page)
  + [File uploads (3-step flow)](#file-uploads-3-step-flow)
* [Property Types](#property-types)
* [API Version 2025-09-03 — Databases vs Data Sources](#api-version-2025-09-03--databases-vs-data-sources)
* [Notion Workers (advanced, requires `ntn`)](#notion-workers-advanced-requires-ntn)
  + [Minimal Worker](#minimal-worker)
  + [Webhook capability](#webhook-capability)
  + [Worker lifecycle commands](#worker-lifecycle-commands)
* [Notion-Flavored Markdown (used by `/markdown` endpoints)](#notion-flavored-markdown-used-by-markdown-endpoints)
