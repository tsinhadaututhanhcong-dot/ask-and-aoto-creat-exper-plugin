...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-lbo-model](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/finance/finance-lbo-model)
...
| 相关 skills | [`excel-author`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-excel-author), [`pptx-author`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-pptx-author), [`dcf-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-dcf-model), [`3-statement-model`](/docs/zh-Hans/user-guide/skills/optional/finance/finance-3-statement-model) |
...
## 关键指令——请先阅读[​](#关键指令请先阅读 "关键指令——请先阅读的直接链接")
...
### 填充颜色调色板——专业蓝灰配色（除非用户/模板另有指定）[​](#填充颜色调色板专业蓝灰配色除非用户模板另有指定 "填充颜色调色板——专业蓝灰配色（除非用户/模板另有指定）的直接链接")
...
## 模板分析阶段——请先执行此步骤[​](#模板分析阶段请先执行此步骤 "模板分析阶段——请先执行此步骤的直接链接")
...
## 填写公式——通用方法[​](#填写公式通用方法 "填写公式——通用方法的直接链接")

对于每个需要公式的单元格，遵循以下优先级：

### 第一步：检查模板[​](#第一步检查模板 "第一步：检查模板的直接链接")
...
### 第二步：检查用户指令[​](#第二步检查用户指令 "第二步：检查用户指令的直接链接")

* 用户是否指定了特定的计算方法？
* 是否有影响此公式的既定假设？
* 是否有特殊需求？

### 第三步：应用标准实践[​](#第三步应用标准实践 "第三步：应用标准实践的直接链接")
...
## 验证清单——完成后运行[​](#验证清单完成后运行 "验证清单——完成后运行的直接链接")

### 运行公式验证[​](#运行公式验证 "运行公式验证的直接链接")
...
### 资产负债表（如适用）[​](#资产负债表如适用 "资产负债表（如适用）的直接链接")
...
### 现金流量（如适用）[​](#现金流量如适用 "现金流量（如适用）的直接链接")
...
### 债务/融资计划（如适用）[​](#债务融资计划如适用 "债务/融资计划（如适用）的直接链接")
...
### 敏感性表格（如适用）[​](#敏感性表格如适用 "敏感性表格（如适用）的直接链接")
...
## 与用户协作——逐节检查点[​](#与用户协作逐节检查点 "与用户协作——逐节检查点的直接链接")
...
## 数据来源——优先使用 MCP，其次使用网络[​](#数据来源优先使用-mcp其次使用网络 "数据来源——优先使用 MCP，其次使用网络的直接链接")
...
* [关键指令——请先阅读](#关键指令请先阅读)
  + [核心原则](#核心原则)
  + [公式颜色约定](#公式颜色约定)
  + [填充颜色调色板——专业蓝灰配色（除非用户/模板另有指定）](#填充颜色调色板专业蓝灰配色除非用户模板另有指定)
  + [数字格式标准](#数字格式标准)
  + [首先明确需求](#首先明确需求)
* [模板分析阶段——请先执行此步骤](#模板分析阶段请先执行此步骤)
* [填写公式——通用方法](#填写公式通用方法)
  + [第一步：检查模板](#第一步检查模板)
  + [第二步：检查用户指令](#第二步检查用户指令)
  + [第三步：应用标准实践](#第三步应用标准实践)
* [常见问题区域](#常见问题区域)
  + [平衡节](#平衡节)
  + [税务计算](#税务计算)
  + [利息与循环引用](#利息与循环引用)
  + [债务还款/现金清扫](#债务还款现金清扫)
  + [回报计算（IRR/MOIC）](#回报计算irrmoic)
  + [敏感性表格](#敏感性表格)
* [验证清单——完成后运行](#验证清单完成后运行)
  + [运行公式验证](#运行公式验证)
  + [节平衡](#节平衡)
  + [收入/运营预测](#收入运营预测)
  + [资产负债表（如适用）](#资产负债表如适用)
  + [现金流量（如适用）](#现金流量如适用)
  + [支持性计划](#支持性计划)
  + [债务/融资计划（如适用）](#债务融资计划如适用)
  + [回报/输出分析](#回报输出分析)
  + [敏感性表格（如适用）](#敏感性表格如适用)
  + [格式](#格式)
  + [逻辑合理性检查](#逻辑合理性检查)
* [常见错误须避免](#常见错误须避免)
* [与用户协作——逐节检查点](#与用户协作逐节检查点)
* [数据来源——优先使用 MCP，其次使用网络](#数据来源优先使用-mcp其次使用网络)
* [归属](#归属)
