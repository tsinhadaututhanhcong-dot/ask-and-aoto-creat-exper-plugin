...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-parallel-cli](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/research/research-parallel-cli)
...
| 相关技能 | [`duckduckgo-search`](/docs/zh-Hans/user-guide/skills/optional/research/research-duckduckgo-search), [`mcporter`](/docs/zh-Hans/user-guide/skills/optional/mcp/mcp-mcporter) |
...
### Homebrew[​](#homebrew "Homebrew的直接链接")
...
### 上下文链式调用 / 后续追问[​](#上下文链式调用--后续追问 "上下文链式调用 / 后续追问的直接链接")
...
## 数据丰富（Enrichment）[​](#数据丰富enrichment "数据丰富（Enrichment）的直接链接")
...
### 非交互式文件运行[​](#非交互式文件运行 "非交互式文件运行的直接链接")
...
### YAML 配置运行[​](#yaml-配置运行 "YAML 配置运行的直接链接")

```
parallel-cli enrich run config.yaml
```

### 状态 / 轮询[​](#状态--轮询 "状态 / 轮询的直接链接")
...
## 监控（Monitor）[​](#监控monitor "监控（Monitor）的直接链接")
...
## 推荐的 Hermes 使用模式[​](#推荐的-hermes-使用模式 "推荐的 Hermes 使用模式的直接链接")

### 快速答案与引用[​](#快速答案与引用 "快速答案与引用的直接链接")

1. 运行 `parallel-cli search ... --json`
2. 解析标题、URL、日期、摘录
3. 仅使用返回的 URL 进行内联引用并汇总

### URL 调查[​](#url-调查 "URL 调查的直接链接")

1. 运行 `parallel-cli extract URL --json`
2. 如有需要，使用 `--objective` 或 `--full-content` 重新运行
3. 引用或汇总提取的 markdown

### 长时研究工作流[​](#长时研究工作流 "长时研究工作流的直接链接")
...
### 结构化数据丰富工作流[​](#结构化数据丰富工作流 "结构化数据丰富工作流的直接链接")
...
## 错误处理与退出码[​](#错误处理与退出码 "错误处理与退出码的直接链接")
...
* [深度研究](#深度研究)
  + [同步模式](#同步模式)
  + [异步启动 + 轮询](#异步启动--轮询)
  + [上下文链式调用 / 后续追问](#上下文链式调用--后续追问)
* [数据丰富（Enrichment）](#数据丰富enrichment)
  + [建议列](#建议列)
  + [规划配置](#规划配置)
  + [内联数据](#内联数据)
  + [非交互式文件运行](#非交互式文件运行)
...
* [推荐的 Hermes 使用模式](#推荐的-hermes-使用模式)
  + [快速答案与引用](#快速答案与引用)
  + [URL 调查](#url-调查)
  + [长时研究工作流](#长时研究工作流)
  + [结构化数据丰富工作流](#结构化数据丰富工作流)
* [错误处理与退出码](#错误处理与退出码)
* [维护](#维护)
* [注意事项](#注意事项)
