# ACP 编辑器集成 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/acp](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/acp)
...
## Hermes 在 ACP 模式下暴露的内容[​](#hermes-在-acp-模式下暴露的内容 "Hermes 在 ACP 模式下暴露的内容的直接链接")
...
## 启动 ACP 服务器[​](#启动-acp-服务器 "启动 ACP 服务器的直接链接")
...
### 浏览器工具（可选）[​](#浏览器工具可选 "浏览器工具（可选）的直接链接")
...
## 编辑器设置[​](#编辑器设置 "编辑器设置的直接链接")

### VS Code[​](#vs-code "VS Code的直接链接")
...
### JetBrains[​](#jetbrains "JetBrains的直接链接")
...
## Registry 清单[​](#registry-清单 "Registry 清单的直接链接")
...
### 会话范围的编辑自动审批[​](#会话范围的编辑自动审批 "会话范围的编辑自动审批的直接链接")
...
### ACP agent 未出现在编辑器中[​](#acp-agent-未出现在编辑器中 "ACP agent 未出现在编辑器中的直接链接")
...
### ACP 启动后立即报错[​](#acp-启动后立即报错 "ACP 启动后立即报错的直接链接")
...
### Zed registry 启动器找不到 uv[​](#zed-registry-启动器找不到-uv "Zed registry 启动器找不到 uv的直接链接")
...
* [Hermes 在 ACP 模式下暴露的内容](#hermes-在-acp-模式下暴露的内容)
* [安装](#安装)
* [启动 ACP 服务器](#启动-acp-服务器)
  + [浏览器工具（可选）](#浏览器工具可选)
* [编辑器设置](#编辑器设置)
  + [VS Code](#vs-code)
  + [Zed](#zed)
  + [JetBrains](#jetbrains)
* [Registry 清单](#registry-清单)
* [配置与凭据](#配置与凭据)
* [会话行为](#会话行为)
* [工作目录行为](#工作目录行为)
* [审批](#审批)
  + [会话范围的编辑自动审批](#会话范围的编辑自动审批)
* [故障排查](#故障排查)
  + [ACP agent 未出现在编辑器中](#acp-agent-未出现在编辑器中)
  + [ACP 启动后立即报错](#acp-启动后立即报错)
  + [缺少凭据](#缺少凭据)
  + [Zed registry 启动器找不到 uv](#zed-registry-启动器找不到-uv)
* [另请参阅](#另请参阅)
