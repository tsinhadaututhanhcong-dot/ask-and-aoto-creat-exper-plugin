# 健身营养 — 健身房训练计划与营养追踪 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/health/health-fitness-nutrition](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/health/health-fitness-nutrition)
...
### 动作查询（wger API）[​](#动作查询wger-api "动作查询（wger API）的直接链接")
...
### 营养查询（USDA FoodData Central）[​](#营养查询usda-fooddata-central "营养查询（USDA FoodData Central）的直接链接")

优先使用 `USDA_API_KEY` 环境变量，否则回退到 `DEMO_KEY`。
DEMO\_KEY = 每小时 30 次请求。免费注册密钥 = 每小时 1,000 次请求。
...
* [使用时机](#使用时机)
* [操作流程](#操作流程)
  + [动作查询（wger API）](#动作查询wger-api)
  + [营养查询（USDA FoodData Central）](#营养查询usda-fooddata-central)
  + [离线计算器](#离线计算器)
* [注意事项](#注意事项)
* [验证](#验证)
* [快速参考](#快速参考)