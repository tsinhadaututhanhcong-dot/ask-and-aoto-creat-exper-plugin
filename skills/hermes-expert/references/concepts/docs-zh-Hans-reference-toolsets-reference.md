# 工具集参考 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/toolsets-reference](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/toolsets-reference)

本页总览

工具集（Toolset）是工具的命名集合，用于控制 agent 可以执行的操作。它是按平台、按会话或按任务配置工具可用性的主要机制。

## 工具集的工作原理[​](#工具集的工作原理 "工具集的工作原理的直接链接")
...
## 配置工具集[​](#配置工具集 "配置工具集的直接链接")

### 按会话（CLI）[​](#按会话cli "按会话（CLI）的直接链接")
...
### 按平台（config.yaml）[​](#按平台configyaml "按平台（config.yaml）的直接链接")
...
## 平台工具集[​](#平台工具集 "平台工具集的直接链接")

平台工具集定义了部署目标的完整工具配置。大多数消息平台使用与 `hermes-cli` 相同的配置：

| 工具集 | 与 `hermes-cli` 的差异 |
| --- | --- |
| `hermes-cli` | 完整工具集——交互式 CLI 会话的默认配置。包含 file、terminal、web、browser、memory、skills、vision、image\_gen、todo、tts、delegation、code\_execution、cronjob、session\_search、clarify 和 `safe`（只读）套件，以及标准消息工具。 |
...
## 动态工具集[​](#动态工具集 "动态工具集的直接链接")

### MCP server 工具集[​](#mcp-server-工具集 "MCP server 工具集的直接链接")
...
## 与 `hermes tools` 的关系[​](#与-hermes-tools-的关系 "与-hermes-tools-的关系的直接链接")

`hermes tools` 命令提供基于 curses 的 UI，用于按平台切换单个工具的启用/禁用状态。该操作在工具级别进行（比工具集更细粒度），并持久化到 `config.yaml`。即使工具集已启用，被禁用的工具也会被过滤掉。

另请参阅：[工具参考](/docs/zh-Hans/reference/tools-reference)，获取所有单个工具及其参数的完整列表。

* [工具集的工作原理](#工具集的工作原理)
* [配置工具集](#配置工具集)
  + [按会话（CLI）](#按会话cli)
  + [按平台（config.yaml）](#按平台configyaml)
  + [交互式管理](#交互式管理)
* [核心工具集](#核心工具集)
* [平台工具集](#平台工具集)
* [动态工具集](#动态工具集)
  + [MCP server 工具集](#mcp-server-工具集)
  + [插件工具集](#插件工具集)
  + [自定义工具集](#自定义工具集)
  + [通配符](#通配符)
* [与 `hermes tools` 的关系](#与-hermes-tools-的关系)
