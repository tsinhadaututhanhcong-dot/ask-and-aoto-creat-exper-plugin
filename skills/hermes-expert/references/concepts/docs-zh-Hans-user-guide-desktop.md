...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/desktop](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/desktop)
...
* **CLI** (`hermes`) and **[TUI](/docs/zh-Hans/user-guide/tui.md)** (`hermes --tui`) — terminal interfaces.
* **[Web Dashboard](/docs/zh-Hans/user-guide/features/web-dashboard.md)** (`hermes dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Hermes Desktop](/docs/zh-Hans/getting-started/installation.md).
...
## What's in the app[​](#whats-in-the-app "What's in the app的直接链接")
...
#### Status bar[​](#status-bar "Status bar的直接链接")

The bar along the bottom of the chat shows live session state and exposes quick controls without opening Settings:

* **Per-session YOLO toggle** — flip YOLO on or off for just this session (matching the TUI). YOLO bypasses the dangerous-command approval prompts, so know what you're turning off — see [Security → YOLO Mode](/docs/zh-Hans/user-guide/security.md#yolo-mode).

Chatting against a Hermes instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Hermes Desktop to a remote backend](/docs/zh-Hans/user-guide/features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend).

#### Choosing a model[​](#choosing-a-model "Choosing a model的直接链接")
...
### File browser[​](#file-browser "File browser的直接链接")
...
### Voice[​](#voice "Voice的直接链接")

Talk to Hermes and hear it back, the same [voice mode](/docs/zh-Hans/user-guide/features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.

### Settings & onboarding[​](#settings--onboarding "Settings & onboarding的直接链接")
...
### Management panes[​](#management-panes "Management panes的直接链接")

The app also surfaces the broader Hermes management surface so you don't have to drop to a terminal:

* **Skills** — browse, install, and manage [skills](/docs/zh-Hans/user-guide/features/skills.md).
* **Cron** — view and manage [scheduled jobs](/docs/zh-Hans/reference/cli-commands.md#hermes-cron).
* **Profiles** — switch between [Hermes profiles](/docs/zh-Hans/user-guide/profiles.md) (isolated config/skills/sessions).
* **Messaging** — set up gateway channels.
* **Agents** and **Command Center** — orchestration surfaces for multi-agent work.

### Keyboard & navigation[​](#keyboard--navigation "Keyboard & navigation的直接链接")
...
### Sessions & profiles[​](#sessions--profiles "Sessions & profiles的直接链接")
...
* **Concurrent multi-profile sessions** — run sessions across multiple [profiles](/docs/zh-Hans/user-guide/profiles.md) at the same time, and reference a session in another profile with cross-profile `@session` links.

## Updating[​](#updating "Updating的直接链接")
...
## Uninstalling[​](#uninstalling "Uninstalling的直接链接")
...
## CLI reference: `hermes desktop`[​](#cli-reference-hermes-desktop "cli-reference-hermes-desktop的直接链接")
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Nous Research](/docs/zh-Hans/user-guide/features/web-dashboard.md#default-provider-nous-research).

### On the backend (the remote machine)[​](#on-the-backend-the-remote-machine "On the backend (the remote machine)的直接链接")
...
### In the app[​](#in-the-app "In the app的直接链接")
...
The remote gateway host is configured per [profile](/docs/zh-Hans/user-guide/profiles.md), so each profile can point at its own remote backend (or stay on its local one). Switching profiles switches which remote host the app connects to.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Hermes Desktop to a remote backend](/docs/zh-Hans/user-guide/features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Hermes Desktop](/docs/zh-Hans/reference/environment-variables.md#web-dashboard--hermes-desktop).

## Troubleshooting[​](#troubleshooting-1 "Troubleshooting的直接链接")
...
### "Build desktop app" stuck on Electron download[​](#build-desktop-app-stuck-on-electron-download "\"Build desktop app\" stuck on Electron download的直接链接")
...
## Building from source[​](#building-from-source "Building from source的直接链接")
...
* [CLI Guide](/docs/zh-Hans/user-guide/cli.md) — the terminal interface
* [TUI](/docs/zh-Hans/user-guide/tui.md) — the modern terminal UI used by `hermes --tui` and the dashboard chat tab
* [Web Dashboard](/docs/zh-Hans/user-guide/features/web-dashboard.md) — browser admin panel with an embedded chat tab
* [Configuration](/docs/zh-Hans/user-guide/configuration.md) — config that the desktop app reads and writes
* [Windows (Native)](/docs/zh-Hans/user-guide/windows-native.md) — native Windows install path
...
* [What's in the app](#whats-in-the-app)
  ...
  + [Management panes](#management-panes)
  + [Keyboard & navigation](#keyboard--navigation)
  + [Sessions & profiles](#sessions--profiles)
* [Updating](#updating)
* [Uninstalling](#uninstalling)
* [CLI reference: `hermes desktop`](#cli-reference-hermes-desktop)
* [How it works](#how-it-works)
* [Connecting to a remote backend](#connecting-to-a-remote-backend)
  + [On the backend (the remote machine)](#on-the-backend-the-remote-machine)
  + [In the app](#in-the-app)
  + [Troubleshooting](#troubleshooting)
* [Troubleshooting](#troubleshooting-1)
  + ["Build desktop app" stuck on Electron download](#build-desktop-app-stuck-on-electron-download)
...
- [https://hermes-agent.nousresearch.com/docs/getting-started/updating](./docs-getting-started-updating.md)
