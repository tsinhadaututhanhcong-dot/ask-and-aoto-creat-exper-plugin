# X (Twitter) 搜索 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/x-search](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/x-search)
...
| 凭据 | 来源 | 配置方式 |
| --- | --- | --- |
| **SuperGrok / X Premium+ OAuth**（推荐） | 在 `accounts.x.ai` 浏览器登录，自动刷新 | `hermes auth add xai-oauth` — 参见 [xAI Grok OAuth (SuperGrok / X Premium+)](/docs/zh-Hans/guides/xai-grok-oauth) |
| **`XAI_API_KEY`** | 付费 xAI API 密钥 | 在 `~/.hermes/.env` 中设置 |

两者使用相同的 endpoint 和相同的请求体，区别仅在于 bearer token。**当两者同时配置时，SuperGrok OAuth 优先**，x\_search 将消耗你的订阅配额而非付费 API 用量。
...
当 xAI 凭据（OAuth token 或 `XAI_API_KEY`）存在时自动启用。如不需要，可通过 `hermes tools` → Search → x\_search 显式禁用。
...
### "No xAI credentials available"[​](#no-xai-credentials-available "\"No xAI credentials available\"的直接链接")

当两种认证路径均失败时，工具会显示此错误。请在 `~/.hermes/.env` 中设置 `XAI_API_KEY`，或运行 `hermes auth add xai-oauth` 并完成浏览器登录。然后重启会话，让 agent 重新加载工具注册表。

### "`x_search` is not enabled for this model"[​](#x_search-is-not-enabled-for-this-model "x_search-is-not-enabled-for-this-model的直接链接")
...
### 工具未出现在 schema 中[​](#工具未出现在-schema-中 "工具未出现在 schema 中的直接链接")
...
### `degraded: true` — 回答无引用来源[​](#degraded-true--回答无引用来源 "degraded-true--回答无引用来源的直接链接")
...
* [xAI Grok OAuth (SuperGrok / Premium+)](/docs/zh-Hans/guides/xai-grok-oauth) — OAuth 配置指南
* [Web 搜索与提取](/docs/zh-Hans/user-guide/features/web-search) — 用于一般（非 X）网页搜索
* [工具参考](/docs/zh-Hans/reference/tools-reference) — 完整工具目录
...
* [故障排查](#故障排查)
  ...
  + [工具未出现在 schema 中](#工具未出现在-schema-中)
  + [`degraded: true` — 回答无引用来源](#degraded-true--回答无引用来源)
* [另请参阅](#另请参阅)
