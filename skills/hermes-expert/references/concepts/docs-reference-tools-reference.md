...
**Source:** [https://hermes-agent.nousresearch.com/docs/reference/tools-reference](https://hermes-agent.nousresearch.com/docs/reference/tools-reference)
...
## `browser` toolset (CDP-gated tools)[​](#browser-toolset-cdp-gated-tools "Direct link to browser-toolset-cdp-gated-tools")
...
## `computer_use` toolset[​](#computer_use-toolset "Direct link to computer_use-toolset")

| Tool | Description | Requires environment |
| --- | --- | --- |
| `computer_use` | Background desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list\_apps, focus\_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS, Windows, and Linux. | `cua-driver` on `$PATH` (install via `hermes tools`). |
...
## `kanban` toolset[​](#kanban-toolset "Direct link to kanban-toolset")
...
## `project` toolset[​](#project-toolset "Direct link to project-toolset")
...
## `video` toolset[​](#video-toolset "Direct link to video-toolset")
...
## `video_gen` toolset[​](#video_gen-toolset "Direct link to video_gen-toolset")
...
## `x_search` toolset[​](#x_search-toolset "Direct link to x_search-toolset")

| Tool | Description | Requires environment |
| --- | --- | --- |
| `x_search` | Search X (Twitter) posts, profiles, and threads using xAI's built-in `x_search` Responses tool. Use this for current discussion, reactions, or claims on X rather than general web pages. Off by default — opt in via `hermes tools` → 🐦 X (Twitter) Search. Schema is only registered when xAI credentials are configured (check\_fn-gated). | XAI\_API\_KEY **or** xAI Grok OAuth (SuperGrok / Premium+) login |
...
* [`browser` toolset (CDP-gated tools)](#browser-toolset-cdp-gated-tools)
...
* [`computer_use` toolset](#computer_use-toolset)
...
* [`video` toolset](#video-toolset)
* [`video_gen` toolset](#video_gen-toolset)
* [`web` toolset](#web-toolset)
* [`x_search` toolset](#x_search-toolset)
