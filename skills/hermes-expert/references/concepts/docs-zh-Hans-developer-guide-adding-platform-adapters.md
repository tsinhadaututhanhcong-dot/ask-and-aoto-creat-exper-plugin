# 添加平台适配器 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/adding-platform-adapters](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/adding-platform-adapters)
...
## Plugin 路径（推荐）[​](#plugin-path-recommended "Plugin 路径（推荐）的直接链接")
...
### plugin.yaml[​](#pluginyaml "plugin.yaml的直接链接")
...
### adapter.py[​](#adapterpy "adapter.py的直接链接")
...
### Plugin 系统自动处理的内容[​](#plugin-系统自动处理的内容 "Plugin 系统自动处理的内容的直接链接")
...
| send\_message 工具 | 通过实时 gateway 适配器路由 |
...
## 环境变量驱动的自动配置[​](#环境变量驱动的自动配置 "环境变量驱动的自动配置的直接链接")
...
## YAML→env 配置桥接[​](#yamlenv-配置桥接 "YAML→env 配置桥接的直接链接")
...
### 进程外 cron 投递[​](#进程外-cron-投递 "进程外 cron 投递的直接链接")
...
## 在 `hermes config` 中暴露环境变量[​](#surfacing-env-vars-in-hermes-config "surfacing-env-vars-in-hermes-config的直接链接")
...
## 平台专属慢速 LLM 用户体验[​](#平台专属慢速-llm-用户体验 "平台专属慢速 LLM 用户体验的直接链接")
...
### 模式：子类化 `_keep_typing` 以叠加飞行中 UX[​](#模式子类化-_keep_typing-以叠加飞行中-ux "模式子类化-_keep_typing-以叠加飞行中-ux的直接链接")
...
### 模式：子类化 `send` 以通过缓存路由而非立即发送[​](#模式子类化-send-以通过缓存路由而非立即发送 "模式子类化-send-以通过缓存路由而非立即发送的直接链接")

如果你的慢速响应 UX 缓存响应以供稍后检索（LINE 的 postback 流程），你的 `send` 覆盖需要识别三种模式：

1. **此聊天存在待处理的 postback** → 将响应缓存在 request\_id 下，不发送任何可见内容。
...
### 参考实现（Plugin 路径）[​](#参考实现plugin-路径 "参考实现（Plugin 路径）的直接链接")
...
## 分步清单（内置路径）[​](#step-by-step-checklist "分步清单（内置路径）的直接链接")

备注

此清单用于将平台直接添加到 Hermes 核心代码库 — 通常由核心贡献者为官方支持的平台执行。社区/第三方平台应使用上方的 [Plugin 路径](#plugin-path-recommended)。

### 1. Platform 枚举[​](#1-platform-枚举 "1. Platform 枚举的直接链接")
...
### 3. Gateway 配置（`gateway/config.py`）[​](#3-gateway-配置gatewayconfigpy "3-gateway-配置gatewayconfigpy的直接链接")
...
### 4. Gateway Runner（`gateway/run.py`）[​](#4-gateway-runnergatewayrunpy "4-gateway-runnergatewayrunpy的直接链接")

五个接触点：

1. **`_create_adapter()`** — 添加 `elif platform == Platform.NEWPLAT:` 分支
2. **`_is_user_authorized()` allowed\_users 映射** — `Platform.NEWPLAT: "NEWPLAT_ALLOWED_USERS"`
3. **`_is_user_authorized()` allow\_all 映射** — `Platform.NEWPLAT: "NEWPLAT_ALLOW_ALL_USERS"`
...
### 6. CLI 集成[​](#6-cli-集成 "6. CLI 集成的直接链接")

1. **`hermes_cli/config.py`** — 将所有 `NEWPLAT_*` 变量添加到 `_EXTRA_ENV_KEYS`
2. **`hermes_cli/gateway.py`** — 在 `_PLATFORMS` 列表中添加条目，包含 key、label、emoji、token\_var、setup\_instructions 和 vars
3. **`hermes_cli/platforms.py`** — 添加带 label 和 default\_toolset 的 `PlatformInfo` 条目（供 `skills_config` 和 `tools_config` TUI 使用）
...
### 8. Toolset[​](#8-toolset "8. Toolset的直接链接")
...
### 9. 可选：平台提示[​](#9-可选平台提示 "9. 可选：平台提示的直接链接")
...
| `website/docs/reference/environment-variables.md` | 所有 NEWPLAT\_\* 环境变量 |
...
### 回调/Webhook 适配器[​](#回调webhook-适配器 "回调/Webhook 适配器的直接链接")
...
* [Plugin 路径（推荐）](#plugin-path-recommended)
  ...
  + [配置](#配置)
  + [Plugin 系统自动处理的内容](#plugin-系统自动处理的内容)
* [环境变量驱动的自动配置](#环境变量驱动的自动配置)
* [YAML→env 配置桥接](#yamlenv-配置桥接)
* [Cron 投递](#cron-投递)
  + [进程外 cron 投递](#进程外-cron-投递)
* [在 `hermes config` 中暴露环境变量](#surfacing-env-vars-in-hermes-config)
* [平台专属慢速 LLM 用户体验](#平台专属慢速-llm-用户体验)
  + [模式：子类化 `_keep_typing` 以叠加飞行中 UX](#模式子类化-_keep_typing-以叠加飞行中-ux)
  + [模式：子类化 `send` 以通过缓存路由而非立即发送](#模式子类化-send-以通过缓存路由而非立即发送)
  + [何时适用此模式](#何时适用此模式)
  + [参考实现](#参考实现)
  + [参考实现（Plugin 路径）](#参考实现plugin-路径)
* [分步清单（内置路径）](#step-by-step-checklist)
  + [1. Platform 枚举](#1-platform-枚举)
  + [2. 适配器文件](#2-适配器文件)
  + [3. Gateway 配置（`gateway/config.py`）](#3-gateway-配置gatewayconfigpy)
  + [4. Gateway Runner（`gateway/run.py`）](#4-gateway-runnergatewayrunpy)
  + [5. 跨平台投递](#5-跨平台投递)
  + [6. CLI 集成](#6-cli-集成)
  + [7. 工具](#7-工具)
  + [8. Toolset](#8-toolset)
