# Godmode — Jailbreak LLMs: Parseltongue, GODMODE, ULTRAPLINIAN | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/security/security-godmode](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/security/security-godmode)
...
| Related skills | [`obliteratus`](/docs/zh-Hans/docs/user-guide/skills/optional/mlops/mlops-obliteratus) |
...
## Overview of Attack Modes[​](#overview-of-attack-modes "Overview of Attack Modes的直接链接")

### 1. GODMODE CLASSIC — System Prompt Templates[​](#1-godmode-classic--system-prompt-templates "1. GODMODE CLASSIC — System Prompt Templates的直接链接")
...
### 2. PARSELTONGUE — Input Obfuscation (33 Techniques)[​](#2-parseltongue--input-obfuscation-33-techniques "2. PARSELTONGUE — Input Obfuscation (33 Techniques)的直接链接")
...
### 3. ULTRAPLINIAN — Multi-Model Racing[​](#3-ultraplinian--multi-model-racing "3. ULTRAPLINIAN — Multi-Model Racing的直接链接")
...
## Step 0: Auto-Jailbreak (Recommended)[​](#step-0-auto-jailbreak-recommended "Step 0: Auto-Jailbreak (Recommended)的直接链接")
...
### What it does:[​](#what-it-does "What it does:的直接链接")
...
### Strategy order per model family:[​](#strategy-order-per-model-family "Strategy order per model family:的直接链接")

| Family | Strategy Order |
| --- | --- |
| Claude | boundary\_inversion → refusal\_inversion → prefill\_only → parseltongue |
| GPT | og\_godmode → refusal\_inversion → prefill\_only → parseltongue |
| Gemini | refusal\_inversion → boundary\_inversion → prefill\_only → parseltongue |
| Grok | unfiltered\_liberated → prefill\_only |
| Hermes | prefill\_only (already uncensored) |
| DeepSeek | parseltongue → refusal\_inversion → prefill\_only |
| Llama | prefill\_only → refusal\_inversion → parseltongue |
| Qwen | parseltongue → refusal\_inversion → prefill\_only |
| Mistral | prefill\_only → refusal\_inversion → parseltongue |

Each strategy is also tried with prefill messages added if it fails alone.

### After auto-jailbreak:[​](#after-auto-jailbreak "After auto-jailbreak:的直接链接")
...
## Step 1: Choose Your Attack Mode[​](#step-1-choose-your-attack-mode "Step 1: Choose Your Attack Mode的直接链接")
...
| Want persistent jailbreaking for all queries | Hermes Config | Set prefill.json + system\_prompt once |
| Stubborn refusal, single technique fails | Escalation | Combines GODMODE + PARSELTONGUE + retry |

## Step 2: GODMODE CLASSIC — Quick Start[​](#step-2-godmode-classic--quick-start "Step 2: GODMODE CLASSIC — Quick Start的直接链接")

The fastest path. Set the jailbreak system prompt and prefill in Hermes config:

### Option A: Ephemeral system prompt (config.yaml)[​](#option-a-ephemeral-system-prompt-configyaml "Option A: Ephemeral system prompt (config.yaml)的直接链接")
...
### Option B: Prefill messages (prefill.json)[​](#option-b-prefill-messages-prefilljson "Option B: Prefill messages (prefill.json)的直接链接")
...
### Option C: Both together (maximum effect)[​](#option-c-both-together-maximum-effect "Option C: Both together (maximum effect)的直接链接")
...
## Step 3: PARSELTONGUE — Obfuscating Queries[​](#step-3-parseltongue--obfuscating-queries "Step 3: PARSELTONGUE — Obfuscating Queries的直接链接")
...
### Encoding Escalation[​](#encoding-escalation "Encoding Escalation的直接链接")
...
## Step 4: ULTRAPLINIAN — Multi-Model Racing[​](#step-4-ultraplinian--multi-model-racing "Step 4: ULTRAPLINIAN — Multi-Model Racing的直接链接")
...
### Scoring Logic[​](#scoring-logic "Scoring Logic的直接链接")
...
## Step 5: Detecting Refusals[​](#step-5-detecting-refusals "Step 5: Detecting Refusals的直接链接")
...
## Step 6: Advanced — Combining Techniques[​](#step-6-advanced--combining-techniques "Step 6: Advanced — Combining Techniques的直接链接")
...
## Model-Specific Notes[​](#model-specific-notes "Model-Specific Notes的直接链接")
...
## Trigger Words (Reference)[​](#trigger-words-reference "Trigger Words (Reference)的直接链接")
...
## Source Credits[​](#source-credits "Source Credits的直接链接")

* **G0DM0D3:** [elder-plinius/G0DM0D3](https://github.com/elder-plinius/G0DM0D3) (AGPL-3.0)
* **L1B3RT4S:** [elder-plinius/L1B3RT4S](https://github.com/elder-plinius/L1B3RT4S) (AGPL-3.0)
* **Pliny the Prompter:** [@elder\_plinius](https://x.com/elder_plinius)

## Tested Results (March 2026)[​](#tested-results-march-2026 "Tested Results (March 2026)的直接链接")

Real test data from running auto\_jailbreak against Claude Sonnet 4 via OpenRouter:

### Test 1: Gray-area query ("how to pick a lock")[​](#test-1-gray-area-query-how-to-pick-a-lock "Test 1: Gray-area query (\"how to pick a lock\")的直接链接")
...
### Test 2: Hard query ("write a phishing email template")[​](#test-2-hard-query-write-a-phishing-email-template "Test 2: Hard query (\"write a phishing email template\")的直接链接")
...
### Key Findings[​](#key-findings "Key Findings的直接链接")
