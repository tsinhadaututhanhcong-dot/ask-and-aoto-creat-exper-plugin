# Jupyter Live Kernel — 通过实时 Jupyter 内核进行迭代式 Python 开发（hamelnb） | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/data-science/data-science-jupyter-live-kernel](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/data-science/data-science-jupyter-live-kernel)
...
## 何时使用本 Skill 与其他工具[​](#何时使用本-skill-与其他工具 "何时使用本 Skill 与其他工具的直接链接")

| 工具 | 使用场景 |
| --- | --- |
| **本 skill** | 迭代式探索、跨步骤保持状态、数据科学、机器学习、"试试看再检查" |
| `execute_code` | 需要访问 Hermes 工具（web\_search、文件操作）的一次性脚本。无状态。 |
...
### 启动 JupyterLab[​](#启动-jupyterlab "启动 JupyterLab的直接链接")
...
### 为 REPL 使用创建 Notebook[​](#为-repl-使用创建-notebook "为 REPL 使用创建 Notebook的直接链接")
...
### 1. 发现服务器和 notebook[​](#1-发现服务器和-notebook "1. 发现服务器和 notebook的直接链接")
...
### 2. 执行代码（主要操作）[​](#2-执行代码主要操作 "2. 执行代码（主要操作）的直接链接")
...
### 3. 检查实时变量[​](#3-检查实时变量 "3. 检查实时变量的直接链接")
...
### 4. 编辑 notebook 单元格[​](#4-编辑-notebook-单元格 "4. 编辑 notebook 单元格的直接链接")
...
### 5. 验证（重启并全部运行）[​](#5-验证重启并全部运行 "5. 验证（重启并全部运行）的直接链接")
...
## 超时默认值[​](#超时默认值 "超时默认值的直接链接")

脚本每次执行的默认超时为 30 秒。对于长时间运行的操作，传入 `--timeout 120`。初始设置或大量计算时，建议使用较宽松的超时值（60 秒以上）。

* [Skill 元数据](#skill-元数据)
* [参考：完整 SKILL.md](#参考完整-skillmd)
* [何时使用本 Skill 与其他工具](#何时使用本-skill-与其他工具)
* [前置条件](#前置条件)
* [设置](#设置)
  + [启动 JupyterLab](#启动-jupyterlab)
  + [为 REPL 使用创建 Notebook](#为-repl-使用创建-notebook)
* [核心工作流](#核心工作流)
  + [1. 发现服务器和 notebook](#1-发现服务器和-notebook)
  + [2. 执行代码（主要操作）](#2-执行代码主要操作)
  + [3. 检查实时变量](#3-检查实时变量)
  + [4. 编辑 notebook 单元格](#4-编辑-notebook-单元格)
  + [5. 验证（重启并全部运行）](#5-验证重启并全部运行)
* [实践经验提示](#实践经验提示)
* [超时默认值](#超时默认值)