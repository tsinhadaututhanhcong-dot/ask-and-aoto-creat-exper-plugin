# Browser CDP Supervisor — 设计文档 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/browser-supervisor](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/browser-supervisor)
...
## 后端能力矩阵（2026-04-23 实测验证）[​](#后端能力矩阵2026-04-23-实测验证 "后端能力矩阵（2026-04-23 实测验证）的直接链接")
...
### CDPSupervisor[​](#cdpsupervisor "CDPSupervisor的直接链接")
...
## Agent 接口（PR 1）[​](#agent-接口pr-1 "Agent 接口（PR 1）的直接链接")

### 一个新工具[​](#一个新工具 "一个新工具的直接链接")
...
### `browser_snapshot` 扩展[​](#browser_snapshot-扩展 "browser_snapshot-扩展的直接链接")
...
* **`recent_dialogs`**：最近关闭的最多 20 个对话框的环形缓冲区，带有 `closed_by` 标签——`"agent"`（我们响应了）、`"auto_policy"`（本地 auto\_dismiss/auto\_accept）、`"watchdog"`（must\_respond 超时触发）或 `"remote"`（浏览器/后端主动关闭，例如 Browserbase）。这是 Browserbase 上的 agent 仍能了解发生了什么的方式。
...
### 可用性门控[​](#可用性门控 "可用性门控的直接链接")

两个接口均通过 `_browser_cdp_check` 进行门控（supervisor 只能在 CDP 端点可达时运行）。在 Camofox / 无后端会话中，对话框工具被隐藏，快照省略新字段——不产生 schema 膨胀。

## 跨域 iframe 交互[​](#跨域-iframe-交互 "跨域 iframe 交互的直接链接")

在对话框检测工作的基础上，`browser_cdp(frame_id=...)` 通过 supervisor 已连接的 WebSocket，使用 OOPIF 的子 `sessionId` 路由 CDP 调用（尤其是 `Runtime.evaluate`）。Agent 从 `browser_snapshot.frame_tree.children[]` 中 `is_oopif=true` 的条目获取 frame\_id，并将其传递给 `browser_cdp`。对于同源 iframe（无专用 CDP 会话），agent 改用顶层 `Runtime.evaluate` 中的 `contentWindow`/`contentDocument`——当 `frame_id` 属于非 OOPIF 时，supervisor 会返回指向该回退方案的错误。

在 Browserbase 上，这是 iframe 交互的**唯一**可靠路径——无状态 CDP 连接（每次 `browser_cdp` 调用时打开）会遭遇签名 URL 过期，而 supervisor 的长连接则保持有效会话。

## Camofox（后续跟进）[​](#camofox后续跟进 "Camofox（后续跟进）的直接链接")
...
## 涉及文件（PR 1）[​](#涉及文件pr-1 "涉及文件（PR 1）的直接链接")
...
* [问题](#问题)
* [后端能力矩阵（2026-04-23 实测验证）](#后端能力矩阵2026-04-23-实测验证)
* [架构](#架构)
  + [CDPSupervisor](#cdpsupervisor)
  + [生命周期](#生命周期)
  + [对话框策略](#对话框策略)
* [Agent 接口（PR 1）](#agent-接口pr-1)
  + [一个新工具](#一个新工具)
  + [`browser_snapshot` 扩展](#browser_snapshot-扩展)
  + [可用性门控](#可用性门控)
* [跨域 iframe 交互](#跨域-iframe-交互)
* [Camofox（后续跟进）](#camofox后续跟进)
* [涉及文件（PR 1）](#涉及文件pr-1)
  + [新增](#新增)
  + [修改](#修改)
* [非目标](#非目标)
