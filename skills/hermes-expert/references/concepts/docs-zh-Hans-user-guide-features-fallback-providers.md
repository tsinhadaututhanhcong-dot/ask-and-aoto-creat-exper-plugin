# 备用提供商 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/fallback-providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/fallback-providers)

本页总览

Hermes Agent 具备三层弹性机制，在提供商出现问题时保持会话正常运行：

1. **[凭据池](/docs/zh-Hans/user-guide/features/credential-pools)** — 在*同一*提供商的多个 API 密钥之间轮换（优先尝试）
...
`fallback_model` 与 `fallback_providers`
...
## 辅助任务备用[​](#辅助任务备用 "辅助任务备用的直接链接")

Hermes 为附属任务使用独立的轻量级模型。每个任务都有自己的提供商解析链，充当内置的备用系统。

### 具有独立提供商解析的任务[​](#具有独立提供商解析的任务 "具有独立提供商解析的任务的直接链接")
...
以上每个任务均遵循相同的 **provider / model / base\_url** 模式。上下文压缩在 `auxiliary.compression` 下配置：
...
### 辅助任务的提供商选项[​](#辅助任务的提供商选项 "辅助任务的提供商选项的直接链接")

以下选项仅适用于 `auxiliary:`、`compression:` 和 `fallback_model:` 配置——`"main"` **不是**顶层 `model.provider` 的有效值。对于自定义端点，请在 `model:` 部分使用 `provider: custom`（参见 [AI 提供商](/docs/zh-Hans/integrations/providers)）。
...
## 辅助任务容量错误备用[​](#辅助任务容量错误备用 "辅助任务容量错误备用的直接链接")
...
### 可选：每任务备用链[​](#可选每任务备用链 "可选：每任务备用链的直接链接")
...
### 触发备用的提供商配额错误[​](#触发备用的提供商配额错误 "触发备用的提供商配额错误的直接链接")
...
完整配置详情参见[子 Agent 委派](/docs/zh-Hans/user-guide/features/delegation)。

---

## Cron 任务提供商[​](#cron-任务提供商 "Cron 任务提供商的直接链接")
...
完整配置详情参见[定时任务（Cron）](/docs/zh-Hans/user-guide/features/cron)。
...
* [主模型备用](#主模型备用)
  ...
  + [备用适用范围](#备用适用范围)
* [辅助任务备用](#辅助任务备用)
  + [具有独立提供商解析的任务](#具有独立提供商解析的任务)
  + [自动检测链](#自动检测链)
  + [配置辅助提供商](#配置辅助提供商)
  + [辅助任务的提供商选项](#辅助任务的提供商选项)
  + [直接端点覆盖](#直接端点覆盖)
* [辅助任务容量错误备用](#辅助任务容量错误备用)
  + [可选：每任务备用链](#可选每任务备用链)
  + [触发备用的提供商配额错误](#触发备用的提供商配额错误)
* [上下文压缩备用](#上下文压缩备用)
* [委派提供商覆盖](#委派提供商覆盖)
* [Cron 任务提供商](#cron-任务提供商)
* [总结](#总结)
