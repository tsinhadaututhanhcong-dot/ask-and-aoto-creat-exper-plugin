# 用 Cron 自动化一切 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/automate-with-cron](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/automate-with-cron)

本页总览

[每日简报机器人教程](/docs/zh-Hans/guides/daily-briefing-bot)涵盖了基础内容。本指南更进一步——五种真实的自动化模式，可直接改造用于你自己的工作流。

完整功能参考请见 [定时任务（Cron）](/docs/zh-Hans/user-guide/features/cron)。

核心概念

Cron 任务在全新的 agent 会话中运行，不保留当前对话的任何记忆。Prompt（提示词）必须**完全自包含**——把 agent 需要知道的一切都写进去。

不需要 LLM？你有两种零 token 方案。

* **循环看门狗**：脚本本身已能生成精确消息（内存告警、磁盘告警、心跳）时，使用 [纯脚本 cron 任务](/docs/zh-Hans/guides/cron-script-only)。相同的调度器，无需 LLM。你可以在对话中让 Hermes 帮你设置——`cronjob` 工具知道何时选择 `no_agent=True` 并为你编写脚本。
* **已在运行的脚本发起的一次性通知**（CI 步骤、post-commit hook、部署脚本、外部调度的监控）：使用 [`hermes send`](/docs/zh-Hans/guides/pipe-script-output) 将 stdout 或文件直接推送到 Telegram / Discord / Slack 等，无需设置 cron 条目。

---

## 模式一：网站变更监控[​](#模式一网站变更监控 "模式一：网站变更监控的直接链接")
...
## 模式二：每周报告[​](#模式二每周报告 "模式二：每周报告的直接链接")
...
## 模式三：GitHub 仓库监控[​](#模式三github-仓库监控 "模式三：GitHub 仓库监控的直接链接")
...
## 模式四：数据采集管道[​](#模式四数据采集管道 "模式四：数据采集管道的直接链接")
...
## 模式五：多技能工作流[​](#模式五多技能工作流 "模式五：多技能工作流的直接链接")
...
*完整的 cron 参考——所有参数、边界情况和内部机制——请见 [定时任务（Cron）](/docs/zh-Hans/user-guide/features/cron)。*

* [模式一：网站变更监控](#模式一网站变更监控)
* [模式二：每周报告](#模式二每周报告)
* [模式三：GitHub 仓库监控](#模式三github-仓库监控)
* [模式四：数据采集管道](#模式四数据采集管道)
* [模式五：多技能工作流](#模式五多技能工作流)
* [管理你的任务](#管理你的任务)
* [投递目标](#投递目标)
* [使用技巧](#使用技巧)
