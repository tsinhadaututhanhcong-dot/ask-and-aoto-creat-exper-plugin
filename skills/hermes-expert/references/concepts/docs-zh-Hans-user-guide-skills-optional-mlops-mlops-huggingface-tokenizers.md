# Huggingface Tokenizers — 为研究和生产优化的快速 tokenizer | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-huggingface-tokenizers](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/optional/mlops/mlops-huggingface-tokenizers)
...
## 何时使用 HuggingFace Tokenizers[​](#何时使用-huggingface-tokenizers "何时使用 HuggingFace Tokenizers的直接链接")
...
### 加载预训练 tokenizer[​](#加载预训练-tokenizer "加载预训练 tokenizer的直接链接")
...
### 训练自定义 BPE tokenizer[​](#训练自定义-bpe-tokenizer "训练自定义 BPE tokenizer的直接链接")
...
### 批量编码与 padding[​](#批量编码与-padding "批量编码与 padding的直接链接")
...
### BPE（字节对编码）[​](#bpe字节对编码 "BPE（字节对编码）的直接链接")
...
### WordPiece[​](#wordpiece "WordPiece的直接链接")
...
## 分词流水线[​](#分词流水线 "分词流水线的直接链接")

完整流水线：**归一化 → 预分词 → 模型 → 后处理**

### 归一化（Normalization）[​](#归一化normalization "归一化（Normalization）的直接链接")
...
### 预分词（Pre-tokenization）[​](#预分词pre-tokenization "预分词（Pre-tokenization）的直接链接")
...
### 后处理（Post-processing）[​](#后处理post-processing "后处理（Post-processing）的直接链接")
...
## 与 transformers 集成[​](#与-transformers-集成 "与 transformers 集成的直接链接")

### 使用 AutoTokenizer 加载[​](#使用-autotokenizer-加载 "使用 AutoTokenizer 加载的直接链接")
...
### 将自定义 tokenizer 转换为 transformers 格式[​](#将自定义-tokenizer-转换为-transformers-格式 "将自定义 tokenizer 转换为 transformers 格式的直接链接")
...
### 从迭代器训练（大型数据集）[​](#从迭代器训练大型数据集 "从迭代器训练（大型数据集）的直接链接")
...
### 启用 truncation 和 padding[​](#启用-truncation-和-padding "启用 truncation 和 padding的直接链接")
...
* [何时使用 HuggingFace Tokenizers](#何时使用-huggingface-tokenizers)
* [快速开始](#快速开始)
  + [安装](#安装)
  + [加载预训练 tokenizer](#加载预训练-tokenizer)
  + [训练自定义 BPE tokenizer](#训练自定义-bpe-tokenizer)
  + [批量编码与 padding](#批量编码与-padding)
* [分词算法](#分词算法)
  + [BPE（字节对编码）](#bpe字节对编码)
  + [WordPiece](#wordpiece)
  + [Unigram](#unigram)
* [分词流水线](#分词流水线)
  ...
  + [预分词（Pre-tokenization）](#预分词pre-tokenization)
  + [后处理（Post-processing）](#后处理post-processing)
* [对齐追踪](#对齐追踪)
* [与 transformers 集成](#与-transformers-集成)
  + [使用 AutoTokenizer 加载](#使用-autotokenizer-加载)
  + [将自定义 tokenizer 转换为 transformers 格式](#将自定义-tokenizer-转换为-transformers-格式)
* [常见模式](#常见模式)
  + [从迭代器训练（大型数据集）](#从迭代器训练大型数据集)
  + [启用 truncation 和 padding](#启用-truncation-和-padding)
  + [多进程处理](#多进程处理)
* [性能基准](#性能基准)
  + [训练速度](#训练速度)
  + [分词速度](#分词速度)
