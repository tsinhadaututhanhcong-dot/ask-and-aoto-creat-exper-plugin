# Ascii Video — ASCII 视频：将视频/音频转换为彩色 ASCII MP4/GIF | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-ascii-video](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/creative/creative-ascii-video)
...
3. **SCENE\_FN** — 场景函数渲染到像素画布（`uint8 H,W,3`）。通过 `_render_vf()` + 像素混合模式组合多个字符网格。参见 `references/composition.md`
...
### 第一步：创作愿景[​](#第一步创作愿景 "第一步：创作愿景的直接链接")
...
### 第三步：构建脚本[​](#第三步构建脚本 "第三步：构建脚本的直接链接")
...
### 第四步：质量验证[​](#第四步质量验证 "第四步：质量验证的直接链接")
...
### 亮度——使用 `tonemap()`，而非线性乘数[​](#亮度使用-tonemap而非线性乘数 "亮度使用-tonemap而非线性乘数的直接链接")
...
* [工作流程](#工作流程)
  ...
  + [第四步：质量验证](#第四步质量验证)
* [关键实现注意事项](#关键实现注意事项)
  + [亮度——使用 `tonemap()`，而非线性乘数](#亮度使用-tonemap而非线性乘数)
  + [字体单元高度](#字体单元高度)
  + [ffmpeg 管道死锁](#ffmpeg-管道死锁)
  + [字体兼容性](#字体兼容性)
  + [逐片段架构](#逐片段架构)
* [性能目标](#性能目标)
* [参考资料](#参考资料)
