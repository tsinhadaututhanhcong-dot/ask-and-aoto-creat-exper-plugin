# Segment Anything Model — SAM：通过点、框、掩码实现零样本图像分割 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/mlops/mlops-models-segment-anything](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/mlops/mlops-models-segment-anything)
...
## 何时使用 SAM[​](#何时使用-sam "何时使用 SAM的直接链接")
...
### 使用 SamPredictor 的基本用法[​](#使用-sampredictor-的基本用法 "使用 SamPredictor 的基本用法的直接链接")
...
### Prompt 类型[​](#prompt-类型 "Prompt 类型的直接链接")
...
## 交互式分割[​](#交互式分割 "交互式分割的直接链接")

### 点 prompt[​](#点-prompt "点 prompt的直接链接")
...
### 框 prompt[​](#框-prompt "框 prompt的直接链接")
...
### 组合 prompt[​](#组合-prompt "组合 prompt的直接链接")
...
## 自动掩码生成[​](#自动掩码生成 "自动掩码生成的直接链接")

### 基本自动分割[​](#基本自动分割 "基本自动分割的直接链接")
...
### 每张图像多个 prompt[​](#每张图像多个-prompt "每张图像多个 prompt的直接链接")
...
### 使用 ONNX 模型[​](#使用-onnx-模型 "使用 ONNX 模型的直接链接")
...
### 工作流 1：标注工具[​](#工作流-1标注工具 "工作流 1：标注工具的直接链接")
...
### 工作流 2：对象提取[​](#工作流-2对象提取 "工作流 2：对象提取的直接链接")
...
### 工作流 3：医学图像分割[​](#工作流-3医学图像分割 "工作流 3：医学图像分割的直接链接")
...
### COCO RLE 格式[​](#coco-rle-格式 "COCO RLE 格式的直接链接")
...
| 推理缓慢 | 使用 ViT-B，减小 points\_per\_side |
| 掩码质量差 | 尝试不同 prompt，使用框 + 点组合 |
| 边缘伪影 | 使用 stability\_score 过滤 |
| 小对象漏检 | 增大 points\_per\_side |
...
* [快速开始](#快速开始)
  + [安装](#安装)
  + [下载检查点](#下载检查点)
  + [使用 SamPredictor 的基本用法](#使用-sampredictor-的基本用法)
...
* [批量推理](#批量推理)
  + [多张图像](#多张图像)
  + [每张图像多个 prompt](#每张图像多个-prompt)
* [ONNX 部署](#onnx-部署)
  + [导出模型](#导出模型)
  + [使用 ONNX 模型](#使用-onnx-模型)
* [常见工作流](#常见工作流)
  + [工作流 1：标注工具](#工作流-1标注工具)
  + [工作流 2：对象提取](#工作流-2对象提取)
  + [工作流 3：医学图像分割](#工作流-3医学图像分割)
* [输出格式](#输出格式)
  + [掩码数据结构](#掩码数据结构)
