...
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/spotify](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/spotify)
...
### 一键完成：`hermes tools` 或首次运行设置[​](#一键完成hermes-tools-或首次运行设置 "一键完成hermes-tools-或首次运行设置的直接链接")
...
#### 2. 运行登录向导[​](#2-运行登录向导 "2. 运行登录向导的直接链接")
...
### 创建 Spotify 应用（向导所需内容）[​](#创建-spotify-应用向导所需内容 "创建 Spotify 应用（向导所需内容）的直接链接")
...
### 通过 SSH / 在无头环境中运行[​](#通过-ssh--在无头环境中运行 "通过 SSH / 在无头环境中运行的直接链接")
...
关于跳板机/堡垒机设置及其他注意事项（mosh、tmux、端口冲突），请参阅 [OAuth over SSH / Remote Hosts](/docs/zh-Hans/guides/oauth-over-ssh)。
...
#### `spotify_playback`[​](#spotify_playback "spotify_playback的直接链接")
...
#### `spotify_devices`[​](#spotify_devices "spotify_devices的直接链接")
...
### Home Assistant 管理的音箱[​](#home-assistant-管理的音箱 "Home Assistant 管理的音箱的直接链接")
...
#### `spotify_queue`[​](#spotify_queue "spotify_queue的直接链接")
...
#### `spotify_search`[​](#spotify_search "spotify_search的直接链接")

搜索曲库。`query` 为必填项。可选：`types`（`track` / `album` / `artist` / `playlist` / `show` / `episode` 的数组）、`limit`、`offset`、`market`。

#### `spotify_playlists`[​](#spotify_playlists "spotify_playlists的直接链接")
...
#### `spotify_albums`[​](#spotify_albums "spotify_albums的直接链接")
...
#### `spotify_library`[​](#spotify_library "spotify_library的直接链接")
...
### 功能矩阵：免费版 vs Premium 版[​](#功能矩阵免费版-vs-premium-版 "功能矩阵：免费版 vs Premium 版的直接链接")

只读工具在免费账号上可用。任何修改播放状态或队列的操作都需要 Premium。

| 免费版可用 | 需要 Premium |
| --- | --- |
| `spotify_search`（全部） | `spotify_playback` — play、pause、next、previous、seek、set\_repeat、set\_shuffle、set\_volume |
| `spotify_playback` — get\_state、get\_currently\_playing、recently\_played | `spotify_queue` — add |
...
## 定时任务：Spotify + cron[​](#定时任务spotify--cron "定时任务：Spotify + cron的直接链接")

由于 Spotify 工具是普通的 Hermes 工具，在 Hermes 会话中运行的 cron 任务可以按任意计划触发播放，无需编写额外代码。

### 早晨唤醒播放列表[​](#早晨唤醒播放列表 "早晨唤醒播放列表的直接链接")
...
完整 cron 参考：[Cron Jobs](/docs/zh-Hans/user-guide/features/cron)。
...
## 进阶：自定义 scope[​](#进阶自定义-scope "进阶：自定义 scope的直接链接")
...
## 进阶：自定义 client ID / redirect URI[​](#进阶自定义-client-id--redirect-uri "进阶：自定义 client ID / redirect URI的直接链接")
...
* [前提条件](#前提条件)
* [设置](#设置)
  + [一键完成：`hermes tools` 或首次运行设置](#一键完成hermes-tools-或首次运行设置)
  + [两步流程](#两步流程)
  + [创建 Spotify 应用（向导所需内容）](#创建-spotify-应用向导所需内容)
  + [通过 SSH / 在无头环境中运行](#通过-ssh--在无头环境中运行)
* [验证](#验证)
* [使用方法](#使用方法)
  + [工具参考](#工具参考)
  + [Home Assistant 管理的音箱](#home-assistant-管理的音箱)
  + [功能矩阵：免费版 vs Premium 版](#功能矩阵免费版-vs-premium-版)
* [定时任务：Spotify + cron](#定时任务spotify--cron)
  + [早晨唤醒播放列表](#早晨唤醒播放列表)
  + [夜间收尾](#夜间收尾)
  + [注意事项](#注意事项)
* [退出登录](#退出登录)
* [故障排查](#故障排查)
* [进阶：自定义 scope](#进阶自定义-scope)
* [进阶：自定义 client ID / redirect URI](#进阶自定义-client-id--redirect-uri)
* [文件位置](#文件位置)
