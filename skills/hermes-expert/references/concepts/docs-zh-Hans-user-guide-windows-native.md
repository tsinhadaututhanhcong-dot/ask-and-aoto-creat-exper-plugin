# windows-native | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/windows-native](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/windows-native)

本页总览

## P--- title: "Windows（原生）指南" description: "在 Windows 10 / 11 上原生运行 Hermes Agent — 安装、功能矩阵、UTF-8 控制台、Git Bash、将 gateway 作为计划任务、编辑器处理、PATH、卸载及常见问题" sidebar\_label: "Windows（原生）" sidebar\_position: 3[​](#p---title-windows原生指南description-在-windows-10--11-上原生运行-hermes-agent--安装功能矩阵utf-8-控制台git-bash将-gateway-作为计划任务编辑器处理path卸载及常见问题sidebar_label-windows原生sidebar_position-3 "P--- title: \"Windows（原生）指南\" description: \"在 Windows 10 / 11 上原生运行 Hermes Agent — 安装、功能矩阵、UTF-8 控制台、Git Bash、将 gateway 作为计划任务、编辑器处理、PATH、卸载及常见问题\" sidebar_label: \"Windows（原生）\" sidebar_position: 3的直接链接")
...
如果你只是想安装，[首页](/docs/zh-Hans/) 或[安装页面](/docs/zh-Hans/getting-started/installation#windows%E5%8E%9F%E7%94%9Fpowershell)上的一行命令就够了。遇到意外情况时再回来查阅本页。

想用 WSL？

如果你更倾向于真正的 POSIX 环境（用于 dashboard 内嵌终端、`fork` 语义、Linux 风格文件监视器等），请参阅 **[Windows（WSL2）指南](/docs/zh-Hans/user-guide/windows-wsl-quickstart)**。两者可以干净共存：原生数据存放在 `%LOCALAPPDATA%\hermes`，WSL 数据存放在 `~/.hermes`。
...
### 桌面安装程序（备选方案）[​](#桌面安装程序备选方案 "桌面安装程序（备选方案）的直接链接")
...
### 依赖引导（`dep_ensure`）[​](#依赖引导dep_ensure "依赖引导dep_ensure的直接链接")
...
## 安装程序实际做了什么[​](#安装程序实际做了什么 "安装程序实际做了什么的直接链接")
...
在 Windows 上跳过繁琐的提供商配置

在 Windows 上，逐个配置工具 API key（Firecrawl、FAL、Browser Use、OpenAI TTS）是获得可用 agent 摩擦最大的部分。[Nous Portal](/docs/zh-Hans/user-guide/features/tool-gateway) 订阅通过一次 OAuth 登录即可覆盖模型**以及**所有这些工具。安装程序完成后，运行 `hermes setup --portal` 完成配置。
...
## Hermes 在 Windows 上如何运行 shell 命令[​](#hermes-在-windows-上如何运行-shell-命令 "Hermes 在 Windows 上如何运行 shell 命令的直接链接")
...
## Windows 上的 UTF-8 控制台[​](#windows-上的-utf-8-控制台 "Windows 上的 UTF-8 控制台的直接链接")
...
1. 通过 `kernel32.SetConsoleCP` / `SetConsoleOutputCP` 将控制台代码页切换为 CP\_UTF8（65001）。
...
## 编辑器（`Ctrl-X Ctrl-E`、`/edit`）[​](#编辑器ctrl-x-ctrl-eedit "编辑器ctrl-x-ctrl-eedit的直接链接")

在 PR #21561 之前，在 Windows 上按 `Ctrl-X Ctrl-E` 或输入 `/edit` 会静默无响应。prompt\_toolkit 有一个硬编码的 POSIX 绝对路径回退列表（`/usr/bin/nano`、`/usr/bin/pico`、`/usr/bin/vi` 等），在 Windows 上永远无法解析——即使安装了完整的 Git for Windows 也不行。
...
## CLI 中用 `Ctrl+Enter` 换行[​](#cli-中用-ctrlenter-换行 "cli-中用-ctrlenter-换行的直接链接")
...
## 在 Windows 登录时运行 gateway[​](#在-windows-登录时运行-gateway "在 Windows 登录时运行 gateway的直接链接")
...
### 为什么不用 Windows 服务？[​](#为什么不用-windows-服务 "为什么不用 Windows 服务？的直接链接")
...
## 在 Windows 上运行 Hermes — 实用说明[​](#在-windows-上运行-hermes--实用说明 "在 Windows 上运行 Hermes — 实用说明的直接链接")

### 安装后的 PATH[​](#安装后的-path "安装后的 PATH的直接链接")
...
### Windows 专属环境变量[​](#windows-专属环境变量 "Windows 专属环境变量的直接链接")
...
## 进程管理内部机制[​](#进程管理内部机制 "进程管理内部机制的直接链接")
...
* **[安装](/docs/zh-Hans/getting-started/installation)** — 完整安装页面，包括 Linux/macOS/WSL2/Termux。
* **[Windows（WSL2）指南](/docs/zh-Hans/user-guide/windows-wsl-quickstart)** — 如果你需要 POSIX 语义或 dashboard 终端面板。
* **[CLI 参考](/docs/zh-Hans/reference/cli-commands)** — 所有 `hermes` 子命令。
* **[FAQ](/docs/zh-Hans/reference/faq)** — 常见的非 Windows 专属问题。
* **[消息 Gateway](/docs/zh-Hans/user-guide/messaging/)** — 在 Windows 上运行 Telegram/Discord/Slack。

* [P---
  title: "Windows（原生）指南"
  description: "在 Windows 10 / 11 上原生运行 Hermes Agent — 安装、功能矩阵、UTF-8 控制台、Git Bash、将 gateway 作为计划任务、编辑器处理、PATH、卸载及常见问题"
  sidebar\_label: "Windows（原生）"
  sidebar\_position: 3](#p---title-windows原生指南description-在-windows-10--11-上原生运行-hermes-agent--安装功能矩阵utf-8-控制台git-bash将-gateway-作为计划任务编辑器处理path卸载及常见问题sidebar_label-windows原生sidebar_position-3)
* [快速安装](#快速安装)
  + [桌面安装程序（备选方案）](#桌面安装程序备选方案)
  + [依赖引导（`dep_ensure`）](#依赖引导dep_ensure)
* [安装程序实际做了什么](#安装程序实际做了什么)
* [功能矩阵](#功能矩阵)
* [Hermes 在 Windows 上如何运行 shell 命令](#hermes-在-windows-上如何运行-shell-命令)
* [Windows 上的 UTF-8 控制台](#windows-上的-utf-8-控制台)
* [编辑器（`Ctrl-X Ctrl-E`、`/edit`）](#编辑器ctrl-x-ctrl-eedit)
* [CLI 中用 `Ctrl+Enter` 换行](#cli-中用-ctrlenter-换行)
* [在 Windows 登录时运行 gateway](#在-windows-登录时运行-gateway)
  + [安装](#安装)
  + [管理](#管理)
  + [为什么不用 Windows 服务？](#为什么不用-windows-服务)
* [数据布局](#数据布局)
* [浏览器工具](#浏览器工具)
* [在 Windows 上运行 Hermes — 实用说明](#在-windows-上运行-hermes--实用说明)
  + [安装后的 PATH](#安装后的-path)
  + [环境变量](#环境变量)
  + [Windows 专属环境变量](#windows-专属环境变量)
* [卸载](#卸载)
* [进程管理内部机制](#进程管理内部机制)
* [常见问题](#常见问题)
* [下一步](#下一步)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/](./docs-zh-Hans.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/windows-wsl-quickstart](./docs-zh-Hans-user-guide-windows-wsl-quickstart.md)
