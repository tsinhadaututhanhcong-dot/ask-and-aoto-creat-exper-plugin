# Teams 会议 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/teams-meetings](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/teams-meetings)
...
关于上线后的日常运维、上线检查及运维工作表，请参阅专项指南：[运维 Teams 会议流水线](/docs/zh-Hans/guides/operate-teams-meeting-pipeline)。
...
* 可正常运行的 Hermes 安装
* 若需要 Teams 出站投递，需完成现有的 [Microsoft Teams bot 配置](/docs/zh-Hans/user-guide/messaging/teams)
* 具备订阅所需会议资源权限的 Microsoft Graph 应用凭据
* Microsoft Graph 可调用的公网 HTTPS URL，用于 webhook 投递
* 若需要录音加 STT 回退，需安装 `ffmpeg`

## 第一步：添加 Microsoft Graph 凭据[​](#第一步添加-microsoft-graph-凭据 "第一步：添加 Microsoft Graph 凭据的直接链接")
...
## 第二步：启用 Graph Webhook 监听器[​](#第二步启用-graph-webhook-监听器 "第二步：启用 Graph Webhook 监听器的直接链接")
...
## 第三步：配置 Teams 投递与流水线行为[​](#第三步配置-teams-投递与流水线行为 "第三步：配置 Teams 投递与流水线行为的直接链接")
...
## Teams 投递模式[​](#teams-投递模式 "Teams 投递模式的直接链接")

流水线在现有 Teams 插件内支持两种 Teams 摘要投递模式。

### `incoming_webhook`[​](#incoming_webhook "incoming_webhook的直接链接")
...
## 第五步：创建 Graph 订阅[​](#第五步创建-graph-订阅 "第五步：创建 Graph 订阅的直接链接")
...
Microsoft Graph 将 webhook 订阅上限设为 72 小时，且不会自动续期。你**必须**在上线前调度 `hermes teams-pipeline maintain-subscriptions`，否则通知将在手动创建订阅三天后静默停止。请参阅运维手册中的[自动化订阅续期](/docs/zh-Hans/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production)——提供三种方案（Hermes cron、systemd timer、普通 crontab）。

关于订阅维护和上线后的运维流程，请继续阅读指南：[运维 Teams 会议流水线](/docs/zh-Hans/guides/operate-teams-meeting-pipeline)。
...
* [Microsoft Teams bot 配置](/docs/zh-Hans/user-guide/messaging/teams)
* [运维 Teams 会议流水线](/docs/zh-Hans/guides/operate-teams-meeting-pipeline)

* [功能说明](#功能说明)
* [前提条件](#前提条件)
* [第一步：添加 Microsoft Graph 凭据](#第一步添加-microsoft-graph-凭据)
* [第二步：启用 Graph Webhook 监听器](#第二步启用-graph-webhook-监听器)
* [第三步：配置 Teams 投递与流水线行为](#第三步配置-teams-投递与流水线行为)
* [Teams 投递模式](#teams-投递模式)
  + [`incoming_webhook`](#incoming_webhook)
  + [`graph`](#graph)
* [第四步：启动 Gateway](#第四步启动-gateway)
* [第五步：创建 Graph 订阅](#第五步创建-graph-订阅)
* [验证](#验证)
* [故障排查](#故障排查)
* [相关文档](#相关文档)
