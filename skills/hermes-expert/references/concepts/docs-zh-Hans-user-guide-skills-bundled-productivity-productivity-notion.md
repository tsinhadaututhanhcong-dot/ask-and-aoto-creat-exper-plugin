# Notion — Notion API + ntn CLI：页面、数据库、Markdown、Workers | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-notion](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/skills/bundled/productivity/productivity-notion)
...
### 1. 获取集成 token（两种方式均需要）[​](#1-获取集成-token两种方式均需要 "1. 获取集成 token（两种方式均需要）的直接链接")
...
### 2. 安装 `ntn`（macOS / Linux 上的首选方式）[​](#2-安装-ntnmacos--linux-上的首选方式 "2-安装-ntnmacos--linux-上的首选方式的直接链接")
...
### 3. 运行时选择路径[​](#3-运行时选择路径 "3. 运行时选择路径的直接链接")
...
## Path A — `ntn` CLI（首选，macOS / Linux）[​](#path-a--ntn-cli首选macos--linux "path-a--ntn-cli首选macos--linux的直接链接")

### 原始 API 调用（curl 的简写）[​](#原始-api-调用curl-的简写 "原始 API 调用（curl 的简写）的直接链接")
...
### 读取页面元数据[​](#读取页面元数据 "读取页面元数据的直接链接")

```
ntn api v1/pages/{page_id}
```

### 以 Markdown 格式读取页面（适合 agent 使用）[​](#以-markdown-格式读取页面适合-agent-使用 "以 Markdown 格式读取页面（适合 agent 使用）的直接链接")

```
ntn api v1/pages/{page_id}/markdown
```

### 以块（block）形式读取页面内容[​](#以块block形式读取页面内容 "以块（block）形式读取页面内容的直接链接")

```
ntn api v1/blocks/{page_id}/children
```

### 从 Markdown 创建页面[​](#从-markdown-创建页面 "从 Markdown 创建页面的直接链接")
...
### 用 Markdown 更新页面[​](#用-markdown-更新页面 "用 Markdown 更新页面的直接链接")
...
### 查询数据库（data source）[​](#查询数据库data-source "查询数据库（data source）的直接链接")
...
### 文件上传（单行命令——CLI 最大优势）[​](#文件上传单行命令cli-最大优势 "文件上传（单行命令——CLI 最大优势）的直接链接")
...
## Path B — HTTP + curl（跨平台，Windows 默认方式）[​](#path-b--http--curl跨平台windows-默认方式 "Path B — HTTP + curl（跨平台，Windows 默认方式）的直接链接")
...
### 读取页面元数据[​](#读取页面元数据-1 "读取页面元数据的直接链接")
...
### 以 Markdown 格式读取页面（适合 agent 使用）[​](#以-markdown-格式读取页面适合-agent-使用-1 "以 Markdown 格式读取页面（适合 agent 使用）的直接链接")
...
### 以块形式读取页面内容（需要结构化数据时使用）[​](#以块形式读取页面内容需要结构化数据时使用 "以块形式读取页面内容（需要结构化数据时使用）的直接链接")
...
### 从 Markdown 创建页面[​](#从-markdown-创建页面-1 "从 Markdown 创建页面的直接链接")
...
### 用 Markdown 更新页面[​](#用-markdown-更新页面-1 "用 Markdown 更新页面的直接链接")
...
### 在数据库中创建页面（带类型属性）[​](#在数据库中创建页面带类型属性 "在数据库中创建页面（带类型属性）的直接链接")
...
### 查询数据库（data source）[​](#查询数据库data-source-1 "查询数据库（data source）的直接链接")
...
### 文件上传（三步流程）[​](#文件上传三步流程 "文件上传（三步流程）的直接链接")
...
## API 版本 2025-09-03 — 数据库与 Data Sources[​](#api-版本-2025-09-03--数据库与-data-sources "API 版本 2025-09-03 — 数据库与 Data Sources的直接链接")
...
## Notion Workers（高级功能，需要 `ntn`）[​](#notion-workers高级功能需要-ntn "notion-workers高级功能需要-ntn的直接链接")
...
### 最简 Worker[​](#最简-worker "最简 Worker的直接链接")
...
### Webhook 能力[​](#webhook-能力 "Webhook 能力的直接链接")
...
### Worker 生命周期命令[​](#worker-生命周期命令 "Worker 生命周期命令的直接链接")
...
## Notion 风格 Markdown（用于 `/markdown` 端点）[​](#notion-风格-markdown用于-markdown-端点 "notion-风格-markdown用于-markdown-端点的直接链接")
...
* [配置](#配置)
  + [1. 获取集成 token（两种方式均需要）](#1-获取集成-token两种方式均需要)
  + [2. 安装 `ntn`（macOS / Linux 上的首选方式）](#2-安装-ntnmacos--linux-上的首选方式)
  + [3. 运行时选择路径](#3-运行时选择路径)
* [API 基础](#api-基础)
* [Path A — `ntn` CLI（首选，macOS / Linux）](#path-a--ntn-cli首选macos--linux)
  + [原始 API 调用（curl 的简写）](#原始-api-调用curl-的简写)
  + [搜索](#搜索)
  + [读取页面元数据](#读取页面元数据)
  + [以 Markdown 格式读取页面（适合 agent 使用）](#以-markdown-格式读取页面适合-agent-使用)
  + [以块（block）形式读取页面内容](#以块block形式读取页面内容)
  + [从 Markdown 创建页面](#从-markdown-创建页面)
  + [用 Markdown 更新页面](#用-markdown-更新页面)
  + [查询数据库（data source）](#查询数据库data-source)
  + [文件上传（单行命令——CLI 最大优势）](#文件上传单行命令cli-最大优势)
  + [常用环境变量](#常用环境变量)
* [Path B — HTTP + curl（跨平台，Windows 默认方式）](#path-b--http--curl跨平台windows-默认方式)
  + [搜索](#搜索-1)
  + [读取页面元数据](#读取页面元数据-1)
  + [以 Markdown 格式读取页面（适合 agent 使用）](#以-markdown-格式读取页面适合-agent-使用-1)
  + [以块形式读取页面内容（需要结构化数据时使用）](#以块形式读取页面内容需要结构化数据时使用)
  + [从 Markdown 创建页面](#从-markdown-创建页面-1)
  + [用 Markdown 更新页面](#用-markdown-更新页面-1)
  + [在数据库中创建页面（带类型属性）](#在数据库中创建页面带类型属性)
  + [查询数据库（data source）](#查询数据库data-source-1)
  + [创建数据库](#创建数据库)
  + [更新页面属性](#更新页面属性)
  + [向页面追加块](#向页面追加块)
  + [文件上传（三步流程）](#文件上传三步流程)
* [属性类型](#属性类型)
* [API 版本 2025-09-03 — 数据库与 Data Sources](#api-版本-2025-09-03--数据库与-data-sources)
* [Notion Workers（高级功能，需要 `ntn`）](#notion-workers高级功能需要-ntn)
  + [最简 Worker](#最简-worker)
  + [Webhook 能力](#webhook-能力)
  + [Worker 生命周期命令](#worker-生命周期命令)
* [Notion 风格 Markdown（用于 `/markdown` 端点）](#notion-风格-markdown用于-markdown-端点)
* [选择合适的路径](#选择合适的路径)
* [注意事项](#注意事项)