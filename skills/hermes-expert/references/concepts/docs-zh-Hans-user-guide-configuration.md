# 配置 | Hermes Agent
**Source:** [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuration](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuration)
...
关于 AI provider 设置（OpenRouter、Anthropic、Copilot、自定义端点、自托管 LLM、回退模型等），请参阅 [AI Providers](/docs/zh-Hans/integrations/providers)。

### Provider 超时[​](#provider-超时 "Provider 超时的直接链接")
...
### Local 后端[​](#local-后端 "Local 后端的直接链接")
...
### Docker 后端[​](#docker-后端 "Docker 后端的直接链接")
...
### Modal 后端[​](#modal-后端 "Modal 后端的直接链接")
...
### Daytona 后端[​](#daytona-后端 "Daytona 后端的直接链接")
...
### Singularity/Apptainer 后端[​](#singularityapptainer-后端 "Singularity/Apptainer 后端的直接链接")
...
### 常见终端后端问题[​](#常见终端后端问题 "常见终端后端问题的直接链接")
...
### 拆卸时远程到宿主文件同步[​](#拆卸时远程到宿主文件同步 "拆卸时远程到宿主文件同步的直接链接")
...
### Docker 卷挂载[​](#docker-卷挂载 "Docker 卷挂载的直接链接")
...
### Docker 凭据转发[​](#docker-凭据转发 "Docker 凭据转发的直接链接")
...
### 以宿主用户身份运行容器[​](#以宿主用户身份运行容器 "以宿主用户身份运行容器的直接链接")
...
### 可选：将启动目录挂载到 `/workspace`[​](#可选将启动目录挂载到-workspace "可选将启动目录挂载到-workspace的直接链接")
...
### 持久 Shell[​](#持久-shell "持久 Shell的直接链接")
...
有关每个后端的详细信息，请参阅[代码执行](/docs/zh-Hans/user-guide/features/code-execution)和 [README 的终端部分](/docs/zh-Hans/user-guide/features/tools)。
...
有关在您自己的技能中声明配置设置的详细信息，请参阅[创建技能 — 配置设置](/docs/zh-Hans/developer-guide/creating-skills#config-settings-configyaml)。

### Agent 创建技能写入的守卫[​](#agent-创建技能写入的守卫 "Agent 创建技能写入的守卫的直接链接")
...
## 工具输出截断限制[​](#工具输出截断限制 "工具输出截断限制的直接链接")
...
## Git Worktree 隔离[​](#git-worktree-隔离 "Git Worktree 隔离的直接链接")
...
### 三个旋钮的交互方式[​](#三个旋钮的交互方式 "三个旋钮的交互方式的直接链接")
...
## 上下文引擎[​](#上下文引擎 "上下文引擎的直接链接")

上下文引擎控制在接近模型 token 限制时如何管理对话。内置的 `compressor` 引擎使用有损摘要（参见[上下文压缩](/docs/zh-Hans/developer-guide/context-compression-and-caching)）。插件引擎可以用替代策略替换它。
...
有关内存插件的类似单选系统，请参阅[内存 Providers](/docs/zh-Hans/user-guide/features/memory-providers)。
...
`agent.api_max_retries` 控制 Hermes 在回退 provider 切换启动**之前**对瞬时错误（速率限制、连接断开、5xx）重试 provider API 调用的次数。默认为 `3` —— 总共四次尝试。如果您配置了[回退 providers](/docs/zh-Hans/user-guide/features/fallback-providers) 并希望更快地故障转移，请将其降至 `0`，这样主 provider 上的第一个瞬时错误会立即切换到回退，而不是对不稳定的端点进行重试。

### API 超时[​](#api-超时 "API 超时的直接链接")
...
选项：`fill_first`（默认）、`round_robin`、`least_used`、`random`。完整文档请参阅[凭据池](/docs/zh-Hans/user-guide/features/credential-pools)。

## Prompt 缓存[​](#prompt-缓存 "Prompt 缓存的直接链接")
...
Qwen Cloud（阿里巴巴 DashScope）上游将缓存 TTL 限制为 5 分钟，因此 Hermes 在那里使用 5 分钟断点 TTL。其他通过第三方的 Claude 路径（AWS Bedrock、Azure Foundry）回退到 provider 自己的缓存默认值。xAI Grok 使用单独的会话固定对话 ID 机制 —— 参阅 [xAI prompt 缓存](/docs/zh-Hans/integrations/providers#xai-grok--responses-api--prompt-caching)。
...
### 交互式配置辅助模型[​](#交互式配置辅助模型 "交互式配置辅助模型的直接链接")
...
辅助任务的可用 providers：`auto`、`main`，以及[provider 注册表](/docs/zh-Hans/reference/environment-variables)中的任何 provider —— `openrouter`、`nous`、`openai-codex`、`copilot`、`copilot-acp`、`anthropic`、`gemini`、`qwen-oauth`、`zai`、`kimi-coding`、`kimi-coding-cn`、`minimax`、`minimax-cn`、`minimax-oauth`、`deepseek`、`nvidia`、`xai`、`xai-oauth`、`ollama-cloud`、`alibaba`、`bedrock`、`huggingface`、`arcee`、`xiaomi`、`kilocode`、`opencode-zen`、`opencode-go`、`azure-foundry` —— 或您 `custom_providers` 列表中任何命名的自定义 provider（例如 `provider: "beans"`）。

MiniMax OAuth

`minimax-oauth` 通过浏览器 OAuth 登录（无需 API 密钥）。运行 `hermes model` 并选择 **MiniMax (OAuth)** 进行认证。辅助任务自动使用 `MiniMax-M2.7-highspeed`。参阅 [MiniMax OAuth 指南](/docs/zh-Hans/guides/minimax-oauth)。

xAI Grok OAuth

`xai-oauth` 通过浏览器 OAuth 为 SuperGrok 和 X Premium+ 订阅者登录（无需 API 密钥）。运行 `hermes model` 并选择 **xAI Grok OAuth (SuperGrok / Premium+)** 进行认证。相同的 OAuth token 可重用于每个直接到 xAI 的接口（聊天、辅助任务、TTS、图像生成、视频生成、转录）。参阅 [xAI Grok OAuth 指南](/docs/zh-Hans/guides/xai-grok-oauth)，如果 Hermes 在远程主机上，请参阅 [SSH/远程主机上的 OAuth](/docs/zh-Hans/guides/oauth-over-ssh)。

`"main"` 仅用于辅助任务

`"main"` provider 选项表示"使用我的主 agent 使用的任何 provider" —— 它仅在 `auxiliary:`、`compression:` 和 `fallback_model:` 配置中有效。它**不是**顶级 `model.provider` 设置的有效值。如果您使用自定义 OpenAI 兼容端点，请在 `model:` 部分设置 `provider: custom`。所有主模型 provider 选项请参阅 [AI Providers](/docs/zh-Hans/integrations/providers)。

### 完整辅助配置参考[​](#完整辅助配置参考 "完整辅助配置参考的直接链接")
...
每个辅助任务都有可配置的 `timeout`（秒）。默认值：vision 120s、web\_extract 360s、approval 30s、compression 120s。如果您为辅助任务使用慢速本地模型，请增加这些值。Vision 还有单独的 `download_timeout`（默认 30s）用于 HTTP 图像下载 —— 对于慢速连接或自托管图像服务器，请增加此值。

信息

上下文压缩有自己的 `compression:` 块用于阈值，以及 `auxiliary.compression:` 块用于模型/provider 设置 —— 参阅上方的[上下文压缩](#context-compression)。主备用链使用顶层的 `fallback_providers:` 列表 —— 参阅[备用提供商](/docs/zh-Hans/integrations/providers#fallback-providers)。三者都遵循相同的 provider/model/base\_url 模式。

### 辅助任务的每任务回退链[​](#辅助任务的每任务回退链 "辅助任务的每任务回退链的直接链接")
...
### OpenRouter 路由和辅助任务的 Pareto Code[​](#openrouter-路由和辅助任务的-pareto-code "OpenRouter 路由和辅助任务的 Pareto Code的直接链接")

当辅助任务解析到 OpenRouter（显式或通过 `provider: "main"` 而您的主 agent 在 OpenRouter 上）时，主 agent 的 `provider_routing` 和 `openrouter.min_coding_score` 设置**不会传播** —— 按设计，每个辅助任务是独立的。要为特定辅助任务设置 OpenRouter provider 偏好或使用 [Pareto Code 路由器](/docs/zh-Hans/integrations/providers#openrouter-pareto-code-router)，请通过 `extra_body` 按任务设置：
...
### Provider 选项[​](#provider-选项 "Provider 选项的直接链接")
...
运行 `hermes model` 并选择 **MiniMax (OAuth)** 自动登录并设置此项。对于中国区域，基础 URL 将是 `https://api.minimaxi.com/anthropic`。完整演练请参阅 [MiniMax OAuth 指南](/docs/zh-Hans/guides/minimax-oauth)。
...
### 静态消息的 UI 语言[​](#静态消息的-ui-语言 "静态消息的 UI 语言的直接链接")
...
### 运行时元数据页脚（仅限 gateway）[​](#运行时元数据页脚仅限-gateway "运行时元数据页脚（仅限 gateway）的直接链接")
...
## 语音模式（CLI）[​](#语音模式cli "语音模式（CLI）的直接链接")
...
在 CLI 中使用 `/voice on` 启用麦克风模式，使用 `record_key` 开始/停止录音，使用 `/voice tts` 切换口语回复。端到端设置和平台特定行为请参阅[语音模式](/docs/zh-Hans/user-guide/features/voice-mode)。

## 流式传输[​](#流式传输 "流式传输的直接链接")

将 token 实时流式传输到终端或消息平台，而不是等待完整响应。

### CLI 流式传输[​](#cli-流式传输 "CLI 流式传输的直接链接")
...
### Gateway 流式传输（Telegram、Discord、Slack）[​](#gateway-流式传输telegramdiscordslack "Gateway 流式传输（Telegram、Discord、Slack）的直接链接")
...
有关行为详情和示例，请参阅[会话](/docs/zh-Hans/user-guide/sessions)和 [Discord 指南](/docs/zh-Hans/user-guide/messaging/discord)。

## 未授权 DM 行为[​](#未授权-dm-行为 "未授权 DM 行为的直接链接")
...
## Web 搜索后端[​](#web-搜索后端 "Web 搜索后端的直接链接")
...
**SearXNG** 是一个免费、自托管、尊重隐私的元搜索引擎，查询 70+ 个搜索引擎。无需 API 密钥 —— 只需将 `SEARXNG_URL` 设置为您的实例（例如 `http://localhost:8080`）。SearXNG 仅限搜索；`web_extract` 需要单独的提取 provider（设置 `web.extract_backend`）。Docker 设置说明请参阅 [Web 搜索设置指南](/docs/zh-Hans/user-guide/features/web-search)。
...
完整对话框工作流请参阅[浏览器功能页面](/docs/zh-Hans/user-guide/features/browser#browser_dialog)。

浏览器工具集支持多个 provider。有关 Browserbase、Browser Use 和本地 Chromium 系 CDP 设置的详细信息，请参阅[浏览器功能页面](/docs/zh-Hans/user-guide/features/browser)。
...
破坏性文件操作之前的自动文件系统快照。详情请参阅[检查点与回滚](/docs/zh-Hans/user-guide/checkpoints-and-rollback)。
...
## 上下文文件（SOUL.md、AGENTS.md）[​](#上下文文件soulmdagentsmd "上下文文件（SOUL.md、AGENTS.md）的直接链接")
...
* [终端后端配置](#终端后端配置)
  ...
  + [SSH 后端](#ssh-后端)
  + [Modal 后端](#modal-后端)
  + [Daytona 后端](#daytona-后端)
  + [Singularity/Apptainer 后端](#singularityapptainer-后端)
  + [常见终端后端问题](#常见终端后端问题)
  + [拆卸时远程到宿主文件同步](#拆卸时远程到宿主文件同步)
  + [Docker 卷挂载](#docker-卷挂载)
  + [Docker 凭据转发](#docker-凭据转发)
  + [以宿主用户身份运行容器](#以宿主用户身份运行容器)
  + [可选：将启动目录挂载到 `/workspace`](#可选将启动目录挂载到-workspace)
  + [持久 Shell](#持久-shell)
* [技能设置](#技能设置)
  + [Agent 创建技能写入的守卫](#agent-创建技能写入的守卫)
* [内存配置](#内存配置)
* [文件读取安全](#文件读取安全)
* [工具输出截断限制](#工具输出截断限制)
* [全局工具集禁用](#全局工具集禁用)
...
* [辅助模型](#辅助模型)
  ...
  + [通用配置模式](#通用配置模式)
  + [完整辅助配置参考](#完整辅助配置参考)
  + [辅助任务的每任务回退链](#辅助任务的每任务回退链)
  + [OpenRouter 路由和辅助任务的 Pareto Code](#openrouter-路由和辅助任务的-pareto-code)
  + [更改视觉模型](#更改视觉模型)
...
* [显示设置](#显示设置)
  + [文件变更验证器](#文件变更验证器)
  + [静态消息的 UI 语言](#静态消息的-ui-语言)
  + [运行时元数据页脚（仅限 gateway）](#运行时元数据页脚仅限-gateway)
  + [每平台进度覆盖](#每平台进度覆盖)
* [隐私](#隐私)
* [语音转文字（STT）](#语音转文字stt)
* [语音模式（CLI）](#语音模式cli)
* [流式传输](#流式传输)
  + [CLI 流式传输](#cli-流式传输)
  + [Gateway 流式传输（Telegram、Discord、Slack）](#gateway-流式传输telegramdiscordslack)
* [群聊会话隔离](#群聊会话隔离)
* [未授权 DM 行为](#未授权-dm-行为)
* [快速命令](#快速命令)
* [人类延迟](#人类延迟)
...
* [委托](#委托)
* [澄清](#澄清)
* [上下文文件（SOUL.md、AGENTS.md）](#上下文文件soulmdagentsmd)
* [工作目录](#工作目录)

## Related Files
> **LLM Navigation:** Các tệp dưới đây được liên kết trực tiếp từ tài liệu này. Hãy đọc chúng nếu cần thêm ngữ cảnh.

- [https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/context-compression-and-caching](./docs-zh-Hans-developer-guide-context-compression-and-caching.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/minimax-oauth](./docs-zh-Hans-guides-minimax-oauth.md)
...
- [https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/web-search](./docs-zh-Hans-user-guide-features-web-search.md)
