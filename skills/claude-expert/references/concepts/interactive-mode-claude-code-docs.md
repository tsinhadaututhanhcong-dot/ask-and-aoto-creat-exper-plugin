...
**Source:** [https://code.claude.com/docs/en/interactive-mode](https://code.claude.com/docs/en/interactive-mode)
...
In print mode they are off by default. Pass [`--prompt-suggestions`](/docs/en/cli-reference#cli-flags) with `--output-format stream-json --verbose` to emit a `prompt_suggestion` message after each turn instead.
...
[Tools reference](/docs/en/tools-reference)[Checkpointing](/docs/en/checkpointing)
