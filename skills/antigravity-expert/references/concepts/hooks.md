...
*Source URL*: https://antigravity.google/docs/hooks
...
info**Note**: For `PreInvocation`, `PostInvocation`, and `Stop`, the structure is simpler (a list of handlers directly under the event key) and the matcher is ignored.
...
* **`find_by_name`**: Search for files and directories using glob patterns. **Đây là một native tool thật sự có sẵn trên Antigravity agent** (không chỉ là tên dùng cho hook matchers).
